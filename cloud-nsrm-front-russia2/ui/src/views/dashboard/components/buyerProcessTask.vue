    ...
    <div class="task-header">
      ...
      >
        <el-tab-pane
          ...
          :name="item.name"
        >
          <span v-if="['ceeaFindMyRunningProcess', 'queryTodoCurrent'].includes(item.name)" slot="label">
...
export default {
  ...
  methods: {
    ...
    goToForm (row) {
      if (['queryTodoCurrent', 'queryDone'].includes(this.activeName)) {
        this.flowGoForm(row)
      } else {
        this.taskGoForm(row)
      }
    ...
    async taskGoForm (row) {
      ...
      this.$router.push({
        ...
        params: {
          ...
          row,
          // 1-待办 2-已办
          taskIndex: ['ceeaFindMyRunningProcess', 'ceeaFindMyWorkedProcess'].indexOf(this.activeName) + 1
        }
      })
    },
    // 流程代办跳转
    flowGoForm (row) {
      ...
      this.$router.push({
        ...
        query: {
          ...
          formId: formId, // 业务单据ID
          formNo: formNo, // 业务单据No
          status: row.status || '1' // 待审批状态，2：起草，1：执行中
        }
      })
    }
  }
