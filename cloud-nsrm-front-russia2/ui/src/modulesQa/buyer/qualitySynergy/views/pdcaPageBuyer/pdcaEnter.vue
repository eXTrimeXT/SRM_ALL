  ...
  <el-container class="flex-container" direction="vertical">
    <el-main>
      ...
      >
        <el-collapse v-model="activeDims" class="tab-form-style">
              ...
              <el-table-column align="center" prop="reasonAnalysis" :label="$t('quality.reasonAnalysis')">
                <template slot-scope="scope">
                  <el-form-item label-width="0" :prop="'causeList[' + scope.$index + '].reasonAnalysis'" :rules="rules.reasonAnalysis">
                    <el-input v-model="scope.row.reasonAnalysis" :placeholder="$t('qualitySynergy.causeAnalysis')" :disabled="isDisabled" />
                  </el-form-item>
                </template>
              </el-table-column>
          ...
          <el-collapse-item :title="$t('quality.verification')" name="3">
            ...
            >
              ...
              <el-table-column
                width="210"
                align="center"
                prop="veriflcationDate"
                :label="$t('quality.veriflcationDate')"
              >
                <template slot-scope="scope">
          ...
          <el-collapse-item :title="$t('quality.validateFlow')" name="4">
            <srm-row :gutter="30">
              <srm-col :initCol="2">
                <el-form-item required :label="$t('quality.rectificationStatus')">
                  <el-radio-group v-model="approveForm.rectificationStatus" :disabled="false">
                    <el-radio label="Y">
                      {{ $t('common.yes') }}
                    </el-radio>
                    <el-radio label="N">
              ...
              <srm-col :initCol="2">
                <el-form-item required :label="$t('quality.approvalAdvice')">
                  <el-input v-model="approveForm.approvalAdvice" :disabled="false" type="textarea" />
                </el-form-item>
              </srm-col>
            </srm-row>
          </el-collapse-item>
    ...
    <CToolbar>
      <template slot="right">
        ...
        <template>
          <el-button size="primary" @click="handleApprove">
            {{ $t('common.toApprove') }}
          </el-button>
          <el-button type="ghost" size="small" @click="handleReject">
            {{ $t('common.toRefuse') }}
          </el-button>
        </template>
      </template>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        exceptionCatelog: [{ required: true, message: this.$t('common.pleaseInput') }],
  ...
  methods: {
    handleApprove () {
      if (this.approveForm.rectificationStatus === null) return this.$message.warning(this.$t('quest.rectificationStatus'))
      if (this.approveForm.approvalAdvice === null) return this.$message.warning(this.$t('quality.fillApprovalAdvice'))
      let data = this.$attrs.params.params
      console.log(data)
      let param = {
        recordId: data.recordId,
        status: 'APPROVAL',
        rectificationStatus: this.approveForm.rectificationStatus,
        approvalAdvice: this.approveForm.approvalAdvice
      }
      let transformParams = transformMQL.save('spcPdcaRecordBuyer', [param], 'update')
