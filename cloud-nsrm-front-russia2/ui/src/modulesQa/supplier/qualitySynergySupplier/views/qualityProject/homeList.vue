  ...
  <el-container class="flex-container the_quick_list__outter_wrapper" direction="vertical">
    <el-main>
      ...
      <MainHeader>
        <template slot="left">
          ...
          <el-button
            ...
            @click="editTab('change')"
          >
            修改
          </el-button> -->
          <!-- spc暂时屏蔽功能 -->
          <ExportExcel
            page-url="/api-qc/api-ql/spcMonitorList/query"
      ...
      <TableView
        ...
        url="/api-qc/api-ql/spcMonitorList/query"
      />
      <!--弹框区域-->
      <srm-dialog
        ...
        class="c-form-dialog"
        width="70%"
        :title="$t('quality.project.jiankongweihu')"
      >
        <el-form
          ref="monitorsForm"
...
import { qualityProject } from '@/modulesQa/supplier/qualitySynergySupplier/api'
import { parseTime } from '@/utils'
import { tabTodoWatch, tabTodoMixin } from '@/utils/mixins'
import { transformMQL } from '@/library/utils/util'
import { mapGetters } from 'vuex'
import ExportExcel from 'lib@/components/export-excel'
const { pageCondition, listMyMonitors, addMonitors, monitorModify, deleteMonitors } = qualityProject
...
export default {
  ...
  methods: {
    ...
    getQuerydata (params = {}) {
      ...
      this.queryParam = transformMQL.listGetData('spcMonitorList', params, 'lastUpdateDate', undefined, 'query')
      this.$nextTick(() => {
        this.$refs[this.gridId].query()
      })
    },
    ...
    editTab (type, param) {
      ...
      if (type == 'SPCStandard') {
        ...
        tab = {
          ...
          params: { flag: 'common', param: param },
    ...
    // 删除数据
    deleteOne () {
      deleteMonitors(this.selectDel).then(response => {
        const { data } = response
        if (response.success) {
          this.$refs[this.gridId].query()
        }
