      ...
      <div class="form-container">
        <el-collapse v-model="activeDims" class="tab-form-style">
          ...
          <el-form ref="form" :model="form" :rules="rules">
            <el-collapse-item :title="$t('orderMod.outerBoxBarcodeBasicInformation')" name="1">
              <srm-row :gutter="32">
                <srm-col>
                  <el-form-item prop="outerBoxCode" :label="$t('orderMod.outerBoxCode')">
                ...
                <srm-col v-show="!outShow&&!editMode">
                  <el-form-item prop="currentLotQuantity" :label="$t('orderMod.currentLotQuantity')">
          ...
          <!-- 条码扩展信息 -->
          <el-collapse-item v-show="expandFields.length > 0" :title="$t('orderMod.outerBoxBarcodeExtensionInformation')" name="2">
          ...
          <el-collapse-item v-show="!editMode" :title="$t('orderMod.generateBarcodeInformation')" name="3">
            ...
            >
              <el-table-column
                align="center"
                prop="outerBoxCode"
                :label="$t('orderMod.outerBoxCode2')"
              ...
              <el-table-column
                ...
                align="center"
                prop="outerBoxId"
                :label="$t('hierarchical.Outercarton')"
                :show-overflow-tooltip="true"
              />
              <!-- <el-table-column
                  align="center"
            ...
            <!-- <el-button
                ...
                >{{ $t('cusEntry.supplement20250211.allBarcodesSaved') }}</el-button
              > -->
          </el-collapse-item>
        </el-collapse>
      </div>
      ...
      <CToolbar>
        <template #right>
          ...
          <!-- <el-button
            ...
            @click="save('2')"
          >
            {{ $t('cusEntry.supplement20250211.autoOuterBoxBinding') }}
          </el-button> -->
          <el-button
            v-show="!editMode"
...
import { outerBoxCodeApi, boxtagconfigApi } from 'mods@/barcodeSupplier/api'
    ...
    return {
      ...
      columns: [
        // 外箱id
        {
          attrs: {
            prop: 'outerBoxId',
            label: this.$t('cusEntry.supplement20250211.outerBoxId')
          },
          slot: 'outerBoxId'
        },
        // 扩展字段类型
        {
          attrs: {
            prop: 'fieldType',
            label: this.$t('cusEntry.supplement20250211.fieldType')
          },
          slot: 'fieldType'
        },
        // 扩展字段编码（dto属性字段）
        {
          attrs: {
            prop: 'fieldCode',
            label: this.$t('cusEntry.supplement20250211.fieldCode')
          },
          slot: 'fieldCode'
        },
        // 扩展字段描述
        {
          attrs: {
            prop: 'fieldDesc',
            label: this.$t('cusEntry.supplement20250211.fieldDesc')
          },
          slot: 'fieldDesc'
        },
        // 扩展字段数值
        {
          attrs: {
            prop: 'fieldVal',
            label: this.$t('cusEntry.supplement20250211.fieldVal')
          },
          slot: 'fieldVal'
        },
        {
      ...
      rules: {
        categoryName: [
          { required: true, message: this.$t('hierarchical.information') }
        ],
        materialName: [
          { required: true, message: this.$t('hierarchical.ItemInformation') }
        ],
        vendorName: [
          { required: true, message: this.$t('hierarchical.SupplierInformation') }
        ],
        productionDate: [
          { required: true, message: this.$t('hierarchical.productiondate') }
        ]
        // currentLotQuantity:[
        //   { required: true, message: this.$t('hierarchical.quantitylynumber') }
        // ],
        //  orderNumber: [
        //   { required: true, message: this.$t('cusEntry.supplement20250211.inputOrderNumber') }
        // ],
        // orderLineNum: [
        //   { required: true, message: this.$t('cusEntry.supplement20250211.inputOrderLineNum') }
        // ],
      },
      formExpandrules: {},
      readOnly: false,
    ...
    outShow: {
      handler () {
        if (this.outShow) {
          ...
          this.$set(this.rules, 'currentLotQuantity', [{ required: true, message: this.$t('hierarchical.quantitylynumber') }])
        }
      },
      deep: true,
      immediate: true
    ...
    */
        ...
        if (flag === 'add') {
          ...
          if (type == '1') {
            ...
            const loading = this.$loading({
              lock: true,
              // 自动生成内外箱条码，并且自动绑定，请稍等！！！
              text: this.$t('cusEntry.supplement20250211.message10'),
      ...
      this.$http({
        ...
        .then((data) => {
          ...
          if (isNull(data.data.categoryCode)) {
            this.$message({
              type: 'warning',
              message: this.$t('hierarchical.categorynumber')
            })
          } else {
        ...
        .catch(_ => {
          this.$message({
            type: 'warning',
            // 系统没有维护此物料信息，请管理员查看！
            message: this.$t('cusEntry.supplement20250211.message6')
          })
        })
    },
    updateOutList () {
    ...
    onInputBlur () {
      if (this.form.comments.length >= 60) {
        this.$message({
          type: 'warning',
          // 当前字符长度超过字数限制，最大值60个字符
          message: this.$t('cusEntry.supplement20250211.message7')
        })
      }
    }
  }
