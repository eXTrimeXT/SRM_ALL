...
import {
  ...
  exportExcelSegment
} from 'lib@/components/render-engine/schema-segments'// @ts-ignore
// @ts-ignore
import {
  defineSchemas,
  expression,
      ...
      query: {
        ...
        properties: generateXindexInOrder({
          vendorId: {
            type: 'string',
            'x-hidden': true,
            default: '{{app.$store.getters.userInfo?.companyId}}',
      ...
      toolbar: {
        ...
        properties: {
          exportExcel: {
            ...
            'x-component-props': {
              ...exportExcelSegment,
              pageUrl: '/api-sup-ce/api-ql/DeliveryNoticeDetailVendor/list',
              dictCodes: {
                status: 'DELIVERY_NOTICE_DETAIL_STATUS_NEW' // 行状态
              }
            },
            'x-reactions': expression(`(field) => {
                $form.query('DeliveryNoticeDetailVendor.table').take(fields =>{
                    ...
                    field.componentProps.tableHeader = transformColumns(columns,[{
                      ...
                      title: $t('orderMod.buyerOrderSynergy.deliveryNoticeNum')
                    },{
                      targetFiled: 'deliveryNoticeNumberAndLineNum',
                      field: 'lineNum',
                      title: $t('orderMod.deliveryLineNum')
                  },{
                      ...
                      title: $t('orderMod.buyerOrderSynergy.orderLineNum')
                  }])
                })
              }`)
          }
...
<template>
  <RenderEngine :schema="schema" :scope="scope" schemaKey="VendorDeliveryNoticeDetailList" />
</template>
