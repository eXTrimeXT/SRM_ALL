<template>
  <el-container
    class="flex-container-notab the_planOrders_wrapper"
    direction="vertical"
  >
    <el-main>
      <FormWrapper
...
export default {
  ...
  data () {
    return {
      ...
      preArr: [
        ...
        { prop: 'status', label: this.$t('common.status') }, // 状态
        {
          prop: 'planMonth',
  ...
  created () {
    this.tableHeader = [
      ...
      { prop: 'purOrg', label: this.$t('common.orgName'), minWidth: 100 }, // 采购组织
      {
        prop: 'demandDate',
    ...
    this.$nextTick(() => {
      ...
      for (let i = 1; i < 5; i++) {
        listdata.push({
          ...
          itemCode: this.$t('common.materialCode') + i,
          itemName: this.$t('common.materialName') + i,
          demandAmount: '10' + i,
          purOrg: this.$t('cusEntry.supplement20250211.oouuRefrigeratorDivision') + i, // OOUU_冰箱事业部_顺德工厂
