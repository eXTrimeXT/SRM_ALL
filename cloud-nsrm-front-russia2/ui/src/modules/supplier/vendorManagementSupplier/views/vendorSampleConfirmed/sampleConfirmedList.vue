    ...
    $route: {
      ...
      handler () {
        if (
          ...
          this.$route.params.funName === 'vendorSampleConfirmed'
        ) {
        ...
        buttons: [
          {
            ...
            show: function (row) {
              if (row.approveStatus === 'PUBLISHED') {
                return true
              } else {
                return false
              }
  ...
  methods: {
    ...
    delRowData (row) {
      ...
      this.$confirm(this.$t('common.confirmDelete'), {
        ...
        .then(() => {
          sampleConfirmed.quaSampleBathDel([id]).then((res) => {
            this.$message({
              message: res.message,
              type: 'success'
            })
    ...
    editTab (type, row) {
      ...
      if (type === 'add') {
        ...
        this.$confirm(this.$t('vendorMod.isRejectSampleConfirmation'), this.$t('common.tips'), {
          ...
          sampleConfirmed.saveOrUpdatuOrderByStatus(url, submitData).then(res => {
            this.$message({
              type: 'success',
              message: res.message
            })
