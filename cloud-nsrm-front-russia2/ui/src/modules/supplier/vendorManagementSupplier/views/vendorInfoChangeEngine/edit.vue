...
import { tabs } from "mods@/vendorManagementSupplier/views/vendorInfoChangeEngine/components/tabs";
...
const $submits = (type, $form, $queryEngine, $message, $t, $bus) => {
  ...
  if (allData.companyInfoChange.registCurrency == '' || allData.companyInfoChange.registCurrency == null) {
    app.$message.error($t('vendorMod.enterCurrency'))
    return false
  }
  ...
  if (type == 'SAVE') { // 暂存的时候
    ...
    if ([null, 'DRAFT', 'SUBMITTED', 'VENDOR_SUBMITTED'].includes(changeStatus)) { // 新增或者编辑的时候
      allData.changeStatus = 'SUBMITTED'
    }
    $queryEngine.request.save(allData, { customizeAction: 'vendorSubmit', query: { '*':{} } }).then((res) => {
      $message.success($t('common.successSave'))
      $bus.$emit('vendorInfoChange')
      emitTabRemove(attrs.tabName)
    })
