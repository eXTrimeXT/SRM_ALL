...
const $submits = (type, $form, $queryEngine, $message, $t, $bus) => {
  ...
  const planConfirmId = attrs.params.row?.planConfirmId || null
  if (type == 'SAVE') { // 暂存的时候
    if ([null, 'DRAFT'].includes(planConfirmId)) { // 新增或者编辑的时候
...
<template>
  <RenderEngine schemaKey="vendorSiteReviewPlanConfirmDetail" :pageAttrs="$attrs" :schema="schema" :scope="scope" :components="components" />
</template>
