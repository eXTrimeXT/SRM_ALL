    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          />
          <span v-else>{{ $parseTime(scope.row.authDate) }}</span>
        </template>
      </el-table-column>
      <!-- 认证机构 -->
      <el-table-column
      ...
      >
        <template slot-scope="scope">
          <el-date-picker
            ...
            value-format="yyyy-MM-dd"
          />
          <span v-else>{{ $parseTime(scope.row.endDate) }}</span>
        </template>
      </el-table-column>
      <!-- 操作 -->
      <el-table-column
...
export default {
  ...
  inject: ['addSceneAttachment', 'innerHandleUploadSuccess', 'innerButtonClick', 'handleAttachmentRemove1', 'setFormatValue', 'sceneAttachmentHandel', 'relevantRowDelHandel'],
  computed: {
