...
const $batchAbandon = async ($self: any, $queryEngine: any, $message: any, $confirm: any) => {
  ...
  if (i.length) {
    ...
    // 序号${sequence}已绑定外箱，不可作废
    return $message.warning($t('cusEntry.supplement20250211.batchAbandonTip1', { sequence }))
  }
  const j = rows.filter((row: any) => row.status === 'N')
  if (j.length) {
    const sequence = j.map((item: any) => item.sequence).join()
    // 序号${sequence}已作废，不可再作废
    return $message.warning($t('cusEntry.supplement20250211.batchAbandonTip2', { sequence }))
  }
        ...
        properties: generateXindexInOrder({
          ...
          innerBoxCode: {
            ...
            'x-render-table-column': {
              title: i18nExpression('orderMod.innerBoxBarcode'), // 内箱条码
              minWidth: 120,
              customRender: true
            },
          ...
          relationMaterialQuantity: {
            ...
            'x-render-table-column': {
              ...
              minWidth: 150,
              'title-prefix': { content: $t('buyerDeliveryOrder.prompt21') } // 即1个内箱可装的物料数量，贴1个内箱标签
            }
          },
          tagNo: {
            type: 'string',
          ...
          materialQuantity: {
            ...
            'x-render-table-column': {
              ...
              minWidth: 120,
              'title-prefix': { content: $t('cusEntry.supplement20250211.innerBoxBarcodeMaterialQuantity') } // 本次生成内箱条码时对应的物料数量
            }
          },
          tagRuleName: {
            type: 'string',
