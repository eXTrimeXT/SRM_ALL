...
const $batchAbandon = async ($self: any, $queryEngine: any, $message: any, $confirm: any) => {
  ...
  if (k.length) {
    ...
    // 送货状态为拟定或已取消时才可以作废，序号${sequence}不可作废
    return $message.warning($t('cusEntry.supplement20250211.batchAbandonTip7', {sequence}))
  }
        ...
        properties: generateXindexInOrder({
          ...
          generateQuantity: {
            ...
            'x-render-table-column': {
              ...
              minWidth: 160,
              'title-prefix': { content: i18nExpression('cusEntry.supplement20250211.generateQuantityTip') } // 本次生成外箱条码的物料数量
            }
          },
          tagNo: {
            type: 'string',
          ...
          generateTotalQuantity: {
            type: 'string',
            'x-render-table-column': {
              title: i18nExpression('cusEntry.supplement20250211.generateTotalQuantity'), // 该送货明细行累计已生成数量
              minWidth: 160
            }
          },
          deliveryQuantity: {
          ...
          deliveryNoteStatus: {
            ...
            'x-render-table-column': {
              title: i18nExpression('buyerDeliveryOrder.deliveryStatus'), // 送货状态
              minWidth: 120
            },
            'x-query-engine-skip': true
          },
