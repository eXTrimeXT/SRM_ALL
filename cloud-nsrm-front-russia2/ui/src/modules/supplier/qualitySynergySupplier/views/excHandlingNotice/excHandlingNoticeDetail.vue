    ...
    <el-main>
      ...
      <CToolbar>
        <template #right>
          ...
          <el-button
            v-if="form.orderStatus == 'published' && curRole === 'BUYER'"
            @click="save('nullify')"
          >
            {{ $t('common.cancelled') }}
          </el-button>
  ...
  data () {
    return {
      ...
      form: {
        ...
        vendorName: null,
        vendorCode: null,
        releaseDate: parseTime(new Date(), '{y}-{m}-{d}', true),
        orderStatus: 'draft',
        orgId: null,
        orgName: null,
        purchaserName: null,
