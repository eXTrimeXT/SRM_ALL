  ...
  >
    <el-collapse v-model="activeDims" class="tab-form-style">
      ...
      <el-collapse-item
        ...
        name="5"
      >
        <OriginBondConfig :business-type="BUSINESS_TYPE_ENUM.BARGAIN_LTS" :base-data.sync="bargainBaseInfo" />
      </el-collapse-item>
      ...
      <!-- 报价控制 -->
      <el-collapse-item :title="$t('bidMod.priceControl')" name="8">
        <BargainControl
          :bargain-base.sync="bargainBaseInfo"
      ...
      <!-- 智能推荐供应商控制 -->
      <el-collapse-item :title="$t('bidMod.recommendSupControl')" name="9">
        <VendorsControl :bargain-base.sync="bargainBaseInfo" />
      </el-collapse-item>
    </el-collapse>
  </el-form>
</template>
...
import BargainControl from './projectInfo/bargainControl'
...
export default {
      ...
      rules: {
            ...
            validator: (_rule, value, callback) => {
              if (value) {
                ...
                if (startDate.getTime() > endDate.getTime()) {
                  // '价格有效期至需要小于价格有效期自'
                  callback(new Error(this.$t('bidMod.priceEndTimeLessStart')))
                }
              }
              callback()
            },
        ...
        orderEndTime: [
          { required: true, message: this.$t('bidMod.bidMsgList[36]') },
          {
            validator: (_rule, value, callback) => {
              if (value) {
                const startDate = new Date(this.bargainBaseInfo.orderStartTime)
                const endDate = new Date(value)
                if (startDate.getTime() > endDate.getTime()) {
                  // '截止时间需要小于开始时间'
                  callback(new Error(this.$t('bidMod.orderEndTimeLessStart')))
                }
              }
              callback()
            },
        ...
        bankAccountName: [
          {
            validator: (_rule, value, callback) => {
              if (this.bondConfigVisible && !value) {
                // '请输入账户名称'
                callback(new Error(this.$t('vendorMod.msgBankAccountName')))
              }
              callback()
            },
            trigger: ['change', 'blur']
        ...
        bankBranchName: [
          {
            validator: (_rule, value, callback) => {
              if (this.bondConfigVisible && !value) {
                // '请输入开户支行'
                callback(new Error(this.$t('bidMod.bidMsgList.17')))
              }
              callback()
            },
            trigger: ['change', 'blur']
  ...
  computed: {
    ...
    showEnrollEndDatetime () {
      ...
      return (this.bargainBaseInfo.processConfigId && (this.enabledNodeMenu || []).includes('signUpManagement')) ||
        !this.bargainBaseInfo.processConfigId ||
        // 只读，但是有值
        (this.readonly && this.bargainBaseInfo.signUpEndTime)
    }
  },
  ...
  methods: {
    ...
    validateNeedEncryptPrice () {
      // 密封报价时工作小组需分配解密权限
      if (this.bargainBaseInfo.needEncryptPrice === 'Y') {
    ...
    async tempSaveProjectInfo (type) {
      ...
      if (this.bargainBaseInfo.bargainType === SOU_BRG_TYPE_ENUM.TECHNOLOGY_BUSINESS) {
        ...
        if (count !== 1) {
          // '工作小组中必须有且只能有一个技术评委！'
          this.$message.warning(this.$t('cusEntry.supplement20250211.technicalJudgeInGroup'))
          return
        }
      }
      ...
      const data = await brgBuyerHttp.init.editProjectInfo(submitData)
      if (!data) {
        return
      }
      ...
      if (!this.bargainBaseInfo.projectId) {
        this.bargainBaseInfo.projectId = data.data
      }
      // 等待ID更新
      await this.$nextTick()
      // 发起保存成功回调
