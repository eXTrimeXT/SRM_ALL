...
export default {
  ...
  data () {
    return {
      tableViewUrl: brgBuyerHttp.signUp.page,
      tableHeader: [
        // 供应商编码
        {
          prop: 'vendorCode',
          ...
          buttons: [
            // 确认
            {
              ...
              show: row => (this.pageFlag.isEdit || this.pageFlag.isAdd) &&
                this.bargainBase.publishScope === SOU_PUBLISH_SCOPE_ENUM.OPEN_TENDER &&
  ...
  computed: {
    queryParam () {
      return { projectId: this.bargainBase.projectId }
    }
  },
  ...
  methods: {
    ...
    async changeSignUpEndTime (isStopNow) {
      ...
      const response = await brgBuyerHttp.signUp.changeSignUpEndTime(params)
      if (response) {
    ...
    async submitConfirmOrReject (row, type) {
      ...
      const response = await brgBuyerHttp.signUp.confirmSignUp(param)
      if (response) {
