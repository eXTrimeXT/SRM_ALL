<template>
  ...
  >
    <SearchForm :pre-form-obj="{ round: bargainBase.currentRound || 1 }" @get="getQueryData" />
...
import ModelQuoteReadDialog from 'lib@/composition/bargainLts/modelQuote/modelQuoteReadDialog'
import FormulaPriceDialog from 'lib@/composition/bargainLts/formulaPriceDialog'
...
export default {
  ...
  data () {
    return {
      tableViewUrl: brgBuyerHttp.select.listPageUrl,
    ...
    bargainBase: {
      handler (val) {
        ...
        const tableHeaderData = [
          ...
          {
            prop: 'winStatus',
            label: this.$t('bidMod.currentInList'),  // '本轮入围情况'
            minWidth: 120,
          ...
          // 原币未税单价
          {
            ...
            label: this.$t('bidMod.orderNotaxPrice'),  // '原币未税单价'
            minWidth: 120
          },
          // 原币含税单价
          {
            prop: 'orderTaxPrice',
            label: this.$t('bidMod.orderTaxPrice'),  // '原币含税单价'
            minWidth: 150
          },
          // 本币未税单价
          {
  ...
  methods: {
    ...
    async intelligentEvaluation () {
      const response = await brgBuyerHttp.select.intelligent({
        projectId: this.bargainBase.projectId
      })
      if (response) {
    ...
    async changeWinStatus (type) {
      ...
      const response = await brgBuyerHttp.select.changeWinStatus({ orderItemIds, toWin })
      if (response) {
        // 入围成功！
    ...
    async changeSelectStatus (type) {
      ...
      // 发起中标
      const response = await brgBuyerHttp.select.changeSelectStatus({
        orderItemIds,
    ...
    /* 公示本轮结果 */
    async openResult () {
      const response = await brgBuyerHttp.select.openResult(this.bargainBase.projectId)
      if (response) {
        // 结果已公示！
    ...
    async createPricingApproval () {
      ...
      await createPricingApproval(
        ...
        // apiUrl
        brgBuyerHttp.select.getCreatePricingApprovalUrl(this.bargainBase.projectId),
        {},
        this.$router
      )
    },
    ...
    async submitQuotaQuantity (valid) {
      ...
      const responseChangeWinAmount = await brgBuyerHttp.select.changeWinAmount(data)
    ...
    async getPriceCompareInfos (row) {
      ...
      const response = await brgBuyerHttp.select.getPriceCompareInfos({
    ...
    openPriceComparisonTab (row) {
      const tab = {
        ...
        params: {
          businessType: BUSINESS_TYPE_ENUM.BARGAIN_LTS,
          pricingType: this.bargainBase.orderType,
          row: {
            id: this.bargainBase.projectId,
            number: this.bargainBase.souNo,
            round: row.round
          }
        },
        closable: true,
        title: `${this.bargainBase.souNo} ${this.$t('bidMod.priceCompare')}`,  // 比价
        name: `priceComparison-${this.bargainBase.souNo}`
      }
