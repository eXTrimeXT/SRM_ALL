 ...
 */
import { expression, i18nExpression, generateCharFunctionExpression } from '@meicloud/render-engine'
...
const EditQuoteTimeSegment: Record<any, any> = {
  editQuoteTimeDialog: {
    type: 'void',
    title: i18nExpression('bidMod.chooseNewQuoteTime'),  // '选择新的报价结束时间'
    'x-component': 'RDialog',
    'x-component-props': {
      class: 'dialogMain',
      size: 'small',
        ...
        $form.query($self.address.concat('editQuoteTimeForm')).take(async (field: any) => {
          ...
          $form.validate(field.address.concat('*'))
            .then(async () => {
              ...
              if (response) {
                $message.success(i18nExpression('bidMod.endQuoteTime'))  //'修改报价截止时间成功!'
            ...
            'x-validator': {
              ...
              validator: generateCharFunctionExpression(({ $dayjs, $self, $values }) => {
                if ($dayjs($self.value).unix() <= $dayjs($values.orderStartTime).unix()) {
                  return i18nExpression('bidMod.quoteEndTimeMsg')  // '报价截止时间必须大于报价开始时间！'
                }
              })
            }
          }
