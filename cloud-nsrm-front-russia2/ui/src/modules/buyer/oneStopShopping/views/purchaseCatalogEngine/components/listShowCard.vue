<template>
  <div class="list-show-card">
    <div v-if="itemResData.length > 0" class="method-card">
      <el-row :gutter="16">
        <el-col v-for="(item, index) in itemResData" :key="index" :span="6">
          <div class="itemWrpper">
            ...
            <div class="item-info">
              ...
              <p :title="item.materialName">
                ...
                <span class="item-desc-font">{{ item.materialName }}</span>
              </p>
              <p>
                {{ $t('oneStopShopping.specification') }}：
                <span class="item-desc-font">{{ item.specification }}</span>
              </p>
              <p>
                {{ $t('oneStopShopping.unit') }}：
                <span class="item-desc-font">{{ $getDictLabel('unit', item.unitCode) }}</span>
              </p>
              <p :title="item.vendorName" class="the_tip">
                {{ $t('oneStopShopping.vendor') }}：
                <span class="item-desc-font">{{ item.vendorName }}</span>
              </p>
              <el-button
                type="primary"
