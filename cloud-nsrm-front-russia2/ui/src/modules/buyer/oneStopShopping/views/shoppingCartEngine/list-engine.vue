        ...
        $queryEngine.request.baseRequest({
          ...
          if (res.data) {
            if (res.data.length > 0) {
              ...
              app.$confirm($t('oneStopShopping.createProjectMsg4'), $t('components.approvalHead.tips.tip'), {
...
const $keep = ($form:any, $queryEngine:any) => {
  ...
  if (selects.some(item => item.status != 'DRAFT')) {
    // '请勾选未提交的数据'
    return app.$message.warning($t('cusEntry.supplement20250211.checkUnsubmittedData'))
  }
