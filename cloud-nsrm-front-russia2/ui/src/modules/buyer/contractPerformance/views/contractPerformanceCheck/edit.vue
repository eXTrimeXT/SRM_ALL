...
const updateButtonConfig = ($form: any) => {
  setTimeout(() => {
    ...
    componentInstance.setWorkflowBusinessId($form.values.perAcceptanceId)
    componentInstance.setWorkflowTabDisabled(['DRAFT', 'SUPPLIER_SUBMITTED', 'FIRST_PASS'].includes($form.values.status))
  ...
  PerAcceptance: {
    ...
    properties: {
      SchemaWorkflow: {
        ...
        'x-component-props': {
          'business-id': expression('$attrs.params.row?.perAcceptanceId || null'),
              ...
              collapse: {
                ...
                properties: {
                      ...
                      layout: {
                        ...
                        properties: {
                          ...
                          perAcceptanceNo: {
                            type: 'string',
                            'x-decorator': 'FormItem',
                            title: i18nExpression('bid_mod.perAcceptanceNo'),  // '合同验收单号'
                            'x-component-props': {
                              'disabled': true
                            }
                          },
                          ...
                          'perPlanId.perPlanNo': {
                            type: 'string',
                            'x-decorator': 'FormItem',
                            title: i18nExpression('bid_mod.perPlanNo'),  // '合同履约计划单号's
                            'x-component-props': {
                              'disabled': true
                            }
                          },
                    ...
                    properties: {
                      perPlanMilestoneId: {
                        ...
                        properties: generateXindexInOrder({
                          ...
                          nodePersonName: {
                            type: 'string',
                            title: i18nExpression('common.nodeLeader'),  // '节点负责人'
                            'x-render-table-column': {
                              minWidth: 130
                            }
                          },
                          ...
                          // 计划结束时间
                          planEndDate: {
                            title: i18nExpression('perfMod.planEndDate'),  // '计划结束时间'
                            ...yearMonthDaySelectorSegment,
                            'x-component-props': {
                              ...yearMonthDaySelectorSegment['x-component-props'],
                              formatter: expression(`({ cellValue, row, column }) => {
                          ...
                          nodePlanNum: {
                            ...
                            `),
                            'x-render-table-column': {
                              title: i18nExpression('perfMod.numberOfNodesDelivered'),  // '节点交付数量'
                              minWidth: 130
                            },
                            ...editTableFormItemValid
                          },
                          ...
                          practicallyEndDate: {
                            ...
                            `),
                            'x-render-table-column': {
                              title: i18nExpression('perfMod.actualEndTime'),  // '实际结束时间'
                              minWidth: 130
                            },
                            ...editTableFormItemValid
                          },
                          ...
                          remarks: {
                            type: 'string',
                            title: i18nExpression('perfMod.specialRemarks'),  // '特殊备注'
                            'x-render-table-column': {
                              minWidth: 130
                            }
                          },
                  ...
                  perAcceptanceConf: {
                    ...
                    'x-component': 'CollapseItem',
                    'x-component-props': {
                      title: i18nExpression('bidMod.performanceProcessEvaluation')  // '履约过程评价'
                    },
                    properties: {
                      perAcceptanceConfList: {
                        type: 'string',
                  ...
                  delivery: {
                    ...
                    'x-component': 'CollapseItem',
                    'x-component-props': {
                      title: i18nExpression('bidMod.deliveryInstructions')  // '交付说明'
                    },
                    properties: {
                      // 交付说明
                      deliveryExplain: {
...
const $saveBill = (type: string, $form: any, $queryEngine: any, $confirm: any, $message: any, $bus: any) => {
  ...
  if (type === 'SAVE') {
    $submitData('save', data, $form, $queryEngine, $confirm, $message, $bus)
  } else if (type === 'SUBMIT') {
    $form.validate().then(() => {
      $submitData('submit', data, $form, $queryEngine, $confirm, $message, $bus)
    })
...
const $submitData = (type: string, $values: any, $form: any, $queryEngine: any, $confirm: any, $message: any, $bus: any) => {
  if (type === 'submit') {
    ...
    for (let i = 0; i < $values.perPlanMilestoneId.length; i += 1) {
      if (!$values.perPlanMilestoneId[i].practicallyEndDate) {
        // '请填写里程碑---实际结束时间'
        $message(i18nExpression('cusEntry.supplement20250205.milestoneActualEndTime'))
        return
      }
    }
  }
  ...
  $queryEngine.request.save($values, { customizeAction: vendor() && type === 'submit' ? 'vendorSubmit' : undefined })
    .then((res: any) => {
      if (type === 'save') {
        $message.success(t('common.successSave'))
      }
      ...
      getFormDetail(res.data?.[0], $form, $queryEngine).then(() => {
        ...
        const componentInstance = $form.query('.SchemaWorkflow').take().componentProps.componentInstance
        componentInstance.setWorkflowBusinessId(res.data?.[0].perAcceptanceId || null)
