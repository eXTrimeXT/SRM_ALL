        ...
        <el-collapse v-model="colValue">
          <el-collapse-item :title="$t('supRisk.baseInfo')" name="1">
            ...
            >
              <srm-row :gutter="32">
                ...
                <srm-col>
                  <!-- 合同履约计划单号 -->
                  <el-form-item prop="perPlanNo" label="bid_mod.perPlanNo">
          ...
          <el-collapse-item ref="perPlanMilestone" :title="$t('contractMod.milestone')" name="2">
            <el-table border stripe :data="perPlanMilestoneList">
              ...
              <el-table-column prop="planStartDate" :formatter="(row, column, cellValue) => $parseTime(cellValue)" :label="$t('perfMod.planStartDate')" min-width="130" show-overflow-tooltip />
                <!-- 计划结束时间 -->
              <el-table-column prop="planEndDate" :formatter="(row, column, cellValue) => $parseTime(cellValue)" label="$t('perfMod.planEndDate')" min-width="130" show-overflow-tooltip />
              <!-- 节点交付数量 -->
              ...
              <el-table-column prop="practicallyEndDate" min-width="150">
                ...
                <template slot-scope="scope">
                  <el-date-picker v-model="scope.row.practicallyEndDate" :disabled="disabledFlag || ['SUPPLIER_SUBMITTED','FIRST_PASS'].includes(form.status) || form.dataCreationType === 'VENDOR'" :placeholder="$t('purchaseDemand.datePicker')" value-format="yyyy-MM-dd" :format="$formatDatePicker" />
                </template>
              </el-table-column>
              <!-- 特殊备注 -->
          ...
          <!-- 履约过程评价 -->
          <el-collapse-item ref="perAcceptance" :title="$t('bidMod.performanceProcessEvaluation')" name="3">
            <i-field-view
