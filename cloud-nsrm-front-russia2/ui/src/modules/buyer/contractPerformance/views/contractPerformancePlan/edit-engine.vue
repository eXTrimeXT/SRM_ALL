          ...
          collapse: {
            ...
            properties: {
                  ...
                  baseinfo: {
                    ...
                    properties: {
                      ...
                      contractNo: {
                        ...
                        title: i18nExpression('bidMod.compactIndex'),  // 合同序号
                      ...
                      includeTaxAmount: {
                        type: 'string',
                        'x-decorator': 'FormItem',
                        title: i18nExpression('purSettlementMod.includeTaxAmount'),  // 合同总金额（含税）
                        'x-component-props': {
                          disabled: true
                        }
                      },
              ...
              IPayformPlan: {
                ...
                'x-component': 'CollapseItem',
                'x-component-props': {
                  title: '{{$t(\'route.buyerPaymentPlan\')}}'  // '付款计划'
                },
                properties: {
                  toolbar: {
                    type: 'void',
...
const $submitData = (type: string, $values: any, $form: any, $queryEngine: any, $confirm: any, $message: any, $bus: any) => {
  if (type === 'submit') {
    ...
    if (!perPlanMilestoneList.length) {
      // '请填写里程碑相关信息'
      $message.error(i18nExpression('cusEntry.supplement20250211.milestoneInfo'))
      return
    }
    for (let item of perPlanMilestoneList) {
      if (!item.nodePersonName) {
        // '里程碑---节点负责人必填'
        $message.error(i18nExpression('cusEntry.supplement20250211.milestoneNodePersonInChargeMustFill'))
        return
      }
      if (!item.planStartDate) {
        // '里程碑---计划开始时间必填'
        $message.error(i18nExpression('cusEntry.supplement20250211.milestonePlanStartTimeRequired'))
        return
      }
      if (!item.planEndDate) {
        // '里程碑---计划结束时间必填'
        $message.error(i18nExpression('cusEntry.supplement20250211.milestonePlanEndTimeRequired'))
        return
      }
      if (new Date(item.planStartDate).getTime() > new Date(item.planEndDate).getTime()) {
        // '里程碑---计划完成时间必须晚于里程碑计划开始时间'
        $message.error(i18nExpression('cusEntry.supplement20250211.milestonePlanEndTimeMustBeLaterThanStartTime'))
        return
      }
    }
    if (!perPayPlanList.length) {
      // '请填写付款计划相关信息'
      $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanInfo'))
      return
    }
    let totalPercent = 0
    ...
    for (let item of perPayPlanList) {
      if (!item.milestoneType) {
        // '付款计划---里程碑名称必填'
        $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanMilestoneNameRequired'))
        return
      }
      if (!item.nodePersonName) {
        // '付款计划---节点负责人必填'
        $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanNodePersonInChargeMandatory'))
        return
      }
      if (!item.paymentStage) {
        // '付款计划---付款阶段必填'
        $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanPaymentStageRequired'))
        return
      }
      if (!item.payExplain) {
        // '付款计划---付款条件必填'
        $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanPaymentConditionsRequired'))
        return
      }
      if (!item.payMethod) {
        // '付款计划---付款方式必填'
        $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanPaymentMethodRequired'))
        return
      }
      if (!item.paymentRatio) {
        // '付款计划---付款比例必填'
        $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanPaymentRatioRequired'))
        return
      }
      if (!item.palnPaymentDate) {
        // '付款计划---计划开始时间必填'
        $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanStartTimeRequired'))
        return
      }
      if (!item.planInvoiceCompleteDate) {
        // '付款计划---计划结束时间必填'
        $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanEndTimeRequired'))
        return
      }
      if (new Date(item.palnPaymentDate).getTime() > new Date(item.planInvoiceCompleteDate).getTime()) {
        // '付款计划---计划结束时间必须晚于计划开始时间'
        $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanEndTimeMustBeLaterThanStartTime'))
        return
      }
    ...
    for (let key in typeArr) {
      if (typeArr[key] > 1) {
        // '付款计划---里程碑重复，请修改'
        $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanMilestonesDuplicate'))
        return
      }
    }
    if (totalPercent.toString() !== '100') {
      // '付款比例相加应该等于100'
      $message.error(i18nExpression('cusEntry.supplement20250211.paymentProportionSum'))
      return
    }
  } else {
    ...
    for (let item of perPayPlanList) {
      if (item.perPlanId) {
        ...
        if (!item.stagePaymentAmount) {
          // '付款计划---阶段付款金额必填'
          $message.error(i18nExpression('cusEntry.supplement20250211.paymentPlanStageAmountRequired'))
          return
        }
        if (!item.payExplain) {
          // '付款计划---付款条件必填'
...
const $handover = (row: any, type: any, IFileList: any, $confirm: any, $router: any) => {
  if (type === 'deliver') {
    // '确认要生成合同验收单么？'
    $confirm(i18nExpression('cusEntry.supplement20250211.confirmGenerateContractAcceptanceForm'), {
