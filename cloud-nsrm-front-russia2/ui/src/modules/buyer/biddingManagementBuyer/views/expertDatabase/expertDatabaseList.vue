  ...
  >
    <el-main>
      ...
      <TableView
        ...
        url="/api-sou/bidExpert/listPage"
      />
    </el-main>
  </el-container>
</template>
  ...
  methods: {
    ...
    deleteExpertDatabase (row) {
      this.$confirm(this.$t('common.delRow'), {
        ...
        this.$http({
          url: `api-sou/bidExpert/delete/${row.expertId}`,
          method: 'DELETE',
          loading: true
        }).then(() => {
          // 删除成功
    ...
    validExpertDatabase (row) {
      this.$confirm(this.$t('dataConfMod.confirmationValid'), {
        ...
        this.$http({
          url: `/api-sou/bidExpert/valid/${row.expertId}`,
          method: 'POST',
          loading: true
        }).then(() => {
          // 失效成功
    ...
    invalidExpertDatabase (row) {
      ...
      this.$confirm(this.$t('bidMod.confirmInvalid'), {
        ...
        this.$http({
          url: `/api-sou/bidExpert/invalid/${row.expertId}`,
          method: 'POST',
          loading: true
        }).then(() => {
          // 失效成功
