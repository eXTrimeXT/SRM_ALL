...
export default {
  ...
  methods: {
    ...
    openBatchMaintainPaymentTypeDialog () {
      if (this.requirementLineList.length === 0) {
        // '请至少添加一行需求！'
        this.$message.warning(this.$t('competition.requirementLineListTip'))
        return
      }
    ...
    async saveRequirement (type) {
      ...
      if (!validateRequiredColumn(
        ...
        this.requirementLineList,
        {
          validateScope: true,
          // '需求明细'
          tableTitle: this.$t('bidMod.demandDetail')
        }
      )) {
        return { status: false }
      }
      ...
      if (this.purchasePaymentTermList.length === 0) {
        // '请批量维护付款条款'
        this.$message.warning(this.$t('competition.purchasePaymentTermListTip'))
        return { status: false }
      }
