    ...
    >
      ...
      >
        <template v-slot="scope">
          ...
          />
          <span v-else>{{ $parseTime(scope.row.requireDate) }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        prop="priceStartTime"
        :label="$t('contractMod.price_startDate')"
        min-width="150"
        show-overflow-tooltip
        :render-header="_addStarToColumn"
      >
        <template v-slot="scope">
          <el-date-picker
            ...
            :disabled="readonly"
          />
          <span v-else>{{ $parseTime(scope.row.priceStartTime) }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        prop="priceEndTime"
        :label="$t('dataConfMod.priceExpirationDate')"
        min-width="150"
        show-overflow-tooltip
        :render-header="_addStarToColumn"
      >
        <template v-slot="scope">
          <el-date-picker
            ...
            :disabled="readonly"
          />
          <span v-else>{{ $parseTime(scope.row.priceEndTime) }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        prop="warrantyPeriod"
        :label="$t('competition.warrantyPeriod')"
        width="150"
        :render-header="_addStarToColumn"
      >
        <template v-slot="scope">
...
export default {
  ...
  methods: {
    ...
    openBatchMaintainPaymentTypeDialog () {
      if (this.requirementLineList.length === 0) {
        this.$message.warning(this.$t('competition.requirementLineListTip'))  // '请至少添加一行需求！'
        return
      }
    ...
    itemCodeQuickSearchBeforeOpen (row, callback) {
      if (!row.orgOuId || !row.orgInvId) {
        this.$message.warning(this.$t('bidMod.selectOuAndOrgMsg'))  // '请先选择业务实体以及库存组织'
        callback(null)
      }
    },
    ...
    async saveRequirement (type) {
      ...
      if (!validateRequiredColumn(
        ...
        {
          validateScope: false,
          tableTitle: this.$t('bidMod.demandDetail'),  // '需求明细'
      ...
      for (let i = 0; i < this.requirementLineList.length; i++) {
        ...
        if (item.noCodeItem !== 'Y' && !item.itemCode) {
          this.$message.warning(`${this.$t('components.ocr.diTip')}${i + 1}${this.$t('competition.requirementIndexTip')}`)  // 行的物料编码不能为空
          return { status: false }
        }
      }
      ...
      if (this.purchasePaymentTermList.length === 0) {
        this.$message.warning(this.$t('competition.purchasePaymentTermListTip'))  // '请批量维护付款条款'
        return { status: false }
      }
