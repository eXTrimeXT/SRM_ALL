    ...
    detailData: {
      handler (val) {
        ...
        if (val && Array.isArray(val)) {
          const newVal = val.concat()
          this.innerFiles = newVal.filter(item => item.fileType === 'INNER')
          this.outerFiles = newVal.filter(item => item.fileType === 'OUTER')
        }
      },
      immediate: true,
      deep: true
  ...
  methods: {
    ...
    validateOuterFilesTable () {
      ...
      return validateRequiredColumn(
        ...
        {
          validateScope: false,
          tableTitle: this.$t('bidMod.supplierFileList')  // '供方查看附件'
        }
      )
    },
