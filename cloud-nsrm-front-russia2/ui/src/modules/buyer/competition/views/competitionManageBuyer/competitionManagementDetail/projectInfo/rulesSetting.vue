...
<template>
  <SrmRow>
    ...
    <SrmCol v-if="baseInfoData.auctRule === 'NO_ALLOW_SAME_PRICE'" :init-col="3">
      <el-form-item :label="$t('competition.noAllowSamePriceCount')" prop="auctSouProject.noAllowSamePriceCount">
    ...
    <template v-if="baseInfoData.allowExtendTime === 'Y'">
      ...
      <!--延时触发的最大次数-->
      <SrmCol :init-col="3">
        <el-form-item :label="$t('competition.extendTriggerCount')" prop="auctSouProject.extendTriggerCount">
          <el-input v-model="baseInfoData.extendTriggerCount" v-input-format="{type:'float',negative:false}" />
        </el-form-item>
      </SrmCol>
      <!--首次延时触发后最长延时时间限制 -->
      <SrmCol :init-col="3">
        <el-form-item :label="$t('competition.extendMaxMinute')" prop="auctSouProject.extendMaxMinute">
