<template>
  <el-container class="flex-container the_main_po_list" direction="vertical">
    <el-main>
      <CWorkflowMulti
        ref="workflowMultiOrder"
        v-model="activeTabName"
        :fun-params="workflowParamsInfo"
        :button-config-info="buttonConfigInfo"
        @tab-click="workflowView"
        @workflow-handler="workflowHandler"
        @click-handler="type => solveHandler(type)"
        @submit-direct="type => solveHandler(type)"
        @confirm="(type, comment) => solveHandler(type, comment)"
        @close-tab="cancelBill"
      >
        <!-- 进度条-未测试先注释-linyk7 -->
        <!-- <div class="form-container">
          <el-steps v-if="isFinish" :active="activeStep" finish-status="success" align-center>
            <el-step :title="$t('orderMod.waitPush')" />
            <el-step v-if="form.ceeaIfSupplierConfirm === 'Y'" :title="$t('orderMod.waitConfirmSupplier')" />
            <el-step :title="$t('orderMod.orderReception')" />
            <el-step v-if="form.orderStatus !== 'REFUSED'" :title="$t('vendorMod.closed')" />
            <el-step v-if="form.orderStatus === 'REFUSED'" :title="$t('sourcingBuyer.refused')" />
          </el-steps>
        </div> -->
        <el-form
          ref="detailRef"
        ...
        >
          <el-collapse v-model="activeDims" class="tab-form-style">
            ...
            <el-collapse-item :title="$t('orderMod.buyerOrderSynergy.orderDetailsList')" name="3">
              <div class="btn_line">
                ...
                <el-button
                  ...
                  :disabled="
                    isReadOnly ||
                      form.demandType === 'NONPRODUCTIVE_DEMAND' ||
                      ifsourceSystem === 'DEMAND'
                  "
                  class="detail-pbtn"
                  @click="openDialog"
                >
              ...
              <BaseInfor
                ...
                :index="false"
                :selection="true"
                @selection-change="handlePriceChange"
              ...
              >
                ...
                <template #ceeaPlanReceiveDate="{scope}">
                  <el-date-picker
                    ...
                    @change="datePickerChange(scope.$index, scope.row)"
                  />
                </template>
                ...
                <template #usedContractQuantity="{scope}">
                  <el-button
                    type="text"
                    :disabled="!scope.row.orderDetailId"
            ...
            <el-collapse-item :title="$t('orderMod.orderAttachInfo')" name="4">
              ...
              <upload-attach :attachData="orderFileList" :fileInfo="fileInfo" :readonly="isReadOnly" />
            </el-collapse-item>
    ...
    <CToolbar>
      <template slot="right">
        <el-button
          v-if="buttonConfigInfo.cancel.view"
          @click="cancelBill"
        >
          {{ $t('common.cancel') }}
        </el-button>
        ...
        <!-- <el-button
          ...
          @click="orderClose"
        >
          {{ $t('orderMod.orderClose') }}
        </el-button> -->
        <el-button
          v-if="buttonConfigInfo.close.view"
          @click="cancelBill"
        >
          {{ $t('orderMod.closePage') }}
        </el-button>
        <el-button
          v-if="buttonConfigInfo.save.view"
          type="primary"
        ...
        <el-button
          v-if="buttonConfigInfo.submit.view"
          type="primary"
    ...
    >
      ...
      <div slot="footer">
        ...
        <el-button type="primary" @click="handleCloseCause">
          {{ $t('common.confirm') }}
        </el-button>
      </div>
    </srm-dialog>
...
import { formItems, detailColumn, reportHeader } from './data/detail'
import purchaseDetail from './components/purchaseDetailSelect'
import materialDetail from './components/materialDetailSelect'
...
export default {
  ...
  computed: {
    ...
    workflowBusinessId () {
      // 用来指定工作流的业务ID
      return this.form.orderId ? this.form.orderId : null
    },
    // 不是审批按钮进来或者是送单号进来则禁用流程tab
    workflowTabDisabled () {
      return (
        this.$attrs.params.flag !== 'approvalOnly' || this.$attrs.params.flag === 'approveNumber'
      )
    },
    isReadOnly () {
      return this.$attrs.params.showType === 'readOnly'
    },
    // 订单关闭显示状态判断(已生效、待供方确认且行明细为接受)
    isClose () {
      let hasAccept = this.detailModelReal.some(row => ['ACCEPT', 'WAITING_VENDOR_CONFIRM'].includes(row.orderDetailStatus))
      return ['APPROVED', 'APPROVED_INVALID'].includes(this.$attrs.params.row.orderStatus) && hasAccept
    },
    // 进度条步骤
    // ['DRAFT', 'SUBMITTED', 'UNDER_APPROVAL', 'WITHDRAW', 'REJECT'], // 待发布:拟定、已提交、审批中、已撤回、已驳回（未进行供方确认的状态）
    // ['APPROVED_INVALID'], // 待供方确认：待供方确认，是否供方确认为是显示
    // ['APPROVED'], // 订单接收:已生效
    // ['CLOSED', 'ABANDONED'], // 已关闭：关闭、已作废
    // ['REFUSED'] // 已拒绝:：已拒绝，是否供方确认为是显示
    activeStep () {
      ...
      return mapStatus.get(this.form.orderStatus) || 0
    }
  },
  created () {
    this.initButton()
  ...
  methods: {
    ...
    async concatContract (row) {
      if (!row.orderDetailId) return this.$message.warning(this.$t('orderMod.saveBeforContractTip'))
      this.contractConcat.row = row
      this.contractConcat.params = await this.queryContractList(row)
      // 给每一行设置一个字段存储初始关联数量
      this.contractConcat.params.orderContractMappingList.forEach(row => {
        this.$set(row, 'oldCorrelatedQuantity', row.correlatedQuantity || 0)
      })
      this.concatSelectRow = row
    ...
    getCheckConcatObj () {
      return this.contractConcat.params.orderContractMappingList.map((row, i) => {
        ...
        if (row.isFrameworkAgreement === 'N') {
          ...
          // 修改后的关联数量 - 打开弹窗请求回的关联数量值 > 未关联
          if (row.correlatedQuantity - row.oldCorrelatedQuantity > row.unusedContractQuantity) checkNumber = [true, i + 1]
        }
        return {
          isZero: zeros, // 判断数量0
          isEmpty: emps, // 判空
    ...
    // 设置要求到货日期校验
    setDateValidate (rule, _value, callback = {}) {
      if (!_value) callback(new Error(this.$t('purchaseOrder.prompt1'))) // '请输入要求到货日期'

      const index = Number(rule.field.split('.')[1]) // 获取行号

      const sign = this.detailModelReal.some((row, i) => {
        return (
          index === i &&
          this.getTime(row.ceeaPlanReceiveDate) < this.getTime(this.form.ceeaPurchaseOrderDate)
        )
      })

      // 要求到货日期及供方承诺到货日期,应晚于创建订单日期！
      if (sign) callback(new Error(this.$t('orderMod.dateValidate')))
      callback()
    },
    // 选择需求类型为生产性需求则隐藏并清=空预算编号及id
    setBugetInfor (val) {
    ...
    addOneContent (multipleSelection) {
      if (multipleSelection.length === 0) return this.$message.error(this.$t('orderMod.msgOrder[9]'))

      const ids = this.detailModelReal.map(item => {
    ...
    addNewOne (selection) {
      ...
      const ids = this.detailModelReal.map(item => item['ceeaRequirementLineId'])
      const sign = this.setRepeatData(
        ids,
        this.detailModelReal,
        selection,
        'requirementLineId',
        row => {
          return {
            ceeaIfRequirement: 'Y',
            ceeaPromiseReceiveDate: row.requirementDate, // 订单创建时的承诺到货时间默认为需求日期 change byliwenhong
            ceeaTaxRate: row.taxRate,
            ceeaTaxKey: row.taxKey
          }
        },
      )
      !sign && (this.dialogFormVisible = false)
    },
    /**
     * @Description: 添加明细去重
     * @param data  表格数据
    ...
    setRepeatData (ids, data, selection, condition, lineSet) {
      ...
      data.push(...dataArr)
      // 设置行号
      data.forEach((row, i) => this.$set(row, 'lineNum', Number(i + 1)))
      // 已剔除重复勾选数据，明细行应唯一！
      if (isTip) return this.$message.warning(this.$t('orderMod.checkDataRowUnique'))
    },
    ...
    initButton () {
      ...
      this.buttonConfigInfo.save.view = isEdit // 保存按钮
      this.buttonConfigInfo.submit.view = isEdit // 提交按钮
      this.buttonConfigInfo.cancel.view = !this.isReadOnly // 返回按钮
      this.buttonConfigInfo.close.view = this.isReadOnly // 关闭按钮
    },
    async initDetail () {
      ...
      if (flag === 'add') {
        ...
        await this.queryDetails(orderId)
      }
      this.isFinish = true
    },
    ...
    // 收货地点选择
    changeSiteInfo (form, { element }) {
      this.$set(form, 'receiveContact', element.receiver)
      this.$set(form, 'receiveTelephone', element.receiverPhone)
      this.$set(form, 'receiveAddress', element.siteName)
    },
    // 预算编号快查关闭回写值
    getBudgetNumObj (val) {
    ...
    handleCloseCause () {
      ...
      // line:明细行关闭；complate:整单关闭
      if (this.closeMethods === 'line') {
        const ids = this.selectOrderDetail.map(item => item.orderDetailId)
        this.$http({
          ...
          this.queryDetails(this.form.orderId)
        })
      } else {
        this.$http({
          url: '/api-sup-ce/order/order/closeOrder',
          method: 'POST',
          data: {
            closedCause: this.closedCause,
            orderId: this.form.orderId
    ...
    async orderClose () {
      ...
      this.closeMethods = 'complate'
    },
    handlePriceChange (select) {
      this.selectOrderDetail = select
    },
    // 订单明细关闭
    closeOrderList () {
      if (this.selectOrderDetail.length < 1) {
        // 请选择需关闭行数据！
        return this.$message.warning(this.$t('purchaseDemand.selectClosed'))
      }

      const sign = this.selectOrderDetail.every(item => ['ACCEPT', 'WAITING_VENDOR_CONFIRM'].includes(item.orderDetailStatus))
      if (!sign) {
        // 请选择接受状态的订单行进行关闭！
        this.$message.warning(this.$t('purchaseDemand.selectAccept'))
        return
      }
      this.closeVisibleDialog = true
      this.closeMethods = 'line'
    },
    datePickerChange (index, row) {
      let moment = this.detailModelReal[index]
      this.$set(row, 'ceeaPromiseReceiveDate', row.ceeaPlanReceiveDate)
      this.$set(this.detailModelReal, index, moment)
    },
    async getWorkflowBusinessVariables () {
      return {
        Amount: this.form.ceeaTaxAmount
      }
    },
    ...
    // 获取工作流的tabName
    getCWorkflowRefName () {
      return 'workflowMultiOrder' // 对应CWorkflowMulti标签中的ref
    },
    ...
    async getConcatInfo (scope, companyId) {
      ...
      scope.ceeaSupplierContacts = data.contactName
      scope.ceeaReceiveOrderTelephone = data.ceeaContactMethod
    },
    ...
    setRowAmount (row) {
      console.log(row.orderNum, 'orderNum')
    ...
    // 校验付款条款
    checkPaymentEmpty () {
      let displayMaterialItem2Watch = false
      this.paymentInfor.forEach(item => {
        if (
          item.paymentStage &&
          item.paymentTerm &&
          item.paymentPeriod &&
          ...
          displayMaterialItem2Watch = true
        }
      })
      return displayMaterialItem2Watch
    },
    async submitBill (type) {
      // 校验付款条款 - 请先维护付款条款
      // if (!this.checkPaymentEmpty()) return this.$message.warning(this.$t('orderMod.msgOrder[14]'))
      let fileArr = this.orderFileList.filter(item => !!item.fileuploadId)
      this.$http({
        url: '/api-sup-ce/po/order/submitOrder',
        method: 'POST',
        data: {
          order: { ...this.form },
          paymentProvisionList: this.paymentInfor,
          attachList: fileArr,
          detailList: this.detailModelReal.map(item => ({
            ...item,
    ...
    async solveHandler (type) {
      ...
      // 验证form表单
      if (flag) {
        if (!this.detailModelReal.length) {
          this.$message({
            type: 'error',
        ...
        // 2023.04.09新增校验
        if (this.detailModelReal.find(item => item.orderNum < item.usedContractQuantity)) {
          this.$message({
            type: 'error',
            message: this.$t('purchaseOrder.prompt2') // '订单行数量不能小于合同已关联数量，请检查！'
          })
          return
        }
    ...
    saveBill (_type) {
      ...
      if (this.$attrs.params.flag === 'edit') {
        purchaseOrderApi.save(params).then(async res => {
          this.$message({
            type: 'success',
            message: res.message
          })
          await this.queryDetails(res.data)
          if (this.form.orderStatus === 'RETURNED') {
            purchaseOrderApi.publish(params).then(data => {
              this.$message({
                type: 'success',
    ...
    openDialog (_row) {
      ...
      this.$set(this.queryForm, 'orgId', this.form.ceeaOrgId)
