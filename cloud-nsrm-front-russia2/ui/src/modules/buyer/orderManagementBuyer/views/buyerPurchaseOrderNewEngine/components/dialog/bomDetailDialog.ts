...
export default {
  ...
  properties: {
      ...
      properties: {
        bomDetailList: {
          ...
          properties: generateXindexInOrder({
            ...
            componentOrderNum: {
              type: 'string',
              title: i18nExpression('purchaseApplication.componentOrderNum'), // 组件订单数量
              'x-render-table-column': {
                minWidth: 120
              }
            }
    ...
    pagination: {
      ...
      'x-component-props': {
        pageNum: expression('$form.query(\'Order\').get(\'data\').bomDetailListPageNum'),
        pageSize: expression('$form.query(\'Order\').get(\'data\').bomDetailListPageSize'),
        total: expression('$form.query(\'Order\').get(\'data\').bomDetailListTotal'),
        pageSizes: [5, 15, 30, 60, 120, 300, 600, 1000, 1500],
        '@current-change': expression(`(num) => {
                $form.query('Order').get('data').bomDetailListPageNum = num
                $getBomDetailListList($form)
              }`),
        '@size-change': expression(`(size) => {
                $form.query('Order').get('data').bomDetailListPageSize = size
                $getBomDetailListList($form)
              }`)
      }
    }
