...
export default {
  ...
  computed: {
    ...
    approvalHeaderDataRules () {
      let rules = {
        // '请输入标题'
        approvalTitle: [{ required: true, message: this.$t('dataConfMod.msgNoticeTitle') }]
      }
      if (this.pageType.isHandMake) {
        ...
        rules = {
          ...rules,
          // '请选择决标方式'
          awareWay: [{ required: true, message: this.$t('bidMod.bidMsgList.8') }],
          // '请选择是否更新到价格库'
          ifUpdatePriceLibrary: [{ required: true, message: this.$t('bidMod.chooseUpdatePriceLibrary') }],
          // '请选择币种'
          standardCurrency: [{ required: true, message: this.$t('vendorMod.msgCurrencyCode') }],
          // '请选择价格精度'
          priceNum: [{ required: true, message: this.$t('bidMod.selectPricePrecision') }],
          // '请选择汇率类型'
