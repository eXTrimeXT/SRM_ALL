      ...
      >
        ...
        >
          <el-row>
            ...
            <el-col v-if="configForm.pwdCreated.DEFAULT.switcher=='RANDOM'">
              ...
              <!-- 密码生成字段 -->
              <el-form-item
                :label="$t('cusEntry.supplement20250211.passwordGenerationField')"
                label-width="108px"
              >
                <div>
                  <el-checkbox
            ...
            <el-col v-if="configForm.pwdCreated.DEFAULT.switcher=='SOLID'">
              ...
              >
                ...
                <div>
                  <!-- 允许特殊字符有 `~!@#$%^&*()-_=+[{}];:'",?. -->
                  {{ $t("cusEntry.supplement20250211.allowSpecialChars") }}
                </div>
              </el-form-item>
            </el-col>
            <!-- 属性值 -->
            <el-col v-if="configForm.pwdCreated.DEFAULT.switcher=='ATTR_VALUE'">
              <div>
                <!-- 开发中... -->
                {{ $t("cusEntry.supplement20250211.developmentInProgress") }}
              </div>
            </el-col>
          </el-row>
        </el-collapse-item>
        ...
        >
          <el-row>
            <el-col>
              <!-- 密码校验字段 -->
              <el-form-item label-width="108px" :label="$t('cusEntry.supplement20250211.passwordValidationField')">
                <el-checkbox
...
export default {
  ...
  data () {
    return {
      ...
      notContainList: [
        {
          label: this.$t('vendorMod.account'), // 账号
          value: 'username-total',
          objValue: { username: { total: 'Y' } }
        },
        {
          label: this.$t('cusEntry.supplement20250211.accountReverseOrder'),  // '账号倒序'
          value: 'username-desc',
  ...
  computed: {
    ...
    sourceTypeList () {
      if (this.userType == 'BUYER') {
        return [
          { value: 'DEFAULT', label: this.$t('cusEntry.supplement20250211.purchaseInitializer') },  // '采购商初始化'
          { value: 'REGISTER', label: this.$t('cusEntry.supplement20250211.newSupplierAccount') }  // '新建采购商账号'
        ]
      } else {
        return [
          { value: 'DEFAULT', label: this.$t('cusEntry.supplement20250211.supplierInitialization') },  // '供应商初始化'
          { value: 'REGISTER', label: this.$t('route.registered') },  // '供应商注册'
          { value: 'VENDOR_GREEN', label: this.$t('route.vendorGreenChannel') }, // '供应商绿色通道'
          { value: 'VENDOR_GREEN1', label: this.$t('cusEntry.supplement20250211.subAccountSource') },  // '子账号来源'
          { value: 'VENDOR_OPENAPI', label: this.$t('cusEntry.supplement20250211.supplierOpenInterface') }  // '供应商开放接口'
        ]
      }
    }
  },
  watch: {
    ...
    configForm: {
      ...
      handler (nVal, oVal) {
        this.isEditFlag += 1
        this.$emit('isChange', this.isEditFlag)
      }
    }
  },
  async created () {
    ...
    async saveConfigFn () {
      ...
      if (this.configForm.pwdCreated.DEFAULT.switcher == 'SOLID') {
        if (!this.configForm.pwdCreated.DEFAULT.value) {
          return this.$message.error(this.$t('cusEntry.supplement20250211.addFixedPassword')) // '请添加添加固定密码'
        } else {
          let patrn = validPatrn
          if (!patrn.test(this.configForm.pwdCreated.DEFAULT.value)) {
            return this.$message.error(this.$t('cusEntry.supplement20250211.fixedPasswordCharacters')) // '固定密码只能是数字、小写字母、大写字母、特殊字符 `~!@#$%^&*()-_=+[{}];:\'",?.'
          }
        }
      } else {
        if (!this.configForm.pwdCreated.DEFAULT.length) {
          return this.$message.error(this.$t('cusEntry.supplement20250211.passwordLength'))  // '请输入密码长度'
        }
        let ruleCount = 0
        ...
        if (ruleCount == 0) {
          return this.$message.error(this.$t('cusEntry.supplement20250211.passwordGenerationStrategyRandomCharactersAtLeastOneOfEachType')) // '密码生成策略的随机字符至少包含数字、小写字母、大写字母、特殊字符其中一种'
        }
      }
      // 密码校验
      // 密码校验长度范围
      if (this.configForm.pwdCheck.minLength > this.configForm.pwdCheck.maxLength) {
        return this.$message.error(this.$t('cusEntry.supplement20250211.passwordLengthRangeLeftValueNotGreaterThanRightValue')) // '密码长度范围左边数值不能大于右边数值'
      }
