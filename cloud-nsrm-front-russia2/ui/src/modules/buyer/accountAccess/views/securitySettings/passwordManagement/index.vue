<template>
  <div class="password-management">
    <div class="password-management-header">
      <span :class="{'is-active': rolesActiveName=='BUYER'}" @click="handleClick('BUYER',false)">
        <!-- 采购商 -->
        {{ $t("common.buyer") }}
      </span>
      <span :class="{'is-active': rolesActiveName=='VENDOR'}" @click="handleClick('VENDOR',false)">
        <!-- 供应商 -->
        {{ $t("common.vendor") }}
      </span>
    </div>
  ...
  methods: {
    handleClick (userType, loading = false) {
      ...
      if (buyerT > 1 || vendorT > 1) {
        // 请确认修改的配置已经保存！
        this.$confirm(this.$t('cusEntry.supplement20250211.confirmModifiedConfigSaved'), {
