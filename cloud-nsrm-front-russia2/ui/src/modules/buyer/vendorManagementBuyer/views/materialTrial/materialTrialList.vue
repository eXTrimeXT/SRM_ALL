...
import sampleConfirmedDetail from 'modb@/vendorManagementBuyer/views/sampleConfirmed/sampleConfirmedDetail'
import { parseTime } from '@/utils'
import { materialTrialApi } from 'modb@/vendorManagementBuyer/api/vendorManagement'
    ...
    $route: {
      ...
      handler () {
        if (
          ...
          this.$route.params.funName === 'materialTrial'
        ) {
  ...
  created () {
    ...
    this.tableHeader = [
      ...
      },
      {
        ...
        callback: function (row) {
          if (!row.reviewFormNumber) return
      ...
      },
      {
        ...
        buttons: [
          ...
          },
          {
            ...
            show: function (row) {
              if (['SUBMITTED', 'WITHDRAW', 'REJECTED'].includes(row.approveStatus) && _this.curRole === 'BUYER') {
                return true
              } else {
                return false
              }
  ...
  methods: {
    ...
    delRowData (row) {
      ...
      this.$confirm(this.$t('common.confirmDelete'), {
        ...
        .then(() => {
          materialTrialApi.materialTrialOrderDel([id]).then((res) => {
            this.$message({
              message: res.message,
              type: 'success'
            })
