          ...
          <el-collapse v-model="colValue">
            ...
            <el-collapse-item :title="$t('accountMod.relevantAttachment')" name="4">
              ...
              <el-table class="mt-10" :data="fileData" border stripe>
                ...
                <el-table-column prop="creationDate" :label="$t('components.fileupload.uploadDate')" />
  ...
  data () {
    return {
      ...
      rules: {
        ...
        purchaseOrgId: [{ required: true, message: this.$t('vendorMod.purchaseOrgId') }],  // '请输入业务实体'
        invId: [{ required: true, message: this.$t('vendorMod.invId') }],  // '请输入库存组织'
        updatedReason: [{ required: true, message: this.$t('dataConfMod.enterUpdateReason') }]  // '请输入更新原因'
      },
      fileData: [],
      fileInfo: {
        fileModular: 'sup',
    ...
    async saveBill (type) {
      ...
      if (type === 'SAVE') {
        saveMethods(params).then((res) => {
          ...
          if (res.data) this.getChangeFormDetail(res.data)
        })
      } else if (type === 'SUBMIT') {
        let validForm
