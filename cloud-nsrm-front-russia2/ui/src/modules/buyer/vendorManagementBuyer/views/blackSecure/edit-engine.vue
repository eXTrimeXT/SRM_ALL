  ...
  async (type: any,$form: any, $queryEngine: any, $message: any, $bus: any, ) => {
    ...
    if (type === 'SAVE') {
      ...
      $form
        .validate()
        .then(() => {
          // '提交审批后，供应商还是正式黑名单状态，审批通过后，供应商进入合格供应商，原来的组织及品类更新为原来的合格状态，不需重新引入。'
          let tips = $t('vendorMod.blackListTips2')
          // '是否确认提交'
          app.$confirm(tips, $t('cusEntry.supplement20250211.isConfirmSubmit'), {
              ...
              properties: {
                ...
                blackRescindCompanyList: {
                  ...
                  properties: generateXindexInOrder({
                    ...
                    lcCode: {
                      type: 'string',
                      title: i18nExpression('cusEntry.supplement20250211.creditCode'), // 统一信用代码
                      'x-render-table-column': {
                        minWidth: 150
                      }
                    },
                    legalPerson: {
                      type: 'string',
                      title: i18nExpression('vendorMod.corporateRepresentative'), // 法人代表
                      'x-render-table-column': {
                        minWidth: 150
                      }
                    },
                    ...
                    agent: {
                      type: 'string',
                      title: i18nExpression('vendorMod.managerAgent'), // 经办人
                      'x-render-table-column': {
                        minWidth: 150
                      }
                    },
                    endDate: {
                      title: i18nExpression('supplierRating.blacklistDeadline'), // 黑名单截止日期
                      ...yearMonthDaySelectorSegment,
                      'x-component-props': {
                        ...yearMonthDaySelectorSegment['x-component-props'],
...
<template>
  <RenderEngine :pageAttrs="$attrs" :schema="schema" :scope="scope" :components="components" schemaKey="BlackDetail"/>
</template>
