  ...
  <div class="technology-score-detail">
    ...
    <div class="display-form-content">
      <SrmRow>
        <!--项目编号-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.bidingNum') }}</span>{{ biddingData.souNo }}
        </SrmCol>
        <!--项目名称-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.bidingName') }}</span>{{ biddingData.souName }}
        </SrmCol>
        <!--发布时间-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.releaseDatetime') }}</span>{{ $parseTime(biddingData.publishTime) }}
        </SrmCol>
        <!--投标截止时间-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.enrollEndDatetime') }}</span>{{ $parseTime(biddingData.orderEndTime) }}
        </SrmCol>
        <!--编制人-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.createdBy2') }}</span>{{ biddingData.createdUserName }}
        </SrmCol>
        <!--投标开始时间-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.bidingStartDatetime') }}</span>{{ $parseTime(biddingData.orderStartTime) }}
        </SrmCol>
        <!--预计投标地点-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.bidingSite') }}</span>{{ biddingData.orderSite }}
        </SrmCol>
        <!--预算金额-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.budgetAmount') }}</span>{{ biddingData.budgetAmount }}
        </SrmCol>
        <!--其他说明-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.bondDesc') }}</span>{{ biddingData.bondDesc }}
        </SrmCol>
      </SrmRow>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getTechProgressReviewDetail () {
      ...
      const response = await bidBuyerHttp.tech.techProgressReviewDetail(this.projectId)
