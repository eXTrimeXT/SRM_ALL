  ...
  >
    <el-header height="80">
      <el-steps :active="active">
        <!-- 选择数据源 -->
        <el-step
          :title="$t('cusEntry.supplement20250211.selectDataSource')"
          :description="$t('cusEntry.supplement20250211.specifiedModuleAndTable')"
        />
        <!-- 基本信息、 配置表单和列表页基本信息 -->
        <el-step
          :title="$t('vendorMod.basicInformation')"
          :description="$t('cusEntry.supplement20250211.configFormAndListPageBasicInfo')"
        />
        <!-- 组件选择 \ 指定是否需要通用组件 -->
        <el-step
          v-if="!isAPI"
          :title="$t('cusEntry.supplement20250211.componentSelection')"
          :description="$t('cusEntry.supplement20250211.needCommonComponent')"
        />
        <!-- 接口配置 \ 指定增删查改接口和请求方式 -->
        <el-step
          v-if="!isAPI"
          :title="$t('route.interfaceconfig')"
          :description="$t('cusEntry.supplement20250211.apiEndpoints')"
        />
        <!-- 生成代码 \ 生成结果下载 -->
        <el-step
          :title="$t('cusEntry.supplement20250211.generateCode')"
          :description="$t('cusEntry.supplement20250211.resultDownload')"
        />
      </el-steps>
    </el-header>
        ...
        <transition-group name="el-fade-in-linear">
            ...
            <div class="source-wrapper">
              ...
              <!-- 模块 -->
                ...
                <template v-if="tableCount === '0'">
                  ...
                  <el-form-item
                    prop="database"
                    :label="$t('cusEntry.supplement20250211.tableName')"
                  >
                    <DictSelect
                ...
                <el-form-item
                  v-if="!isAPI"
                  prop="pageName"
                  :label="$t('cusEntry.supplement20250211.frontEndPageName')"
                >
            ...
            <div class="fields-wrapper">
              ...
              <template v-if="tableCount === '1'">
                <el-collapse v-model="activeCollapse">
                  <!-- 头表 -->
                  <el-collapse-item
                    :title="`${$t('cusEntry.supplement20250211.headTable')} ${source.headerDatabase}`"
                    :label="databaseLabel.header"
                    name="1"
                  >
                    <!-- 头表 -->
                  ...
                  <el-collapse-item
                    ...
                    :key="key"
                    :title="`${$t('common.rowList')} ${lineData.table.name}`"
                    :label="databaseLabel.line"
                    :name="`${key+2}`"
                  >
                    <!-- 行表 -->
          ...
          >
            <div class="component-wrapper">
              ...
              <!-- 导入组件 -->
                <el-form-item
                  prop="importComp"
                  :label="$t('cusEntry.supplement20250211.importComponent')"
                >
                  <el-switch
                ...
                <el-form-item
                  prop="exportComp"
                  :label="$t('cusEntry.supplement20250211.exportComponent')"
                >
                  <el-switch
          ...
          >
            <div class="api-wrapper">
              ...
              <!-- 新增 -->
                ...
                <el-form-item
                  prop="delete"
                  :label="$t('components.common.delete')"
                >
                ...
                <el-form-item
                  prop="query"
                  :label="$t('components.common.search')"
                >
          ...
          >
            <div class="generate-wrapper">
              ...
              >
                <!-- 下载代码 -->
                {{ $t("cusEntry.supplement20250211.downloadCode") }}
              </el-button>
    ...
    <CToolbar>
      <template #right>
        ...
        >
          <!-- 上一步 -->
          {{ $t("components.homepage.previous") }}
        </el-button>
...
export default {
  ...
  data () {
    return {
      ...
      sourceRules: {
        module: [{ required: true, message: this.$t('cusEntry.supplement20250211.moduleRequired') }],  // '模块必选'
        pageName: [{ required: true, message: this.$t('cusEntry.supplement20250211.frontEndPageNameRequired') }],  // '前端页面名称必填'
        database: [{ required: true, message: this.$t('cusEntry.supplement20250211.tableNameRequired') }],  // '表名必填'
        headerDatabase: [{ required: true, message: this.$t('cusEntry.supplement20250211.headTableNameRequired') }],  // '头表表名必填'
        lineDatabase: [{ required: true, message: this.$t('cusEntry.supplement20250211.tableNameRequired2') }]  // '行表表名必填'
      }
    }
  },
  computed: {
    ...
    databaseLabel () {
      if (this.baseData.generateType === '0') {
        return {
          header: this.$t('cusEntry.supplement20250211.headTable'),  // '头表'
          line: this.$t('common.rowList')  // '行表'
        }
      } else {
        return {
          header: this.$t('cusEntry.supplement20250211.tempTable'),  // '临时表'
          line: this.$t('cusEntry.supplement20250211.businessTable')  // '业务表'
        }
      }
    },
    showNext () {
  ...
  methods: {
    ...
    formatOfYesOrNo (row, column, cellValue) {
      return cellValue === 'N' ? this.$t('common.no') : this.$t('common.yes')
    },
    close () {
      this.$emit('close')
    },
      ...
      return new Promise(rs => {
        if (this.$refs.api) {
          ...
          this.$refs.api.validate(boolean => {
            if (!boolean) {
              this.focusError()
              this.$message.error(this.$t('cusEntry.supplement20250211.checkRequiredFields'))  // '请检查必填项'
            }
            rs(boolean)
          })
        }
        ...
        if (this.$refs.source) {
          ...
          this.$refs.source.validate(boolean => {
            if (!boolean) {
              ...
              this.$message.error(this.$t('cusEntry.supplement20250211.tableNameNotStandardPleaseCheck'))  // '表名不规范，请检查'
              rs(false)
            } else {
              rs(true)
              this.queryDatabase()
