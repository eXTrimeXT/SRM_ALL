...
export default {
      ...
      modelElementColumns: [
        ...
        // 基价币种
        {
          attrs: {
            align: 'center',
            minWidth: '140',
            label: () => this.$t('priceModel.costElement.currencyCode'),  // '基价币种'
            prop: 'currencyCode'
          },
          slot: 'currencyCode'
        },
        ...
        // 本位币基价 = 基价（不含税）* 汇率
        {
          attrs: {
            align: 'center',
            minWidth: '100',
            label: () => this.$t('priceModel.costElement.currencyBasePrice'),  // '本位币基价'
      ...
      // 币种
      {
        ...
        itemAttrs: {
          ...
          rules: [
            {
              required: true,
              message: this.$t('vendorMod.msgCurrencyCode'),  // '请选择币种'
              type: 'string'
            }
          ]
        },
      ...
      // 汇率类型
      {
        ...
        itemAttrs: {
          ...
          rules: [
            {
              required: true,
              message: this.$t('dataConfMod.rateTypeRequired'),  // '请选择汇率类型'
              type: 'string'
            }
          ]
        },
      ...
      // 模型名称
      {
        itemAttrs: {
          ...
          rules: [
            {
              required: true,
              message: this.$t('priceModel.priceModel.selectPriceModelName'),  // '请选择模型'
              type: 'string'
            }
          ]
        },
  ...
  methods: {
    ...
    async queryPurchaseExchangeRate (fromCodes) {
      if (!this.mergeForm.clearCurrency) {
        // '请维护基础信息币种数据'
        this.$message.warning(this.$t('priceModel.priceModel.clearCurrencyEdit'))
        return { status: false }
      }
      if (!this.mergeForm.rateType) {
        // '请维护基础信息汇率类型数据'
        this.$message.warning(this.$t('priceModel.priceModel.rateTypeEdit'))
        return { status: false }
      }
      ...
      if (data && data.data && Array.isArray(data.data)) {
        if (data.data.length === 0 && paramData.fromCurrencyCodeList.length === 1) {
          // 只查一个时候查不到给予提示
          this.$message.warning(`$s{this.$t('cusEntry.supplement20250211.configPlease')}${paramData.fromCurrencyCodeList[0]} TO ${paramData.toCurrencyCode} 汇率类型为${this.$getDictLabel('EXCHANGE_RATE_TYPE', this.mergeForm.rateType)}的汇率!`)
          return { status: false }
        }
        return {
          status: true,
