...
export default {
  ...
  created () {
    ...
    accessFlowSetting.listPage(objs).then(res => {
      const list = res.data.list
      let attr = []
      list.forEach(element => {
        const obj = {
          name: element.modelName,
          value: element.modelHeadId
        }
        attr.push(obj)
      })
      this.modelSelect = attr
    })
    this.queryForm = [
      {
        prop: 'quaReviewType',
    ...
    this.tableHeader = [
      ...
      {
        prop: 'node',
        label: () => this.$t('common.node'), // 节点
        minWidth: 110,
        showType: 'button',
        btnStyle: 'text',
        callback: row => this.nodeFun(row),
        show: row => row.isNoShow !== 'Y',
        // 维护节点
        formattor: () => this.$t('common.nodeEdit')
      },
      {
  ...
  methods: {
    ...
    listChange (row) {
      row.ifRepush = 1
      this.$refs.accessTableList.$refs.tableGrid.toggleRowSelection(row, true)
    },
    save () {
      let bol = false
    ...
    fatchDictData () {
      ...
      getDictItemList(dictParamsArr).then(res => {
        ...
        this.entryFileType = adaptDictData(ENTRY_FILE_TYPE.ENTRY_FILE_TYPE)
      })
    },
    // 删除合同定义其中一条数据
    ...
    handleUploadSuccess (file, row) {
      ...
      row.dotemplateFileIdcId = fileId.toString()
