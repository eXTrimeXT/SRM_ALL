      ...
      >
        ...
        >
          ...
          <el-table-column
            ...
            prop="templateDesc"
            :label="$t('dataConfMod.keyConfirmElement')"
            min-width="150"
            :show-overflow-tooltip="true"
          >
            <template slot-scope="scope">
  ...
  created () {
    ...
    this.tableHeader = [
      ...
      },
      {
        ...
        code: 'accessFlowSetting:fileSetting'
      },
      // {
      //   prop: 'ceeaCategoryName',
  ...
  methods: {
    ...
    addTableSelection (row) {
      ...
      if (this.isSampleTableSelection(row)) {
        if (row?.addId) {
          const index = this.tableSelection.findIndex(item => item.addId === row.addId)
          if (index == -1) {
    ...
    addNew () {
      this.$refs.accessTableList.addOneEditableColumn({ isNoShow: 'Y', addId: this.addId })
      const addId = this.addId
      this.addId = addId + 1
    },
    // 选中
