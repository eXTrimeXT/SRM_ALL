      ...
      <el-collapse v-model="activeDims">
        ...
        <el-collapse-item v-show="tabVal === 'tab2'" :title="$t('meeting.issueManagement')" name="3">
          ...
          <el-table class="mt-10" :data="materialFileData" border stripe>
            ...
            <el-table-column prop="creationDate" :label="$t('common.uploadDate')" :formatter="(row, column, cellValue) => $parseTime(cellValue)" />
          </el-table>
          <p ref="waitMake">
    ...
    async goToRelation (scope) { // 跳转关联单号
      ...
      if (row.topicType === 'SS') {
        ...
        tab = {
          ...
          // ss上会材料
          title: `${this.$t('cusEntry.supplement20250211.ssMeetingMaterials')}${this.form.meetingCode || ''}`,
          name: `ssMaterials${row.relationBillCode}`
        }
        ...
        tab = {
          ...
          // sd上会材料
          title: `${this.$t('cusEntry.supplement20250211.sdMeetingMaterials')}${this.form.meetingCode || ''}`,
          name: `sdMaterials${row.relationBillCode}`
        }
