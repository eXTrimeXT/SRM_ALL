  ...
  methods: {
    ...
    addRequirementInfo () {
      if (this.sourcingForm.categoryName) {
        ...
        // '请先选择物料品类'
        this.$message.error(this.$t('cusEntry.supplement20250211.chooseMaterialCategory'))
      }
    },
    async beforeUpload () {
    ...
    categoryDownloadTemplate () {
      downloadFileLink(
        ...
        // '物料导入模板下载.xlsx',
        this.$t('cusEntry.supplement20250211.fileDownloadTemplate'),
      ).catch(err => {
    ...
    getItemObj (val, scope) {
      ...
      if (val) {
        ...
        if (hasRow) {
          // '这个物料已经存在！'
          this.$message.warning(this.$t('cusEntry.supplement20250211.existingMaterial'))
          scope.materialCode = ''
          return false
        } else {
