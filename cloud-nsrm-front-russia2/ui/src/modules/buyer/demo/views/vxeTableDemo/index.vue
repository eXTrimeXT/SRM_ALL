<template>
  <div style="padding: 0 10px 10px 10px; margin: 10px;">
    <el-tabs v-model="activeName">
      <!-- 上传组件 -->
      <el-tab-pane
        :label="$t('cusEntry.supplement20250211.uploadComponent')"
        name="uploadFile"
        lazy
      >
        <UploadFileCommon />
      ...
      <!-- 倒计时 -->
      <el-tab-pane
        :label="$t('cusEntry.supplement20250211.countdownTimer')"
        name="first"
        lazy
      >
        <div>
          <h2>
            <!-- 1. 截止时间与当前时间对比 [2022-04-22 00:00:00] -->
            {{ $t("cusEntry.supplement20250211.endTimeComparison") }}
          </h2>
          <!-- 距离截止还剩余: -->
          <DynamicCutoffTime
            :label="$t('cusEntry.supplement20250211.distanceToDeadline')"
          ...
          <h2>
            <!-- 2. 截止时间与当前时间对比 只统计天 [2022-04-22 00:00:00] -->
            {{ $t("cusEntry.supplement20250211.endTimeComparisonDays") }}
          </h2>
          <!-- 距离截止还剩余: -->
          <DynamicCutoffTime
            :label="$t('cusEntry.supplement20250211.distanceToDeadline')"
          ...
          <h2>
            <!-- 3. 截止时间与自定义时间对比 非动态 [2022-01-22 00:00:00 - 2022-04-22 00:00:00] -->
            {{ $t("cusEntry.supplement20250211.customTimeComparisonEnd") }}
          </h2>
          <!-- 距离截止还剩余: -->
          <DynamicCutoffTime
            label="$t('cusEntry.supplement20250211.distanceToDeadline')"
          ...
          <h2>
            <!-- 4. 截止时间与自定义时间对比 只统计年 非动态 [2020-01-22 00:00:00 - 2022-04-22 00:00:00] -->
            {{ $t("cusEntry.supplement20250211.endTimeComparisonWithCustomYearStats") }}
          </h2>
          <!-- 距离截止还剩余: -->
          <DynamicCutoffTime
            :label="$t('cusEntry.supplement20250211.distanceToDeadline')"
          ...
          <h2>
            <!-- 5. 剩余时间 824281008/毫秒 -->
            {{ $t("cusEntry.supplement20250211.remainingTimeInMilliseconds") }}
          </h2>
          <!-- 报名截止还剩余: -->
          <DynamicCutoffTime
            :label="$t('cusEntry.supplement20250211.remainingRegistrationTime')"
          ...
          <h2>
            <!-- 6. 剩余时间 824281008/毫秒 只统计天 -->
            {{ $t("cusEntry.supplement20250211.remainingTimeDays") }}
          </h2>
          <!-- 报名截止还剩余: -->
          <DynamicCutoffTime
            :label="$t('cusEntry.supplement20250211.remainingRegistrationTime')"
          ...
          <h2>
            <!-- 7. 剩余时间 824281008/毫秒 只统计小时 -->
            {{ $t("cusEntry.supplement20250211.remainingTimeInHours") }}
          </h2>
          <!-- 报名截止还剩余: -->
          <DynamicCutoffTime
            :label="$t('cusEntry.supplement20250211.remainingRegistrationTime')"
          ...
          <p>
            <el-button type="primary" @click="dynamicCutoffTimeDiffToString('2022-04-30 00:00:00')">
              ...
              {{ $t("cusEntry.supplement20250211.clickCalculateCountdown") }}
            </el-button>
          </p>
        </div>
      </el-tab-pane>
      ...
      >
        <h2>
          <!-- number 数字 -->
          {{ $t("cusEntry.supplement20250211.numberDigit") }}
        </h2>
        <el-input
          v-model="inputValue1"
        ...
        <h2>
          <!-- integer 整数 -->
          {{ $t("cusEntry.supplement20250211.integerNumber") }}
        </h2>
        <el-input
          v-model="inputValue2"
        ...
        <h2>
          <!-- float 浮点数 -->
          {{ $t("cusEntry.supplement20250211.floatPointNumber") }}
        </h2>
        <el-input
          v-model="inputValue3"
        ...
        <h2>
          <!-- thousandth 数值分位 -->
          {{ $t("cusEntry.supplement20250211.thousandthDigitPlace") }}
        </h2>
        <el-input
          v-model="inputValue4"
        ...
        <h2>
          <!-- filterZhCn 过滤中文 -->
          {{ $t("cusEntry.supplement20250211.filterZhCn") }}
        </h2>
        <el-input
          v-model="inputValue5"
      ...
      <!--vxe table大数据表格-->
      <el-tab-pane
        :label="$t('cusEntry.supplement20250211.vxeTableBigDataGrid')"
        name="three"
        lazy
      >
        <div>
          <vxe-toolbar>
            <template #buttons>
              <el-button @click="insertEvent(-1)">
                <!-- 在最后行插入 -->
                {{ $t("cusEntry.supplement20250211.insertAtEndLine") }}
              </el-button>
            </template>
          </vxe-toolbar>
          ...
          >
            ...
            >
              ...
              <template #default="{ row }">
                {{ parseTime(row.date1, '{y}-{m}-{d} {h}:{i}:{s}') }}
              </template>
            </vxe-column>
            <vxe-column
              align="center"
...
export default {
  ...
  data () {
    return {
      ...
      regionList: [],
      restaurants: [
        { value: this.$t('cusEntry.supplement20250211.frontend'), name: this.$t('cusEntry.supplement20250211.frontend') },
        { value: this.$t('cusEntry.supplement20250211.backend'), name: this.$t('cusEntry.supplement20250211.backend') }
      ],
      priceNodes: {},
  ...
  methods: {
    dynamicCutoffTimeDiffToString (...ret) {
      ...
      this.$alert(result.status ? result.value : this.$t('cusEntry.supplement20250211.englishcalculationError'))  // '计算错误'
    },
    categoryNameChange () {
      console.log('!-----')
    },
