...
export default {
  ...
  properties: {
    formWrapper: {
      ...
      'x-component-props': {
        colLength: 2,
        'form-array': expression('$form.query(\'PaymentApply\').get(\'data\').advancePaymentDialogQueryForm'),
    ...
    advancePaymentTable: {
      ...
      properties: generateXindexInOrder({
        ...
        stayPaymentAmount: {
          type: 'string',
          title: i18nExpression('contract_mod.paymentAmount'), // 代付款金额
          'x-render-table-column': {
            minWidth: 100
          }
        },
    ...
    pagination: {
      ...
      'x-component-props': {
        pageNum: expression('$form.query(\'PaymentApply\').get(\'data\').advancePaymentDialogPageNum'),
        pageSize: expression('$form.query(\'PaymentApply\').get(\'data\').advancePaymentDialogPageSize'),
        total: expression('$form.query(\'PaymentApply\').get(\'data\').advancePaymentDialogTotal'),
        pageSizes: [5, 15, 30, 60, 120, 300, 600, 1000, 1500],
        '@current-change': expression(`(num) => {
            $form.query('PaymentApply').get('data').advancePaymentDialogPageNum = num
            $getAdvancePaymentDialogData($form)
          }`),
        '@size-change': expression(`(size) => {
            $form.query('PaymentApply').get('data').advancePaymentDialogPageSize = size
            $getAdvancePaymentDialogData($form)
          }`)
      }
    }
