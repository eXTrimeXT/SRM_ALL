    ...
    <el-main>
      ...
      >
        <div class="form-container2">
          <el-collapse v-model="activeDims" class="tab-form-style">
            <el-form
              ...
              :disabled="curOpt === 'view' || curOpt === 'approvalOnly'"
            >
                ...
                <srm-row :gutter="32">
                  ...
                  <srm-col>
                    <el-form-item :label="$t('quota.org')" prop="orgId">
                      <OrganizationSelector
                        ...
                        :disabled="isViewApproval || isAllEdit || isPartEdit"
                  ...
                  <srm-col>
                    <el-form-item
                      ...
                      :rules="{
                        required: true,
                        message: $t('purchaseDemand.inputPayMethod')
                      }"
                    >
                      <dict-select
                        v-model="form.payMethod"
                        code="PAYMENT_MODE"
                        :disabled="isViewApproval || isFirstApproveShow"
                      />
                    </el-form-item>
                  </srm-col>
                  <!-- 币种 -->
                  <srm-col>
                    <el-form-item :label="$t('quota.currency')" prop="currencyName">
                      <QuickSearch
                        :disabled="isViewApproval || isAllEdit"
                  ...
                  <srm-col>
                    ...
                    >
                      <dict-select
                        ...
                        code="PAYMENT_PERIOD"
                        :disabled="isAllEdit || isViewApproval"
            ...
            <el-form
              ref="advanceForm"
              :model="advanceForm"
              :disabled="curOpt === 'view' || curOpt === 'approvalOnly' || isFirstApproveShow"
            >
              <!-- 对账单明细 -->
              <el-collapse-item :title="$t('accountMod.statementDetail1')" name="2">
                <p v-if="!isFirstApproveShow" class="btn_line">
                  <el-button
                    type="primary"
                    :disabled="isViewApproval && showType !== 'viewApproval'"
                    class="detail-pbtn"
              ...
              <!-- 绩效考核 -->
              <el-collapse-item :title="$t('route.performanceAssessment')" name="3">
                <p v-if="!isFirstApproveShow" class="btn_line">
                  <el-button
                    :disabled="isViewApproval && showType !== 'viewApproval'"
                    type="primary"
              ...
              <el-collapse-item :title="$t('accountMod.invoiceInfo')" name="5">
                <p v-if="!isFirstApproveShow" class="btn_line">
                  ...
                  <CInvoiceUpload
                    class="invoice-btn"
                    :editMode="false"
                    :isReadonly="isViewApproval && showType !== 'viewApproval'"
                ...
                >
                  ...
                  <el-table-column :label="$t('common.operation')" width="150" fixed="right">
                    <template slot-scope="scope">
                      <el-button v-if="isFirstApproveShow" type="text">
                        {{ $t('common.edit') }}
                      </el-button>
                      <!-- 编辑 -->
                      <CInvoiceUpload
              ...
              <el-collapse-item :title="$t('quota.fileInfo')" name="6">
                ...
                <upload-attach
                  :readonly="isViewApproval && showType !== 'viewApproval'"
        ...
        <template v-slot:buttonTwo>
          <!-- 驳回 （审批已驳回（REJECTED）&& 供方创建）|| 供方已提交 -->
          <el-button
            v-if="isFirstApproveShow"
            @click="rejectForm"
          >
            {{ $t('common.toRefuse') }}
          </el-button>
...
export default {
  ...
  computed: {
    isAllEdit () {
      // 对账单、预付款选必填
      const signArr = [
        this.advanceForm.statementDetailsTable.length > 0,
        this.advanceForm.onlineInvoiceAdvances.length > 0
      ]
      return signArr.some(item => item)
    },
    isPartEdit () {
      return this.onlineInvoicePunishes.length > 0
    },
    // 点击驳回撤回添加拟定等可编辑字段按钮控制
    isViewApproval () {
      return !['viewApproval', 'add', 'edit'].includes(this.curOpt)
    },
    // 用来控制保存、提交按钮是否可见。如果自定义按钮则无需添加
    viewUpdateButton () {
      return this.isViewApproval && this.form.invoiceStatus !== 'APPROVED'
    },
    disabledUpdateButton () {
      return this.form.invoiceStatus === 'SUBMITTED' || this.form.invoiceStatus === 'APPROVING'
    },
    // 用来指定工作流的业务ID
    workflowBusinessId () {
    ...
    isFirstApproveShow () {
      // const isView = this.curOpt !== 'view' // 是否是查看状态
      // const vendorSubmit = this.form.invoiceStatus === 'VENDOR_SUBMITTED' // 供方已提交状态
      // const isRejectOrWithdraw = ['REJECTED', 'WITHDRAW'].includes(this.form.invoiceStatus) // 审批已驳回/撤回状态
      // const vendorCreated = this.form.onlineInvoiceType === 'VENDOR' // 供方创建单据
      // return isView && (vendorSubmit || (isRejectOrWithdraw && vendorCreated))
      return this.curOpt !== 'view' && this.form.onlineInvoiceType === 'VENDOR' && ['VENDOR_SUBMITTED', 'REJECTED', 'WITHDRAW'].includes(this.form.invoiceStatus)
    }
  },
  watch: {
    isFirstApproveShow: {
  ...
  methods: {
    ...
    amountValidator (rule, value, callback) {
      ...
      // 本次核销金额不可大于未核销金额！
      sign && callback(new Error(this.$t('purchaseDemand.amountCheck')))
      callback()
    },
    // 对账单本次开票数量
    invoiceValidator (rule, value, callback) {
      ...
      // 本次开票数量不可大于未开票数量
      sign && callback(new Error(this.$t('purchaseDemand.lessThanInvoiceNumber')))
      callback()
    },
    // 驳回
    rejectForm () {
    ...
    confirmReject () {
      this.$http({
        ...
        method: 'GET',
        params: {
          onlineInvoiceId: this.form.onlineInvoiceId,
    ...
    initButtonsInfor () {
      this.buttonConfigInfo.save.view =
        !this.isFirstApproveShow && !this.isViewApproval
      this.buttonConfigInfo.submit.view = !this.isViewApproval
      this.buttonConfigInfo.cancel.view = !this.isViewApproval
      this.buttonConfigInfo.close.view = this.isViewApproval
    },
    // 付款账期
    ...
    async deleteInvoice (index) {
      // 确定删除该发票？
      const sign = await this.$confirm(this.$t('agentOnlineInvoice.prompt14'), {
    ...
    async getOrderFormDetail (onlineInvoiceId) {
      await this.$http({
        url: '/api-sup-ce/ps/invoice/onlineInvoice/getOnlineInvoiceSaveDTO',
        method: 'GET',
        ...
        if (res && res.data) {
          ...
          this.fileuploads = res.data.onlineInvoiceAttaches.map(row => {
            // 公共上传接口用fileSourceName字段，后台暂存后读取attachName
            this.$set(row, 'fileSourceName', row.attachName)
            return row
          })
    ...
    */
    ...
    async submitBill (type) {
      let url = ''
      if (this.isFirstApproveShow) {
        url = '/api-sup-ce/ps/invoice/onlineInvoice/firstApproval' // 初审
      } else {
        // 明细行可以修改的情况下要先掉暂存接口，获取最新数据，避免其他人操作单据导致数量对不上
        ['add', 'edit'].includes(this.curOpt) && await this.saveHandle(type)
        url = '/api-sup-ce/ps/invoice/onlineInvoice/submit' // 提交
      }
      let fileArr = this.fileuploads.filter(item => !!item.fileuploadId)
      const params = {
        ...
        onlineInvoiceAttaches: fileArr // 附件
      }
