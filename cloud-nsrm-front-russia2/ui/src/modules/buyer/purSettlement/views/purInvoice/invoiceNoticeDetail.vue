  ...
  <el-container class="flex-container the-purInvoice-detail" direction="vertical">
    <el-main>
      ...
      >
        <div class="form-container2">
          <el-form
            ...
            :model="form"
            :rules="formRules"
            :disabled="curOpt === 'view' || isApprovalOnly"
            label-position="top"
                  ...
                  <srm-col>
                    <el-form-item :label="$t('quota.org')" prop="orgId">
                      <OrganizationSelector
                        ...
                        v-model="form.orgId"
                        :disabled="isViewApproval || isInvoiceLength"
                        :parent-id="-1"
                        node-type="OU"
                  ...
                  <srm-col>
                    ...
                    >
                      <dict-select
                        ...
                        code="PAYMENT_PERIOD"
                        :disabled="isViewApproval || form.invoiceNoticeStatus === 'VENDOR_SUBMITTED' || form.userType === 'VENDOR'"
                      />
                    </el-form-item>
                  </srm-col>
                  <!-- 单据状态 -->
      ...
      <srm-dialog
        ...
        :visible.sync="isInvoiceLengthResult"
        :modal-append-to-body="false"
        size="middle"
        style="text-align: center"
      >
        <!-- 请填写驳回原因 -->
        <el-input v-model="rejectReason" type="textarea" :rows="4" :placeholder="$t('purSettlementMod.pleaseFillReasonForRejection')" />
        <div class="topComment">
          <el-button @click="isInvoiceLengthResult = false">
            {{ $t('common.cancel') }}
          </el-button>
        ...
        <div>
          ...
          >
            <srm-row :gutter="32">
              ...
              <!-- 物料名称 -->
              <srm-col :initCol="3">
                <el-form-item prop="materialId" :label="$t('purSettlementMod.materialId')">
                  <QuickSearch
                    :disable="isViewApproval"
...
export default {
  ...
  data () {
    return {
      ...
      form: {
        ...
        currencyCode: '',
        currencyName: '',
        ceeaInvoiceDate: parseTime(new Date(), '{y}-{m}-{d}', true)
      },
  ...
  computed: {
    ...
    viewUpdateButton () {
      return this.isViewApproval && this.form.invoiceNoticeStatus !== 'APPROVED'
    },
    disabledUpdateButton () {
      return (
        this.form.invoiceNoticeStatus === 'SUBMITTED' ||
        this.form.invoiceNoticeStatus === 'APPROVING'
      )
    },
    // 用来指定工作流的业务ID
    workflowBusinessId () {
      return this.form ? this.form.invoiceNoticeId : null
    },
    ...
    isFirstApproveShow () {
      ...
      const vendorSubmit = this.form.invoiceNoticeStatus === 'VENDOR_SUBMITTED' // 供方已提交状态
      const isRejectOrWithdraw = ['REJECTED', 'WITHDRAW'].includes(this.form.invoiceNoticeStatus) // 审批已驳回/撤回状态
      const vendorCreated = this.form.userType === 'VENDOR' // 供方创建单据
  ...
  methods: {
    initButtonsInfor () {
      console.log(this.isFirstApproveShow, 'xxx')
    ...
    // 驳回
    rejectForm () {
      this.isInvoiceLengthResult = true
    },
    confirmReject () {
      this.$http({
        ...
        this.isInvoiceLengthResult = false
        this.backTo()
      })
    },
    // 选择税率
    ...
    async getOrderFormDetail (invoiceNoticeId) {
      await this.$http({
        url: '/api-sup-ce/ps/invoice/invoiceNotice/getInvoiceNoticeSaveDTO',
        method: 'GET',
    ...
    setTotalAmount (row) {
      if (row.invoiceQuantity > row.notInvoiceQuantity) {
        return this.$message.error(this.$t('purSettlementMod.msgArr[14]'))
      }
    },
    reset () {
      // 重置所有过滤条件
    ...
    setPropsInfor () {
      // 对账期间
      this.$set(this.form, 'ceeaReceiveStartDate', this.form.receiveDate ? this.form.receiveDate[0] : '')
      this.$set(this.form, 'ceeaReceiveEndDate', this.form.receiveDate ? this.form.receiveDate[1] : '')
    },
    async saveOrSubmitData (type) {
      if (this.form.ceeaTaxTotalAmount < 0) {
        return this.$message.warning(this.$t('purSettlementMod.ceeaTaxTotalAmountWarning'))
      }
      this.setPropsInfor()
    ...
    async saveData (type) {
      const { code, data } = await this.$http({
        url: '/api-sup-ce/ps/invoice/invoiceNotice/saveTemporary',
        method: 'POST',
        data: {
    ...
    */
    ...
    async submitData (type) {
      ...
      if (this.isFirstApproveShow) {
        url = '/api-sup-ce/ps/invoice/invoiceNotice/firstApproval'
      } else {
        // 明细行可以修改的情况下要先掉暂存接口，获取最新数据，避免其他人操作单据导致数量对不上
        ['add', 'edit'].includes(this.curOpt) && await this.saveData(type)
        url = '/api-sup-ce/ps/invoice/invoiceNotice/submit'
      }
    ...
    queryItemList () {
      ...
      this.$http({
        ...
        console.log(this.displayMaterialItem, 'this.displayMaterialItem')
    ...
    addNewOne () {
      ...
      for (const item of this.selectionItem) {
        if (!warehousingReturnDetailIdArr.includes(item.warehousingReturnDetailId)) {
          ...
          this.$message.warning(this.$t('purSettlementMod.existsDuplicateRow')) // 存在重复行
          return
        }
      }
      this.getTaxCal()
