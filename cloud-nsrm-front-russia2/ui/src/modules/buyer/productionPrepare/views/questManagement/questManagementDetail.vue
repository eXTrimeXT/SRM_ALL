<template>
  <el-container class="questManagementDetail" direction="vertical">
    <el-main>
      <div class="form-container">
        <el-collapse v-model="activeDims" class="tab-form-style">
          <!-- 调查表基本信息 -->
          <el-collapse-item ref="questInfo" :title="$t('vendorMod.questInfo')" name="1">
            <el-form ref="questInfoForm" :model="form" :rules="questInfoRules" :disabled="disabledFlag">
              <srm-row :gutter="32">
                <srm-col>
                  <!-- 调查模板类型 -->
                  <el-form-item :label="$t('vendorMod.questTemplateType')" prop="questTemplateType">
                    <dict-select
                ...
                <srm-col>
                  <el-form-item prop="questTemplateId" :label="$t('vendorMod.questTemplateId')">
                    <QuickSearch
                ...
                <srm-col>
                  <el-form-item :label="$t('components.organization.ORG')">
                    <el-select
                ...
                <srm-col>
                  <el-form-item prop="questName" :label="$t('vendorMod.questName')">
                    <el-input v-model="form.questName" :disabled="curRole === 'VENDOR'" />
                  </el-form-item>
                </srm-col>
                <!-- 燕豪修改 -->
                <srm-col>
                  <el-form-item :label="$t('problemManagement.motorcycleTypeCode')">
                    <el-input v-model="form.modelCode" :disabled="curRole === 'VENDOR'" />
                  </el-form-item>
                </srm-col>
                <srm-col>
                  <el-form-item :label="$t('productionPrepare.stage')">
                    <dict-select v-model="form.stage" code="QUESTIONNAIRE_STAGE" :disabled="curRole === 'VENDOR'" />
                  </el-form-item>
                </srm-col>
                <srm-col>
                  <el-form-item prop="recoveryTime" :label="$t('productionPrepare.recoveryTime')">
                    <!-- 选择回收时间 -->
                    <el-date-picker
                      ...
                      type="date"
                      placeholder="$t('cusEntry.supplement20250211.selectRecycleTime')"
                    />
                  </el-form-item>
                </srm-col>
                <srm-col>
                  <!-- 范围类型 -->
                  <el-form-item prop="rangType" :label="$t('productionPrepare.rangeType')">
                    <dict-select
                ...
                <template v-if="curRole === 'VENDOR' || $attrs.params.flag === 'vendor'">
                  <srm-col>
                    <!-- 反馈提交时间 -->
                    <el-form-item prop="recyclingTime" :label="$t('productionPrepare.recyclingTime')">
                      <!-- 选择日期 -->
                      <el-date-picker
                  ...
                  <srm-col>
                    <!-- 填写人 -->
                    <el-form-item :label="$t('productionPrepare.questFullName')">
                      <el-input v-model="form.questFullName" disabled />
                    </el-form-item>
                  </srm-col>
                  <srm-col :init-col="2">
                    <!-- 驳回原因 -->
                    <el-form-item :label="$t('vendorMod.rejectReason')">
          ...
          <!-- 调查表进度分析 -->
          <el-collapse-item v-if="showProgress" ref="progress" :title="$t('cusEntry.supplement20250211.progressAnalysis')" name="2">
            <div class="btns">
          ...
          <!-- 调查表基本信息 -->
          <!-- 附件 -->
          <el-collapse-item ref="file" :title="$t('components.workedProcess.headers.attachment')" name="3">
            ...
            <el-table :data="fileData" border style="width:100%;margin-top:10px;">
              <!-- 附件上传 -->
              <el-table-column :label="$t('components.upload.fileUpload')" align="center">
              ...
              <!-- 附件名称 -->
              <el-table-column :label="$t('common.fileUploadName')" prop="fileName" align="center" />
              <!-- 上传人 -->
              <el-table-column :label="$t('components.fileupload.uploadUserName')" prop="createdBy" align="center" />
              <!-- 操作 -->
              <el-table-column :label="$t('components.headers.operation')" align="center" width="100">
                <template slot-scope="scope">
          ...
          <!-- 明细信息 -->
          <el-collapse-item ref="supplierInfo" :title="$t('productionPrepare.supplierInfo')" name="4">
            <el-tabs v-model="activeList">
              <!-- 调查表范围 -->
              <el-tab-pane v-if="curRole === 'BUYER' && $attrs.params.flag !== 'vendor'" name="1" :label="$t('vendorMod.questionnaireScope')">
                <!-- 暂存后可导入 -->
                <el-tooltip :content="$t('cusEntry.supplement20250211.tempStorageAndImportable')" :disabled="!!form.questNoForQuery">
                  <!-- 导入 -->
                  <MImport
                    v-if="!disabledFlag"
              ...
              <!-- 调查明细 -->
              <el-tab-pane :label="$t('productionPrepare.questDetailInfo')" name="2">
                <!-- 根据模板配置渲染相关组件 -->
                <renderForm
                  ref="renderFormDom"
              ...
              <!-- 进度跟踪 -->
              <el-tab-pane v-if="curRole === 'BUYER' && $attrs.params.flag !== 'vendor'" name="3" :label="$t('productionPrepare.progressTracking')">
                <FormWrapper
      ...
      <CToolbar>
        <template #right>
          ...
          >
            <!-- 撤回申请 -->
            {{ $t("cusEntry.supplement20250211.withdrawApplication") }}
          </el-button>
    ...
    <!-- 驳回弹窗 -->
    <el-dialog :title="$t('components.approvalHead.headers.refuse')" :visible.sync="dialogFormVisible">
      <el-form :model="rejectForm">
        <!-- 驳回原因 -->
        <el-form-item :label="$t('vendorMod.rejectReason')" :label-width="120" required>
    ...
    <!-- 撤回弹窗 -->
    <!-- 撤回 -->
    <el-dialog :title="$t('components.approvalHead.headers.return')" :visible.sync="recallVisible">
      <el-form :model="recallForm">
        <!-- 撤回原因 -->
        <el-form-item :label="$t('bidMod.withdrawReason')" :label-width="120" required>
    ...
    <!-- 撤回原因 -->
    <el-dialog :title="$t('bidMod.withdrawReason')" :visible.sync="recallReasonVisible">
      <el-table border :data="recallData" :max-height="400">
        <!-- 撤回原因 -->
        <el-table-column prop="recallReason" align="center" :label="$t('bidMod.withdrawReason')" />
        <!-- 撤回时间 -->
        <el-table-column prop="creationDate" align="center" :label="$t('logisticsMod.withdrawTime')" :formatter="(row, column, cellValue) => $parseTime(cellValue)" width="160" />
      </el-table>
    </el-dialog>
  </el-container>
</template>
    ...
    return {
      ...
      tableHeaderExport: [
        ...
        },
        {
          ...
          // '供应商是否已回收'
          label: this.$t('productionPrepare.vendorRecycleFlag'),
          width: 100
        },
        {
          prop: 'vendorReplyFlag',
          // '供应商是否已回复反馈提交时间'
          label: this.$t('productionPrepare.vendorReplyFlag'),
          width: 130
        }
      ],
      dictCodes: {},
      ...
      filterConfig: [
        ...
        {
          prop: 'recyclingType',
          label: this.$t('productionPrepare.recyclingType'),  // '回收状态'
          type: 'dict',
      ...
      questInfoRules: {
        ...
        status: [
          { required: true, message: this.$t('cusEntry.supplement20250211.inputStage') }  // '请输入阶段'
        ],
        recoveryTime: [
          { required: true, message: this.$t('productionPrepare.selectRecoveryTime') }  // '请选择回收时间'
        ],
        rangType: [
          { required: true, message: this.$t('productionPrepare.selectRangeType') }  // '请选择范围类型'
        ],
        recyclingTime: [
          { required: true, message: this.$t('cusEntry.supplement20250211.selectFeedbackTime') }  // '请选择反馈时间'
        ]
      },
      ...
      progressTrackingTableHeader: [
        ...
        },
        {
          prop: 'recyclingType',
          // '回收状态'
          label: this.$t('productionPrepare.recyclingType'), // TO_BE_RECYCLED 待回收 / RECYCLED 已回收 / REJECT 驳回 / REVIEWED 已审核
          minWidth: 100,
          dataType: 'dict',
          code: 'QUESTIONNAIRE_RECOVERY_STATUS'
        },
        {
          ...
          // '供应商回复单号'
          label: this.$t('productionPrepare.vendorReplyCode'),
          minWidth: 150,
          showType: 'button',
          btnStyle: 'text',
          callback: row => {
            this.$emit('tab-add', {
              ...
              // '供应商调查表'
              title: this.$t('cusEntry.supplement20250211.supplierSurveyForm') + row.vendorReplyCode,
        ...
        },
        {
          prop: 'recycleTime',
          // '供应商反馈回收时间'
          label: this.$t('productionPrepare.vendorRecycleTime'),
          minWidth: 160,
          formattor: val => {
            if (val) {
        ...
        },
        {
          prop: 'recallReason',
          // '撤回原因'
          label: this.$t('bidMod.withdrawReason'),
          minWidth: 120,
          showType: 'button',
          btnStyle: 'text',
          callback: row => {
          ...
          // '查看'
          formattor: val => this.$t('common.view')
        },
        {
          prop: 'operation',
          // '操作'
          ...
          buttons: [
            ...
            },
            {
              callback: row => {
                // '确定审核？'
                this.$confirm(this.$t('cusEntry.supplement20250211.determineAudit'), this.$t('components.approvalHead.tips.tip'), {
                  type: 'warning'
                }).then(() => {
              ...
              formattor: () => {
                // '审核'
                return this.$t('common.approveNew')
              }
            },
            {
              callback: row => {
                // '确定同意？'
                this.$confirm(this.$t('productionPrepare.questManagementTip20'), this.$t('components.approvalHead.tips.tip'), {
                  type: 'warning'
                }).then(() => {
              ...
              formattor: () => {
                // '同意撤回'
                return this.$t('productionPrepare.questManagementTitle4')
              }
            },
            {
              callback: row => {
                // '确定拒绝？'
                this.$confirm(this.$t('productionPrepare.questManagementTip21'), this.$t('components.approvalHead.tips.tip'), {
                  type: 'warning'
                }).then(() => {
              ...
              formattor: () => {
                // '拒绝撤回'
                return this.$t('productionPrepare.questManagementTitle5')
              }
            }
          ]
        }
      ...
      companyInfoListTableHeader1: [
        ...
        },
        {
          ...
          // '下一落点'
          label: this.$t('productionPrepare.nextLoadingPoint'),
          minWidth: 150
        },
        {
          prop: 'requiredCompletionTime',
          // '要求达成时间'
          label: this.$t('productionPrepare.requiredCompletionTime'),
          minWidth: 150
        }
      ],
  ...
  methods: {
      ...
      questManagement.getProgressDataPic({ questNo: this.form.questNoForQuery }).then(res => {
        if (res.code === '0') {
          ...
          this.$nextTick(() => {
            this.initChart({
              // '回收情况分析图'
              title: this.$t('cusEntry.supplement20250211.recycleSituationAnalysisChart'),
              el: 'chart-receive',
            ...
            this.initChart({
              // '反馈情况分析图'
              title: this.$t('cusEntry.supplement20250211.feedbackAnalysisChart'),
              el: 'chart-feedback',
    ...
    downloadTemplate () {
      ...
      downloadFileLinkByPost(
        ...
        // '调查表范围导入模板.xlsx',
        this.$t('cusEntry.supplement20250211.surveyRangeTemplateXlsx'),
        params,
      ).catch(() => {
        // '下载失败'
    ...
    progressExport () {
      ...
      downloadFileLinkByPost(
        ...
        // '调查表进度跟踪导出.xlsx',
        this.$t('cusEntry.supplement20250211.surveyProgressTrackingExportXlsx'),
        {
    ...
    recallOk () {
      if (!this.recallForm.recallReason) {
        // '请填写撤回原因！'
        this.$message.warning(this.$t('productionPrepare.questManagementTip7'))
        return
      }
      this.recallVisible = false
    ...
    updateQuestRecy (row, type) {
      ...
      questManagement.updateQuestSupplierApprovalStatus(data).then(res => {
        if (res.code === '0') {
          ...
          let message = type === 'REJECT' ? this.$t('agentOnlineInvoice.prompt5') : this.$t('productionPrepare.questManagementTip6')
        ...
        for (let item of submitData.progressDetails) {
          if (!item.detailData || !item.detailData.length) {
            return this.__jump_error__(
              ...
              null,
              // '调查明细页签不能为空'
              this.$t('cusEntry.supplement20250211.surveyDetailTabNotEmpty')
            )
          }
            ...
            for (let keys of questTemplatePropArr) {
              if (keys.enabledFlag === 'Y' && keys.emptyFlag === 'Y' && !innerItem[keys.questTemplatePropField]) {
                return this.__jump_error__(
                  ...
                  this.$t('productionPrepare.questManagementTip9')
                )
              }
            }
          }
