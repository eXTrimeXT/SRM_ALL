              ...
              <el-collapse v-model="activeDims" class="tab-form-style">
                ...
                <el-collapse-item ref="projectInfo" :title="$t('perfMod.projectInformation')" name="1">
                  <srm-row>
                    ...
                    <srm-col>
                      <el-form-item :label="$t('perfMod.fullPathId')">
                    ...
                    <srm-col>
                      <el-form-item :label="$t('perfMod.scoreEndTime')" prop="scoreEndTime">
                        <el-date-picker
                ...
                <el-collapse-item ref="evaluateVendor" :title="$t('perfMod.evaluateVendor')" name="2">
                  <div class="btn-flex">
                    <QuickSearch
                      ...
                      :disabled="['view', 'approve'].includes(curOpt) || !form.organizationId"
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        // '请选择绩效模型'
        templateHeadId: [{ required: true, message: i18nExpression('perfMod.selectPerformanceModel') }],
        // '请选择绩效开始月份'
        perStartMonth: [{ required: true, message: i18nExpression('perfMod.performanceStartMonth') }],
        // '请选择绩效结束月份'
        perEndMonth: [{ required: true, message: i18nExpression('perfMod.performanceEndMonth') }],
        // '请选择评分开始时间'
        scoreStartTime: [{ required: true, message: i18nExpression('perfMod.gradingStartTime') }],
        // '请选择评分截止时间'
        scoreEndTime: [{ required: true, message: i18nExpression('perfMod.gradingEndTime') }]
      },
  ...
  computed: {
    preQueryData1 () {
      return { 't.KEY_WORD': '', 't.ORG_ID': this.form.organizationId, 't.DATA_SOURCES': 'IMAGE', 't.STATUS': 'APPROVED' }
    },
    preQueryData2 () {
  ...
  methods: {
      ...
      this.$http({
        ...
        list?.forEach((data, index) => {
          data?.perfIndicatorDimScoreList.forEach((data2) => {
            ...
            // 总得分
            const obj = {
              label: this.$getDictLabel('INDICATORS_DIM', data2.indicatorDimensionType) + this.$t('perfMod.totalScore'), // 总得分显示的头信息
    ...
    // 查询有效的绩效模型下拉数据
    getTempateList () {
      performanceManagement.getValidTemplateHeader().then((res) => {
