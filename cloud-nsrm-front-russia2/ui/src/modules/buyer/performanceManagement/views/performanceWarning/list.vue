  ...
  >
    <el-main>
      ...
      <TableView
        ...
        url="/api-pef/scoring/perfOverallScore/listPageScoreUnder60"
      />
    </el-main>
    <srm-dialog
      :title="dialogTitle"
  ...
  data () {
    return {
      ...
      form: {
        ...
        vendorCode: this.$t('common.vendorCode'),  // '供应商编码'
        orgId: this.$t('cusEntry.supplement20250205.purchaseOrgId'),  // '采购组织ID'
        orgCode: this.$t('cusEntry.supplement20250205.purchaseOrgCode'),  // '采购组织编码'
        orgName: this.$t('cusEntry.supplement20250121.purchaseOrg'),  // '采购组织'
        categoryCode: this.$t('components.category.categoryCode'),  // '品类编码'
        categoryName: this.$t('components.category.categoryName'),  // '品类名称'
        planType: this.$t('vendorMod.planType'),  // '计划类型'
        planStartDate: this.$t('cusEntry.supplement20250205.planStartTime'),  // '计划开始时间(YYYY-MM-DD)'
        planProcessStatus: this.$t('vendorMod.planProcessStatus'),  // '计划进度状态'
        planConfirmCode: this.$t('vendorMod.planConfirmCode'),  // '计划落实单号'
        siteReviewCode: this.$t('vendorMod.siteReviewCode'),  // '现场评审单号'
        planStatus: this.$t('vendorMod.planStatus'),  // '计划状态'
        ...
        createdFullName: this.$t('cusEntry.supplement20250205.creatorNewName'),  // '创建人新姓名'
        ...
        lastUpdatedFullName: this.$t('cusEntry.supplement20250205.lastUpdaterName'),  //'最后更新人姓名'
        tenantId: this.$t('monitorBizConfig.tenantId'),  // '租户ID'
      ...
      tableHeader: [
        ...
        {
          prop: 'improveNo',
          label: this.$t('perfMod.improveNo2'),
          showType: 'button',
          btnStyle: 'text',
          ...
          formattor: (val) => {
            ...
            switch (val) {
              case 'DRAFT':
                ...
                break
              case 'IN_FEEDBACK':
                mainHtml = this.$t('perfMod.IN_FEEDBACK')
                break
              case 'WITHDRAWN':
                mainHtml = this.$t('perfMod.WITHDRAWN')
                break
              case 'OBSOLETE':
                mainHtml = this.$t('perfMod.OBSOLETE')
                break
              case 'OBSOLETE':
                mainHtml = this.$t('perfMod.OBSOLETE')
                break
              case 'ASSESSED':
                mainHtml = this.$t('perfMod.inspection')
                break
              default:
                mainHtml = this.$t('perfMod.DRAFT')
            }
        ...
        {
          prop: 'assessmentNo',
          label: this.$t('perfMod.assessmentNo2'),
          showType: 'button',
          btnStyle: 'text',
          callback: row => this.assessmentNoFunction(row),
          width: 100
        },
        {
          prop: 'serviceStatus',
          ...
          buttons: [
            {
              ...
              show: row => row.improveNo == '',
              formattor: () => {
                return this.$t('perfMod.createImprovementOrder')
              }
            },
            {
              callback: row => this.editHandle2(row),
              // code: "pr:requirementApply:edit",
              show: row => row.assessmentNo == '',
              formattor: () => {
                return this.$t('perfMod.createAppraisalForm')
              }
            }
          ]
        }
  ...
  methods: {
    ...
    editHandle1 (row) {
      ...
      let tab = {
        ...
        title: row.companyName,
        name: 'improvementDetail' + row.vendorAssesId
      }
      this.$emit('tab-add', tab)
    },
    editHandle2 (row) {
