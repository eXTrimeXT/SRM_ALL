...
const mutations = {
  ...
  SET_INTERFACE_AUTH_INFO: (state, interfaceAuthInfo) => {
    state.interfaceAuthInfo = interfaceAuthInfo
    Cookies.set('interfaceAuthInfo', JSON.stringify(interfaceAuthInfo))
  }
}

const actions = {
  setInterfaceAuth ({ commit }, authInfo) {
    commit('SET_INTERFACE_AUTH_INFO', authInfo)
  },
  ...
  setSysOpenConfig ({ commit }) {
    return new Promise((resolve, reject) => {
      getOpenConfigBeforeLogin().then(res => {
        ...
        const { globalization={}, allSceneCode = '', buyerSceneCode = '', vendorSceneCode = '', loginSystem = config.loginSystem, loginType = config.loginType, supplierAutoAuth = false, iamBaseUrl, iamContextPath = '', registerVersion = 'old', ...rest } = res.data
        let iamSysBaseUrl = iamBaseUrl + iamContextPath
        localStorage.setItem('globalization', globalization?.enable)
