...
const $qEdit = (row: any, flag: string) => {
  ...
  if (flag === 'add') { // 新增
    name = 'questionDetail'
    title = i18nExpression('bidMod.addChallenge') // 新增质疑
...
const $aEdit = (row: any, flag: string) => {
  ...
  if (flag === 'add') { // 新增
    name = 'answerDetail'
    title = i18nExpression('bidMod.addClarification') // 新增澄清
  } else if (flag === 'clarifyFormQuestion') {
    name = 'answerDetail' + row.questionNum
    title = row.questionNum + '-' + i18nExpression('bidMod.addClarification')
        ...
        properties: {
          ...
          tableQuestion: {
            ...
            properties: generateXindexInOrder({
              ...
              submitTime: {
                title: "{{$t('bidMod.qa.submitTimeBuyer')}}",
                ...yearMonthDaySelectorSegment,
                'x-component-props': {
