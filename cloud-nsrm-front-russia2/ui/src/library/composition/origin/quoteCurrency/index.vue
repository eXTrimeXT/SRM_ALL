    ...
    <SrmRow>
      ...
      <SrmCol :init-col="3">
        <el-form-item :label="$t('bid_mod.currencyChangeDate')" :prop="keyMap.currencyChangeDate">
          <el-date-picker v-model="baseInfoData[keyMap.currencyChangeDate]" type="date" :format="$formatDatePicker" />
        </el-form-item>
      </SrmCol>
    </SrmRow>
      ...
      if (data && data.data && Array.isArray(list)) {
        if (list.length === 0) {
          this.$message.warning(
            `${fromCode} TO ${toCode} ${this.$t('bidMod.common.quoteCurrencyMsg1')}${this.$getDictLabel('EXCHANGE_RATE_TYPE', paramData.rateType)}${this.$t('bidMod.common.quoteCurrencyMsg2')}${this.$parseTime(paramData.exchangeDate)}${this.$t('bidMod.common.quoteCurrencyMsg3')}`
          )
          scope.row.priceTax = ''
          this.currencyList.splice(scope.$index, 1, scope.row)
          return
