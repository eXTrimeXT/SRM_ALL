<template>
  <div class="price-line-chart-wrap">
    ...
    <slot name="tips">
      <p class="chart-tips">
        {{ chartTips || $t('competition.chartTips') }}
      </p>
    </slot>
  ...
  computed: {
    searchFormConfig () {
      ...
      return [
        {
          prop: 'dateList',
          label: this.$t('competition.hisDateList'),
          type: 'daterange'
        }
      ]
    }
  ...
  methods: {
        ...
        vendorKeys.forEach(item => {
          ...
          series.push({
            ...
            data: this.vendorPriceNodes[item]
              .map(itemVendor => {
                xAxis.push(this.$parseTime(itemVendor.date))
                return [this.$parseTime(itemVendor.date), itemVendor.notaxPrice]
              })
              // 时间排序
    ...
    setChartOptions (val) {
      ...
      this.chart.setOption({
        ...
        yAxis: {
          name: this.$t('competition.yAxisNoTaxPrice'),
          axisTick: {
            show: false
          }
        },
