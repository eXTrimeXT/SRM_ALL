<template>
  <div class="template-compile-wrap">
    <el-tabs v-model="activeTabItem" type="card">
      <!--报价属性项-->
      <el-tab-pane
        v-for="item in attrTableList"
        :key="item.attrId"
        :label="item.attrName"
        :name="item.attrId.toString()"
      ...
      >
        ...
        >
          <!-- 汇总tab页  -->
          <template #subItemName="{column,row}">
            <span v-if="row.required" style="color:red">*</span>
            <span>{{ row.subItemName }}</span>
            <el-popover
              v-if="row.headerSlot === 'formulaValueTooltip'"
              placement="top"
              width="430"
              trigger="hover"
            >
              <el-table :data="fieldTypeIsFormulaPropertyMap.get(row.fieldId) || []" style="width: 100%">
                <!-- 应用条件 -->
                <el-table-column
          ...
          <template #subItemAmount="{column,row}">
            <span>{{ row.subItemAmount }}</span>
          </template>
          <!--表头 公式提示-->
  ...
  methods: {
    ...
    arrangeTableColumns () {
      ...
      tempVO.tempLineList.forEach(item => {
        ...
        // 将汇总项放置在tab第一项
        itemColumns.isTotalBoolean ? resultList.unshift(itemColumns) : resultList.push(itemColumns)
      })

      this.attrTableList = resultList
      this.afterArrangeTableColumns(resultList)
      // 编排汇总列
      // let totalItemColumn = resultList.find(item => item.isTotalBoolean)
      // if(totalItemColumn){
      //   this.totalItemColumn = JSON.parse(JSON.stringify(totalItemColumn))
      //   this.arrangeTotalTableColumns(totalItemColumn)
      // }
    },
    ...
    afterArrangeTableColumns (resultList) {
      ...
      if (resultList.length) {
        this.activeTabItem = (resultList || []).map(item => item.attrId)[0].toString()
      }
    ...
    arrangeTableData () {
      ...
      // 总价行处理
      // if(isTotalItem){
      //   let totalItemData = JSON.parse(JSON.stringify(this.templateData[isTotalItem.attrId]))
      //   console.log('totalItemData',totalItemData)
      //   let { tableColumns,...totalRest } = this.totalItemColumn
      //   //去除序号列
      //   tableColumns = tableColumns.filter(item => item.attrs.type !== 'index')
      //   let newTotalItemData = []
      //   if(!totalItemData.length) return
      //   Object.keys(totalItemData[0]).forEach(key => {
      //     tableColumns.forEach(item => {
      //       const {attrs,...rest} = item
      //       if(attrs.prop === key){
      //         newTotalItemData.push({
      //           subItemName:attrs.label,
      //           subItemAmount:totalItemData[0][key],
      //           fieldId:key,
      //           ...rest, // headerSlot slot
      //           ...totalRest,
      //           required:!!attrs.renderHeader // 字段是否必填
      //         })
      //       }
      //     })
      //   })
      //   this.totalItemData = newTotalItemData
      // }
    },
