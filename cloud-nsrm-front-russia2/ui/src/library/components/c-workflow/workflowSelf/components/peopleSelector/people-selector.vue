    ...
    >
      <div class="section-content">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              {{ normalizer ? normalizer(scope) : scope.row.assigneeName }}
            </template>
          </el-table-column>
          <el-table-column
            width="40"
...
export default {
    ...
    dialogVisible: {
      ...
      handler (visible) {
        if (visible) {
          ...
          this.buyerQuery = { ...this.buyerQuery, ...this.searchData }
          if (this.resetSelect) {
            this.resetData()
  ...
  methods: {
    fatchBuyerData () {
      getUserList(this.buyerQuery).then(res => {
        if (res) {
    ...
    // 搜索用户
    searchUser () {
      this.buyerQuery.searchKey = this.searchKey
      this.fatchBuyerData()
    },
    // 处理单个员工选择
    handleBuyerSelect (selection, row) {
      ...
      selection.forEach(select => {
        if (row.assigneeId === select.assigneeId) {
          isChoice = true
        }
      })
      this.buyerChosenData.forEach((employee, index) => {
        if (row.assigneeId === employee.assigneeId) {
          isExist = index
        }
      })
      ...
      if (handleType === 1) {
        ...
        _this.buyerData.forEach(employee => {
          ...
          for (let i = 0; i < _this.buyerChosenData.length; i++) {
            if (employee.assigneeId === _this.buyerChosenData[i].assigneeId) {
              find = true
              break
            }
          }
    ...
    deleteBuyerChosen (index) {
      let targetUserCode = this.buyerChosenData[index].assigneeId
      this.buyerChosenData.splice(index, 1)
      // 如果被删除的存在被勾选，则取消
      for (let i = 0; i < this.buyerData.length; i++) {
        if (this.buyerData[i].assigneeId === targetUserCode) {
    ...
    addAllBuyer () {
      ...
      this.buyerData.forEach(buyer => {
        ...
        for (let i = 0; i < this.buyerChosenData.length; i++) {
          if (buyer.assigneeId === this.buyerChosenData[i].assigneeId) {
            find = true
            break
          }
        }
      ...
      this.$refs.BuyerSelector.toggleAllSelection()
    },
    // 删除全部被选中员工
