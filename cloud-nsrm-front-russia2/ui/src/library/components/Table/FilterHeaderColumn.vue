  ...
  >
    ...
    >
      <span class="el-dropdown-link">{{
        symbols[queryData[col.prop + "Cond"]]
      }}</span>
...
export default {
  ...
  data: function () {
    return {
      ...
      shownames: {
        bt: this.$t('components.condition.bt'), // "之间",
        cn: this.$t('components.condition.cn'), // "包含",
        eq: this.$t('components.condition.eq'), // "等于",
        ne: this.$t('components.condition.ne'), // "不等",
        me: this.$t('components.condition.me'), // "多选",
        bw: this.$t('components.condition.bw'), // "开始于",
        ew: this.$t('components.condition.ew'), // "结束于",
        gt: this.$t('components.condition.gt'), // "大于",
        ge: this.$t('components.condition.ge'), // "大于等于",
        lt: this.$t('components.condition.lt'), // "小于",
        le: this.$t('components.condition.le') // "小于等于"
      },
    ...
    mergeCondition (f) {
      return function (f) {
        if (
          ...
          // 日期、时间、多选框的比较符固定，不可定制
          return this.conditions[f.type].filter(function (v) {
            return f.conditions.indexOf(v) > -1
          }) // 自定义比较符需要合法性过滤
        } else {
  ...
  methods: {
    ...
    handleCommand (command) {
      ...
      if (
        this.queryData[command.col.prop] &&
        (command.col.type == 'number' ||
          this.queryData[command.col.prop].length > 0)
      ) {
        this.onQuery()
      }
    },
