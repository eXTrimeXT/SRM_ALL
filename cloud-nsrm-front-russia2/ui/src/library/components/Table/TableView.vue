          ...
          >
            ...
            <template #default="scope">
              ...
              <template v-else-if="col.showType === 'date'">
                ...
                <span v-else>{{ $parseTime(scope.row[col.prop]) }}</span>
              </template>
...
import { debounce, isNull, parseTime } from '@/utils'
  ...
  methods: {
    ...
    getPropData (data, col) {
      ...
      if (col.dataType === 'dateTime') {
        return parseTime(propData[col.prop])
      }
      if (col.dataType === 'dateTimes') {
        return parseTime(propData[col.prop], '{d}.{m}.{y} {h}:{i}:{s}')
      }
