...
export const checkMD5 = data => {
  return http({
    url: '/api-file/largerfile/checkMd5',
    method: 'POST',
    data
  })
}
...
export const checkChunk = data => {
  return http({
    url: '/api-file/largerfile/checkChunk',
    method: 'POST',
    data
  })
}
...
export const chunkUpload = data => {
  return http({
    url: '/api-file/largerfile/saveChunk',
    method: 'POST',
    data
  })
}
...
export const mergeFile = data => {
  return http({
    url: '/api-file/largerfile/mergeChunk',
    method: 'POST',
    data
  })
}
...
// 判断文件大小
export const getFileSize = id =>
  http({
    url: `/api-file/largerfile/downloadSign?fileuploadId=${id}`,
    method: 'GET'
  })
// 判断IE 10 IE11
function isIE () { //
    ...
    axios({
      ...
      .then(response => {
        ...
        if (data.byteLength != 0) {
          ...
          throw new Error(i18n.t('components.fileupload.fileDestroyed'))
        }
        resolve()
      })
      .catch(error => {
