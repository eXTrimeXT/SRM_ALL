    ...
    <el-main>
      ...
      >
        <div class="form-container2">
          ...
          >
            <el-collapse v-model="activeDims" class="tab-form-style">
              <!-- 意向金开票申请 -->
              <el-collapse-item ref="aptInfo" :title="$t('cusEntry.supplement20250205.applyInvoice')" name="1">
                <srm-row>
                  <srm-col>
                    <el-form-item
                      :label="$t('bidMod.bidsProjectName')"
                      prop="projectName"
                      :rules="{
                        required: true,
                  ...
                  <srm-col v-if="requirementHead.invoiceType == 'RED_INVOICE'" key="1">
                    <!-- 原意向金开票单号 -->
                    <el-form-item
                      :label="$t('cusEntry.supplement20250205.fromDepositInvoiceNo')"
                      :rules="{
                        required: true,
                        message: $t('contractMod.pleaseFillIn')
                      }"
                  ...
                  <srm-col>
                    <!-- 开票公司 -->
                    <el-form-item
                      :label="$t('cusEntry.supplement20250205.vendorName')"
                      :rules="{
                        required: true,
                        message: $t('contractMod.pleaseFillIn')
                      }"
                  ...
                  <srm-col>
                    <el-form-item
                      :label="$t('vendorMod.socialCreditCode3')"
                      prop="taxPayer"
                    >
                  ...
                  <srm-col>
                    <!-- 开户银行名称 -->
                    <el-form-item
                      :label="$t('cusEntry.supplement20250205.bankName')"
                      :rules="{
                        required: true,
                        message: $t('contractMod.pleaseFillIn')
                      }"
                  ...
                  <srm-col>
                    <!-- 开户银行账户 -->
                    <el-form-item
                      :label="$t('cusEntry.supplement20250205.bankAccount')"
                      :rules="{
                        required: true,
                        message: $t('contractMod.pleaseFillIn')
                      }"
                  ...
                  <srm-col>
                    <!-- 委托代缴证明/红票说明 -->
                    <el-form-item
                      :label="$t('cusEntry.supplement20250205.entrustPayVoucher')"
                      :rules="{
                  ...
                  <srm-col v-if="requirementHead.invoiceType !== 'RED_INVOICE'" key="2">
                    <!-- 发票接收邮箱 -->
                    <el-form-item
                      :label="$t('cusEntry.supplement20250205.email')"
                  ...
                  <srm-col>
                    <!-- 开票数量 -->
                    <el-form-item
                      :label="$t('cusEntry.supplement20250121.invoiceQuantity')"
                    >
                      <el-input
                  ...
                  <srm-col>
                    <el-form-item :label="$t('common.creationDate')">
                      <el-date-picker
                  ...
                  <srm-col>
                    <!-- 发票 -->
                    <el-form-item
                      :label="$t('cusEntry.supplement20250205.invoice')"
                      prop="fileId"
                    >
                      <SrmCommonFile
                        :extra-data="fileInfo"
                  ...
                  <srm-col :initCol="1" v-if="requirementHead.invoiceType == 'RED_INVOICE'" key="3">
                    <!-- 开具红字发票原因 -->
                    <el-form-item :label="$t('cusEntry.supplement20250205.redInvoiceReason')">
                      <el-input
                  ...
                  <srm-col :initCol="1" v-if="requirementHead.invoiceType == 'RED_INVOICE'" key="4">
                    <!-- 开票说明： -->
                    <!-- 请按照要求填写开票信息，发票开具后会自动回复至招标平台，请注意查收。注：a.个人(或者公司)代缴的投标意向金，如需开具发票抬头为公司的发票，需出具加盖投标单位公章的委托代付说明文件，否则将无法开具发票；b.发票开具完成后，意向金将不再退还；c.贵司提供发票信息后，大约25个工作日内会收到相应的电子发票，请耐心等待，有问题请电话联系0312-2197731。 -->
                    <p>{{ $t('cusEntry.supplement20250205.redInvoiceReasonTip1') }}</p>
                    <p>{{ $t('cusEntry.supplement20250205.redInvoiceReasonTip2') }}</p>
                  </srm-col>
                  <srm-col :initCol="1" key="5" v-else>
                    <!-- 开票说明： -->
                    <!-- 请按照要求填写开票信息，发票开具后会自动回复至招标平台，请注意查收。注：a.个人(或者公司)代缴的投标意向金，如需开具发票抬头为公司的发票，需出具加盖投标单位公章的委托代付说明文件，否则将无法开具发票；b.发票开具完成后，意向金将不再退还；c.贵司提供发票信息后，大约25个工作日内会收到相应的电子发票，请耐心等待，有问题请电话联系0312-2197731。 -->
                    <!-- 注：开票信息：纳税人识别号、电话、开户银行名称等信息的的修改请在【开票信息】菜单维护。 -->
                    <p>{{ $t('cusEntry.supplement20250205.redInvoiceReasonTip1') }}</p>
                    <p>{{ $t('cusEntry.supplement20250205.redInvoiceReasonTip2') }}</p>
                    <p>{{ $t('cusEntry.supplement20250205.redInvoiceReasonTip3') }}</p>
                  </srm-col>
                </srm-row>
              </el-collapse-item>
            </el-collapse>
        ...
        <template slot="buttonOne" class="buttonOneClass">
          ...
          >
            <!-- 下载委托代付凭证模板 -->
            {{ $t('cusEntry.supplement20250205.downloadTemplate') }}
          </el-button>
        </template>
      </CWorkflowMulti>
    </el-main>
  ...
  methods: {
    ...
    download(){
      downloadFileLink(
        ...
        // '代付凭证模板'
        this.$t('cusEntry.supplement20250205.agencyPayTemplate')
      ).catch(() => {
    ...
    saveBill (allparam) {
      ...
      yixiangJin.save(saveData).then(() => {
        this.$message({
          message: this.$t('sourcingBuyer.stagingSuccess'),
          type: 'success'
        })
        this.back()
      })
