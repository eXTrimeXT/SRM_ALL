...
export default {
  ...
  data () {
    return {
      ...
      preArr: [
        ...
        // 招标专家
        { prop: 'createdFullName', label: this.$t('cusEntry.bidSuperviseReport.souPrincipal') },
        // {
      ...
      tableHeader: [
        ...
        // },
        {
          ...
          buttons: [
            ...
            //   // 项目状态：技术投标中、商务投标中（投标中） && extTenderFlag != 'Y'
            //   show: row => ['TECH_BID', 'BUS_BID'].includes(row.projectStatus) && row.extTenderFlag !== 'Y',
            //   formattor: () => this.$t('cusEntry.biddingSettings.extNotAttend'),
            //   callback: row => this.openConfirmBidDialog(row, 'extNotAttend')
            // },
            // 投标
            {
            ...
            // 点击【确认投标】后 extTenderFlag='Y' 出现【发起质疑】按钮
            {
              ...
              formattor: () => this.$t('cusEntry.supplement20250205.questioned'), // 发起质疑
  ...
  methods: {
    ...
    // 确认投标 or 不参与
    openConfirmBidDialog (row, type='confirm') {
      this.editRow = {
        type,
        id: row.orderId,
        idKey: 'orderId'
    ...
    openQuoteTab (row, flag) {
      ...
      if (flag === 'edit' && this.isExamFlag) {
        ...
        this.$emit('tab-add', {
          ...
          title: flag == 'edit' ? this.$t('cusEntry.supplement20250205.vendorQuota') : this.$t('cusEntry.supplement20250205.vendorQuotaHistory'),
