    ...
    <el-main>
      <el-form ref="form" :model="form" :rules="formRules">
        <el-collapse v-model="colValue">
          ...
          <el-collapse-item :title="$t('cusEntry.supplement20250205.issueListAttachment')" name="2">
            <!-- 仅需求部门可新增 -->
            <FileDynamic
    ...
    async saveBill (type) {
      ...
      if (type === 'SUBMIT') {
        ...
        if (!problemFeedbackListFiles.length) {
          ...
          this.$message.warning(this.$t('cusEntry.supplement20250205.pleaseAddIssueListFeedbackAttachment'))
          return
        }
        for (let item of problemFeedbackListFiles) {
          if (!item.fileId) {
            // this.$message.warning('问题清单反馈附件-附件不能为空')
            this.$message.warning(this.$t('cusEntry.supplement20250205.issueListFeedbackAttachmentCannotBeEmpty'))
            return
          }
        }
