...
export default {
  ...
  data () {
    return {
      ...
      form: {
        siteReviewPlanId: 'ID',
        vendorId: this.$t('cusEntry.supplement20250205.supplierId'),
        vendorName: this.$t('common.companyName'),
        vendorCode: this.$t('common.vendorCode'),
        orgId: this.$t('cusEntry.supplement20250205.purchaseOrgId'),
        orgCode: this.$t('cusEntry.supplement20250205.purchaseOrgCode'),
        orgName: this.$t('cusEntry.supplement20250121.purchaseOrg'),
        categoryCode: this.$t('components.category.categoryCode'),
        categoryName: this.$t('components.category.categoryName'),
        planType: this.$t('vendorMod.planType'),
        planStartDate: this.$t('cusEntry.supplement20250205.planStartTime'),
        planProcessStatus: this.$t('vendorMod.planProcessStatus'),
        planConfirmCode: this.$t('vendorMod.planConfirmCode'),
        siteReviewCode: this.$t('vendorMod.siteReviewCode'),
        planStatus: this.$t('vendorMod.planStatus'),
        createdId: this.$t('monitorBizConfig.createdId'),
        createdBy: this.$t('common.creator'),
        creationDate: this.$t('common.creationTime'),
        createdFullName: this.$t('cusEntry.supplement20250205.creatorNewName'),
        createdByIp: this.$t('monitorBizConfig.createdByIp'),
        lastUpdatedId: this.$t('monitorBizConfig.lastUpdatedId'),
        lastUpdatedBy: this.$t('common.lastUpdatePeople'),
        lastUpdateDate: this.$t('dataConfMod.lastUpdateDate'),
        lastUpdatedByIp: this.$t('monitorBizConfig.lastUpdatedByIp'),
        lastUpdatedFullName: this.$t('cusEntry.supplement20250205.lastUpdaterName'),
        tenantId: this.$t('monitorBizConfig.tenantId'),
        version: this.$t('dataConfMod.version'),
        submitFlag: ''
      },
      rules: {},
      extraData: {
  ...
  created () {
    ...
    this.tableHeader = [
      ...
      {
        prop: 'contractNo',
        label: this.$t('bidMod.category_contractCode'),  // '合同编码'
        width: 120
      },
      {
        prop: 'buOrganizationName',
      ...
      {
        prop: 'warningCode',
        label: this.$t('cusEntry.supplement20250205.warningBill'),  // '预警单据'
        showType: 'button',
        btnStyle: 'text',
      ...
      {
        prop: 'levelName',
        label: this.$t('perfMod.resultsReview'),  // '绩效结果'
        width: 130
      },
      {
        prop: 'bidCode',
      ...
      {
        prop: 'extInvestNo',
        label: this.$t('cusEntry.bidMod.investNum'),  // '投资编号'
        width: 130
      },
      {
        prop: 'bidEndDate',
      ...
      {
        prop: 'bidManagerFullPath',
        label: this.$t('cusEntry.supplement20250205.bidExpertCompanyDept'),  // '招标专家公司部门'
        width: 130
      },
      {
      ...
      {
        prop: 'contractManagerFullPath',
        label: this.$t('cusEntry.supplement20250205.contractHandlerCompanyDepartment'),  // '合同经办人公司部门'
        width: 130
      },
      {
        prop: 'supplierRemark',
        label: this.$t('cusEntry.supplement20250205.processRemarks'),  // '处理备注'
        width: 130
      },
      {
        ...
        buttons: [
          {
            ...
            show: row => ['', null].includes(row.warningCode),
            formattor: () => {
          ...
          },
          {
            ...
            show: row => ['', null].includes(row.supplierRemark),
            formattor: () => {
              return this.$t('cusEntry.supplement20250205.supplierProcessing')  // '供应商处理'
            }
          }
        ]
      }
  ...
  methods: {
    editHandle2 (row) {
      //供应商处理
      this.$prompt(this.$t('cusEntry.supplement20250205.supplierProcessing'), '', {
        ...
        inputErrorMessage: this.$t('common.pleaseTypeContents')
      }).then(({ value }) => {
        const obj = {
          'supplierRemark': value,
          'scoreHeaderId': row.scoreHeaderId
        }
        this.$http({
          url: '/api-pef/projectScoreHeader/supplierReply',
          method: 'POST',
          data: obj,
          loading: true
        }).then(data => {
    ...
    getQuerydata (params) {
      this.queryParam = {
        levelNames: [this.$t('vendorMod.text8')], // '不合格'
        ...params
      }
      this.$nextTick(() => {
