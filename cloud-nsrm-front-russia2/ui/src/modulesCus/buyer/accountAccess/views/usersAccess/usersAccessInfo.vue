<template>
  <el-container class="flex-container the_usersAccessInfo_wrapper" direction="vertical">
    <el-main>
          ...
          <el-row :gutter="32">
            ...
            <el-col :span="8">
              ...
              <el-form-item :label="$t('dataConfMod.userID')" prop="username">
                <el-input v-model.trim="form.username" :disabled="isEdit" @change="usernameChange" />
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <!-- 姓名 -->
            ...
            <el-col :span="8">
              ...
              <el-form-item :label="$t('dataConfMod.employeeId')">
                <el-input v-model="form.ceeaEmpNo" disabled />
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <!-- 板块名称 -->
              <el-form-item :label="$t('cusEntry.dataConfMod.plateName')" prop="extSectorId">
                <QuickSearch
                  :show-input="form.extSectorName"
                  show-key="organizationName"
                  auto-query
                  :scope-data="form"
                  name="scc_base_organization_sector"
                  :disabled="form.userType != 'BUYER'"
                  @close-quicksearch="extSectorChange"
                />
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <!-- 公司名称 -->
              <el-form-item :label="$t('dataConfMod.ceeaCompanyName')" prop="ceeaCompany">
                <QuickSearch
                  :show-input="form.ceeaCompanyDescr"
                  show-key="organizationName"
                  auto-query
                  :scope-data="form"
                  name="scc_base_organization_company"
                  :pre-query-data="{'t.PARENT_ORGANIZATION_IDS': form.extSectorId}"
                  :disabled="form.userType != 'BUYER' || !form.extSector"
            ...
            <el-col :span="8">
              <!-- 部门 -->
              <el-form-item :label="$t('dataConfMod.department')" prop="ceeaDeptId">
                <QuickSearch
                  :show-input="form.department"
                  show-key="descr"
                  auto-query
                  :scope-data="form"
                  name="scc_base_organization_dept"
                  :pre-query-data="{'t.PARENT_ORGANIZATION_IDS': form.ceeaCompany}"
                  :disabled="form.userType != 'BUYER' || !form.ceeaCompany"
            ...
            <el-col :span="8">
              ...
              <el-form-item :label="$t('dataConfMod.positionName')">
                <el-input v-model.trim="form.ceeaJobcodeDescr" />
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <!-- 生效日期 -->
            ...
            <el-col :span="8">
              <!-- 办公电话 -->
              <el-form-item :label="$t('cusEntry.supplement20250205.officePhone')" prop="extOfficePhone">
                <el-input v-model.trim="form.extOfficePhone" />
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <!-- 是否有经验 -->
              <el-form-item :label="$t('cusEntry.supplement20250218.hasExperience')">
                <DictSelect v-model="form.extIfExperienced" code="YES_OR_NO" clearable/>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
      ...
      <el-collapse v-model="activeDims" class="tab-form-style">
        ...
        <el-collapse-item name="1" :title="$t('dataConfMod.orgAccess')">
          ...
          <el-table
            :ref="gridId"
            :data="organizationUsers.slice((pagination.pageNum - 1) * pagination.pageSize, pagination.pageNum * pagination.pageSize)"
            border
            max-height="500px"
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <el-button type="text" class="detail-pbtn" @click.stop="orgDel(scope.row,scope.$index)">
                  {{ $t("common.delete") }}
                </el-button>
              </template>
            </el-table-column>
        ...
        <el-collapse-item name="2" :title="$t('dataConfMod.roleAccess')">
          ...
          <el-table :ref="gridId2" :data="roleUsers" border max-height="350px">
            <!-- 角色名称 -->
    ...
    <srm-dialog
      ...
      :title="this.$attrs.params?.flag == 'edit' ? $t('编辑组织权限') : $t('dataConfMod.addOrgAccess')"
      size="middle"
...
import { accountApi, accessApi } from 'modcb@/accountAccess/api'
...
export default {
  ...
  computed: {
    ...
    rules () {
      return {
        ...
        nickname: [{ required: true, message: this.$t('dataConfMod.msgUserName') }], 
        phone: [
          // { required: this.form.userType == 'BUYER', message: this.$t('common.pleaseInput') },
          {
        ...
        extSectorId: [{ required: this.form.userType == 'BUYER', message: this.$t('common.pleaseSelect') }],
        ceeaCompany: [{ required: this.form.userType == 'BUYER', message: this.$t('common.pleaseSelect') }],
        ceeaDeptId: [{ required: this.form.userType == 'BUYER', message: this.$t('common.pleaseSelect') }]
      }
    }
  },
  ...
  methods: {
    usernameChange (val) {
      this.form.ceeaEmpNo = val
    },
    extSectorChange (val, scope) {
      scope.extSectorId = val ? val.organizationId : null
      scope.extSector = val ? val.organizationCode : ''
      scope.extSectorName = val ? val.organizationName : ''

      scope.ceeaCompanyDescr = ''
      scope.ceeaCompany = null
      scope.ceeaDeptId = null
      scope.department = ''
    },
    getDepObj (val, scope) {
      scope.ceeaDeptId = val ? val.organizationId : null
      scope.department = val ? val.organizationName : ''
    },

    getCompany (val, scope) {
      scope.ceeaCompanyDescr = val ? val.organizationName : ''
      scope.ceeaCompany = val ? val.organizationId : null
    },
    // 选择项发生变化
    ...
    async batchDelete () {
      ...
      // 获取已经存在id的行
      let organizationUserRelIds = []
      let orgIds = []
      selectList.forEach(row => {
        if (row.organizationUserRelId) {
          organizationUserRelIds.push(row.organizationUserRelId)
        } else {
          orgIds.push(row.organizationId)
        }
      })
      if (organizationUserRelIds.length) {
        let transformParams = transformMQL.save('OrganizationUser', organizationUserRelIds, 'delete')
        const response = await this.$http({
          url: '/api-base/api-ql/OrganizationUser/delete',
          method: 'POST',
          data: transformParams,
          loading: true
        })
        if (response) {
          organizationUserRelIds.forEach(id => {
            const realIndex = this.organizationUsers.findIndex(row => row.organizationUserRelId === id)
            this.organizationUsers.splice(realIndex, 1)
          })
          orgIds.length === 0 && this.$message.success(this.$t('common.successDelete'))
        }
      }
      if (orgIds.length) {
        orgIds.forEach(id => {
          const realIndex = this.organizationUsers.findIndex(row => row.organizationId === id)
          this.organizationUsers.splice(realIndex, 1)
        })
        this.$message.success(this.$t('common.successDelete'))
      }
      this.$refs[this.gridId].clearSelection()
    },
    /* 页码变更 */
    ...
    paginationSizeChange (pageSize) {
      ...
      /* 判断数组长度是否达到分页要求 */
      const sourceLength = this.organizationUsers.length
    ...
    async orgListFun () {
      const { data } = await this.newOrganaztionTreehttp({})
      this.$http({
        url: '/api-base/orgUser/getOrganizationUserList',
        method: 'POST',
        data: {
          pageNum: this.orgPageNum,
          pageSize: this.orgPageSize,
          userId: this.userId,
          '_page': this.orgPageNum,
          '_pagesize': this.orgPageSize
        },
        loading: true
      }).then((res) => {
        console.log(res, 'res')
        const _list = getOrgList(
          this.transfromDate(res.data.list),
          data,
          store.fullPathNameMap || new Map()
        )
        this.organizationUsers = _list.orgList.map(item => {
    ...
    initFormData () {
      this.form = {
        userType: 'BUYER',
        extOfficePhone: null,
        extIfExperienced:null,
        ceeaCompany: null,
        ceeaCompanyDescr: null,
        ceeaEmpNo: null,
        extSector: '',
        extSectorId: null,
        extSectorName: '',
    ...
    async orgDel (row, index) {
      const { organizationUserRelId } = row
      const {
        pageNum,
        pageSize
      } = this.pagination
      const realIndex = (pageNum - 1) * pageSize + index
      if (organizationUserRelId) {
        let transformParams = transformMQL.save('OrganizationUser', [organizationUserRelId], 'delete')
        const response = await this.$http({
          url: '/api-base/api-ql/OrganizationUser/delete',
          method: 'POST',
          data: transformParams,
          ...
          if (this.isEdit) {
            accountApi.modifyBuyer(params)
              .then(res => {
                ...
                this.$http({
                  url: '/api-base/base-anon/permission/delRedisByUser',
                  method: 'GET',
