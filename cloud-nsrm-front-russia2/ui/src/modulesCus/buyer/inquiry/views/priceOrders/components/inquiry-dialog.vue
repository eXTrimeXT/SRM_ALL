  ...
  >
    ...
    >
      ...
      <el-table-column
        ...
        min-width="120"
        :formatter="row => {
          return row.orderCurrency ? $getDictLabel('currency', row.orderCurrency) : ''
        }"
      />
      <!-- 转换后币种 -->
      <el-table-column
...
export default {
  ...
  methods: {
    ...
    getQueryData (params = {}) {
      ...
      inqBuyerHttp.price.inquiry(queryParams).then(async res => {
        if (res.data) {
          // 查询多语言物料信息
          const materialIds = res.data.list.map(item => item.itemId)
          ...
          this.tableData = res.data.list.map(item => {
            ...
            return {
              ...item,
              extConvertCurrency: 'RUB', // 转换后币种默认卢布
