    ...
    <el-main>
      ...
      <MainHeader>
        <template slot="left">
          ...
          <AuthorityButton
            type="primary"
            code="inq:priceOrderList:exprotDetail"
            @click="exprotDetail"
          >
...
export default {
  ...
  created () {
    this.preArr = [
      ...
      {
        prop: 'vendorName',
        label: this.$t('cusEntry.inq.vendorName')
      },
      {
        prop: 'extMaterialModel',
        label: this.$t('cusEntry.inq.extMaterialModel')
      }
    ]
    this.tableHeader = [
      {
      ...
      // },
      {
        prop: 'extHighestNoTaxPrice',
        label: this.$t('cusEntry.inq.topNoTaxPrice'),
        minWidth: 120
      },
      {
        prop: 'extPassRejectedFlowDate',
        label: this.$t('cusEntry.inq.extPassRejectedFlowDate'),
        minWidth: 120,
        dataType: 'dateTime'
      },
      {
      ...
      },
      {
        ...
        buttons: [
          {
            show: row => ['DRAFT', 'WITHDRAW', 'REJECTED'].includes(row.fixPriceStatus) && this.curUserId == row.createdId,
          ...
          {
            show: row => ['DRAFT'].includes(row.fixPriceStatus) && this.curUserId == row.createdId,
          ...
          // 审批
          {
            callback: row => this.editTab(row, 'approve'),
            show: row => ['SUBMITTED'].includes(row.fixPriceStatus) && (this.curUserId == row.createdId || row.isApprover == 'Y'),
  ...
  methods: {
    ...
    afterQuery () {
      this.$refs[this.gridList].setTableData(async tableData => {
        const res = await this.$api.base.flowAPI.queryTodo()
        let queryTodoList = res.data || []
        tableData.forEach(tableItem => {
          let obj = queryTodoList.find(todoItem => tableItem.fixPriceHeadId + '' === todoItem.businessId + '')
          if (obj) {
    ...
    exprotDetail () {
      ...
      downloadFileLinkByPost(
        '/api-sou/npm/fix-price/buyer/exportFixPrice',
