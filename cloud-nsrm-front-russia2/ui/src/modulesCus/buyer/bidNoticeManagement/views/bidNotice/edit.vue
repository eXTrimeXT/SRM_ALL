    ...
    async preNextStepHandler () {
      ...
      for (let item of bidNoticeDetails) {
        if (!item.isWin) {
          ...
          this.$message.warning(
            this.$t('cusEntry.supplement20250205.notEmptyWithName', {
              title: this.$t('cusEntry.supplement20250205.bidNoticeInfo'),
              name: this.$t('cusEntry.bidMod.isWin')
            })
          )
          return false
        }
        // if (item.isWin === 'Y' && !item.winAmount) {
        //   // 中/落标信息-中标供应商【中标金额】不能为空
        //   this.$message.warning(
        //     this.$t('cusEntry.supplement20250205.notEmptyWithName', {
        //       title: this.$t('cusEntry.supplement20250205.bidNoticeInfo'),
        //       name: this.$t('cusEntry.supplement20250205.winVendorWithName', { name: this.$t('bidMod.wonBidAmount')})
        //     })
        //   )
        //   return false
        // }
        // if (item.isWin === 'Y' && !item.isPerformanceEvaluated) {
        //   // 中/落标信息-中标供应商【是否履约评价】不能为空
        //   this.$message.warning(
        //     this.$t('cusEntry.supplement20250205.notEmptyWithName', {
        //       title: this.$t('cusEntry.supplement20250205.bidNoticeInfo'),
        //       name: this.$t('cusEntry.supplement20250205.winVendorWithName', { name: this.$t('')})
        //     })
        //   )
        //   return false
        // }
        // if (item.isWin === 'Y' && item.isPerformanceEvaluated === 'N' && !item.nonPerformanceReason) {
        //   // 中/落标信息-中标供应商,是否履约评价为否，【不履约评价的原因】不能为空
        //   this.$message.warning(
        //     this.$t('cusEntry.supplement20250205.notEmptyWithName', {
        //       title: this.$t('cusEntry.supplement20250205.bidNoticeInfo'),
        //       name: this.$t('cusEntry.supplement20250205.winVendorWithName', { name: this.$t('')})
        ...
        if (item.isWin === 'Y' && !item.noticeAttachmentId) {
          // 中/落标信息-中标供应商【通知书附件】不能为空
          this.$message.warning(
            this.$t('cusEntry.supplement20250205.notEmptyWithName', {
              title: this.$t('cusEntry.supplement20250205.bidNoticeInfo'),
              name: this.$t('cusEntry.supplement20250205.winVendorWithName', { name: this.$t('cusEntry.supplement20250121.noticeAttachmentId')})
            })
          )
          return false
        }
        if (item.isWin === 'Y' && (!item.contractSignUnitList || !item.contractSignUnitList.length)) {
          // 中/落标信息-中标供应商【合同签署单位】不能为空
          this.$message.warning(
            this.$t('cusEntry.supplement20250205.notEmptyWithName', {
              title: this.$t('cusEntry.supplement20250205.bidNoticeInfo'),
              name: this.$t('cusEntry.supplement20250205.winVendorWithName', { name: this.$t('cusEntry.supplement20250121.contractSign')})
            })
          )
          return false
        }
      ...
      for (let item of bidNoticeInternals) {
        if (!item.attachmentId) {
          ...
          this.$message.warning(
            this.$t('cusEntry.supplement20250205.notEmptyWithName', {
              title: this.$t('cusEntry.supplement20250205.innerNotice'),
              name: this.$t('cusEntry.supplement20250205.winVendorWithName', { name: this.$t('cusEntry.supplement20250121.noticeAttachmentId')})
            })
          )
          return false
