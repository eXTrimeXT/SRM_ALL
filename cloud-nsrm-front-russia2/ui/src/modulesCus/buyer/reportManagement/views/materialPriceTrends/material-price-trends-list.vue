...
export default {
  ...
  methods: {
    ...
    arrangeData () {
      ...
      result.push({
        label: `${curYear}${this.$t('time.years')}${curMonth}${this.$t('time.months')}`,
      ...
      for (let i = 11; i > 0; i--) {
        if (curMonth > 1) {
          result.push({
            label: `${curYear}${this.$t('time.years')}${curMonth - 1}${this.$t('time.months')}`,
            prop: this.dynamicProps[i - 1],
            minWidth: 120
          })
        } else {
          result.push({
            label: `${curYear - 1}${this.$t('time.years')}${curMonth + 12 - 1}${this.$t('time.months')}`,
            prop: this.dynamicProps[i - 1],
            minWidth: 120
          })
        }
