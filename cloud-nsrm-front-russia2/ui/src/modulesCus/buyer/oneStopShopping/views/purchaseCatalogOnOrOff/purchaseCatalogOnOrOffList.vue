        ...
        <template slot="left">
          ...
          <el-dropdown @command="batchOperation">
            <el-button type="primary">
              <!-- 批量操作 -->
              {{ $t("bidMod.batchOperation") }}<i class="el-icon-arrow-down el-icon--right" />
            </el-button>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="batchOn">
                <!-- 批量上架 -->
                {{ $t("cusEntry.supplement20250205.batchListing") }}
              </el-dropdown-item>
              <el-dropdown-item command="timeOn">
                <!-- 定时上架 -->
                {{ $t("cusEntry.supplement20250205.scheduledPublish") }}
              </el-dropdown-item>
              <el-dropdown-item command="timeOff">
                <!-- 取消定时上架 -->
                {{ $t("cusEntry.supplement20250205.cancelScheduledPublish") }}
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
          <!-- 批量上传图片 -->
          <CUploadFile
            ...
            :title="$t('cusEntry.supplement20250205.batchUploadImages')"
            :limit="100"
            :cus-data="fileInfo"
            :fileList="fileList"
            :showFileList="false"
...
export default {
  ...
  data () {
    return {
      ...
      preArr: [
        ...
        {
          prop: 'status',
          label: this.$t('cusEntry.supplement20250205.productStatus'),  // '商品状态'
          type: 'dict',
      ...
      tableHeader: [
        ...
        {
          prop: 'extEffectiveDate',
          label: this.$t('cusEntry.supplement20250205.priceStartDate'),  // '价格开始日期'
          minWidth: 120,
          formattor (val) {
        ...
        {
          prop: 'extExpirationDate',
          label: this.$t('cusEntry.supplement20250205.priceEndDate'), // '价格结束日期'
          minWidth: 120,
          formattor (val) {
            return val ? parseTime(val, '{y}-{m}-{d}') : ''
          }
        ...
        },
        {
          ...
          label: this.$t('cusEntry.supplement20250205.distanceToShelfRemovalTime'),  // '距离下架时间'
          minWidth: 150,
          formattor: (val, row) => {
        ...
        {
          prop: 'extGoodsCode',
          label: this.$t('cusEntry.supplement20250205.productCode'),  // '商品编码'
          minWidth: 150
        },
        {
          prop: 'categoryName',
  ...
  methods: {
    ...
    afterQuery (data) {
      this.$refs.tableRef.setTableData(async tableData => {
        let response = []
    ...
    async batchOperation (type) {
      if (this.selection.length == 0) {
        this.$message.warning(this.$t('cusEntry.supplement20250205.selectProductFirst'))  // '请先勾选商品'
        return
      }
      ...
      let callback = null
      if (type == 'timeOn') {
        tipMsg = this.$t('cusEntry.supplement20250205.confirmScheduledListing')  // '是否确认定时上架？'
        callback = this.timeOnBill
      } else if (type == 'timeOff') {
        tipMsg = this.$t('cusEntry.supplement20250205.isConfirmCancelScheduledUpload')  // '是否确认取消定时上架？'
        callback = this.timeOffBill
      } else if (type == 'batchOn') {
        tipMsg = this.$t('cusEntry.supplement20250205.isConfirmBatchListing')  // '是否确认批量上架？'
    ...
    onBill () {
      ...
      this.$http({
        ...
        this.$message.success(this.$t('cusEntry.supplement20250205.listingSuccess'))  // '上架成功！'
        this.getQuerydata()
      })
    },
    // 定时上架
    timeOnBill () {
      ...
      this.$http({
        ...
        this.$message.success(this.$t('cusEntry.supplement20250205.scheduledPublishSuccess'))  // '定时上架成功！'
        this.getQuerydata()
      })
    },
    // 取消定时上架
    timeOffBill () {
      ...
      this.$http({
        ...
        this.$message.success(this.$t('cusEntry.supplement20250205.cancelScheduledPublishSuccess'))  // '取消定时上架成功！'
        this.getQuerydata()
      })
    },
    // 多个文件上传成功
    mulUploadSuccess (file, fileList, uploadNum) {
      ...
      if (this.fileAttachList.length == uploadNum) {
        ...
        this.$http({
          ...
          this.$message.success(this.$t('cusEntry.supplement20250205.importImageSuccess'))  // '导入图片成功！'
          this.getQuerydata()
        })
      }
    }
