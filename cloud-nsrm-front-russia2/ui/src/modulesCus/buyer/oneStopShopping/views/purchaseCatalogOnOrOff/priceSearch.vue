  ...
  data () {
    return {
      ...
      tableHeader: [
        ...
        {
          prop: 'extAreaName',
          label: this.$t('vendorMod.area1'),  // '区域'
          minWidth: 150,
          formattor: (val, row) => {
  ...
  methods: {
    radioChange (select) {
      ...
      this.currentRow = {
        ...
        unit: select.unit,
        effectiveDate: parseTime(select.effectiveStartDate, '{y}-{m}-{d}', true), // 价格开始日期
        expirationDate: parseTime(select.effectiveEndDate, '{y}-{m}-{d}', true), // 价格结束日期
