...
const initButtonConfig = ($form: any, $t: any) => {
  setTimeout(() => {
    ...
    let formHeaderValue = getHeaderField($form.values.form)
  ...
  if (companyId) {
    ...
    if (response && response.data && response.data.records.length) {
      ...
      if (baseInfo) {
        ...
        if (isPerson) {
          const { businessStartDate, businessEndDate } = baseInfo
          baseInfo.validityPeriodOfCard = [businessStartDate, businessEndDate]
...
const $submits = async (type, $form, $queryEngine, $message, $t, $bus) => {
  ...
  if (type == 'SAVE') { // 暂存的时候
    ...
    //   let validDate = (overseasRelation === 'INSIDE' && ['INSIDE_1', 'INSIDE_2', 'INSIDE_3', 'INSIDE_10', 'INSIDE_11'].includes(item.authNum)) ||
    //     (overseasRelation === 'PERSONAL' && ['PERSONAL_1', 'PERSONAL_2', 'PERSONAL_3'].includes(item.authNum)) ||
    //     overseasRelation === 'OUT'
    ...
    if (changeStatus == 'VENDOR_SUBMITTED') {
      let formHeaderValue = getHeaderField(allData)
      const componentInstance = $form.query('.SchemaWorkflow').take().componentProps.componentInstance
      ...
      $queryEngine.request.save(allData, { customizeAction: 'submit', query: { '*': {} } }).then(async (res: any) => {
        const changeId = res.records[0]
        let resFormData = await $queryEngine.request.read(changeId, { action: 'query' })
        let formHeaderValue = getHeaderField(resFormData.records[0])
        const componentInstance = $form.query('.SchemaWorkflow').take().componentProps.componentInstance
