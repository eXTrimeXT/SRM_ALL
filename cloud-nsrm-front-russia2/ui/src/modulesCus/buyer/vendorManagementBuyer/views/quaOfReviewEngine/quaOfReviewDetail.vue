...
// @ts-ignore
import { accessCommonApi, reviewFormStandard } from 'modb@/vendorManagementBuyer/api/vendorManagement'
// @ts-ignore
...
// @ts-ignore
import VendorProfileDetailRead from 'modcb@/vendorManagementBuyer/views/vendorProfileEngine/vendorProfileDetailReadEngine'
// @ts-ignore
import sourcingApplicationDetail from 'modb@/sourcing/views/sourcingApplicationBuyer/sourcingApplicationDetail'
import { vendorGreenApi, quaApi } from 'modcb@/vendorManagementBuyer/api/vendorManagement'
...
const initData = async ($form: any) => {
  ...
  if ($form.query('ReviewForm').get('data').curOpt === 'add') {
    ...
    // $form.values.creationDate = `${nowDate.getFullYear()}-${nowDate.getMonth() + 1}-${nowDate.getDate()}`
    if ($attrs.params.row) {
      const {
        companyCode,
        companyId,
...
const $getAllOrg = ($form: any, $message) => {
  if ($form.values.vendorId) {
    quaApi.getAllOrg($form.values.vendorId).then(res => {
      if (res.data) {
        ...
        $form.values.orgJournals = formData.allOrg.slice(0, formData.pageSize)
      }
    })
  }
}
  ...
  if (formData.allOrg.length < 1) {
    ...
    orgs.forEach((item: any) => {
      ...
      if (bol) {
        ...
        formData.allOrg.forEach((el: any, index: any) => {
          if (organizationId === el.orgId) {
            formData.allOrg.splice(index, 1, {
              ...el,
              reqHeadId
            })
          }
...
// 获取品类信息
const $addCategorysList = async (categoryList: any, $form: ViewModel, $message: any) => {
  const formData = $form.query('ReviewForm').get('data')
  if (formData.allCat.length < 1) {
    // const categoryIds = categoryList.map((item: any) => item.categoryId)
    // const res = await quaApi.getCategoryInfoById(categoryIds)
    categoryList.forEach((item: any, index: any) => {
      // const data = res.data.find((it: any) => it.categoryId === item.categoryId)
      // item.existCountOfCompany = data.existCountOfCompany
      // item.supplierCountLimit = data.supplierCountLimit
      // item.supplierCountLimitFlag = data.supplierCountLimitFlag
      item.categoryJournalId = null
      // $form.query('cateJournals').take((field: any) => field.value.push(item))
      formData.allCat.push(item)
      if (index === 0 && $form.values.ifSiteForm === 'N') {
    ...
    categoryList.forEach((item: any) => {
      ...
      if (bol) {
        ...
        formData.allCat.forEach((el: any, index: any) => {
          if (categoryId === el.categoryId) {
            formData.allCat.splice(index, 1, { ...el, reqHeadId })
          }
        })
      }
    })
  }
  formData.catLength = formData.allCat.length
  $form.values.cateJournals = formData.allCat.slice(0, formData.catPageSize)
}
...
const $delSourcingItem = ($form: ViewModel, $table: any, row: any, rowIndex: any) => {
  ...
  $form.values.cateJournals.forEach((item: any, index: any) => {
    if (item.reqHeadId === row.reqHeadId) {
      $delCateJournals($form, item, index)
    }
  })
}
const $delOrgJournals = ($form: ViewModel, item: any, index: any) => {
  let formData = $form.query('ReviewForm').get('data')
  const deleteIndex = formData.allOrg.findIndex(orgItem => orgItem.orgId === item.orgId)
  ...
  /* 处理前端假分页删完当前页页面数据为空 */
  const orgateJournals = formData.allOrg.slice((pageNum - 1) * pageSize, pageNum * pageSize)
  if (orgateJournals.length === 0 && formData.allOrg.length) {
    $form.values.orgJournals = formData.allOrg.slice((pageNum - 2) * pageSize, (pageNum - 1) * pageSize)
...
const $delCateJournals = ($form: ViewModel, item: any, index: any) => {
  let formData = $form.query('ReviewForm').get('data')
  const deleteIndex = formData.allCat.findIndex((it: any) => it.categoryId === item.categoryId)
  formData.allCat.splice(deleteIndex, 1)
  formData.catLength = formData.allCat.length
  const { catPageSize, catPageNum } = formData
  /* 处理前端假分页删完当前页页面数据为空 */
  const cateJournals = formData.allCat.slice((catPageNum - 1) * catPageSize, catPageNum * catPageSize)
  if (cateJournals.length === 0 && formData.allCat.length) {
    $form.values.cateJournals = formData.allCat.slice((catPageNum - 2) * catPageSize, (catPageNum - 1) * catPageSize)
    formData.catPageNum = catPageNum - 1
  } else {
    $form.values.cateJournals = cateJournals
  }
  // 品类清空完后
...
const $validateForm = ($form: any, $message: any) => {
  // 基础信息
  const baseInfoFlag = $form.values.quaReviewType && $form.values.vendorName && $form.values.reviewExplain
  if (!baseInfoFlag) {
    // 请完成资质审查单据必填项
    $message.error($t('vendorMod.requireQualification'))
    return false
  }
  ...
  //   if (!item.orgId || !item.clearCurrency || !item.paymentMethod || !item.paymentTerms) {
    // 请完善财务信息必填项
  //     $message.error($t('vendorMod.requireFinancialInfo'))
  //     financeFlag = false
  //     break
  //   }
  // }
  ...
  for (let item of fileRecords) {
    if (item.ifRequired === 'Y' && !item.fileId && $form.values.ifSiteForm === 'N') {
      // 设置了必传
      // 请上传
    ...
    if (item.ifValidDate === 'Y' && !item.fileValidDate && $form.values.ifSiteForm === 'N') {
      // 设置了有效期必填
...
const $saveBill = (type: string, $form: any, $queryEngine: any, $confirm: any, $message: any, $bus: any) => {
  ...
  const cateJournals = $form.query('ReviewForm').get('data').allCat
  values.orgJournals = orgJournals
  values.cateJournals = cateJournals
  if (type === 'SAVE') {
...
const $submitData = (type: string, $values: any, $form: any, $queryEngine: any, $confirm: any, $message: any, $bus: any) => {
  ...
  // 处理附件信息空字符串报错
  if ($values.ifSiteForm === 'Y') {
    $queryEngine.request.save(form, { loading: true, customizeAction: 'saveWithoutFlow' }).then(async (res: any) => {
      if (res.data && res.data.length > 0) {
        const reviewFormId = res.originalData?.records[0] || ''
        $form.values.reviewFormId = reviewFormId
        await $queryEngine.request.read()
        if (type === 'SUBMIT') {
          $form.values.approveStatus = 'APPROVED'
          const payload = {
            type: 'ReviewForm',
            loading: true,
    ...
    form.fileRecords = $values.ifSiteForm === 'N' ? $handleEmptyField(form.fileRecords, ['score']) : []
    $queryEngine.request.save(form, { loading: true, customizeAction: 'save' }).then(async (res: any) => {
      if (res.data && res.data.length > 0) {
        const reviewFormId = res.originalData?.records[0] || ''
        $form.values.reviewFormId = reviewFormId
        await $queryEngine.request.read()
    ...
    'x-query-engine': {
      ...
      actions: {
        read: {
          ...
          formData.allCat = cateJournals
          formData.catLength = cateJournals.length
          ...
          $form.setValues({
            ...
            cateJournals: cateJournals.slice(0, formData.catPageSize)
          })
          // 获取调查表清单信息
          $getQuestSupplierList($form, detailData.vendorId || $attrs.params.row.vendorId)
        }`)
        },
        save: {
