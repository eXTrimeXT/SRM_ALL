...
import vendorProfileDetailReadEngine from 'modcb@/vendorManagementBuyer/views/vendorProfileEngine/vendorProfileDetailReadEngine'
export default {
  ...
  created () {
    ...
    this.tableHeader = [
      ...
      {
        prop: 'documentType',
        label: () => _this.$t('orderMod.buyerOrderSynergy.idType'),
        width: 150,
        // dataType: 'dict',
        // code: 'ID_TYPE'
        formattor: (val, row) => {
          return val === 'MANAGEMENT_ATTACH' ? _this.$getDictLabel('CERTIFICATE_TYPE', row.authNum) : _this.$getDictLabel('ExpiredCertificateType', val)
        }
      },
      {
        prop: 'endDate',
        ...
        formattor (val) {
          ...
          if (new Date(val) > new Date() && new Date(val) < day30) {
            ...
            return (
              '<span class="el-icon-error" style="color:#F56C6C;padding-right: 11px;"></span>' + _this.$parseTime(val)
            )
          } else {
            return _this.$parseTime(val)
          }
        }
      },
      {
