<template>
  <div class="quote-selection">
    ...
    >
      ...
      />
      <!--b 发起新一轮-->
      <el-button v-if="showQuoteEvaluation" type="primary" @click="continueNextRound">
        {{ $t('bidMod.biddingControl.startNewRound') }}
      </el-button>
      <!--b 总价比价-->
      <el-button v-if="showQuoteEvaluation" type="primary" @click="compareTotalAmount">
        {{ $t('cusEntry.bidMod.totalPriceComparison') }}
      </el-button>
      <!--b 询价结束-->
      <el-button v-if="showQuoteEvaluation" type="primary" @click="stopInquiry">
    ...
    >
      ...
      >
        ...
        <el-table-column
          align="center"
          prop="firstStandardNotaxPrice"
          label="第一轮未税价格"
          min-width="110"
        >
          <template slot-scope="scope">
            {{ showValueByProps(scope.row.orderItemList, 'firstStandardNotaxPrice', item.vendorId) }}
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('bidMod.taxRate2')"
    ...
    <ContinueTalkPriceDialog
      ...
      :header="header"
      :material-list="quoteSelectionTable"
  ...
  methods: {
    ...
    exportDetail () {
      downloadFileLinkByPost(
        '/api-sou/npm/buyer/ext_pur_inq/select/queryItemSelectInfo/downloadExcel',
    ...
    initPageList () {
      this.pageList = []
      let array = this.quoteSelectionTable
      if (this.areaCode) {
        array = array.filter((item) => {
          let areaList = item.area ? item.area.split(',') : []
          return areaList.indexOf(this.areaCode) !== -1
        })
      }
      this.pageTotal = array.length
      let index = this.pageNum
      let size = this.pageSize
      this.pageList = array.filter((item, i) => {
        return (index - 1) * size <= i & i < index * size
      })
      this.$nextTick(() => {
        if (this.$refs.quoteSelectionTable) this.$refs.quoteSelectionTable.doLayout()
      })
    },
    /* 终止询价 */
    stopInquiry () {
      purInqApi.select.stopInquiry(this.header.projectId).then(res => {
    ...
    showSupplierChangeDialog (row) {
      ...
      purInqApi.select.getChangeSuppilers({ souItemId, round }).then(res => {
    ...
    /* 总价比价 */
    compareTotalAmount () {
      purInqApi.select.compareTotalAmount(this.header.projectId).then(res => {
    ...
    getSelectList (winRow) {
      purInqApi.select.getSelectListByProjectId({ projectId: this.header.projectId }).then(res => {
        if (res.data) {
          const {
            itemList,
            vendorList
    ...
    /* 获取并写入评选的筛选条件 */
    async getSearchInfo () {
      const response = await purInqApi.select.getSearchInfo(this.header.projectId)
    ...
    async getQuoteSelectionDetailData () {
      ...
      const response = await purInqApi.select.getSelectingPage(paramsData)
    ...
    /* 智能评选 */
    async evaluateOne () {
      const response = await purInqApi.select.autoSelecting(this.header.projectId)
      if (response) {
        // 报价评选排名计算已完成
    ...
    async changeWinStatus (type) {
      ...
      const response = await purInqApi.select.changeSelectStatus({ orderItemIds, toWin })
      if (response) {
    ...
    async changeSelectResult (type) {
      ...
      const response = await purInqApi.select.changeSelectResult({
    ...
    priceComparisonExport () {
      ...
      downloadFileLink(
        purInqApi.select.getExportPriceCompareInfoUrl(this.header.projectId),
    ...
    /* 公示本轮结果 */
    async openResult () {
      const response = await purInqApi.select.openResult(this.header.projectId)
      if (response) {
        this.$message.success(this.$t('common.successSubmit'))
        // 更新列表
        await this.getQuoteSelectionDetailData()
    ...
    async toApprovalBill () {
      const httpMethod = meiqlCtrl.priceApproval === 'Y' ? purInqApi.select.createPricingApprovalNew : purInqApi.select.getCreatePricingApprovalUrl
      // 公共方法生成价格审批单
    ...
    async saveBatchPaymentType (val) {
      const response = await purInqApi.select.batchUpdatePayment({
        projectId: this.header.projectId,
    ...
    async rowClick (row) {
      ...
      const response = await purInqApi.select.getPriceCompareInfos({
        souItemId: row.souItemId,
        round: row.round
    ...
    async changeQuoteQuantity () {
      ...
      const response = await purInqApi.select.changeQuoteQuantity(this.multipleSelection.map(item => {
        return {
          orderItemId: item.orderItemId,
          winAmount: item.winAmount
        }
