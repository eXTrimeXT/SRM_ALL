<template>
  <div class="wrapper">
    ...
    <!-- 供应商名称 -->
      ...
      <el-table-column
        prop="nature"
        :label="$t('cusEntry.supplement20250121.nature')"
        showOverflowTooltip
      >
        <template v-slot="{row,$index}">
          <!-- <DictSelect v-if="!readonly" v-model="row.nature" code="COMPANY_NATURE_NEW" /> -->
      ...
      <el-table-column
        prop="arrivalRate"
        :label="$t('cusEntry.supplement20250121.arrivalRate')"
        showOverflowTooltip
      >
        <template v-slot="{row,$index}">
