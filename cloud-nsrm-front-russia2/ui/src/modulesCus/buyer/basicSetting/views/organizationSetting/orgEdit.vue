<template>
  <el-container class="flex-container org-edit" direction="vertical">
    <el-main>
      <el-collapse v-model="activeDims">
        <el-collapse-item :title="$t('dataConfMod.detail')" name="1">
          <el-form
            ref="orgform1"
          ...
          >
            <srm-row>
              <srm-col>
                <!-- 组织类型 -->
                <el-form-item :label="$t('dataConfMod.orgType')" prop="organizationTypeCode">
                  <DictSelect
              ...
              <srm-col>
                <!-- 组织编码 -->
                <el-form-item :label="$t('common.orgCode')" prop="organizationCode">
              ...
              <srm-col>
                <!-- 组织名称 -->
                <el-form-item :label="$t('dataConfMod.orgName')" prop="organizationName">
                  <el-input
              ...
              <srm-col>
                <!-- 上层组织名称 -->
                <el-form-item :label="$t('dataConfMod.lastOrgName')" prop="parentOrganizationNames">
                  <div class="org-pop-search">
                    <el-input
              ...
              <srm-col>
                <!-- erp库存组织ID -->
                <el-form-item :label="$t('dataConfMod.erpOrgId')" prop="erpOrgId">
              ...
              <srm-col>
                <!-- 区域 -->
                <el-form-item
                  :label="$t('cusEntry.dataConfMod.organizationRegion')"
                  prop="organizationRegion"
                >
                  <DictSelect
                    v-model="orgDataModel.orgDataform.organizationRegion"
                    ::disabled="!isSrmEdit"
                    code="REGION"
                  />
                </el-form-item>
              </srm-col>
              <srm-col v-if="orgDataModel.orgDataform.organizationTypeCode == 'INV'">
                <el-form-item :label="$t('是否默认库存组织')">
                  <DictSelect
                    v-model="orgDataModel.orgDataform.defaultInv"
                    ::disabled="!isSrmEdit"
                    code="YES_OR_NO"
                  />
                </el-form-item>
        ...
        <!-- 收获地址 -->
        <el-collapse-item :title="$t('dataConfMod.organizationSite')" name="2">
          <!-- 收货地点 -->
          <div class="site-line">
            <!-- srm创建时可操作 -->
            <p v-if="isSrmEdit" class="btn_line">
              <el-button type="primary" @click="addSiteList">
                {{ $t('common.new') }}
              </el-button>
              <el-button type="primary" @click="delSiteList">
                {{ $t('common.delete') }}
              </el-button>
            </p>
            <TableView
        ...
        <!-- 公司基本信息 -->
        <el-collapse-item v-if="companyShow" :title="$t('dataConfMod.companyBasicInfo')" name="3">
          <!-- 公司信息 -->
          <el-form
            ref="companyForm"
            :model="orgCompany"
            ...
            <srm-row>
              ...
              <srm-col>
                ...
                <el-form-item :label="$t('contractMod.isValid')">
                  <DictSelect v-model="orgCompany.enabledFlag" code="YES_OR_NO" />
                </el-form-item>
              </srm-col>
              <srm-col>
                <!-- email -->
        ...
        >
          ...
          <div class="site-line">
            <p v-if="isSrmEdit" class="btn_line">
              <el-button type="primary" @click="addPersonList">
                {{ $t('common.new') }}
              </el-button>
              <el-button type="primary" @click="delPersonList">
                {{ $t('common.delete') }}
              </el-button>
            </p>
            <TableView
        ...
        <!-- 公司账号 -->
        <el-collapse-item v-if="companyShow" :title="$t('dataConfMod.companyAccount')" name="5">
          <!-- 收货地点 -->
          <div class="site-line">
            <p v-if="isSrmEdit" class="btn_line">
              <el-button type="primary" @click="addBankList">
                {{ $t('common.new') }}
              </el-button>
              <el-button type="primary" @click="delBankList">
                {{ $t('common.delete') }}
              </el-button>
            </p>
            <TableView
        ...
        <!-- 公司地址 -->
        <el-collapse-item v-if="companyShow" :title="$t('dataConfMod.companyAddress')" name="6">
          <!-- 收货地点 -->
          <div class="site-line">
            <p v-if="isSrmEdit" class="btn_line">
              <el-button type="primary" @click="addAddressList">
                {{ $t('common.new') }}
              </el-button>
              <el-button type="primary" @click="delAddressList">
                {{ $t('common.delete') }}
              </el-button>
            </p>
            <TableView
        ...
        <!-- 开票信息 -->
        <el-collapse-item v-if="ouShow" :title="$t('开票信息')" name="7">
          <!-- 收货地点 -->
          <div class="site-line">
            <!-- srm创建时可操作 -->
            <p v-if="isSrmEdit" class="btn_line">
              <el-button type="primary" @click="addOrgInvoiceList">
                {{ $t('common.new') }}
              </el-button>
              <el-button type="primary" @click="delOrgInvoiceList">
                {{ $t('common.delete') }}
              </el-button>
            </p>
            <TableView
              ...
              :checkbox="true"
              table-height="200px"
              :table-infor="orgInvoiceInfoList"
              :table-header="orgInvoiceHeader"
              :check-change="orgInvoiceSelectChange"
              :page-enabled="false"
            />
          </div>
        </el-collapse-item>
        <!-- 收票信息 -->
        <el-collapse-item v-if="ouShow" :title="$t('收票信息')" name="8">
          <!-- 收货地点 -->
          <div class="site-line">
            <!-- srm创建时可操作 -->
            <p v-if="isSrmEdit" class="btn_line">
              <el-button type="primary" @click="addOrgCollectInfoList">
                {{ $t('common.new') }}
              </el-button>
              <el-button type="primary" @click="delOrgCollectList">
                {{ $t('common.delete') }}
              </el-button>
            </p>
            <TableView
              ...
              :checkbox="true"
              table-height="200px"
              :table-infor="orgCollectInfoList"
              :table-header="orgCollectHeader"
              :check-change="orgCollectSelectChange"
              :page-enabled="false"
            />
          </div>
        </el-collapse-item>
...
import { organizationSetting } from 'modc@/buyer/basicSetting/api/basicSetting'
...
const orgTypeDictClass = createDictClass()
  .setCustomSelectType('ORG_TYPE_ALL')
  .loadCustomSelectType('ORG_TYPE_ALL')
...
export default {
  ...
  data () {
    return {
      ...
      orgInvoiceSelect: [], // 选中的开票信息
      orgCollectSelect: [], // 选中的收票信息
      siteTableList: [], // 收货地点列表
      orgInvoiceInfoList: [], // 开票信息
      orgCollectInfoList: [], // 收票信息
      siteTableHeader: [], // 收货地点表头
      orgInvoiceHeader: [], // 开票信息表头
      orgCollectHeader: [], // 收票信息表头
  ...
  computed: {
    companyShow () {
      return false // ['COMPANY'].includes(this.orgDataModel.orgDataform.organizationTypeCode)
    },
    ouShow () {
      return false // ['OU'].includes(this.orgDataModel.orgDataform.organizationTypeCode)
    },
    // srm方式 可编辑
    isSrmEdit () {
  ...
  async created () {
    ...
    this.siteTableHeader = [
      ...
      {
        prop: 'isDefault',
        label: () => this.$t('cusEntry.dataConfMod.isDefault'),
        showType: 'dictSelect',
        editable: row => {
          return this.isSrmEdit
        },
    ...
    // 开票信息
    this.orgInvoiceHeader = [
      // '发票类别'
      {
        prop: 'invoiceType',
        label: () => this.$t('cusEntry.dataConfMod.invoiceType'),
        showType: 'dictSelect',
      ...
      {
        prop: 'companyName',
        label: () => this.$t('cusEntry.dataConfMod.companyName'),
        showType: 'input',
        editable: () => {
          return this.isSrmEdit
        }
      ...
      {
        prop: 'openingName',
        label: () => this.$t('cusEntry.dataConfMod.openingName'),
        showType: 'input',
        editable: () => {
          return this.isSrmEdit
        }
      ...
      {
        prop: 'openingAccount',
        label: () => this.$t('cusEntry.dataConfMod.openingAccount'),
        showType: 'input',
        editable: () => {
          return this.isSrmEdit
        }
      ...
      {
        prop: 'taxpayerNum',
        label: () => this.$t('cusEntry.dataConfMod.taxpayerNum'),
        showType: 'input',
        minWidth: '150',
        editable: () => {
          return this.isSrmEdit
      ...
      {
        prop: 'phone',
        label: () => this.$t('cusEntry.dataConfMod.phone'),
        showType: 'input',
        editable: () => {
          return this.isSrmEdit
        }
      ...
      {
        prop: 'address',
        label: () => this.$t('cusEntry.dataConfMod.address'),
        showType: 'input',
        editable: () => {
          return this.isSrmEdit
        }
    ...
    // 收票信息
    this.orgCollectHeader = [
      // 收票人
      {
        prop: 'collectPerson',
        label: () => this.$t('cusEntry.dataConfMod.collectPerson'),
        minWidth: 150,
        editable: () => {
          return this.isSrmEdit
        },
        ...
        name: 'scc_rbac_user_display',
        getObj: (e, row) => {
          row.collectPerson = e.nickname
        }
      },
      // 收票地址
      {
        prop: 'collectAddress',
        label: () => this.$t('cusEntry.dataConfMod.collectAddress'),
        showType: 'input',
        editable: () => {
          return this.isSrmEdit
        }
      ...
      {
        prop: 'collectMail',
        label: () => this.$t('cusEntry.dataConfMod.collectMail'),
        showType: 'input',
        editable: () => {
          return this.isSrmEdit
        }
    ...
    if (this.curOpt == 'add') {
      ...
      // 当前日期
      this.orgDataModel.orgDataform.startDate = parseTime(new Date(), '{y}-{m}-{d}', true)
      this.resetCompany()
      this.resetOu()
    } else {
  ...
  methods: {
    ...
    // 新增开票信息
    addOrgInvoiceList () {
      this.$refs.orgInvoice.addOneEditableColumn({
        tempRowKey: generateUniqueId(),
        invoiceType: '', // 发票类别
        companyName: '', // 公司名称
        openingName: '', // 开户行名称
        openingAccount: '', // 开户行账号
        taxpayerNum: '', // 纳税人识别号
        phone: '', // 电话
        address: '' // 地址
      })
    ...
    // 新增收票信息
    addOrgCollectInfoList () {
      this.$refs.orgCollect.addOneEditableColumn({
        tempRowKey: generateUniqueId(),
        collectPerson: '', // 收票人
        collectAddress: '', // 收票地址
        isDefault: '' // 是否默认
    ...
    // 删除删除开票信息
    delOrgInvoiceList () {
      this.deleteBySelect(this.orgInvoiceSelect, this.orgInvoiceInfoList)
    },
    // 删除收票信息
    delOrgCollectList () {
      this.deleteBySelect(this.orgCollectSelect, this.orgCollectInfoList)
    },
    delPersonList () {
    ...
    // 开票信息选中方法
    orgInvoiceSelectChange (select) {
      this.orgInvoiceSelect = select
    },
    // 收票信息选中方法
    orgCollectSelectChange (select) {
      this.orgCollectSelect = select
    },
    ...
    resetOu () {
      this.siteTableList = []
      this.orgInvoiceInfoList = []
      this.orgCollectInfoList = []
    },
    initOrgCompany () {
      this.orgCompany = {
        companyName: null,
    ...
    async getDataForEdit (organizationId) {
      ...
      if (data) {
        const {
          ...
          orgInvoiceInfoList,
          orgCollectInfoList,
          orgCompanyPersonList,
          orgCompanyBankList,
          orgCompanyAddressList
        } = data
        // 返回数据处理
        this.orgDataModel.orgDataform = Object.assign(this.orgDataModel.orgDataform, organization)
        ...
        const orgInvoiceInfoArray = (orgInvoiceInfoList || []).map(i => ({
          ...i,
          tempRowKey: generateUniqueId()
        }))
        const orgCollectInfArray = (orgCollectInfoList || []).map(i => ({
          ...i,
          tempRowKey: generateUniqueId()
        }))
        this.siteTableList = siteArray
        this.personTableList = orgCompanyPersonArray
        this.orgInvoiceInfoList = orgInvoiceInfoArray
        this.orgCollectInfoList = orgCollectInfArray
    ...
    saveOrUpdateOrgHandle (opt) {
      ...
      let nums1 = 0
      let nums2 = 0
      if (this.siteTableList?.length) {
        this.siteTableList.forEach(data => {
          if (data.isDefault == 'Y') {
            nums1++
          }
        })
        if (nums1 == 0) {
          this.$message.error('收货地点默认值需选择一个为是')
          return false
        }
      }
      if (nums1 > 1) {
        this.$message.error('收货地点默认值只能有一个')
        return false
      }
      if (this.orgCollectInfoList?.length) {
        this.orgCollectInfoList.forEach(data => {
          if (data.isDefault == 'Y') {
            nums2++
          }
        })
        if (nums2 == 0) {
          this.$message.error('收票信息默认值需选择一个为是')
          return false
        }
      }
      if (nums2 > 1) {
        this.$message.error('收票信息默认值只能有一个')
        return false
      }
        ...
        if (valid) {
          organizationSetting
            .orgSaveOrUpdate({
              ...
              orgInvoiceInfoList: this.orgInvoiceInfoList,
              orgCollectInfoList: this.orgCollectInfoList,
    ...
    sharedOrgcomfirmSelect (data = []) {
      ...
      if (data.length > 0) {
        ...
        if (this.curOpt == 'add') {
          ...
          if (this.orgDataModel.orgDataform.organizationCode == orgRow.organizationCode) {
            // 编辑时只能选择自己
            this.orgDataModel.orgDataform.startDate = orgRow.startDate
            this.orgDataModel.orgDataform.endDate = orgRow.endDate
          } else {
