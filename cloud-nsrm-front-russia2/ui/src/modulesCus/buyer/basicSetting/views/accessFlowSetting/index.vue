  ...
  >
    <el-main>
      ...
      <TableView
        ...
        url="/api-pj/pj/sup/entryConfig/listPageByParam"
      >
        ...
        <template #ifAuthSample="{ scope }">
          <div class="checkboxCon">
            <el-checkbox
              v-model="scope.row.pjIfAuthSample"
              true-label="Y"
              false-label="N"
              @change="val => listChange(scope.row)"
            />
        ...
        <template #ifMaterial="{ scope }">
          <div class="checkboxCon">
            <el-checkbox
              v-model="scope.row.pjIfMaterial"
              true-label="Y"
              false-label="N"
              @change="val => listChange(scope.row)"
            />
      ...
      >
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <el-input v-model="scope.row.keyConfirmElement" />
            </template>
          </el-table-column>
          <!-- 附件模板 -->
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              {{$parseTime(scope.row.lastUpdateDate)}}
            </template>
          </el-table-column>
          <!-- 更新人 -->
          <el-table-column
...
import { quaApi } from 'modcb@/vendorManagementBuyer/api/vendorManagement'
...
export default {
  ...
  methods: {
    ...
    listChange (row) {
      const { pjIfMaterial, pjIfAuthSample, pjIfAuth } = row
      row.ifAuthEffective = [pjIfMaterial, pjIfAuthSample, pjIfAuth].includes('Y') ? 'Y' : 'N'
      row.ifRepush = 1
    ...
    fatchDictData () {
      ...
      getDictItemList(dictParamsArr).then(res => {
        ...
        this.entryFileType = adaptDictData(ENTRY_FILE_TYPE.ENTRY_FILE_TYPE).filter(item => ['REVIEW', 'ACCESS'].includes(item.value))
        console.log(this.entryFileType)
      })
    },
    // 删除合同定义其中一条数据
