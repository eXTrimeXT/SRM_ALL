...
const $preOptions = {
  nextStep: async function preNextStepHandler() {
    ...
    if (focusFlag === 'Y') {
      await app.$confirm(t('cusEntry.tipMessage.isFocusFlag'), t('components.approvalHead.tips.tip'), {
...
const $archive = async ($form, $queryEngine, $bus, $message) => {
  ...
  if(!stampAnnexes || !stampAnnexes.length){
    return $message.warning(t('cusEntry.supplement20250205.stampAttachmentInfoNotEmpty'))
  }
  ...
  if(response){
    if($form.values.isFrameworkAgreement !== 'Y'){
      ...
      $message({
        type:'success',
        // 归档成功，请及时创建合同履约计划
        message:t('cusEntry.supplement20250205.archiveSuccessNotice'),
        duration: 5000
      })
    }else{
  ...
  async ($form: ViewModel, $queryEngine, $bus, type = 'submit') => {
    ...
    const run = async (values: Record<string, any>) => {
      ...
      if (!temporaryData && values.effectiveDateFrom && new Date(values.effectiveDateFrom.replace(/-/g, '/')) > new Date(values.effectiveDateTo.replace(/-/g, '/'))) {
        validResult = false
        // 合同有效期有误
        return app.$message.error(t('cusEntry.supplement20250205.contractValidityError') as string)
      }
      ...
      if (!temporaryData && bolpartnerType > 1) {
        validResult = false
        // 只能有一个甲方
        return app.$message.error(t('dataConfMod.msgSelContract2') as string)
      }
      if (!temporaryData && bolpartnerType2 > 1) {
        validResult = false
        // 只能有一个乙方
        return app.$message.error(t('dataConfMod.msgSelContract3') as string)
      }
      if(!temporaryData){
        for(let item of values.partnerData){
          if(item.phone && !RuleRegExp.Mobile.test(item.phone)){
            validResult = false
            // 合作伙伴-${item.partnerType}联系电话格式不正确
            return app.$message.error(`${t('contractMod.partner')}-${item.partnerType}${t('cusEntry.supplement20250205.contactNumberFormatIncorrect')}`)
          }
        }
      }
      ...
      if(!temporaryData && !$isTermination && values.contractType !== 'SUPPLEMENTAL_AGREEMENT'){
        // 合同附件信息-请上传美国成分分析表附件
        let fileMessage1 = t('cusEntry.supplement20250205.contractAttachmentInfoPleaseUploadAmericanCompositionAnalysisFormAttachment')
        // 合同附件信息-请上传阳光协议附件
        let fileMessage2 = t('cusEntry.supplement20250205.contractAttachmentInfoSunshineAgreementUpload')
      ...
      if (eqN(values.isFrameworkAgreement) && workFlow) {
        ...
        if (!$isTermination) {
          ...
          if (!temporaryData && includeTaxAmount != totalMoney && eqN(values.isFrameworkAgreement)) {
            // 阶段付款金额总和应与合同总金额相等
            app.$message.error(t('cusEntry.supplement20250205.totalStagePaymentAmountShouldEqualContractTotalAmount'))
            validResult = false
            return false
          }
        }
        ...
        for(let item of values.payPlanData){
          if(['HONOUR','WIRE_AND_HONOUR'].includes(item.payMethod) && !item.extAcceptanceDate){
            // 付款计划-付款方式为承兑、电汇+承兑时，承兑期限必填
            app.$message.error(t('cusEntry.supplement20250205.paymentPlanWithAcceptanceAndRemittanceWithMandatoryAcceptancePeriod'))
            validResult = false
            return
          }
          if(item.payMethod === 'WIRE_AND_HONOUR' && Number(item.extAcceptanceRatio) + Number(item.extWireTransferRatio) != 100){
            // 付款计划-付款方式为电汇+承兑时，承兑比例和电汇比例之和必须等于100
            app.$message.error(t('cusEntry.supplement20250205.paymentPlanPaymentMethodTelegraphicTransferAcceptanceRatioAndTelegraphicTransferRatioSumMustBeOneHundredPercent'))
            validResult = false
            return
          }
        }
...
const $saveBill = async ($form: any, $queryEngine: any, $bus: any, $eqY: any, type = '') => {
  ...
  if (!!modelHeadId
    ...
    // 现在合同为编辑模式,请切换为浏览模式
    app.$message.warning(t('cusEntry.supplement20250205.editModeToBrowseMode'))
    return false
  }
  $handlePreview($form)
  ...
  refuseQuery: {
    ...
    properties: {
      refuseDialog: {
        type:'void',
        // 驳回
        title:t('components.approvalHead.headers.refuse'),
        'x-component': 'RDialog',
        'x-component-props': {
          size:'small',
          ...
          form:{
            ...
            properties:generateXindexInOrder({
              vendorRejectReason:{
                type:'string',
                // 驳回原因说明
                title:t('cusEntry.supplement20250205.rejectionReasonDescription'),
                'x-decorator':'FormItem',
                required:true,
                'x-component-props':{
                  type:'textarea',
    ...
    'x-data': {
      ...
      progress:[
              ...
              },
              {
                code:'basisFileInfo',
                // 合同依据
                name: t('cusEntry.supplement20250205.contractBasis'),
                percentage: 0
              },
              {
              ...
              },
              {
                code: 'itemInfo',
                // 合同签约明细
                name: t('cusEntry.supplement20250205.contractSigningDetails'),
                percentage: 0
              },
              {
                code: 'paymentPlan',
              ...
              },
              {
                code:'sealFileInfo',
                // 盖章附件信息
                name:t('cusEntry.supplement20250205.stampAttachmentInfo'),
                percentage: 0
              },
              {
                code:'operateRecord',
              ...
              collapse: {
                ...
                properties: generateXindexInOrder({
                      ...
                      layout: {
                        ...
                        properties: {
                          ...
                          buName:{
                            type:'string',
                            // 我方签约主体
                            title:t('cusEntry.supplement20250205.ourSigningEntity'),
                            'x-decorator': 'FormItem',
                          ...
                          extContractHandlerName: {
                            type:'string',
                            // 合同经办人
                            title:t('cusEntry.supplement20250205.contractManager'),
                            'x-decorator':'FormItem',
                            'x-component':'QuickSearchWrapper',
                            'x-component-props':{
                          ...
                          extHandlerBuId: {
                            type: 'string',
                            // 经办人部门
                            title: t('cusEntry.supplement20250205.handlerDepartment'),
                            'x-decorator':'FormItem',
                            'x-component': 'OrganizationSelector',
                            'x-component-props': {
                              'parent-id': -1,
                          ...
                          extPricePoolFlag:{
                            type:'string',
                            // 是否进价格库
                            title:t('bid_mod.isSyncToPriceLibrary'),
                            'x-decorator':'FormItem',
                            'x-decorator-props':{
                              // 进入价格库物资，协议期内采购无需询比价
                              tooltip:t('cusEntry.supplement20250205.enterPriceLibraryMaterials'),
                              tooltipLayout: 'icon'
                            },
                            'x-component':'DictSelect',
                            'x-component-props':{
                          ...
                          extInvestNo:{
                            type:'string',
                            // 投资编号
                            title:t('cusEntry.bidMod.investNum'),
                            'x-decorator':'FormItem',
                            'x-component-props':{
                              disabled:expression(`$form.values.contractType == 'SUPPLEMENTAL_AGREEMENT'`)
                            }
                          ...
                          sealName: {
                            type: 'string',
                            // 我方印章
                            title: t('cusEntry.supplement20250205.ourSeal'),
                            'x-decorator': 'FormItem',
                            'x-component': 'QuickSearchWrapper',
                            'x-component-props': {
                          ...
                          extIncome: {
                            type: 'string',
                            // 收支方向
                            title: t('cusEntry.supplement20250205.incomeAndExpenseDirection'),
                            'x-decorator': 'FormItem',
                            'x-component': 'DictSelect',
                            'x-component-props': {
                          ...
                          extRent: {
                            type: 'string',
                            // 是否租赁
                            title: t('cusEntry.supplement20250205.isRented'),
                            'x-decorator': 'FormItem',
                            'x-component': 'DictSelect',
                            'x-component-props': {
                              code: 'YES_OR_NO'
                          ...
                          extCycle: {
                            type: 'string',
                            // 是否周期合同
                            title: t('cusEntry.supplement20250205.isPeriodContract'),
                            'x-decorator': 'FormItem',
                            'x-component': 'DictSelect',
                            'x-component-props': {
                              code: 'YES_OR_NO'
                  ...
                  basisFileInfo:{
                    ...
                    'x-component':'FormCollapse.Item',
                    'x-component-props':{
                      // 合同依据（长城内部可见，用于合同发起的背景说明或技术需求）
                      title:t('cusEntry.supplement20250205.contractBasis2')
                    },
                    properties:{
                      toolbar:{
                        type:'void',
                      ...
                      basisAnnexes:{
                        ...
                        properties: generateXindexInOrder({
                          ...
                          remark:{
                            type:'string',
                            // 备注
                            title:t('components.eio.headers.remark'),
                            'x-render-table-column': {
                              minWidth: 150
                            },
                  ...
                  itemInfo: {
                    ...
                    'x-component-props': {
                      ...
                      title: i18nExpression('cusEntry.supplement20250205.contractSigningDetails')
                    },
                    'x-visible': expression(`!$isTermination`), 
                      ...
                      materialListData: {
                        ...
                        properties: generateXindexInOrder({
                          ...
                          extInvoiceType:{
                            type: 'string',
                            // 发票类型
                            title:t('components.ocr.invoiceType'),
                            'x-render-table-column': {
                              minWidth: 150
                            },
                            'x-component':'DictSelect',
                          ...
                          shelfLife: {
                            ...
                            default: undefined,
                            // 质保期(自然日)
                            title: t('cusEntry.bidMod.warrantyPeriod'),
                            'x-component-props': {
                              controls: false,
                            },
                            'x-render-table-column': {
                    ...
                    properties: {
                      ...
                      payPlanData: {
                        ...
                        properties: generateXindexInOrder({
                          ...
                          extAcceptanceDate: {
                            // 承兑期限
                            title:t('cusEntry.supplement20250205.acceptancePeriod'),
                            'x-render-table-column':{
                              minWidth: 120
                            },
                            ...yearMonthDaySelectorSegment,
                          ...
                          extAcceptanceRatio:{
                            ...
                            default: undefined,
                            // 承兑比例%
                            title: t('cusEntry.supplement20250205.acceptanceRatioPercentage'),
                            'x-render-table-column':{
                              minWidth:120
                            },
                            'x-component-props': {
                          ...
                          extWireTransferRatio:{
                            ...
                            default: undefined,
                            // 电汇比例%
                            title: t('cusEntry.supplement20250205.telegraphicTransferRatioPercentage'),
                            'x-render-table-column':{
                              minWidth:120
                            },
                            'x-component-props': {
                  ...
                  */
                      ...
                      partnerData: {
                        ...
                        properties: generateXindexInOrder({
                          ...
                          contactName: {
                            type: 'string',
                            // 授权代表
                            title: t('cusEntry.supplement20250205.authorizedRepresentative'),
                            'x-render-table-column': {
                              minWidth: 120
                            },
                            'x-reactions':{
                  ...
                  fileInfo: {
                    ...
                    'x-component-props': {
                      ...
                      title: t('cusEntry.supplement20250205.contractAttachmentInfo')
                    },
                    properties: {
                      toolbar: {
                        type: 'void',
                  ...
                  sealFileInfo: {
                    ...
                    'x-component-props': {
                      // 盖章附件信息
                      title: t('cusEntry.supplement20250205.stampAttachmentInfo')
                    },
                    properties: {
                      toolbar: {
                        type: 'void',
                      ...
                      operationLogs:{
                        ...
                        properties: generateXindexInOrder({
                          operationType:{
                            type:'string',
                            // 操作类型
                            title:t('dataConfMod.operateType'),
                            'x-render-table-column':{
                              minWidth:130
                            },
                            'x-component':'DictSelect',
                          ...
                          createdFullName:{
                            type:'string',
                            // 操作人
                            title:t('components.workedProcess.headers.fdHandlerName'),
                            'x-render-table-column':{
                              minWidth:130
                            }
                          }
