  ...
  <div style="padding-top: 10px">
    ...
    >
      ...
      <el-table-column
        align="center"
        prop="handerMode"
        :label="$t('cusEntry.supplement20250121.handerMode')"
      ...
      >
        <template slot-scope="scope">
          {{$parseTime(scope.row.refundPaymentDate)}}
        </template>
      </el-table-column>
      <el-table-column
        align="center"
      ...
      >
        <template slot-scope="scope">
          ...
          >
            <!-- 缴纳处理 -->
            {{ $t('cusEntry.supplement20250121.handerMode') }}
          </el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      >
        ...
        >
          <template v-slot="scope">
            ...
            />
            <span v-else>{{ $parseTime(scope.row.expectRefundTime) }}</span>
          </template>
        </el-table-column>
        <el-table-column
          align="center"
    ...
    <srm-dialog
      ...
      size="small"
      :title="$t('cusEntry.supplement20250121.handerMode')"
      append-to-body
    ...
    >
      <el-form ref="payForm" :model="payForm">
        ...
        <el-form-item
          prop="handerMode"
          :label="$t('cusEntry.supplement20250121.handerModeMethod')"
        ...
        <el-form-item
          prop="causeDesc"
          :label="$t('common.explanationOfReasons')"
          :rules="[{ required: true, message: $t('contractMod.pleaseFillIn')}]"
        >
  ...
  methods: {
    ...
    async openDeductDialog (flag) {
      ...
      if (flag == 'edit') {
        ...
        if (onlineFlag) {
          // 缴纳处理为线上核对缴纳，才允许发起保证金扣款
          this.$message.error(this.$t('cusEntry.supplement20250121.handerModeTip'))
          return
        }
      } else {
    ...
    async openRefundDialog (flag) {
      ...
      if (flag == 'edit') {
        ...
        let onlineFlag = this.selectedList.some(item => item.handerMode !== 'ON_LINE')
        if (onlineFlag) {
          // 缴纳处理为线上核对缴纳，才允许发起保证金退回
          this.$message.error(this.$t('cusEntry.supplement20250121.handerModeTip1'))
          return
        }
      } else {
