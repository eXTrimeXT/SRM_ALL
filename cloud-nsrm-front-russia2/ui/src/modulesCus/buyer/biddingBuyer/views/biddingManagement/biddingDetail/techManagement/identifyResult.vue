<template>
  <!-- 投标公司标书查重结果 -->
  <SrmDialog
    :title="`${$t('cusEntry.supplement20250205.duplicateResult')}——${extProjectNo}——${souName}`"
    size="fullscreen"
    ...
    <div class="content">
      <div v-if="resultList.length">
        <div v-for="result in resultList" :key="result.taskId" class="result-box">
          <div class="text-num">
            <!-- 识别字数：{{ result.compareWordSize }}个字 -->
            <span>{{ $t('cusEntry.supplement20250205.recognizedWords', { num: result.compareWordSize }) }}</span>
            <!-- 识别时间 -->
            <span>{{ $t('cusEntry.supplement20250205.recognizedTime') }}：{{ $parseTime(result.createTime) }}</span>
          </div>
          ...
          <div class="conclusion">
            <p style="margin-bottom:4px;">
              <!-- 识别结论 -->
              {{ $t('cusEntry.supplement20250205.recognizedResult') }}
            </p>
      ...
      <div v-else class="no-data">
        ...
        <!-- 结果正在识别中，请耐心等待...... -->
        <p>{{ $t('cusEntry.supplement20250205.recognizedTips') }}</p>
      </div>
    </div>
  </SrmDialog>
</template>
  ...
  data () {
    return {
      ...
      tableColumns: [
        {
          prop: 'ycompanyName',
          label: this.$t('cusEntry.supplement20250205.ycompanyName') // 公司名称-文件名称
        }
      ],
      resultList: []
    }
