        ...
        <CToolbar v-if="activeTab !== 'createApproval'">
          <template slot="right">
            ...
            <p v-if="biddingBaseInfo.createApprovalStatus=='REJECTED'" style="margin-right:20px">
              {{ $t('vendorMod.refuseMemo') }}: {{ biddingBaseInfo.approveRejectDesc }}
            </p>
            <!-- 返回 -->
            ...
            <el-button
              ...
              @click="bidApproval"
            >
              {{ $t('cusEntry.supplement20250121.bidApproval') }}
            </el-button>
            ...
            <el-button
              ...
              @click="bidNotice"
            >
              {{ $t('cusEntry.supplement20250121.bidNotice') }}
            </el-button>
            ...
            <el-button
              ...
              @click="goToArchive"
            >
              {{ $t('cusEntry.supplement20250121.goToArchive') }}
            </el-button>
            ...
            <el-button
              ...
              @click="toBidArchive"
            >
              {{ $t('cusEntry.supplement20250121.archive') }}
            </el-button>
          </template>
        </CToolbar>
      </el-main>
...
export default {
  ...
  async created () {
    ...
    // else {
    //   this.$nextTick(() => {
    //     this.$refs.projectInfo?.$refs.enclosure.loadFileInfo()
    //   })
    // }
  },
  ...
  methods: {
    ...
    async menuItemClick (data) {
      ...
      if (this.biddingBaseInfo.projectStatus == 'DRAW_UP' && ['bondManagement', 'bidingControl', 'techManagement',
        ...
        // 招标状态为拟定，不可查看标前准备后续环节
        this.$message.error(this.$t('cusEntry.supplement20250121.bidTips1'))
        return
      }
      ...
      if (this.userId === leaderObj.userId && !['techManagement'].includes(data.key)) {
        // 无查看权限
        this.$message.error(this.$t('cusEntry.supplement20250121.bidTips2'))
        return
      }
      ...
      !['BUS_BID_OPEN', 'CONFIRM_BID', 'WIN_LOSS_NOTICE', 'NOTICE_ING', 'ARCHIVE_TODO', 'ARCHIVE_DONE'].includes(this.biddingBaseInfo.projectStatus)) {
        this.$message.error(this.$t('cusEntry.supplement20250121.bidTips3'))
        return
      }
    ...
    handleOperation (type) {
      let confirmStr = null
      if (type == 'WITHDRAW') {
        confirmStr = this.$t('cusEntry.supplement20250121.bidTips4') // 确定撤回？
      } else if (type == 'APPROVED') {
        confirmStr = this.$t('cusEntry.supplement20250121.bidTips5') // 确定通过审批？
      }
    ...
    handleReject () {
      this.$prompt(this.$t('vendorMod.rejectReason'), {
        ...
        inputPlaceholder: this.$t('bidMod.msgRejectReason'),
        inputValidator: (value) => {
          if (!value) {
            return this.$t('bidMod.msgRejectReason')
          }
        } }).then(({ value }) => {
        const params = {
          businessId: this.biddingBaseInfo.projectId,
    ...
    bidApproval () {
      // 确定发起定标审批？
      this.$confirm(this.$t('cusEntry.supplement20250121.bidTips6'), {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      }).then(() => {
    ...
    bidNotice () {
      // 确定生成中/落标通知书？
      this.$confirm(this.$t('cusEntry.supplement20250121.bidTips7'), {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      }).then(() => {
    ...
    goToArchive () {
      // 确定进入项目归档？
      this.$confirm(this.$t('cusEntry.supplement20250121.bidTips8'), {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      }).then(() => {
    ...
    toBidArchive () {
      // 确定归档？
      this.$confirm(this.$t('cusEntry.supplement20250121.bidTips9'), {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      }).then(() => {
