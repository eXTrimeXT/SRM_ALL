...
export default {
  ...
  data () {
    return {
      ...
      rightSelected: [],
      // 组合1必选
      // groupNumListMust: ['数量/工程量', '未税单价（卢布）', '未税总价（卢布）'],
      groupNumListMust: ['extQuantity', 'extPriceNoTax', 'extPriceSumNoTax'],
      // 组合1
      // groupNumList: ['数量/工程量', '含税单价（卢布）', '含税总价（卢布）', '未税单价（卢布）', '未税总价（卢布）'],
      groupNumList: ['extQuantity', 'extPriceTax', 'extPriceSumTax', 'extPriceNoTax', 'extPriceSumNoTax'],
      // 组合2必选
      // groupTempNumListMust: ['暂定数量/工程量', '固定未税单价（卢布）', '暂定未税总价（卢布）'],
      groupTempNumListMust: ['requireQuantity', 'extFixedPriceNoTax', 'extProvPriceSumNoTax'],
      // 组合2
      // groupTempNumList: ['暂定数量/工程量', '固定含税单价（卢布）', '暂定含税总价（卢布）', '固定未税单价（卢布）', '暂定未税总价（卢布）'],
      groupTempNumList: ['requireQuantity', 'extFixedPriceTax', 'extProvPriceSumTax', 'extFixedPriceNoTax', 'extProvPriceSumNoTax'],
  ...
  methods: {
    ...
    selectedAdd () {
      ...
      let rightRemove = []
      let leftSelected = this.leftSelected.map(item => item.columnCode)
      this.leftSelected.map(item => {
        if (this.groupNumList.includes(item.columnCode)) {
          groupNumFlag = true
        }
        if (this.groupTempNumList.includes(item.columnCode)) {
      ...
      if (groupNumFlag && groupTempNumFlag) {
        // 组合1与组合2中的字段不可同时选中
        this.$message.warning(this.$t('cusEntry.supplement20250205.selectTips1'))
        return
      }
      // 选中组合1 groupNumList
      if (groupNumFlag && !groupTempNumFlag) {
      ...
      !!rightRemove && rightRemove.forEach(item => {
        // 如果右边包含需要移除的内容
        if (this.templateDataRight.some(v => v.columnCode == item)) {
          let hasIndex = this.templateDataLeft.findIndex(i => (i.columnCode == item))
          if (hasIndex < 0) {
            let column = this.templateDataRight.find(i => (i.columnCode == item))
            this.templateDataLeft.push(column) // 左边添加
          }
          let index = this.templateDataRight.findIndex(v => (v.columnCode == item))
          if (index > -1) {
      ...
      leftSelected.forEach(item => {
        let hasIndex = this.templateDataRight.findIndex(i => (i.columnCode == item))
        if (hasIndex < 0) {
          let column = this.templateDataLeft.find(i => (i.columnCode == item))
          this.templateDataRight.push(column) // 右边添加
        }
        let index = this.templateDataLeft.findIndex(v => (v.columnCode == item))
        if (index > -1) {
    ...
    // 删除
    selectedDel () {
      let rightSelected = this.rightSelected.map(i => i.columnCode)
      this.rightSelected.map(item => {
        if (this.groupNumListMust.includes(item.columnCode)) {
          rightSelected = [...this.groupNumList, ...rightSelected]
        }
        if (this.groupTempNumListMust.includes(item.columnCode)) {
        ...
        if (item.starFlag) {
          this.itemList.some((it, index) => {
            ...
            if (!it[item.columnCode] || (this.biddingBase.mergeFlag && !it.extPackageName)) {
              valid = true
              // 报价信息第${index + 1}行缺少必填项
              tipMsg = this.$t('cusEntry.supplement20250205.validTipMsg', { index: (index + 1) })
              return true
            }
          })
        }
