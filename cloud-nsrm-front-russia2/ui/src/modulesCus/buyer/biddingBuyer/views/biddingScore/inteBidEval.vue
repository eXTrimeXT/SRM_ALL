<template>
    <div class="intBidEval-box">
        <div class="header-box">
            <div class="title-box">
                <span style="margin-right: 12px;font-size: 16px;">{{ souName }}-{{ $t('cusEntry.supplement20250205.technicalBid') }}</span>
                <el-select v-model="curSupplier" :placeholder="$t('common.pleaseSelect')" @change="handleSupplyChange">
                    <el-option
            ...
            <div class="dialogue" @click="goDialogue">
                ...
                <!-- 去对话 -->
                <span>{{ $t('cusEntry.supplement20250205.bidEvaTip11') }}</span>
            </div>
        </div>
        <div class="content">
            <div class="eval-box" :style="{ width: isExpand ? 'calc(50% - 130px)' : 'calc(50% - 5px)' }">
                <div class="title-box">
                    <!-- 评审要求 -->
                    <div class="title">{{ $t('cusEntry.supplement20250205.bidEvaTip12') }}</div>
                    <el-select v-model="selectReviewItem" clearable filterable :placeholder="$t('components.common.searchWord')"
                    ...
                    <div class="item-box" v-for="item in scoreList" :key="item.reviewItem"
                        ...
                        <div class="item-con">
                            <div class="item-con-inner" v-for="temp in item.vendorScoreList" :key="temp.vendorId">
                                <div class="tip" v-if="temp.isSave && temp.vendorId === curSupplier">
                                    <!-- 已填写评审说明 -->
                                    {{ $t('cusEntry.supplement20250205.bidEvaTip13') }}
                                </div>
                                <div class="according" v-show="temp.vendorId === curSupplier">
                                    <div class="title" v-if='temp.answerAndQuotationList.length && temp.isAnswer'>
                                        <!-- 原文引用依据 -->
                                        {{ $t('cusEntry.supplement20250205.bidEvaTip14') }}
                                    </div>
                                    <div class="text-box">
                                ...
                                <div class="describe" v-show="temp.vendorId === curSupplier">
                                    <div class="desc-box">
                                        <!-- 评审说明 -->
                                        <div class="title">{{ $t('cusEntry.supplement20250205.bidEvaTip15') }}</div>
                                        <el-input
                                            type="textarea"
                                            :rows="3"
                                    ...
                                    <div class="score-box" v-if="item.maxScore && Number(item.maxScore) > 0">
                                        <div class="title" style="margin-top: 12px;">
                                            <!-- 评分(此项最高分{{ item.maxScore }}分) -->
                                            {{ $t('cusEntry.supplement20250205.bidEvaTip16', { maxScore: item.maxScore }) }}
                                        </div>
                                        <el-input
                                            :placeholder="$t('bidMod.msgEnterARating')"
                                            style="margin-top: 8px;"
                                            v-model="temp.score"
                                            :disabled="isReadOnly"
                                            v-input-format="{
            ...
            <div class="file-box">
                ...
                <div class="file-box-con"
                    ...
                    <!-- 澄清文件 -->
                    <div class="file-title">{{ $t('cusEntry.supplement20250205.bidEvaTip17') }}</div>
                    <div class="file-item">
                    ...
                    <!-- 投标文件 -->
                    <div class="file-title">{{ $t('cusEntry.bidMod.bidFile') }}</div>
                    <div class="file-item">
