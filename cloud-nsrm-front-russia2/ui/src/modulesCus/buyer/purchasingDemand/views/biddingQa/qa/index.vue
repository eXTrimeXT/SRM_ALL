  ...
  <el-container class="flex-container-notab" direction="vertical">
    <el-main>
      ...
      >
        ...
        >
          <el-table-column prop="vendorName" :label="$t('common.vendorName')" />
        </el-table>
      </SrmDialog>

      <!-- 澄清查看 or 澄清回复 -->
      <SrmDialog
        :title="openHuifuType == 'view' ? $t('cusEntry.supplement20250121.respondView') : $t('cusEntry.supplement20250121.respond')"
        size="middle"
...
export default {
      ...
      return [
        ...
        },
        {
          ...
          buttons: [
            // 采购商 - 澄清质疑
            {
              // 采购商 && 质疑状态 !== 已回复/已澄清 && 招标专家、技术专家
              show: row => this.userTypeFlag.isBuyer && !['CLARIFIED', 'REPLAYED'].includes(row.questionStatus) && [row.extBidUserId, row.extTechUserId].includes(this.userId),
              formattor: () => this.$t('cusEntry.supplement20250121.qaRespond'), // 质疑回复
      ...
      if (this.userTypeFlag.isBuyer) {
        return [
          ...
          },
          {
            ...
            buttons: [
              ...
              // 采购商 - 撤回澄清
              {
                show: row => this.userTypeFlag.isBuyer && row.answerStatus === 'ISSUED' && row.extBidUserId == this.userId,
                formattor: () => this.$t('bidMod.withdraw'),
              ...
              },
              {
                show: row => this.userTypeFlag.isBuyer && row.answerStatus === 'ISSUED' && (row.extBidUserId == this.userId && row.createdId != row.extBidUserId) &&
                  // 2024.3.7 新加校验，extReplayFlag为Y代表已经点过【回复关闭】
                  row.extReplayFlag !== 'Y',
                formattor: () => this.$t('cusEntry.supplement20250121.endReply'), // 结束回复
              ...
              // 供应商 - 接受澄清
              {
                ...
                formattor: () => this.$t('cusEntry.supplement20250121.respond'), // 澄清回复
        ...
        return [
          ...
          // 澄清回复状态
          {
            ...
            minWidth: 120,
            formattor: val => (val == null || val == '' || !val) ? this.$t('cusEntry.supplement20250121.noReplay') : this.$getDictLabel('ANSWER_REPLAY_STATUS', val)
          },
          // 项目名称
          {
            prop: 'souName',
  ...
  methods: {
    ...
    async finishReplay (row) {
      // 确定结束回复？
      const confirmResult = await this.$confirm(this.$t('cusEntry.supplement20250121.endReplyTips'), {
    ...
    async withdrawAnswerNew (row) {
      // 确定撤回？
      const confirmResult = await this.$confirm(this.$t('cusEntry.supplement20250121.bidTips4'), {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      })
    ...
    openQuestionDetailDialog (type, row = {}) {
      if (type == 'view' && !this.userTypeFlag.isBuyer) {
        ...
        const obj = {
          ...
          source: this.$t('cusEntry.supplement20250121.qaView') // '质疑查看'
        }
        this.$http({
      ...
      if (type == 'view' && !this.userTypeFlag.isBuyer) {
        ...
        if (isFromQuestion == 1) {
          obj = {
            ...
            supplierName: users?.companyName,
            supplierCode: users?.companyCode,
            source: this.$t('cusEntry.supplement20250121.respond') // 澄清回复
          }
        } else {
          obj = {
            supplierId: users?.companyId,
            supplierName: users?.companyName,
            supplierCode: users?.companyCode,
            source: this.$t('cusEntry.supplement20250121.respondView') // 澄清查看
          }
        }
