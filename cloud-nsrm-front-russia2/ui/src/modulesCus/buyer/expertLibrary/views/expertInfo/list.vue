      ...
      >
        ...
        <template slot="buttonOne" class="buttonOneClass">
          ...
          <el-button v-if="form.applyStatus === 'APPROVED' && !form.noApprovalPassExpertApplyId" type="primary" @click="handleUpgrade">
            <!-- 专家信息变更 -->
            {{ $t("cusEntry.supplement20250205.expertInfoChange") }}
          </el-button>
          <el-button v-if="form.noApprovalPassExpertApplyId" type="primary" @click="viewUpgrade">
            <!-- 查看专家信息变更 -->
            {{ $t("cusEntry.supplement20250205.viewExpertInfoChange") }}
          </el-button>
        </template>
      </CWorkflowMulti>
  ...
  methods: {
    ...
    async saveBill (type) {
      ...
      if (handleType === 'submit') {
        ...
        //   this.$message.warning(this.$t('cusEntry.supplement20250205.addWorkExperience'))
        //   return
        // }
        // if (!workRelationList.length) {
        //   // this.$message.warning('请至少新增一条亲属工作单位')
        //   this.$message.warning($t('cusEntry.supplement20250205.addRelativeWorkUnit'))
        //   return
        // }
      }
    ...
    handleUpgrade () {
      const tab = {
        ...
        // title: '专家信息变更',
        title: this.$t('cusEntry.supplement20250205.expertInfoChange'),
        name: 'expertUpgrade'
      }
    ...
    async viewUpgrade () {
      ...
      if (response.data.records.length) {
        ...
        const tab = {
          ...
          // title: '查看专家信息变更',
          title: this.$t('cusEntry.supplement20250205.viewExpertInfoChange'),
