...
export default {
  ...
  'x-component-props': {
    title: i18nExpression('accountMod.advancePaymentDetail3') // 合同履约预付款明细
  },
  properties: {
    toolbar: {
      type: 'void',
    ...
    contracts: {
      ...
      'x-query-engine-relation': 'contracts:*',
        ...
        currentPaymentAmount: {
          ...
          'x-validator': {
            ...
            validator: expression(`(value, rule) => {
              if(value <= 0){
                return $t('advancePayment.prompt4')
              }else if(value > Number($self.query('.stayPaymentAmount').get('value'))){
                return $t('advancePayment.prompt5')
              }
            }`)
          }
        },
