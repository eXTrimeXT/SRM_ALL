  ...
  <div :class="classObj" class="app-wrapper">
    ...
    <div :class="['main-container',{ hasTagsView: needTagsView && device !== 'device-xs' }]">
      ...
      <div ref="sysHeader" :class="['sys-header', { 'fixed-header': fixedHeader }]">
        <!-- 头部信息 -->
        <AppHeader pageType="home" @themeConfig="themeConfigFn" @timeZone="timeZoneFn" />
      </div>
    ...
    <!--时区配置-->
    <srm-dialog
      :title="$t('base.navbar.timeZone')"
      :visible.sync="timeZoneFnVisible"
      :close-on-click-modal="false"
      size="small"
    >
      <el-form
        v-if="timeZoneFnVisible"
        ref="orgform"
        label-position="top"
      >
        <srm-row>
          <srm-col :init-col="1">
            <!-- 常用时区 -->
            <el-form-item
              :label="$t('common.commonTimeZones')"
              prop="categoryCode"
            >
              <QuickSearch
                :show-input="timeZoneData[0].customizeConf"
                show-key="GMT"
                :scope-data="timeZoneData[0]"
                name="scc_base_time_zone"
                @close-quicksearch="getGMT"
              />
            </el-form-item>
          </srm-col>
          <srm-col :init-col="1">
            <!-- 日期时间格式 -->
            <el-form-item
              :label="$t('common.dateAndTimeFormat')"
              prop="categoryName"
            >
              <QuickSearch
                :show-input="timeZoneData[1].customizeConf"
                show-key="dateAndTimeFormat"
                :scope-data="timeZoneData[1]"
                name="scc_base_date_time_format"
                @close-quicksearch="getDateAndTime"
              />
            </el-form-item>
          </srm-col>
          <srm-col :init-col="1">
            ...
            >
              <QuickSearch
                :show-input="timeZoneData[2].customizeConf"
                show-key="dateAndTimeFormat"
                :scope-data="timeZoneData[2]"
                name="scc_base_date_format"
                @close-quicksearch="getDate"
              />
            </el-form-item>
          </srm-col>
          <srm-col :init-col="1">
            ...
            <el-form-item
              ...
              prop="numberFormat"
            >
              <DictSelect v-model="timeZoneData[3].customizeConf" code="NUMBER_FORMAT" />
            </el-form-item>
          </srm-col>
        </srm-row>
      </el-form>
      <div
        ...
        class="dialog-footer"
      >
        <el-button @click="timeZoneFnVisible = false">
          <!-- 取 消 -->
          {{ $t('common.cancel') }}
        </el-button>
        <el-button
...
export default {
  ...
  data () {
    return {
      timeZoneData: [{
        "customizeConf": "",
        "customizeType": "TIME_ZONE"
      },{
        "customizeConf": "",
        "customizeType": "DATE_TIME_FORMAT"
      },{
        "customizeConf": "",
        "customizeType": "DATE_FORMAT"
      },{
        "customizeConf": "",
        "customizeType": "NUMBER_FORMAT"
      }],
      appMainStyle: null,
      screenWidth: null,
      themeSelectVisible: false,
      timeZoneFnVisible: false,
  ...
  async created () {
    ...
    this.timeZoneData[0].customizeConf = localStorage.getItem('cloudTimeZoneFormat')
    this.timeZoneData[1].customizeConf = localStorage.getItem('cloudDateTimeFormat')
    this.timeZoneData[2].customizeConf = localStorage.getItem('cloudDateFormat')
    this.timeZoneData[3].customizeConf = localStorage.getItem('cloudNumberFormat')
  },
  mounted () {
    this.getStyle()
    // const _that = this
    ...
    this.$nextTick(() => {
      ...
      document.body.classList.add(`lang_${this.$store.getters.language}`)
    })
  },
  destroyed () {
    // 移除监听鼠标事件
  ...
  methods: {
    timeZoneFn (visible) {
      this.timeZoneFnVisible = visible
    },
    // 快查回调
    getGMT (val) {
      this.timeZoneData[0].customizeConf = val?.timeZoneStandard + val?.timeZoneDifference
    },
    getDateAndTime (val) {
      this.timeZoneData[1].customizeConf = val?.timeFormat
    },
    getDate (val) {
      this.timeZoneData[2].customizeConf = val?.timeFormat
    },
    // 时区设置点击确定
    confirm () {
      this.$http({
        url: '/api-rbac/user/customize/config',
        method: 'POST',
        data: this.timeZoneData,
        loading: true
      }).then(data => {
        // this.$message.success(data.message)
        this.timeZoneFn(false)
        localStorage.setItem('cloudTimeZoneFormat', this.timeZoneData[0]?.customizeConf)
        localStorage.setItem('cloudDateFormat', this.timeZoneData[1]?.customizeConf)
        localStorage.setItem('cloudDateTimeFormat', this.timeZoneData[2]?.customizeConf)
        this.$confirm(this.$t('common.isReload'), {
...
<style>
.lang_ru_RU .el-date-table td.today:not(.selected)>div:after {
  content: 'Сегодня' !important;
}
.lang_ru_RU .el-date-table td.in-range.start-date div:after {
  content: 'Начало' !important;
}
.lang_ru_RU .el-date-table td.in-range.end-date div:after {
  content: 'Конец' !important;
}
</style>
