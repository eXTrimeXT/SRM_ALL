<template>
  <srm-dialog
    :title="$t('route.biddingFlowSetting')"
    size="large"
      ...
      >
        <srm-row>
          ...
          <srm-col :init-col="4">
            ...
            >
              <dict-select
                v-model="form.bidingScope"
                code="BID_SCOPE"
                @change="getBidingScope"
              />
            </el-form-item>
          </srm-col>
          <!--招标类型-->
          <srm-col :init-col="4">
            ...
            >
              <dict-select
                v-model="form.bidingType"
                code="BID_TYPE"
                @change="getBidingType"
              />
            </el-form-item>
          </srm-col>
          <!--评分规则-->
          <srm-col :init-col="4">
            <el-form-item
              v-if="form.bidingType !== 'Logistics'"
      ...
      >
        ...
        >
          <el-table-column
            ...
            :formatter="() => $getDictLabel('BID_SCOPE', form.bidingScope)"
          />
        </el-table-column>
        ...
        >
          <el-table-column
            ...
            width="85"
            show-overflow-tooltip
            :formatter="() => $getDictLabel('BID_TYPE', form.bidingType)"
          />
        </el-table-column>
        ...
        >
          <el-table-column
            ...
            width="85"
            show-overflow-tooltip
            :formatter="() => $getDictLabel('BID_GRADING', form.evaluateMethod)"
          />
        </el-table-column>
          ...
          >
            <template v-slot="scope">
              <el-checkbox
                ...
                :disabled="flowNodeDisabled.bidEvaluation"
                true-label="Y"
                false-label="N"
              />
            </template>
      ...
      handler (val) {
        if (val) {
          ...
          if (this.dialogType === 'add') {
            ...
            if (this.form.bidingType === 'TECHNOLOGY_BUSINESS') {
  ...
  methods: {
    ...
    transformOptions (options) {
      return options.map(item => {
        return {
          ...item,
          disabled: item.value === 'COMPOSITE_SCORE' && this.form.bidingType === 'BUSINESS'
        }
      })
    },
    ...
    /* 设置流程，设置打勾 */
    setFlowNode () {
      if (this.form.bidingType === 'Logistics') {
        // 招标类型---物流招标 已经没有这个选项，后续可以去掉
        return
      }

      // 招标范围
      if (this.form.bidingScope === 'OPEN_TENDER') {
        // 公开招标
        // 不可勾选邀请供应商
        this.flowTable[0].inviteSupplier = 'N'
      } else if (this.form.bidingScope === 'INVITE_TENDER') {
        // 邀请招标
        // 勾选邀请供应商且不能取消
      ...
      // 招标类型
      if (this.form.bidingType === 'BUSINESS') {
        // 商务招标
        // 取消技术标
        this.flowTable[0].technicalManagement = 'N'
      } else if (this.form.bidingType === 'TECHNOLOGY_BUSINESS') {
        // 技术 + 商务
        // 勾选技术标
    ...
    confirmAddNew () {
      ...
      this.$http({
        url: '/api-bid/bidProcessConfig/bidProcessConfig/saveOrUpdateConfig',
        method: 'POST',
        data: params,
        loading: true
      }).then(() => {
