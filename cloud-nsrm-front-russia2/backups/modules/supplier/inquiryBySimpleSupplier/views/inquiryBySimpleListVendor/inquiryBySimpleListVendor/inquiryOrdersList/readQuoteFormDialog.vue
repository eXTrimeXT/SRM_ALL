<template>
  ...
  >
    <srm-row class="the_top_header">
      <srm-col :initCol="3">
        <p>{{ $t('bidMod.inquiryNo') }}:<span style="padding-left: 11px">{{ quoteRow.inquiryNo }}</span></p>
      </srm-col>
      <srm-col :initCol="3">
        <p>{{ $t('bidMod.quoteNo') }}:<span style="padding-left: 11px">{{ quoteRow.quoteNo }}</span></p>
      </srm-col>
      <srm-col :initCol="3" />
      <srm-col :initCol="3" />
    </srm-row>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{ $getDictLabel('QUOTE_STATUS', scope.row.quoteStatus) }}</span>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          ...
          >
            <p class="the_title1">
              物料编码：{{ thisItemCode }}
              <span style="padding-left: 31px">物料名称：</span>
              {{ thisItemName }}
              <span style="padding-left: 31px">{{ thisItemcategoryName }}</span>
            </p>
            <p><span>预计数量：</span><span>{{ thisDemandQuality }}</span></p>
            <el-table
