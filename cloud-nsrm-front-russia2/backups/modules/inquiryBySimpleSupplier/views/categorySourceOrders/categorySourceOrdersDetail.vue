<template>
  <el-container
    class="the-categorySourceOrdersDetail-detail"
    direction="vertical"
  >
    <el-main>
      <main-header>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <el-input v-model="scope.row.uploadDate" />
              </template>
            </el-table-column>
            <el-table-column
              label="操作"
...
export default {
  name: 'CategorySourceOrdersDetail',
  components: {
    MainHeader,
    CToolbar
  },
  ...
  methods: {
    ...
    backTo () {
      if (this.$attrs.params.flag == 'edit') {
        this.$emit(
          'tab-remove',
          'categorySourceOrdersDetail' + this.$attrs.params.row.biddingNum
        )
      } else {
        this.$emit('tab-remove', 'categorySourceOrdersDetail')
      }
...
.the-categorySourceOrdersDetail-detail {
  .sub_header {
    padding: 4px 11px;
    background: #eee;
  }
