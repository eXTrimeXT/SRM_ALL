  ...
  watch: {
    currencyList () {
      this.$nextTick(() => {
        ...
        this.getFormDetail(this.$attrs.params.orderId)
      })
    }
  },
  created () {
        ...
        .then((res) => {
          if (res.data) {
            ...
            if (res.data.attachList) {
              this.approvalFileList = res.data.attachList.filter(
                (v) => v.orderAttachType == 'APPROVAL'
              )
              this.orderFileList = res.data.attachList.filter(
                (v) => v.orderAttachType == 'ORDER'
              )
            }
          }
        })
