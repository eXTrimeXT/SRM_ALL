    ...
    >
      ...
      >
        <el-row>
          ...
          >
            <div class="bg-purple-light">
              {{ this.requirementHead.inquiryNo }}
            </div>
          </el-col>
        </el-row>
        <el-row>
          ...
          >
            <div class="bg-purple-light">
              {{ this.requirementHead.organizationName }}
            </div>
          </el-col>
        </el-row>
        ...
        <el-row>
          ...
          >
            <div class="bg-purple-light">
              {{ this.requirementHead.inquiryRule }}
            </div>
          </el-col>
        </el-row>
        ...
        <el-row>
          ...
          >
            <div class="bg-purple-light">
              {{ this.requirementHead.quoteRule }}
            </div>
          </el-col>
        </el-row>
        ...
        <el-row>
          ...
          >
            <div class="bg-purple-light">
              {{ this.requirementHead.publishDate }}
            </div>
          </el-col>
        </el-row>
        <el-row>
      ...
      >
        <div class="LineList">
          <el-row>
            <el-col :span="12">
              <div>
                <span>共{{ this.requirementAttaches.length }}条</span>
        ...
        <div class="card">
          ...
          >
            <el-carousel-item
              v-for="(item, index) in this.requirementAttaches"
              :key="index"
              name="index"
            >
              <el-row>
      ...
      <!-- <el-collapse-item title="随附技术文件" name="3">
        ...
        <div class="card">
          <el-carousel :autoplay="false" ref="carousel" heigth="20px">
            <el-carousel-item
              v-for="(item, index) in this.requirementAttaches"
              :key="index"
              name="index"
            >
              <el-row>
                <el-col :span="12"><div class="bg-purple">附件名称:</div></el-col>
                <el-col :span="12"
                  ><div class="bg-purple-light">

              <c-download-link
                :id="item.fileuploadId"
                :name="item.fileFullname"
                ellipsis
                class="download-link-item"
              />
              ...
              <el-row>
                <el-col :span="12"
                  ><div class="bg-purple">上传人:</div></el-col
                >
                <el-col :span="12"
                  ><div class="bg-purple-light">
                    {{ item.createdBy }}
               ...
               <el-row>
                <el-col :span="12"
                  ><div class="bg-purple">上传时间:</div></el-col
                >
                <el-col :span="12"
                  ><div class="bg-purple-light">
                    {{ item.creationDate }}
    ...
    currencyList () {
      this.$nextTick(() => {
        // 此时就可以获取到在created赋值后的assessmentTypeList了
         let data = { inquiryId: this.$attrs.params.row.inquiryId, vendorId: this.$attrs.params.row.vendorId }
    this.getFormDetail(data)
  ...
  methods: {
    fatchDictData () {
      ...
      let dictParamsArr = [
         ...
         { dictCode: 'RFQ_SCORE_RULE' },
         { dictCode: 'RFQ_QUOTE_TYPE' }
        // AQL_ASSET_TYPE 资产类别 ceeaAssetType
        // ACCEPTANCE_STATUS 验收状态 acceptStatus
      ]
      getDictItemList(dictParamsArr).then((res) => {
        const [RFQ_SCORE_RULE, RFQ_QUOTE_TYPE] = res.data
        this.currencyList = adaptDictData(
          RFQ_SCORE_RULE.RFQ_SCORE_RULE,

          'dict'
        )
        this.currencyListA = adaptDictData(
    ...
    getFormDetail (data) {
      this.$http({
        url: '/api-inq/inquiry/vendor/getInquiryInfo',
        method: 'POST',
        data,
        loading: true
      })
        .then((res) => {
          if (res) {
            ...
            this.requirementLineList = res.data.outerFiles
            this.requirementAttaches = res.data.items
              this.currencyList.forEach((item) => {
              if (item.value == this.requirementHead.inquiryRule) {
                this.requirementHead.inquiryRule = item.label
              }
               })
                 this.currencyListA.forEach((item) => {
              if (item.value == this.requirementHead.quoteRule) {
                this.requirementHead.quoteRule = item.label
              }
               })
          }
        })
