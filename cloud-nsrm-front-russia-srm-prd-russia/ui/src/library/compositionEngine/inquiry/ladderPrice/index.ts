...
const saveQuoteOrLadderPrice = ($form: any, $self: any, $getFieldParentFieldFormPath: any, $message: any, done: any) => {
  ...
  if ($self.data.mode === 'quote') {
    ...
    $form.values.orderItemList[$self.data.editRowIndex].ladderPriceList = ladderList.concat()
    $form.values.orderItemList[$self.data.editRowIndex].orderNotaxPrice = ladderList[0].orderNotaxPrice
    $form.values.orderItemList[$self.data.editRowIndex].orderTaxPrice = ladderList[0].orderTaxPrice
  ...
  for (const [index, item] of ladderList.entries()) {
    ...
    if (index !== 0 && beginQuantity !== ladderList[index - 1].endQuantity) {
      $message.warning(`第${index + 1}行 阶梯价不允许断层报价，接下来的一层只能从上一层的终点数量开始算!`)
      return
    }
  }
  ...
  console.log(ladderPriceForm, 'ladderType')
  // 立项
  $form.values.orderItemList[$self.data.editRowIndex].inqSouItem.ladderType = ladderPriceForm.ladderType
  $form.values.orderItemList[$self.data.editRowIndex].ladderList = ladderList.concat()
