<template>
  ...
  >
    <div
      v-if="isBuyer && !isReadOnlyByBuyer && !isReadOnlyByBuyer"
    ...
    >
      ...
      >
        <model-quote
          ...
          :is-read-only-by-buyer="isReadOnlyByBuyer"
    ...
    >
      ...
      <!--提交-->
      <el-button
        v-if="!isReadOnlyByBuyer && !isReadOnlyByBuyer"
        type="primary"
        @click="save"
      >
        {{ $t("common.confirm") }}
  ...
  created() {
    if (this.sourceLine) {
      if (this.sourceLine.modelQuoteLines) {
        ...
        this.modelQuoteCollapse = (this.sourceLine.modelQuoteLines).map((_item, index) => `collapseKey-${index}`)
