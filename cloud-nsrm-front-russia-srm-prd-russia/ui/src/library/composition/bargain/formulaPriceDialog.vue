  ...
  >
    <div class="ladder-price-form-row">
      ...
      <div class="form-col">
        ...
        <span class="value">{{ viewRow.targetDesc }}</span>
      </div>
      <div class="form-col">
        <!--预计数量-->
        <span class="label">{{ $t('bidMod.demandQuantity') }}:</span>
        <span class="value">{{ viewRow.quantity }}</span>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    targetNum () {
      return targetNumReveal(this.viewRow.targetNum)
    }
  },
  ...
  methods: {
    ...
    getQuoteFormulaPrices () {
      this.$http({
        url: '/api-brg/supplierCooperate/orderHead/getQuoteFormulaPrices',
        method: 'GET',
        loading: true,
        params: {
          requirementLineId: this.viewRow.requirementLineId || '',
          orderLineId: this.viewRow.orderLineId || '',
          // 币种，用于基材价格根据汇率转换
          currencyCode: this.viewRow.currencyType
        }
      }).then(data => {
