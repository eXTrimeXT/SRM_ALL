...
export const createPricingApproval = async (businessType, apiUrl, params = {}, router) => {
  ...
  const confirmSubmitResult = await MessageBox.confirm('确定生成寻源结果审批单吗？', {
  ...
  const confirmResult = await MessageBox.confirm('价格审批单创建成功，需要跳转到价格审批单页面吗？', {
  ...
  if (confirmResult === 'confirm') {
    ...
    router.push({
      ...
      params: {
        from: 'fromFun',
        // 旧版 approvalHeaderId 新版 approvalId
