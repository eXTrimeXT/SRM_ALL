  ...
  <div class="uploadTableDialog">
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button v-if="editModeV" type="primary" @click="editInvoiceRow">保存</el-button>
        <el-button v-if="hasInvoice && !editModeV" class="ml8" :type="tempFileList.length < 2 ? 'primary' : 'default'" @click="saveCurrent">保存当页</el-button>
        <el-button v-if="tempFileList.length > 1" class="ml8" type="primary" @click="saveAllPage">保存全部</el-button>
      </span>
    </srm-dialog>
      ...
      formItems: [
        ...
        },
        {
          ...
          itemAttrs: {
            ...
            rules: [{ required: true, message: '请选择增值税发票类型' }]
          },
          listeners: {
        ...
        },
        {
          ...
          itemAttrs: {
            ...
            rules: [{ required: true, message: '请输入采购方税号' }]
          },
          computedUIAttrs: _ => {
            return {
        ...
        },
        {
          ...
          computedItemAttrs: _ => {
            return {
              rules: [{ required: this.isCheckCode, message: '请输入校验码' }]
            }
          },
          computedUIAttrs: _ => {
            return {
        ...
        },
        {
          ...
          itemAttrs: {
            ...
            rules: [{ required: true, message: '请输入采购方' }]
          },
          computedUIAttrs: _ => {
            return {
              key: 'purchaserName'
        ...
        },
        {
          ...
          itemAttrs: {
            ...
            rules: [{ required: true, message: '请输入未税金额' }]
          },
          computedUIAttrs: _ => {
            return {
  ...
  computed: {
    ...
    checkAllMsg () {
      ...
      if (noSaveIndex.length > 0) {
        msg = `第${noSaveIndex.join('、')}张发票信息缺失或错误`
      }
  ...
  methods: {
    checkTaxRate (rule, value, callback) {
      ...
      if (value < 0 || value > 100) {
        return callback(new Error('请输入范围 0-100 之间'))
      }
      callback()
    },
    // 空的时候新增发票
    ...
    async closeDialog (done) {
      ...
      this.$emit('closeInvoice', this.fileList, `第${noSaveIndex}张发票未保存，是否离开？`)
    },
    // 编辑数据
    ...
    checkFileFormat (file) {
      ...
      if (!file.type.includes('image') && !file.type.includes('pdf')) {
        return {
          status: 'error',
          msg: `${file.name}: 上传失败,仅支持JPG、PNG、JPEG、PDF格式`
        }
      }
    ...
    async uploadFiles (event) {
      ...
      if (listLength > 50) {
        return this.$message.warning('发票上传数量不能超过50张')
      }

      if (files.length > 5) {
        return this.$message.warning('单次上传数量不能超过5张')
      }
      ...
      await axios({
        ...
        if (data) {
          ...
          if (data.failItems.length > 0) {
            ...
            console.log(failItems, '本次的failItems上传失败')
          }
    ...
    async checkUploadFail () {
      ...
      if (deleteIds.length > 0) {
        await this.$confirm(
          `第${deleteIds.join('、')}张识别失败，票据不清晰或为不可识别类型`,
          {
            confirmButtonText: '保留并手动录入',
            cancelButtonText: '删除',
            type: 'warning'
          }
        ).catch(() => {
