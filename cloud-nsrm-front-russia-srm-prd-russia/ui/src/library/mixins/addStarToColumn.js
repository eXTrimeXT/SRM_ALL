...
export const validateRequiredColumn = (tableRef, tableData, options = {}) => {
  ...
  if (validateResultKeys.length > 0) {
    if (!validateScope) {
      ...
      Message({
        ...messageConfig,
        message: `${tableTitle} 第${firstKey}行 ${validateResult[firstKey][0].innerText}必填！`,
        type: 'warning'
      })
    } else {
      // 校验全部
      ...
      validateResultKeys.forEach(key => {
        message += validateResult[key].map(item => {
          return `<p>第${key}行 ${item.innerText}必填！</p>`
        }).join('')
      })

      MessageBox.alert(message, `${tableTitle}校验提示`, {
