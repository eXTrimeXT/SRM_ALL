...
const updateButtonConfig = ($form: any) => {
  setTimeout(() => {
    ...
    if ($form.values.status === 'SUBMITTED' && componentInstance.workflowParamsInfo.integrationMode === 'Push') {
                  ...
                  perPlanMilestoneList: {
                    ...
                    'x-component-props': {
                      ...
                      '@handover': expression(`(row, type) => {
                            $handover(row, type, $self.query('.IFileList'), $confirm, $router,$form)
                          }`)
                    }
                  }
                }
...
const $submitData = (type: string, $values: any, $form: any, $queryEngine: any, $confirm: any, $message: any, $bus: any) => {
  ...
  $queryEngine.request.save({ ...form, perPlanMilestoneList: [...form.perPlanMilestoneList, ...$form.query('PerPlan').get('data').perPlanMilestoneDeleteList] }).then((res: any) => {
    if (res.data && res.data.length > 0) {
      $message.success($t('common.successSave'))
      const perPlanId = res.originalData?.records[0] || ''
      $form.query('PerPlan').get('data').perPlanMilestoneDeleteList = []
...
const $handover = (row: any, type: any, IFileList: any, $confirm: any, $router: any, $form:any) => {
  if (type === 'deliver') {
    $confirm('确认要生成合同验收单么？', {
  ...
  if (type === 'score') {
    ...
    $router.push({
      ...
      params: {
        from: 'contract', // 来源路由name
        row: {
          contractNo: $form.values.contractNo, // 合同编号
          milestoneType: row?.milestoneType,
          contractId: $form.values.contractHeadId
        }
      }
    })
  }
...
const $processNumClose = (node: any, $form: any, $queryEngine: any) => {
  ...
  $queryEngine.request.baseRequest({
    ...
    if (res?.originalData?.records) {
      // 记录原来的id
      if (!$form.query('PerPlan').get('data').perPlanMilestoneDeleteList.length) {
        $form.query('PerPlan').get('data').perPlanMilestoneDeleteList = form.perPlanMilestoneList.map(item => ({ '$delete': item.perPlanMilestoneId }))
      }
      form.perPlanMilestoneList = res?.originalData?.records.map((id: number) => {
        ...
        Reflect.deleteProperty(record, 'perPlanMilestoneId')
        return record
      })
      // form.perPayPlanList = []
...
<template>
  <RenderEngine
    schemaKey="contractPerformancePlanDetail"
    :pageAttrs="$attrs"
    :schema="schema"
    :scope="scope"
