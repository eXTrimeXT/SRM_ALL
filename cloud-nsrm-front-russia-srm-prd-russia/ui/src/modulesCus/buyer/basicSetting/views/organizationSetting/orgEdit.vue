...
export default {
  ...
  computed: {
    ...
    ouShow () {
      return ['OU'].includes(this.orgDataModel.orgDataform.organizationTypeCode) // && this.curOpt === 'edit'
    },
    // srm方式 可编辑
    isSrmEdit () {
  ...
  async created () {
    ...
    if (this.curOpt == 'add') {
      ...
      // 当前日期
      this.orgDataModel.orgDataform.startDate = parseTime(new Date(), '{y}-{m}-{d}')
      this.resetCompany()
      this.resetOu()
    } else {
