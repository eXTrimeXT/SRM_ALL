...
import { tabs } from 'modcb@/vendorManagementBuyer/views/vendorInfoChangeEngine/components/tabs'
...
const fatchCompanyData = async (companyId:number, $form:any) => {
  if (companyId) {
    ...
    if (response && response.data && response.data.records.length) {
      ...
      $form.query('state').get('data').userType = baseInfo.overseasRelation
      const isPerson = baseInfo.overseasRelation === 'PERSONAL'
      $form.query('state').get('data').beforeChangeJson = JSON.stringify(response.data.records[0])
      ...
      if (baseInfo) {
        if (!isPerson) {
          $form.query('.companyTypeAfter').take().value = baseInfo
          $form.query('.companyTypeBefore').take().value = JSON.parse(JSON.stringify(baseInfo))
          $form.query('.financeInfoBeforeForm').take().value = baseInfo
          $form.query('.financeInfoAfterForm').take().value = JSON.parse(JSON.stringify({ totalAssets, currentAssets, fixedAssets, avgAnnualOutput, avgAnnualProfit }))
          $form.query('.enterpriseThreeCertificatesAfter').take().value = baseInfo
          $form.query('.enterpriseThreeCertificatesBefore').take().value = JSON.parse(JSON.stringify(baseInfo))
        }
        baseInfo.ceeaBusinessModel = baseInfo.ceeaBusinessModel ? baseInfo.ceeaBusinessModel.split(',') : []
        $form.query('.companyBaseInfoAfter').take().value = baseInfo
        $form.query('.companyBaseInfoBefore').take().value = JSON.parse(JSON.stringify(baseInfo))
      }
      if (!isPerson) {
        $form.query('.siteInfosBefore').take().value = npmCompanySizes || []
        $form.query('.siteInfosAfter').take().value = npmCompanySizes.map(item => {
          const {
            totalNumber,
          ...
          return {
            ...
            developerNumber: developerNumber || null,
            productionNumber: productionNumber || null,
            overUndergraduateNumber: overUndergraduateNumber || null
          }
        }) || []
        $form.query('.financeReport').take().value = npmFinanceReports || []
        $form.query('.financeReportChange').take().value = JSON.parse(JSON.stringify(npmFinanceReports)) || []
        $form.query('.qualificationInfoBefore').take().value = managementAttaches || []
        $form.query('.qualificationInfoAfter').take().value = JSON.parse(JSON.stringify(managementAttaches)) || []
      } else {
        $form.query('.personBaseInfoBefore').take().value = baseInfo
        $form.query('.personBaseInfoAfter').take().value = JSON.parse(JSON.stringify(baseInfo))
      }
      ...
      $form.query('.bankInfoBefore').take().value = bankInfos || []
      $form.query('.bankInfoAfter').take().value = JSON.parse(JSON.stringify(bankInfos)) || []
      $form.query('.contactDataBefore').take().value = contactInfos || []
...
const $submits = async (type, $form, $queryEngine, $message, $t, $bus) => {
  ...
  const financeInfoAfterForm = $form.query('.financeInfoAfterForm').get('value')
  const financeReportChange = $form.query('.financeReportChange').get('value')
  const companySizes = $form.query('.siteInfosAfter').get('value')
  ...
  let allData = {
    ...
    ...values.form,
    userType,
    // companyInfoChange: Object.assign({ companyChangeId: $form.query('state').get('data')?.companyChangeId }, companyTypeAfter, enterpriseThreeCertificatesAfter, financeInfoAfterForm, personFormAfter, authInfoAfter),
    companyInfoChange: Object.assign({ companyChangeId: $form.query('state').get('data')?.companyChangeId }, companyTypeAfter, enterpriseThreeCertificatesAfter, financeInfoAfterForm, personFormAfter),
    ...
    npmFinanceReportChanges: financeReportChange,
  ...
  if (allData.changeId) {
    ...
    allData.financeReportChange || [].forEach((e) => {
      e.changeId = allData.changeId
    })
  }
  // 校验
  ...
  if (type == 'SAVE') { // 暂存的时候
    ...
    let overseasRelation = $form.query('state').get('data').userType
    let validate = 0
    ...
    if (overseasRelation == 'INSIDE') {
      ...
      positionList = allData.contactInfoChanges
        .filter(item => !!item.position)
        .map(itm => itm.position)
      if (!(positionList.includes('SALES_MANAGER') && positionList.includes('SENIOR_LEADER'))) {
        $message.warning($t('cusEntry.tipMessage.atLeastManageAndLeader'))
        return
      }
      let validContact = true
      allData.contactInfoChanges.some(item => {
        if (item.ceeaDefaultContact === 'Y' && !item.socialSecurityCertificateFileId) {
          validContact = false
        }
      })
      if (!validContact) {
        $message.warning($t('cusEntry.tipMessage.socialSecurityCertificateMsg'))
        return
      }
    }
    if (overseasRelation !== 'OUT') {
      ...
      if (!ceeaDefaultList.length || ceeaDefaultList.length > 1) {
        $message.warning('联系人默认联系人只能有一个')
        return
      }
      /* 校验银行主账号账户名称和企业名称一致 */
      const companyName = allData.companyInfoChange.companyName
      const mainAccountBankAccountName = allData.bankInfoChanges.find(item => item.ceeaMainAccount === 'Y')?.bankAccountName
      console.log(mainAccountBankAccountName, 'mainAccountBankAccountName')
      if (!mainAccountBankAccountName && overseasRelation != 'OUT') {
        $message.warning('银行信息必须有一个主账号')
        return false
      }
      if (companyName !== mainAccountBankAccountName) {
        $message.warning($t('cusEntry.tipMessage.companyAndBankAccount'))
        return
      }
    }
    /* 校验服务范围品类不能为空 */
    if (serviceRange.length === 0) {
      $message.warning($t('cusEntry.tipMessage.atLeastCategory'))
      return false
    }
    let validFlag = true
    ...
    if (!validFlag) {
      $message.warning($t('cusEntry.tipMessage.serviceRangeCategoryRequired'))
      return false
    }
    // 校验服务范围品类不能重复
    const categoryIdList = new Set(serviceRange.map(item => item.categoryId))
    if (serviceRange.length !== categoryIdList.size) {
      $message.warning($t('cusEntry.tipMessage.serviceRangeCategoryRepeat'))
      return false
    }
