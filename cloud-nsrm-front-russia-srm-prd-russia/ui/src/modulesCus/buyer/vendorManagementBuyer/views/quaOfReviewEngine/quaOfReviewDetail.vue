...
const $validateForm = ($form: any, $message: any) => {
  ...
  if (!ceeaEnabled.length && $form.values.ifSiteForm === 'N') {
    // 供应商银行信息至少需要启用一个
...
const $submitData = (type: string, $values: any, $form: any, $queryEngine: any, $confirm: any, $message: any, $bus: any) => {
  ...
  // 处理附件信息空字符串报错
  if (type === 'SUBMIT' && $values.ifSiteForm === 'Y') {
    ...
    $queryEngine.request.save(form, { loading: true, customizeAction: (type === 'SAVE' && $values.ifSiteForm === 'Y') ? 'saveWithoutFlow' : 'save' }).then((res: any) => {
