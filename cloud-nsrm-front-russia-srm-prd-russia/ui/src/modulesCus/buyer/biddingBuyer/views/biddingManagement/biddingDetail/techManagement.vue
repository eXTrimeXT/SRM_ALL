<template>
  <div>
    <h3 style="margin: 10px 0 20px 0">
      ...
      <!-- 9.13新加校验条件 发起过技术方案分析且结果回传后才可以点击 -->
      <!-- 11月13日新增-只有一个供应商进行报名投标，不需要进行技术方案分析， 可直接确认评标-->
      <el-button
        type="primary"
        :disabled="biddingBase.projectStatus!=='TECH_BID_EVA' || biddingBase.extConfirmFlag=='Y' || isLeader || isBidEval"
...
export default {
  ...
  computed: {
    ...
    // 当前登录人为评标组长 (评标组长在技术标页面只有两个按钮权限：技术开标、查看技术方案)
    isLeader () {
  ...
  methods: {
    ...
    openTechAnalysisTab () {
      let tab = {
        ...
        title: `技术方案分析-${this.biddingBase.extProjectNo}`,
        name: `技术方案分析-${this.biddingBase.extProjectNo}`
      }
    ...
    openTechScoreTab (row) {
      let tab = {
        ...
        title: this.biddingBase.extProjectNo + '-' + '评分详情',
    ...
    // 开始评标
    async startEvaTech () {
      const confirmResult = await this.$confirm('开始评标？', {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      }).catch(() => { /* nothing */ })
    ...
    async confirmTechEva () {
      const confirmResult = await this.$confirm('确认评标？', {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      }).catch(() => { /* nothing */ })
    ...
    async openTech () {
      ...
      if (extBidEvaluatorNum != evaGroupListNum) {
        this.$message.error('评标小组人数与评标总人数不一致，不可开技术标')
        return
      }
      const confirmResult = await this.$confirm('确定发起技术开标？', {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      }).catch(() => { /* nothing */ })
