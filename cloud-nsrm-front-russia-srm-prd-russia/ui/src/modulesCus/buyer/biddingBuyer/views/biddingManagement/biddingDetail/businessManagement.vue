...
export default {
  ...
  data () {
    return {
      ...
      busOpenUserList: [], // 可以点击商务开标的角色（招标负责人+招标部长）
      businessItemList: [],
      negotiationList: [],
  ...
  methods: {
    ...
    openPriceComparison () {
      // 未开商务标 不可查看
      if (!this.quoteFlag) {
        this.$message.error('商务标未开标，不可查看报价信息')
        return
      }
      let tab = {
        ...
        title: this.biddingBase.extProjectNo + '-' + '比价表',
    ...
    downLoadBusFile () {
      ...
      downloadFileLink(
        `/api-file/bid/businessfile/downloadBusinessFile?projectId=${this.biddingBase.projectId}`,
