<template>
  <div class="pdf-wrap">
    <div class="pdf-handler-bar">
      <el-button icon="el-icon-zoom-in" class="handle-icon" title="放大" @click="handleZoom('in')" />
      <el-button icon="el-icon-zoom-out" class="handle-icon" title="缩小" @click="handleZoom('out')" />
      <i class="devider" />
      <el-button icon="el-icon-c-scale-to-original" class="handle-icon" title="重置尺寸" @click="handleReset" />
      <i class="devider" />
      <el-button icon="el-icon-arrow-up" class="handle-icon arrow" title="上一页" @click="handlePage('prev')" />
      <el-button icon="el-icon-arrow-down" class="handle-icon arrow" title="下一页" @click="handlePage('next')" />
    </div>
        ...
        <div class="pdf-page-box"
          ...
          <div class="loading-page" v-if="!item['data'] || item['error']"
            ...
            <template v-if="!item['error']">
              <i class="el-icon-loading" style="margin-right: 10px; fontSize:18px;" />文档加载中，请稍后...
            </template>
            <template v-else>
              <!-- 文档加载失败 -->
