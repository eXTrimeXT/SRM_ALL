<template>
  ...
  >
      ...
      <el-collapse v-model="activeNum">
        <el-collapse-item :title="$t('cusEntry.biddingSettings.techFile')" name="1">
          ...
          <div style="margin: 20px 0">
            ...
            />
            <el-button type="primary" @click="goInteBidEval">智能评标</el-button>
            <el-button type="primary" @click="refresh" v-if="!isReadOnly">刷新</el-button>
          </div>
          ...
          >
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <el-input
                    v-if="scope.row.scoreItem !== '总得分' && scope.row.scoreItem === 'TEH_REVIEW'"
    ...
    >
      <div>
        <p>智能评标指南：</p>
        <p>①本工具作为辅助评标的专业助手，所提供的检索内容仅供参考，旨在提升评标过程精准度与效率。</p>
        <p>②若文件名前有"*"号，表示该文件为扫描文件或表格，无法自动检索内容，评标人员需手动预览或线下评审。</p>
        <p>③智能评标检索后，需评标组主动将检索结果内容输入至评审说明处，完成输入后点击空白处即为自动保存。</p>
        <p>④智能评标对话功能：在评标页面精准选定目标供应商，对话操作将检索该供应商所有文件，分析出相对应评审内容。</p>
      </div>
  ...
  methods: {
      ...
      getFileResult({ projectId: this.projectId }).then(res => {
        if(res && res.code + '' === '0') {
          if (res.data) {
            ...
            this.$message.warning('文件正在解析中，请稍后...')
          }
        }
      })
    },
    ...
    nextStep () {
      ...
      let tab = {
        ...
        title: `智能评标-${this.extProjectNo}`,
        name: `智能评标-${this.extProjectNo}`
      }
      ...
      vendorIdList.some(item => {
        ...
        sumItem.map(item => {
          if (item.score > item.maxScore) {
            flag = true
            errorMsg = `供应商(${sumItem[0].vendorName})${item.reviewItem}技术评分项超过最高分值`
            return false
          }
        })
        /* 计算总分 */
        ...
        if (totalScore > 10000) {
          flag = true
          errorMsg = `供应商(${sumItem[0].vendorName})技术评分总和超过100`
          return true
        }
      })
      if (flag) {
