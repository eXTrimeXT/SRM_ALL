          ...
          buttons: [
            {
              ...
              // orderStatus: 已撤回、已驳回、拟定、已拒绝
              // extStatus 只有2种状态: ONGOING 执行中（在途）、FINISHED 已完成
              show: row => ['WITHDRAW', 'REJECT', 'DRAFT'].includes(row.orderStatus)
            },
            {
            ...
            {
              callback: row => this.readOne(row, 'manage'),
              formattor: () => this.$t('purchaseDemand.manage'),
              // 审批中 (创建人可点管理按钮，进去撤回单据, 采购科长进去审批单据)
              show: row => row.orderStatus === 'UNDER_APPROVAL' && [row.createdId, row.extApproveUserId].includes(this.userId)
            },
            {
