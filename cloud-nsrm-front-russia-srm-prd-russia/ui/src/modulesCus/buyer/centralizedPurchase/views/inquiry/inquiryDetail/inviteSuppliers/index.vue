    ...
    >
      ...
      >
        ...
        <el-table-column
          ...
          show-overflow-tooltip
          :formatter="row => {
            return row.sourceFromType ? $getDictLabel('EXT_PUR_INQ_SOU_VENDOR_SOURCE_FROM_TYPE', row.sourceFromType) : ''
          }"
        />
        <!-- 是否新增供应商 -->
        <el-table-column
    ...
    async deleteSuppliersItem (index, row) {
      ...
      if (row.sourceFromType === 'DESIGN') {
        this.$prompt(this.$t('cusEntry.tipMessage.deleteReason'), this.$t('cusEntry.tipMessage.tip'), {
          ...
          await purInqApi.init.deleteVendor({ souVendorId, delReason: value })
