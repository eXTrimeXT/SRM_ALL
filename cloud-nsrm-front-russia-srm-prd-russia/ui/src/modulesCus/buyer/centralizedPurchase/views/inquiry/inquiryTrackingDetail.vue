...
export default {
  ...
  computed: {
    inquiryTrackingInfoRow () {
      return [
        ...
        { label: this.$t('cusEntry.bidMod.applyDepartment'), value: this.header.orgDeptName },
        // 审核状态
  ...
  methods: {
    ...
    async getQuoteSelectionDetail () {
      const response = await purInqApi.select.getManagementDetail(this.tabRow.projectId)
      if (response) {
        const {
          currentRoundTotalCtn,
          currentRoundQuotedCtn,
    ...
    /* 立即开始 */
    async startNow () {
      const response = await purInqApi.select.changeBeginQuote({
        // 询价单ID
