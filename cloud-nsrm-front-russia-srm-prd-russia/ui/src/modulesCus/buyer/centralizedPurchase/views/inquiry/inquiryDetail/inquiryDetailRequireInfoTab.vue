<template>
  <div class="inquiry-detail-require-info-tab">
    <el-button
      v-if="!readonly"
      type="primary"
      @click="copyLadderPrice"
    >
      {{ $t('cusEntry.common.copyLadderPrice') }}
    </el-button>
    <!-- 表格 -->
    <vxe-table
      ref="xTable"
      ...
      :checkbox-config="{
        ...
        range: true,
        checkMethod: ({ row }) => {
          return row.isLadder === 'Y' && !row.sourceRowFlag
        }}"
      :edit-config="{
        trigger: 'click',
        mode: 'row',
    ...
    >
      ...
      <vxe-column
        type="radio"
        width="60"
        :title="$t('cusEntry.centralizedPurchase.copySource')"
      />
      <!--序号-->
      <vxe-column
        type="seq"
      ...
      <vxe-column
        field="area"
        :title="$t('cusEntry.centralizedPurchase.supplyScope')"
        width="150"
      >
        <template #default="{ row }">
          {{ $getDictLabel('REGION', row.area) }}
        </template>
      </vxe-column>
      <!--t 物料编码-->
      <vxe-column
      ...
      >
        <template #edit="scope">
          <el-input v-model="scope.row.extBrand" />
        </template>
      </vxe-column>
      <!--t 是否阶梯报价-->
      <vxe-column
...
export default {
  ...
  watch: {
    items: {
      handler (val) {
        ...
        this.$set(this.page, 'total', this.itemsTableData.length)
        this.$nextTick(() => {
  ...
  methods: {
    tableFun (datas) {
      ...
      const indexStart = (currentPage-1) * 15
      datas.forEach((data, index) => {
        if (index >= indexStart && index < indexStart + 15) {
          newDatas.push(data)
        }
      })
      return newDatas
    ...
    /* 被复制行 */
    radioChange ({ oldValue, row }) {
      this.$set(row, 'sourceRowFlag', true)
      /* 获取已选中的行 */
      const selectIds = this.$refs.xTable.getCheckboxRecords().map(item => item.souItemId)
      if (selectIds.includes(row.souItemId)) {
        /* 去除选中 */
        this.$refs.xTable.setCheckboxRow(row, false)
      }
      if (oldValue) {
        this.$set(oldValue, 'sourceRowFlag', false)
      }
      this.sourceRow = row
    },
    /* 复制阶梯价 */
    copyLadderPrice () {
      /* 判断被复制行是否是阶梯价以及是否已经维护阶梯价 */
      const {
        ladderList
      } = this.sourceRow
      if (Object.keys(this.sourceRow).length === 0) {
        this.$message.warning(this.$t('cusEntry.tipMessage.selectSourceRow'))
        return false
      }
      if (ladderList.length === 0) {
        this.$message.warning(this.$t('cusEntry.tipMessage.writeSourceRowLadderPrice'))
        return false
      }
      /* 获取需要被复制阶梯价行 */
      let targetRows = this.$refs.xTable.getCheckboxRecords()
      if (targetRows.length === 0) {
        this.$message.warning(this.$t('cusEntry.tipMessage.selectCopyLadderPriceRows'))
        return false
      }
      targetRows.forEach(item => {
        item.ladderList = ladderList.map(itm => {
          const {
            beginQuantity,
            endQuantity
          } = itm
          return { beginQuantity, endQuantity }
        })
      })
      this.$message.success(this.$t('cusEntry.tipMessage.copyLadderPirceSuccess'))
    },
    /* 报价类型过滤 */
