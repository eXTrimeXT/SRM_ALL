              ...
              >
                <template slot-scope="scope">
                  <el-form-item
                    ...
                    :rules="{ required: true, message: '请选择SAP成本中心', trigger: 'blur' }"
                  >
                    <QuickSearch
                      :disabled="isReadonly"
              ...
              >
                <template slot-scope="scope">
                  <el-form-item
                    ...
                    :rules="{ required: true, message: '请选择发票用途', trigger: 'blur' }"
                  >
                    <dict-select
              ...
              >
                <template slot-scope="scope">
                  <el-form-item
                    ...
                    :rules="{ required: true, message: '请选择考核时间', trigger: 'blur' }"
                  >
                    <el-date-picker
              ...
              >
                <template slot-scope="scope">
                  <el-form-item
                    ...
                    :rules="{ required: true, message: '请填写考核事件', trigger: 'blur' }"
                  >
              ...
              >
                <template slot-scope="scope">
                  <el-form-item
                    ...
                    :rules="{ required: true, message: '请填写实际考核未税金额', trigger: 'blur' }"
                  >
                    <el-input-number
              ...
              >
                <template slot-scope="scope">
                  <el-form-item
                    ...
                    :rules="{ required: true, message: '请填写实际考核含税金额', trigger: 'blur' }"
                  >
                    <el-input-number
              ...
              >
                <template slot-scope="scope">
                  <el-form-item
                    ...
                    :rules="{ required: true, message: '请选择物料编码', trigger: 'blur' }"
                  >
                    <QuickSearch
...
export default {
  ...
  data () {
    return {
      ...
      formRules: {
        ...
        extSource: [{ required: true, message: '请选择单据来源' }]
      },
      detailsForm: {
  ...
  methods: {
    ...
    invoiceValidator (rule, value, callback) {
      ...
      sign && callback(new Error('本次开票数量不可大于可开票数量'))
      callback()
    },
    // 对账单-尾差调整
    extAdjustAmountValidator (rule, value, callback) {
      if (value > 1 || value < -1) {
        callback(new Error('尾差调整仅可填写[-1, 1]范围内的数值'))
      }
      callback()
    },
    // 对账单-转出税额
    extInputTaxAmountValidator (rule, value, callback) {
      !value && callback(new Error('转出税额必须大于0'))
      const index = Number(rule.field.split('.')[1]) // 获取行号
      const sign = this.detailsForm.statementDetailsTable.some((row, i) => {
        if (index === i) {
      ...
      sign && callback(new Error('转出税额不可大于税额'))
      callback()
    },
    // 对账单-SAP成本中心
    getSAPObj (val, scope) {
    ...
    amountValidator (rule, value, callback) {
      ...
      sign && callback(new Error('本次核销金额不可大于可核销金额'))
      callback()
    },
    ...
    async saveOrSubmitData (type) {
      ...
      if (flag) {
        if (this.isEmptyDetailTable) {
          return this.$message.error('至少添加一条对账单/验收单明细')
        }
