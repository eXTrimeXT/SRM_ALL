...
export default {
  ...
  methods: {
    ...
    async preNextStepHandler () {
      ...
      let flag = this.vendorList.some(item => !item.vendorName || !item.inspectAddress || !item.inspectCause || !item.inspectTime || !item.inspectContent)
      if (flag) {
    ...
    getBidObj (val, scope) {
      ...
      scope.bidingHead = val ? val.extSouPrincipal : '' // 招标负责人
      scope.bidingTechHead = val ? val.extTechPrincipal : '' // 技术负责人
      ...
      this.$refs.detailForm.validate(valid => {
        if (valid) {
          ...
          let flag = this.vendorList.some(item =>
            !item.vendorName || !item.inspectAddress || !item.inspectCause || !item.inspectTime || !item.inspectContent)
          if (flag) {
            this.$message.warning('请维护考察单位必填项')
            return
          }
