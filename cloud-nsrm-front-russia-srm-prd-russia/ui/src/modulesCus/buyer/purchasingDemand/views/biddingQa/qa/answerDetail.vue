    ...
    >
      <SrmRow>
        ...
        <SrmCol v-if="userTypeFlag.isBuyer" :init-col="3">
          <el-form-item prop="questionTitle" label="质疑标题">
        ...
        <SrmCol v-if="userTypeFlag.isBuyer" :init-col="3">
          <el-form-item prop="creationDate" label="创建日期">
            <el-input v-model="answerFormData.creationDate" disabled />
          </el-form-item>
        </SrmCol>

        <SrmCol :init-col="3">
          <el-form-item prop="answerStatus" label="澄清状态">
            <DictSelect
        ...
        <SrmCol v-if="userTypeFlag.isBuyer" :init-col="3">
          <el-form-item prop="createdUserName" label="创建人">
            <el-input v-model="answerFormData.createdUserName" disabled />
          </el-form-item>
        </SrmCol>

        <SrmCol v-if="userTypeFlag.isBuyer" :init-col="3">
          <el-form-item prop="lastUpdateDate" label="最后更新日期">
            <el-input v-model="answerFormData.lastUpdateDate" disabled />
          </el-form-item>
        </SrmCol>

        <SrmCol v-if="userTypeFlag.isBuyer" :init-col="3">
          <el-form-item prop="extBidNickname" label="招标负责人">
            <el-input v-model="answerFormData.extBidNickname" disabled />
          </el-form-item>
        </SrmCol>

        <SrmCol v-if="!userTypeFlag.isBuyer" :init-col="3">
          <el-form-item prop="lastUpdateDate" label="最新回复时间">
            <el-input v-model="answerFormData.lastUpdateDate" disabled />
          </el-form-item>
        </SrmCol>
  ...
  methods: {
    ...
    xianshangqianshu () {
      ...
      if (!this.multipleSelection.length) {
        return this.$message.warning('请先勾选需要线上签署的文件')
      }
    ...
    async acceptAnswer () {
      ...
      if (this.contractVerification == 'Y' && this.allowClearWithoutSealFlag != 'Y') {
        ...
        if (!flag && this.sceneFilesHuiFu.length) {
          this.$message.error('供方回复附件未签署，若已签署，请点击【刷新签署状态】')
          return
        }
      }
      const sceneFilesHuiFuAll = this.sceneFilesHuiFu.concat(this.sceneFilesHuiFuDel)
