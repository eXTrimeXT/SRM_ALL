...
export default {
  ...
  methods: {
    ...
    async addOneSuppliers (val) {
      ...
      // 判断新增供应商与已经邀请供应商是否存在关联关系
      const { data } = await inqBuyerHttp.init.supplierRelation({ projectId: this.baseInfo.projectId, vendorIds: [val.companyId] })
      if (!data) {
        this.$message.warning(this.$t('cusEntry.tipMessage.suppliersRelation'))
        return false
      }
      let isFocusVendor = false
      if (val && val.focusFlag === 'Y') {
        await this.$confirm(this.$t('cusEntry.tipMessage.focusFlagTitle'), this.$t('cusEntry.tipMessage.tip'), {
    ...
    async deleteSuppliersItem (index, row) {
      if (row.sourceFromType === 'AUTO_RECOMMEND') {
        this.$prompt(this.$t('cusEntry.tipMessage.deleteReason'), this.$t('cusEntry.tipMessage.tip'), {
          confirmButtonText: this.$t('common.confirm'),
          cancelButtonText: this.$t('common.cancel'),
          inputErrorMessage: this.$t('cusEntry.tipMessage.deleteReason'),
          ...
          await inqBuyerHttp.init.deleteSupplier({ ...row, delReason: value, projectId: this.baseInfo.projectId })
          this.$message.success(this.$t('common.successDelete'))
          this.inviteSuppliersList.splice(index, 1)
        })
      } else {
        await inqBuyerHttp.init.deleteSupplier({ ...row, projectId: this.baseInfo.projectId })
        this.$message.success(this.$t('common.successDelete'))
        this.inviteSuppliersList.splice(index, 1)
      }
      this.$nextTick(() => {
    ...
    async saveRecommendVendor (vendorList = []) {
      if (this.addType === 'historyLowPrice') {
        // 判断新增供应商与已经邀请供应商是否存在关联关系
        const { data } = await inqBuyerHttp.init.supplierRelation({ projectId: this.baseInfo.projectId, vendorIds: vendorList.map(item => item.vendorId) })
        if (!data) {
          this.$message.warning(this.$t('cusEntry.tipMessage.suppliersRelation'))
          return false
        }
