...
import { tabs } from 'modcs@/vendorManagementSupplier/views/vendorInfoChangeEngine/components/tabs'
...
const fatchCompanyData = async (companyId:number, $form:any) => {
  if (companyId) {
    ...
    if (response && response.data && response.data.records.length) {
      ...
      if (rest) {
        if (!isPerson) {
          $form.query('.companyTypeAfter').take().value = rest
          $form.query('.companyTypeBefore').take().value = JSON.parse(JSON.stringify(rest))
          $form.query('.financeInfoBeforeForm').take().value = rest
          ...
          $form.query('.enterpriseThreeCertificatesAfter').take().value = rest
          $form.query('.enterpriseThreeCertificatesBefore').take().value = JSON.parse(JSON.stringify(rest))
        }
        rest.ceeaBusinessModel = rest.ceeaBusinessModel ? rest.ceeaBusinessModel.split(',') : []
        $form.query('.companyBaseInfoAfter').take().value = rest
        $form.query('.companyBaseInfoBefore').take().value = JSON.parse(JSON.stringify(rest))
      }
      if (!isPerson) {
        $form.query('.siteInfosBefore').take().value = npmCompanySizes || []
...
const $submits = async (type, $form, $queryEngine, $message, $t, $bus) => {
  ...
  if (type == 'SAVE') { // 暂存的时候
    ...
    let positionList = []
    if ($form.query('state').get('data').userType == 'INSIDE') {
    ...
    if ($form.query('state').get('data').userType !== 'OUT') {
      ...
      if (!mainAccountBankAccountName && $form.query('state').get('data').userType != 'OUT') {
