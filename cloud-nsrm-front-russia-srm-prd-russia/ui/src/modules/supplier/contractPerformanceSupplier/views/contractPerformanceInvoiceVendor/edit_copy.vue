  ...
  <el-container class="flex-container wrapper" direction="vertical">
          ...
          <el-table border stripe class="mt-10" :data="perInvoiceDetailList">
            ...
            <el-table-column min-width="180" show-overflow-tooltip>
              <template slot="header">
                <i class="toRequired">*</i>
                {{ $t('other.key23') }}
                <el-tooltip class="item" effect="dark" content="一个开票单只能选择一种里程碑" placement="top">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                ...
                />
                <span v-show="!scope.row.editabled">{{ scope.row.invoiceDate }}</span>
              </template>
            </el-table-column>
            <!-- 校验码 -->
            <el-table-column
    ...
    >
      ...
      >
        ...
        <el-table-column prop="creationDate" :label="$t('common.creationTime')" show-overflow-tooltip />
      </el-table>
      <CPagination
  ...
  data () {
    return {
      ...
      rules: {
        ...
        currencyCode: [{ required: true, message: '请选择币种' }],
        taxKey: [{ required: true, message: '请选择税率' }]
      },
  ...
  methods: {
    ...
    invoicingVerification () { // 校验是否一个开票单只能有一种里程碑
      if (this.perInvoiceDetailList.length > 0) {
        ...
        if (!bol2) {
          this.__jump_error__('invocie', 'component', this.$t('请输入里程碑节点'))
          return bol2
        }
        if (!bol) {
          this.__jump_error__('invocie', 'component', this.$t('一个开票单只能选择一种里程碑'))
        }
        return bol
      } else {
    ...
    async saveBill (type) {
      ...
      if (type === 'SAVE') {
        ...
        if (!perInvoiceDetailList.length) {
          this.jumpErrorInvoice('请填写开票明细相关信息')
          return
        }
        for (let item of perInvoiceDetailList) {
          if (!item.contractNo) {
            this.jumpErrorInvoice('开票明细---合同序号必填')
            return
          }
          if (!item.milestoneType) {
            this.jumpErrorInvoice('开票明细---里程碑节点必填')
            return
          }
          if (!item.currentUntaxedAmount) {
            this.jumpErrorInvoice('开票明细---本次开票金额未税必填')
            return
          }
        }
