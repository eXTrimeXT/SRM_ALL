...
const $batchAccept = async ($self: any, $queryEngine: any, $message: any, $confirm: any) => {
  ...
  if (sequences) {
    return $message.warning(`只有待供方确认的单据可以接受，序号${sequences}不可接受`)
  }

  sequences = fn((row: any) => !row.ceeaPromiseReceiveDate)
  if (sequences) {
    return $message.warning(`序号${sequences}没有填写供方承诺到货日期`)
  }
...
const $acceptOne = (rows: any, $queryEngine: any, $message: any) => {
  let params = rows.map((item: any) => {
    return {
      ...
      ceeaPromiseReceiveDate: parseTime(item.ceeaPromiseReceiveDate, '{y}-{m}-{d} {h}:{i}:{s}')
    }
  })
...
const $batchRreject = async ($self: any, $queryEngine: any, $message: any, $prompt: any) => {
  ...
  if (sequences) {
    return $message.warning(`只有待供方确认的单据可以拒绝，序号${sequences}不可拒绝`)
  }
                  ...
                  accept: {
                    ...
                    'x-component-props': {
                      ...
                      '@confirm': expression(`({ row }) => {  
                        if(!row.ceeaPromiseReceiveDate) return $message.warning('请填写供方承诺到货日期')
