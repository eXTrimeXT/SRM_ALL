<template>
  ...
  >
    <el-main>
              ...
              <srm-col>
                ...
                >
                  <DictSelect
                    ...
                    :code="formData.organizationId"
                    :custom-select-type="formData.organizationId ? 'RECEIVE_ADDRESS' : ''"
          ...
          >
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <el-form-item
                    :rules="visitorsNotes.rules.visitorName"
                    :prop="`list.${scope.$index}.visitorName`"
                  >
                    <el-input v-model="scope.row.visitorName" />
                  </el-form-item>
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template slot-scope="scope">
                  <el-form-item
                    :rules="visitorsNotes.rules.idType"
              ...
              >
                <template slot-scope="scope">
                  <el-form-item
                    :rules="visitorsNotes.rules.idNo"
              ...
              >
                <template slot-scope="scope">
                  <el-form-item
                    :rules="visitorsNotes.rules.linkPhone"
      ...
      <CToolbar>
        <template slot="right">
          <el-button @click="cancelBill">
            {{ editPage ? $t('common.cancel') : $t('common.close') }}
          </el-button>
          <el-button
            v-if="editPage"
            type="primary"
      ...
      <DeliveryAppointmentsCheckBoxs
        :id="appointmentDetailIds"
        :visible="deliveryNotes.show"
        @on-ok="addDelivery"
        @close="deliveryNotes.show = false"
      />
    </el-main>
  </el-container>
</template>
...
import DeliveryAppointmentsCheckBoxs from './deliveryAppointmentsCheckBoxs'
...
export default {
  ...
  data () {
    return {
      ...
      formData: {
        ...
        carType: null,
        licensePlate: null,
        entryTime: parseTime(new Date(), '{y}-{m}-{d}'),
      ...
      rules: {
        ...
        receiveAddressId: [{ required: true, message: this.$t('orderMod.msgOrder[25]') }],
  ...
  computed: {
    appointmentDetailIds () {
      return this.deliveryNotes.list.map((i) => i.deliveryNoteId)
    },
    editPage () {
  ...
  methods: {
    ...
    selectHandler (node, value, scope) {
      ...
      if (this.formData.organizationId) {
        ...
        this.formData.organizationCode = null
        this.formData.organizationName = null
      }
    },
    selectHandler2 (node, value, scope) {
      if (node) {
        this.$set(this.formData, 'organizationId', node.organizationId)
        this.$set(this.formData, 'organizationCode', node.organizationCode)
        this.$set(this.formData, 'organizationName', node.organizationName)
      }
    },
    // 快速查询受访人员
    ...
    // 新增送货单据
    addDelivery (selection) {
      setRepeatData(this.deliveryNotes.list, selection, 'deliveryNoteId')
      this.deliveryNotes.show = false
    },
    // 删除送货单据
    delDelivery (index) {
    ...
    queryDetail () {
      deliveryAppointmentsApi.getDeliveryAppointById({
    ...
    // 提交
    async submitBill () {
      const { flag, message } = await getCheckForm([this.$refs['deliveryAppoint']])
      if (flag) {
        if (!this.deliveryNotes.list.length) {
          this.$message({
            type: 'warning',
            message: this.$t('orderMod.msgVendorOrder[14]')
          })
          return
        }
        if (!this.visitorsNotes.list.length) {
          this.$message({
            type: 'warning',
            message: this.$t('orderMod.msgVendorOrder[15]')
          })
          return
        }
    ...
    // 数据提交type(save暂存，submit提交)
    handleSubmit (type) {
      let appointDeliveryNotes = this.deliveryNotes.list.map(({ deliveryNoteId }) => ({
        deliveryNoteId
      }))
      let modelData = {
        deliveryAppoint: this.formData,
        appointDeliveryNotes: appointDeliveryNotes,
        deliveryAppointVisitors: this.visitorsNotes.list
      }
      let tabName =
        this.flag === 'add'
          ? 'deliveryAppointmentDetail'
          : 'deliveryAppointmentDetail' + this.$attrs.params.row.deliveryAppointId
      if (type === 'save') {
        this.$http({
          url: '/api-sup-ce/order/deliveryAppoint/temporarySave',
          method: 'POST',
          data: modelData
        }).then((res) => {
          this.$message({
        ...
        this.$http({
          url: '/api-sup-ce/order/deliveryAppoint/submit',
          method: 'POST',
          data: modelData
        }).then((res) => {
          this.$message({
    ...
    cancelBill () {
      ...
      this.__setTabTodo('deliveryAppointmentDetail.getQueryData')
    }
  }
}
</script>
