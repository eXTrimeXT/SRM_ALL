...
const $save = ($form: any, $queryEngine: any, $message: any) => {
  let params = $form.values.detailList.map((item: any) => {
    return {
      parentLineId: item.parentLineId, confirmNum: item.confirmNum, promiseReceiveDate: item.promiseReceiveDate ? parseTime(item.promiseReceiveDate, '{y}-{m}-{d} {h}:{i}:{s}') : null, refusedReason: item.refusedReason
    }
  })
...
const $confirm = ($form: any, $queryEngine: any, $message: any, $bus: any) => {
  $form.validate().then(() => {
    ...
    const params = detailList.map((item: any) => {
      return {
        parentLineId: item.parentLineId, confirmNum: item.confirmNum, promiseReceiveDate: parseTime(item.promiseReceiveDate, '{y}-{m}-{d} {h}:{i}:{s}'), refusedReason: item.refusedReason
      }
    })
