<template>
  <el-container class="flex-container bidding-orders-sign-up" direction="vertical">
    <el-main>
...
export default {
  ...
  methods: {
    /* 查询报名详情 */
    async getSignUpInfo () {
      const response = await bidSupplierHttp.signUp.getSignUpInfo(this.paramsRow.projectId)
    ...
    async saveOrSubmit (type) {
      ...
      const response = await bidSupplierHttp.signUp.vendorSignUp({
