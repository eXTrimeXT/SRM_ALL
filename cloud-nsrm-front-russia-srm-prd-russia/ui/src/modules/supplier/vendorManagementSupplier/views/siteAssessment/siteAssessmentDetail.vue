                ...
                <srm-col :span="6">
                  ...
                  <el-form-item :label="$t('vendorMod.planName3')">
                    <QuickSearch
                      ...
                      :pre-query-data="{'t.VENDOR_ID':this.allParams.siteForm.vendorId}"
              ...
              >
                <template slot-scope="scope">
                  <SrmCommonFile
                    ...
                    @on-change="({file}) => tableHandleUploadSuccess(value, scope.row)"
                  />
                </template>
              </el-table-column>
              <!-- 评审人员 -->
...
import { siteReviewModel, sampleConfirmed, siteReviewPlan } from 'mods@/vendorManagementSupplier/api'
...
// reviewModelId
    ...
    getFininfo (vendorId) {
      // 根据供应商ID获取上次评审人员和时间
      siteReviewPlan.getLastSiteFormMessage({ vendorId }).then(res => {
        if (res.data) {
    ...
    fatchAccessFlow (quaReviewType) {
      siteReviewPlan.getEntryConfigByQuaReviewType({ quaReviewType }).then(res => {
        if (res) {
    ...
    saveDataHandle (type) {
      ...
      return new Promise(rs => {
        this.$http({
          url: '/api-sup/review/siteForm/saveOrUpdateSiteForm',
          method: 'POST',
          data: this.allParams,
          loading: true
        })
