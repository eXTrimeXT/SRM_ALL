  ...
  <el-container class="flex-container-notab the_functionMaintenance_wrapper" direction="vertical">
    <el-main>
      ...
      <MainHeader :l-span="22" :r-span="2">
        <template slot="left">
          <AuthorityButton type="primary" code="rbac:user:add" @click="addOne">
            {{ $t("common.add") }}
          </AuthorityButton>
      ...
      >
        ...
        <div class="form-subTitle-btn">
          <span style="padding-right: 22px">{{
            $t("dataConfMod.roleAccess")
          }}</span>
          <el-button type="primary" @click="addOneRole">
            {{ $t("common.new") }}
          </el-button>
        </div>
        <div class="roleCtrl">
          <el-table :data="roleUsers" style="width: 100%" border height="150px">
            <!-- 角色名称 -->
            <el-table-column align="center" prop="roleId" :label="$t('dataConfMod.roleName')">
              <template slot-scope="scope">
                <el-select v-model="scope.row.roleId" style="display: block">
                  <el-option
            ...
            >
              <template slot-scope="scope">
                <el-button type="text" @click="bankeDel(scope.$index, scope.row)">
                  {{ $t("common.delete") }}
                </el-button>
              </template>
            </el-table-column>
    ...
    saveData () {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (!this.isEdit) {
            // 新增
            accountApi.addVendor(submitData).then(res => {
              this.$message({
                message: this.$t('dataConfMod.passwordSendEmail'),
                type: 'success'
              })
              this.getQuerydata()
            ...
            // 编辑
            accountApi.modifyVendor(submitData).then(res => {
              this.$message({
                message: res.message,
                type: 'success'
              })
