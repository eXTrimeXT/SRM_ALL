...
export default {
  ...
  methods: {
    ...
    async getSignUpInfo () {
      ...
      const response = await carVendorHttp.signUp.getSignUpInfo(transformParams)
      if (response?.data?.records.length) {
        const { signUpFileList } = response.data.records[0]
    ...
    async saveOrSubmit (type) {
      ...
      const response = await carVendorHttp.signUp.vendorSignUp(transformParams)
      if (response) {
