...
export default {
  ...
  properties: {
    ...
    pagination: {
      ...
      'x-component-props': {
        pageNum: expression(`$form.query('InvoiceNotice').get('data').invoiceDialogPageNum`),
        pageSize: expression(`$form.query('InvoiceNotice').get('data').invoiceDialogPageSize`),
        total: expression(`$form.query('InvoiceNotice').get('data').invoiceDialogTotal`),
        pageSizes: [5, 15, 30, 60, 120, 300, 600, 1000, 1500],
        '@current-change': expression(`(num) => {
          $form.query('InvoiceNotice').get('data').invoiceDialogPageNum = num
          $getInvoiceDialogData($form)
        }`),
        '@size-change': expression(`(size) => {
          console.log('size:::',size)
          $form.query('InvoiceNotice').get('data').invoiceDialogPageSize = size
          $getInvoiceDialogData($form)
        }`)
      }
    }
