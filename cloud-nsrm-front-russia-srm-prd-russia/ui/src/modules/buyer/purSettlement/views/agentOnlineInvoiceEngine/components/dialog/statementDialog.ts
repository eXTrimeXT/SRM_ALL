...
export default {
  ...
  properties: {
    formWrapper: {
      ...
      'x-component-props': {
        colLength: 2,
        'form-array': expression('$form.query(\'OnlineInvoice\').get(\'data\').statementDetailsDialogQueryForm'),
    ...
    pagination: {
      ...
      'x-component-props': {
        pageNum: expression('$form.query(\'OnlineInvoice\').get(\'data\').statementDialogPageNum'),
        pageSize: expression('$form.query(\'OnlineInvoice\').get(\'data\').statementDialogPageSize'),
        total: expression('$form.query(\'OnlineInvoice\').get(\'data\').statementDialogTotal'),
        pageSizes: [5, 15, 30, 60, 120, 300, 600, 1000, 1500],
        '@current-change': expression(`(num) => {
              $form.query('OnlineInvoice').get('data').statementDialogPageNum = num
              $getStatementDialogData($form)
            }`),
        '@size-change': expression(`(size) => {
              $form.query('OnlineInvoice').get('data').statementDialogPageSize = size
              $getStatementDialogData($form)
            }`)
      }
    }
