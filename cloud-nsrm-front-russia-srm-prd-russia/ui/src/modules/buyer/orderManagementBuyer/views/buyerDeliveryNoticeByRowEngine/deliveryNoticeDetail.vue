...
const $computational = (list: any) => {
  let flag = false
  let orderNumberAndOrderDetailLineNum = null
  for (let i = 0; i < list.length; i++) {
    let total = list[i].reduce((prev: any, cur: any) => prev + Number(cur.noticeSum || 0), 0)
    console.log(total, list[i][0].remainingNoticeQuantity)
    if (total > list[i][0].remainingNoticeQuantity) {
      // 允许供应商更改订单数量和拒绝订单时-本次通知送货数量大于剩余可通知数量
      flag = true
      orderNumberAndOrderDetailLineNum = list[i][0].orderNumberAndOrderDetailLineNum
      break
    }
  }
  return { flag, orderNumberAndOrderDetailLineNum }
}
...
const $saveData = ($form: any, $queryEngine: any, $message: any, $bus: any, type: any) => {
  ...
  form.detailList.forEach((item: any) => {
    item.receiveDate = item.receiveDate ? parseTime(item.receiveDate, '{y}-{m}-{d} {h}:{i}:{s}') : ''
  })
  ...
  if (type === 'DRAFT') {
    ...
    $form.validate().then(() => {
      if (form.detailList.length === 0) {
        return $message.warning($t('buyerDeliveryNotice.prompt2')) // 请选择订单明细
      } else {
        let map = new Map()
        let detailListNew = []
        form.detailList.forEach((item: any) => {
          map.has(item.orderNumberAndOrderDetailLineNum) ? map.get(item.orderNumberAndOrderDetailLineNum).push(item) : map.set(item.orderNumberAndOrderDetailLineNum, [item])
        })
        detailListNew = [...map.values()]
        console.log(detailListNew, 111)
        const { flag, orderNumberAndOrderDetailLineNum } = $computational(detailListNew)
        if (flag) {
        // 订单编号|行号【x】累计本次通知数量>剩余可通知数量，请检查！
          return $message.warning(`${$t('orderMod.buyerOrderSynergy.orderNumber')}|${$t('vendorMod.relegation.lineNumber')}【${orderNumberAndOrderDetailLineNum}】${$t('buyerDeliveryNotice.prompt5')}`)
        }
        saveFetch($form, form, $queryEngine, $message, $bus, type)
      }
    }).catch((err: any) => {
