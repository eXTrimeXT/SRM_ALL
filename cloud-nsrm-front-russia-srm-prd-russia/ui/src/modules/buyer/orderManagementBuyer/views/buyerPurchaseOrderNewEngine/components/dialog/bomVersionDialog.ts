...
export default {
  ...
  properties: {
    ...
    pagination: {
      ...
      'x-component-props': {
        pageNum: expression('$form.query(\'Order\').get(\'data\').bomVersionListPageNum'),
        pageSize: expression('$form.query(\'Order\').get(\'data\').bomVersionListPageSize'),
        total: expression('$form.query(\'Order\').get(\'data\').bomVersionListTotal'),
        pageSizes: [5, 15, 30, 60, 120, 300, 600, 1000, 1500],
        '@current-change': expression(`(num) => {
                $form.query('Order').get('data').bomVersionListPageNum = num
                $getBomVersionList($form, $queryEngine)
              }`),
        '@size-change': expression(`(size) => {
                $form.query('Order').get('data').bomVersionListPageSize = size
                $getBomVersionList($form, $queryEngine)
              }`)
      }
    }
