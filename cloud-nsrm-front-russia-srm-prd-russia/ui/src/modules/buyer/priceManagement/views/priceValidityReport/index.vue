  ...
  data () {
    return {
      ...
      tableHeader: [
        ...
        // 价格有效期自
        { prop: 'effectiveDate', label: this.$t('quota.priceStartTime'), minWidth: 140 },
        // 价格有效期至
        { prop: 'expirationDate', label: this.$t('quota.priceEndTime'), minWidth: 140 },
        // 剩余天数
        { prop: 'remainDays', label: '剩余天数', minWidth: 140 }
      ],
      tableData: [],
      preArr: [
        // f 物料编码
  ...
  methods: {
    ...
    exportExcelBeforeOpen (value, callback) {
      if (!Object.keys(this.queryParam).find(key => this.queryParam[key])) {
        this.$message.warning('请至少输入一项查询条件后点击查询，再执行导出！')
        callback(null)
      }
    }
  }
