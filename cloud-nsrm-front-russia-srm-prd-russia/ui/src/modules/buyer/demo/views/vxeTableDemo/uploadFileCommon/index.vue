        ...
        <srm-row>
          ...
          <srm-col :init-col="3">
            <SrmCommonFile
              ...
              upload-tips="请上传.xlsx/.doc文件，大小不大于5MB！"
              :validate-options="{
                size: 1024 * 5,
                accept: ['.xlsx', '.doc']
              }"
    ...
    <div>
      ...
      <el-table :data="tableData">
        ...
        <SrmCommonFile
          ...
          drag
          :table-column-options="{
            label: '图片卡片，配置title和tips为空只显示图标',
            prop: 'fileId2',
            nameProp: 'fileName2'
          }"
          :dragger-options="{
...
export default {
  ...
  data () {
    return {
      ...
      formRules: {
        fileId: [{ required: true, message: '请上传附件！', type: 'number', trigger: ['change', 'blur'] }],
        fileIdA: [{ required: true, message: '请上传附件！', type: 'number', trigger: ['change', 'blur'] }]
      },
      extraData: {
        fileModular: 'base',
      ...
      tableData: [
        {
          ...
          fileName: '文件模板(1).xlsx',
          fileIdRead: 358642365452416,
          fileNameRead: '文件模板(2).xlsx',
          fileId2: 360938475217024,
          fileName2: '竞价流程概览.jpg'
        },
        { date: '2016-05-04' }
      ]
    }
  ...
  methods: {
    ...
    /* 提交 */
    async submit () {
      const valid = await this.$refs.demoForm.validate().catch(() => this.__focus_error__('请检查必填项是否已填写！'))
