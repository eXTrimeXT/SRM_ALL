      ...
      tableHeader: [
        ...
        },
        {
          ...
          buttons: [
            ...
            {
              callback: (row) => this.questSupplierResultView(row),
              // code: "pr:requirementApply:edit",
              show: (row) =>
                this.curRole === 'BUYER' &&
                [
            ...
            },
            {
              ...
              show: (row) =>
                this.curRole === 'BUYER' &&
                ((['PRE_PASS'].includes(row.approvalStatus) && row.integrationMode !== 'None') || ['WRITED'].includes(row.approvalStatus) ||
                  (this.flowWithTabMode.includes(row.integrationMode) && row['workflowAuditStatus'] === 'WAIT'))
            },
            {
              callback: function (row) {
    ...
    afterQuery () {
      this.$refs[this.gridId].setTableData(async (tableData) => {
        for (let i = 0; i < tableData.length; i++) {
          ...
          this.$set(tableItem, 'integrationMode', this.integrationMode)
        }
        if (this.notSearchTodoMode.includes(this.integrationMode)) {
          return
        }
        await this.listQueryTodo()
        console.log('this query todo', this.queryTodoList)
        for (let i = 0; i < tableData.length; i++) {
