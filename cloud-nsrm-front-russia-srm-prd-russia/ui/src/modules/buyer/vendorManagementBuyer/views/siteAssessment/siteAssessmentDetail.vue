<template>
  ...
  >
        ...
        >
          ...
          >
            <el-form
              ...
              :disabled="curOpt === 'view' || orderStatus == 'PUBLISH' || orderStatus == 'SUBMITTED' || orderStatus == 'APPROVED' || orderStatus == 'WITHDRAW' || orderStatus == 'REJECTED'"
            >
              <srm-row :gutter="32">
                <srm-col :span="6">
                  <!-- 供应商评审类型 -->
                  <el-form-item
                    ref="assessmentType"
                    :label="$t('vendorMod.siteType2')"
                    prop="assessmentType"
                  >
                    <DictSelect
                      v-model="allParams.siteForm.assessmentType"
                      :disabled="bol1 != 1 || orderStatus == 'PUBLISH' || orderStatus == 'SUBMITTED' || orderStatus == 'APPROVED' || orderStatus == 'WITHDRAW' || orderStatus == 'REJECTED' || disVendorName || allParams.siteForm.reviewFormNumber != ''"
                ...
                <srm-col :span="6">
                  ...
                  >
                    <QuickSearch
                      ...
                      :disabled="bol1 != 1 || disVendorName || !allParams.siteForm.assessmentType || allParams.siteForm.reviewFormNumber != ''"
                      name="scc_sup_company_info2"
                      @close-quicksearch="getCompanyObj"
                    />
                  </el-form-item>
                </srm-col>
                <srm-col v-if="allParams.siteForm.assessmentType == 'ACCESS_ASSESSMENT'" :span="6">
                  <!-- 资质审查单号 -->
                  <el-form-item
                ...
                <srm-col :span="6">
                  <!-- 供应商评审单号 -->
                  <el-form-item
                    :label="$t('vendorMod.siteOrderInfoV')"
                    prop="siteFormNumber"
                  >
                    <el-input
                ...
                <srm-col :span="6">
                  <!-- 关联计划 -->
                  <el-form-item :label="$t('vendorMod.planName3')">
                    <QuickSearch
                      :show-input="allParams.siteForm.planName"
                      show-key="planName"
                      :scope-data="allParams.siteForm"
                      :disabled="bol1 != 1 || addBol"
                      :pre-query-data="{'t.VENDOR_ID':allParams.siteForm.vendorId}"
                ...
                <srm-col>
                  <!-- 是否允许供方查看结果 -->
                  <el-form-item :label="$t('vendorMod.supplierViewFlag')">
                    <DictSelect
                      v-model="allParams.siteForm.supplierViewFlag"
                      code="YES_OR_NO"
                      :disabled="bol1 != 1"
                    />
                  </el-form-item>
                ...
                <srm-col :span="24">
                  <!-- 供应商评审说明 -->
                  <el-form-item :label="$t('vendorMod.siteExplainV')">
                    <el-input
          ...
          <el-collapse-item
            ref="historySiteAssessment"
            :title="$t('vendorMod.historySiteAssessment')"
            name="2"
          >
            <TableView
              ref="siteFormHistoryList"
              :table-header="historyTableHeader"
              ...
              >
                <template slot-scope="scope">
                  <el-checkbox
                    ...
                    true-label="true"
                    false-label="false"
                    :disabled="addressReadOnly || bol1 == 4 || orderStatus == 'PUBLISH' || orderStatus == 'SUBMITTED' || orderStatus == 'APPROVED'"
                  />
                </template>
              </el-table-column>
              <!-- 国家/地区 -->
          ...
          <!-- 认证组织和品类 -->
          <el-collapse-item
            ref="authOrganization"
            :title="$t('vendorMod.authOrganization')"
            name="4"
          >
            <el-table
              ref="orgCateJournals"
              :data="allOrgCateJournalsList"
              style="width: 100%"
              border
              max-height="250px"
            ...
            <!-- <TableView
              ref="orgCateJournals"
              :table-header="orgCateHeader"
              :openCustomTable="false"
              :checkbox="true"
              :checkChange="handleCurrentChange"
              :tableInfor="allOrgCateJournalsList"
              :pageEnabled="false">
          ...
          >
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <QuickSearch
                    v-if="scope.row.personType == 'GRAND_JURY'"
              ...
              <el-table-column
                align="center"
                prop="reviewModelId"
                :label="$t('vendorMod.reviewModelId')"
                min-width="100"
                :show-overflow-tooltip="true"
              >
                <template slot-scope="scope">
                  <el-select
                    ...
                    filterable
                    :placeholder="$t('common.pleaseSelect')"
                    :disabled="addressReadOnly || bol1 != 1 || orderStatus == 'PUBLISH' || orderStatus == 'SUBMITTED' || orderStatus == 'APPROVED'"
                    clearable
                  >
                    <el-option
                      v-for="item in modelList"
          ...
          >
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <SrmCommonFile
                    ...
                    :readonly="curOpt == 'view' || bol1 == 3 || bol1 == 4"
                    @on-change="({file}) => tableHandleUploadSuccess(file,scope.row)"
                  />
                </template>
              </el-table-column>
              <!-- 评审人员 -->
              ...
              <el-table-column
                align="center"
                :prop="$t('vendorMod.authResult')"
                :label="$t('vendorMod.result')"
                min-width="100"
                :show-overflow-tooltip="true"
              >
                <template slot-scope="scope">
                ...
                >
                  ...
                  >
                    <DictSelect
                      ...
                      :disabled="(bol1 == 2 && accountNameId != allParams.siteForm.createdId) || bol1 == 4 || orderStatus == 'APPROVED'"
    ...
    <CFillProgress
      ref="siteProgress"
      :nodeName="$t('route.siteAssessment2')"
      :data="computedNode"
      :percentage="true"
...
export default {
    ...
    return {
      ...
      historyTableHeader: [
        // 状态
        {
          ...
          formattor: (val) => _this.$getDictLabel('SUPPLIER_APPROVE_STATUS_TYPE', val)
        },
        // 现场评审编号
        {
          prop: 'siteFormNumber',
          label: this.$t('vendorMod.siteFormNumber')
        },
        // 现场评审类型
        {
          prop: 'assessmentType',
          label: this.$t('vendorMod.assessmentType'),
          formattor: (val) => _this.$getDictLabel('CEEA_ASSESSMENT_TYPE', val)
        },
        // 资质审查单号
        {
          prop: 'reviewFormNumber',
          label: this.$t('vendorMod.reviewFormNumber2')
        },
        // 供应商名称
        {
          prop: 'vendorName',
          label: this.$t('vendorMod.vendorName')
        },
        // 评审地址
        {
          prop: 'siteAdress',
          label: this.$t('vendorMod.siteAdress')
        },
        {
          prop: 'createdFullName',
          label: this.$t('vendorMod.createdFullName')
        },
        {
          prop: 'creationDate',
          label: this.$t('vendorMod.creationDate2')
        },
        // 评审结论
        {
          prop: 'reviewResult',
          label: this.$t('vendorMod.reviewResult'),
          formattor: (val) => _this.$getDictLabel('CEEA_RESULT_TYPE', val)
        }
      ...
      nodeData: [
        ...
        {
          code: 'siteAssessment',
          name: this.$t('route.siteAssessment2'),
          percentage: 0
        },
        {
          code: 'historySiteAssessment',
          name: this.$t('vendorMod.historySiteAssessment'),
          percentage: 0
        },
        {
          code: 'authBaseInfo',
          name: this.$t('vendorMod.authBaseInfo'),
          percentage: 0
        },
        {
          code: 'authOrganization',
          name: this.$t('vendorMod.authOrganization'),
          percentage: 0
        },
        {
          code: 'workingGroupStaff',
          name: this.$t('vendorMod.workingGroupStaff'),
          percentage: 0
        },
        {
          code: 'attachment',
          name: this.$t('vendorMod.attachment'),
          percentage: 0
        }
      ],
      curOpt: 'add',
  ...
  computed: {
    currentOrgCateRows () {
      return this.allOrgCateJournalsList.filter(item => item.quoted === 'Y')
    },
  ...
  created () {
    this.buttonConfigInfo.save.view = this.viewUpdateButton
    this.buttonConfigInfo.submit.view = this.viewUpdateButton
    this.buttonConfigInfo.save.name = this.$t('common.staging') // 暂存
    this.buttonConfigInfo.submit.name = this.$t('common.submit') // 提交
    ...
    if (this.$attrs.params.flag == 'adds') {
      ...
      // 经测试与产品沟通从其他页面带过来的暂不回显供应商
      let { vendorId, vendorCode, vendorName, planName, siteReviewPlanId } = this.$attrs.params.row
      ...
      this.addBol = true
      const obj = {
        siteReviewPlanId: siteReviewPlanId,
        planName: planName
      }
  ...
  methods: {
    ...
    getCategoryObj2 (val, scope) {
      ...
      this.$set(this.allParams.siteForm, 'planName', val.planName)
      console.log(val)
      if (!val) {
        return false
      }
...
// reviewModelId
    ...
    stateDetermination () {
      ...
      if (this.curOpt == 'add' || this.orderStatus == 'DRAFT') {
        this.bol1 = 1
    ...
    // 上传附件成功
    tableHandleUploadSuccess (file, row) {
    ...
    getSiteFormHistory () {
      ...
      //   method: 'POST',
      //   data: { companyName,  pageNum: 1, pageSize: 999 },
      //   loading: true
      // })
      //   .then(res => {
      //     let result = res.data.list || []
      //     this.allParams.siteFormHistoryList = result
      //   })
      this.queryParam = { vendorName: this.allParams.siteForm.vendorName }
      this.$nextTick(() => {
        this.$refs['siteFormHistoryList'].query()
      })
    },
    // 选择供应商回调
    getCompanyObj (val, data) {
      if (val) {
        ...
        this.getSiteFormHistory()
        if (this.allParams.siteForm.assessmentType && this.allParams.siteForm.assessmentType != 'ACCESS_ASSESSMENT') {
          this.getorgCateJournal(this.allParams.siteForm.vendorCode) // 根据供应商ID查询供应商档案组织与品类状态
        }
    ...
    getReviewFormObj (val, data) {
      ...
      if (val) {
        ...
        this.getorgCateJournalByFormId(val.reviewFormId, true) // 根据资质审查单Id查询组织和品类信息
    ...
    // 根据供应商ID查询供应商档案组织与品类状态
    getorgCateJournal (companyCode) {
      this.$http({
        url: '/api-sup/info/orgCategory/listPageOrgCategoryByParam',
        method: 'POST',
        data: { companyCode, companyStatus: 'QUALIFIED', serviceStatus: 'QUALIFIED', pageNum: 1, pageSize: 999 },
        loading: true
      })
        .then(res => {
          let result = res.data.list || []

          this.allOrgCateJournalsList = []
            result.forEach(item => {
              let orgCateObj = {
              ...
              this.allOrgCateJournalsList.push(orgCateObj)
            })
        })
        .catch(err => {
          console.log(err)
    ...
    handleCurrentChange (val) {
      ...
      this.allOrgCateJournalsList.forEach(item => {
        item.quoted = 'N'
      })
      val.forEach(selected => {
        let index = this.allOrgCateJournalsList.findIndex(item => {
          return `${selected.orgId}-${selected.categoryId}` == `${item.orgId}-${item.categoryId}`
        })
        if (index > -1) this.allOrgCateJournalsList[index].quoted = 'Y'
      })
    },
    rewiteOrgCateJournals () {
      let _this = this
      this.rowSelectFlag = true
      _this.$nextTick(() => {
        _this.allOrgCateJournalsList.forEach(item => {
          if (item.quoted == 'Y') {
            _this.$refs.orgCateJournals.toggleRowSelection(item)
          }
        })
        _this.rowSelectFlag = false
      })
    },
    // 根据资质审查单Id查询组织和品类信息
    getorgCateJournalByFormId (reviewFormId, isAdd = false) {
      this.$http({
        ...
        .then(res => {
          if (res.data) {
            ...
            if (isAdd) {
              // 需要将实体数组和品类数组打平
              this.allOrgCateJournalsList = []
              this.orgData.forEach(oItem => {
                this.catData.forEach(cItem => {
                  let orgCateObj = {
    ...
    assessmentTypeChange (val) {
      this.allParams.siteForm.vendorName = ''
      ...
      this.$http({
        ...
        .then(res => {
          ...
          if (res.data.siteForm) {
            ...
            if (this.orderStatus == 'WITHDRAW' || this.orderStatus == 'REJECTED' || this.orderStatus == 'APPROVED') {
              this.addressReadOnly = true
            }
            if (this.orderStatus == 'APPROVED') {
              ...
              if (reviewFormId) this.getorgCateJournalByFormId(reviewFormId, false) // 根据资质审查单Id查询组织和品类信息
            ...
            if (res.data.orgCateJournals) {
              this.allOrgCateJournalsList = res.data.orgCateJournals || []
              this.rewiteOrgCateJournals()
            }
            ...
            // if ((this.allParams.siteForm.approveStatus == 'PUBLISH' && this.allParams.siteForm.reviewResult) || this.allParams.siteForm.approveStatus == 'SUBMITTED' || this.allParams.siteForm.approveStatus == 'APPROVED' || this.allParams.siteForm.approveStatus == 'REJECTED' || this.allParams.siteForm.approveStatus == 'WITHDRAW') {
            //   // 切换到工作流tab页
    ...
    getOrgJournals () {
      let orgJournals = []
      if (!this.orgData.length) return orgJournals
      this.currentOrgCateRows.forEach(item => {
        let index = this.orgData.findIndex(o => o.orgId === item.orgId)
        let checkIsExist = orgJournals.findIndex(o => o.orgId === item.orgId)
        if (checkIsExist < 0) orgJournals.push(this.orgData[index])
      })
      return orgJournals
    },
    getCateJournals () {
      let cateJournals = []
      if (!this.catData.length) return cateJournals
      this.currentOrgCateRows.forEach(item => {
        let index = this.catData.findIndex(o => o.categoryId === item.categoryId)
        let checkIsExist = cateJournals.findIndex(o => o.categoryId === item.categoryId)
        if (checkIsExist < 0) cateJournals.push(this.catData[index])
      })
      return cateJournals
    },
            ...
            this.$refs['form'].validate((valid2, object2) => {
              ...
              if (bol) {
                this.__jump_error__(
                  'sceneAttachment',
                  null,
                  this.$t('bidMod.pleaseUploadFile')
                )
                return false
              }
    ...
    async saveDataHandle (type) {
      ...
      let orgBol = 1
        this.allOrgCateJournalsList.forEach((e) => {
          if (e.quoted == 'Y') {
            orgBol = 0
          }
        })
      if (orgBol) {
        this.$message.error('请选择认证组织和品类')
        return false
      }
      const bol1Save = this.bol1
      let urls = ''
      if (((bol1Save == 1 && this.allParams.siteForm.reviewResult == '') || !this.allParams.siteForm.approveStatus) && type == 'SAVE') {
        urls = '/api-sup/review/siteForm/saveOrUpdateSiteFormTempSaveByCreator'
      } else if ((bol1Save == 1 && this.allParams.siteForm.reviewResult == '') || !this.allParams.siteForm.approveStatus) {
        urls = '/api-sup/review/siteForm/saveOrUpdateSiteFormPublishByCreator'
      } else if (bol1Save == 2 || (this.allParams.siteForm.reviewResult != '' && type == 'SAVE') || (bol1Save == 3 && type == 'SAVE')) {
        urls = '/api-sup/review/siteForm/saveOrUpdateSiteFormForReviewer'
      } else if ((bol1Save == 3 && type != 'SAVE') || (bol1Save == 1 && this.allParams.siteForm.reviewResult != '')) {
        urls = '/api-sup/review/siteForm/saveOrUpdateSiteFormSubmitByCreator'
      } else {
        await this.handlerAfter(type)
        return false
      }
      console.log(bol1Save)
      this.allParams.orgJournals = this.getOrgJournals()
      ...
      this.allParams.cateJournals = this.getCateJournals()
      this.allParams.orgCateJournals = this.allOrgCateJournalsList
      if (this.allParams.orgCateJournals.length < 1) {
        this.__jump_error__(
          'orgCateJournals',
          null,
          this.$t('vendorMod.msgOrgCatInfo')
        )
        return false
      }
          ...
          .then(async res => {
            ...
            if (bol1Save == 3) {
              ...
              if (this.$attrs.params.flag === 'edit') {
                this.$emit('tab-remove', this.$attrs.params.tabName)
                this.$router.push('/vendorManagement/siteAssessment')
              } else {
                this.$emit('tab-remove', 'siteAssessmentDetail')
                this.$router.push('/vendorManagement/siteAssessment')
              }
              this.__setTabTodo('SiteAssessmentList.getQuerydata')
...
.app-main .the-siteAssessmentDetail-detail {
  overflow: auto;
  padding-right: 181px;
}
.info-fill-progress {
