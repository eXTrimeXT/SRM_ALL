  ...
  >
    <el-main>
      ...
      <table-view
        ...
        :source="$api.cmPerform.buyer.main.performOrder.vendorListPerOrder"
      />
    </el-main>
  </el-container>
</template>
...
import vendorContractPerformanceBillEdit from './edit.vue'
import ExportExcel from 'lib@/components/export-excel'

export default {
  name: 'VendorContractPerformanceBillList',
  components: {
    TableView,
    MainHeader,
    FormWrapper,
  ...
  data () {
    return {
      name: 'vendorContractPerformanceBillList',
      tableName: 'vendorContractPerformanceBillList',
      pageSize: 15,
      gridId: 'list',
      currentRows: [],
      tableHeader: [],
  ...
  created () {
    this.tableHeader = [
      ...
      },
      {
        ...
        label: this.$t('orderMod.deliveryNoteStatus'),
        width: 150,
        formattor: (val) => this.$getDictLabel('CONTRACT_STATE', val)
      },
      {
        prop: 'buName',
        label: this.$t('bid_mod.businessEntity'),
  ...
  methods: {
    ...
    viewHandle (row) {
      this.mode = 'view'
      const tab = {
        component: vendorContractPerformanceBillEdit,
        params: {
          row,
          flag: this.mode,
    ...
    addHandle (row) {
      ...
      const tab = {
        ...
        params: {
          row,
          flag: this.mode,
          tabName: 'vendorContractPerformanceBillEdit'
        },
    ...
    editHandle (row) {
      ...
      const tab = {
        ...
        params: {
          row,
          flag: this.mode,
          tabName: 'vendorContractPerformanceBillEdit' + row.perOrderId
        },
        title: `${this.$t('common.edit')} - ${row.perOrderNo}`,
        name: 'vendorContractPerformanceBillEdit' + row.perOrderId
      }
      this.$emit('tab-add', tab)
    },
    handOver (row) {
