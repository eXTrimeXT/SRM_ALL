          ...
          <div class="head-info">
            ...
            <div class="head-info-item flex-block">
              <div>
                ...
                <span class="ml20">{{ $t('sourcingBuyer.expirationTime') }}：{{ sourcingInfo.expirationTime }}</span>
                <span class="ml20">{{ $t('sourcingBuyer.creationDate1') }}：{{ sourcingInfo.creationDate }}</span>
              </div>
...
const dictClass = createDictClass(
  {
    condition: [
      { id: 1, value: 'gt', label: '大于' },
      { id: 2, value: 'eq', label: '等于' },
      { id: 3, value: 'lt', label: '小于' }
    ]
  },
  false,
)
  ...
  methods: {
    ...
    async submitOne (type) {
      ...
      if (data.length) {
        ...
        this.$message.error('只能对已报名且入围的供应商进行评分')
      }
    },
    backOne () {
