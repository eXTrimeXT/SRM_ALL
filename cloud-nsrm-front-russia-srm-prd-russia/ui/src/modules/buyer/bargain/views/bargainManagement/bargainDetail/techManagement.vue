  ...
  <div>
    <SrmRow class="the_biding_control_row" style="margin-top: 5px">
      ...
      <SrmCol :init-col="3">
        ...
        <el-input v-model="needToReviewCount" disabled />
      </SrmCol>

      <!-- 已完成评审的供应商 -->
      <SrmCol :init-col="3">
        <span class="label-title">{{ $t('bidMod.alreadyReviewCount') }}</span>
        <el-input v-model="alreadyReviewCount" disabled />
      </SrmCol>
    </SrmRow>
    ...
    <SrmRow class="the_biding_control_row">
      ...
      <SrmCol :init-col="3" style="line-height: 32px">
        <!-- 查询 -->
        <el-button
          type="primary"
          :disabled="readonly"
...
import { judgeManagement } from 'lib@/composition/bargainLts/utils'
...
export default {
  ...
  methods: {
    ...
    async getTechProgress () {
      if (!this.bargainBase.projectId) {
        return
      }
      const response = await brgBuyerHttp.tech.techProgress({
    ...
    /* 技术开标 */
    async openTech () {
      const confirmResult = await this.$confirm('确定发起技术开标吗？', {
      ...
      const response = await brgBuyerHttp.tech.openTech(this.bargainBase.projectId)
      if (response) {
