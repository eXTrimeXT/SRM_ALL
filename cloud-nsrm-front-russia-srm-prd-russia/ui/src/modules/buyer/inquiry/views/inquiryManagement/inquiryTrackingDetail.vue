    ...
    <el-main>
      <MainHeader>
        <template #right>
          <!--b 立即开始 报价未开始-->
          <el-button
            v-if="header.extProjectStatus === SOU_PROJECT_STATUS_ENUM.ORDER_NOT_START"
            type="primary"
            @click="startNow"
          >
            {{ $t('bidMod.startNow') }}
      ...
      <div class="the_progress form-container">
        <el-steps :active="activeNumber" finish-status="success">
          <el-step :title="$t('bidMod.inQstatus2')" :description="header.publishDate" />
          <el-step :title="$t('bidMod.inQstatus3')" :description="header.orderStartTime" />
          <el-step :title="$t('bidMod.inQstatus4')" :description="header.orderEndTime" />
      ...
      <el-collapse v-model="activeCollapse" class="tab-form-style">
        ...
        >
          <QuoteSelection
            ref="quoteSelection"
            :header="header"
            @update="getQuoteSelectionDetail"
...
export default {
  ...
  computed: {
    inquiryTrackingInfoRow () {
      return [
        // 询价标题
        { label: this.$t('bidMod.inquiryTitle'), value: this.header.souName },
        // 询价单号
        { label: this.$t('bidMod.inquiryNo'), value: this.header.souNo },
        // 询价单状态
        { label: this.$t('bidMod.inQstatus'), value: this.$getDictLabel('SOU_PROJECT_STATUS', this.header.extProjectStatus) },
        // 审核状态
        { label: this.$t('bidMod.auditStatus'), value: this.$getDictLabel('SOU_APPROVAL_STATUS', this.header.createApprovalStatus) },
        // 评分规则
        { label: this.$t('bidMod.inquiryRule'), value: this.$getDictLabel('SOU_SCORE_RULE_TYPE', this.header.scoreRuleType) },
        // 报价方式
        { label: this.$t(('bidMod.quoteRule')), value: this.$getDictLabel('SOU_ORDER_WAY', this.header.orderWay) }
      ]
    },
    ...
    activeNumber () {
      ...
      const STATUS_ENUM = {
        // 接受报价中
        [SOU_PROJECT_STATUS_ENUM.ACCEPT_ORDER]: 2,
        // 已截止报价
        [SOU_PROJECT_STATUS_ENUM.ORDER_END]: 3,
        // 评选中
        [SOU_PROJECT_STATUS_ENUM.EVALUATING]: 3,
        // 定价中
        [SOU_PROJECT_STATUS_ENUM.PRICING]: 3,
        // 已定价
        [SOU_PROJECT_STATUS_ENUM.PRICE_END]: 5
      }
    ...
    showQuoteEvaluation () {
      return [
        SOU_PROJECT_STATUS_ENUM.ORDER_END,
        SOU_PROJECT_STATUS_ENUM.EVALUATING,
        SOU_PROJECT_STATUS_ENUM.PRICING,
        SOU_PROJECT_STATUS_ENUM.PRICE_END,
        SOU_PROJECT_STATUS_ENUM.PRICE_REJECT
  ...
  methods: {
    ...
    refresh () {
      ...
      if (this.showQuoteEvaluation && this.$refs.quoteSelection) {
        this.$refs.quoteSelection.getQueryData({})
      }
    },
    ...
    async getQuoteSelectionDetail () {
      ...
      if (response) {
        ...
        this.header = header
        this.currentRoundTotalCtn = currentRoundTotalCtn
        this.currentRoundQuotedCtn = currentRoundQuotedCtn
        // 判断并标记是否存在相同IP的供应商
        this.trackingList = judgeListRepeatValueWarnTag(trackingList, 'submitByIp', 'ipWarn')
      }
    },
