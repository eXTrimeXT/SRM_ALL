...
export default {
  ...
  data () {
    return {
      ...
      searchFormConfig: [
        // 交流标题
        { prop: 'technicalExchangeTitle', label: '交流标题' },
        // 交流单号
  ...
  methods: {
    ...
    async deleteItem (row) {
      const confirm = await this.$confirm('确定删除该单据？', {
    ...
    async cancelItem (row) {
      const confirm = await this.$confirm('确定取消该单据？', {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      })
    ...
    async closureItem (row) {
      const confirm = await this.$confirm('请确认是否立即结束供应商反馈！', {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      })
