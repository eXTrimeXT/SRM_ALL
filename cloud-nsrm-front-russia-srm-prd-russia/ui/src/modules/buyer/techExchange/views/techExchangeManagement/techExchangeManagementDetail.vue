<template>
  ...
  >
    <el-tabs
      ...
      type="card"
    >
      <el-tab-pane label="交流信息" name="detail" />
      <el-tab-pane label="供应商反馈" name="feedback" />
    </el-tabs>
...
export default {
  ...
  data () {
    return {
      ...
      detailFormRules: {
        technicalExchangeTitle: [{ required: true, message: '请输入交流标题' }],
        orgOuId: [{ required: true, message: '请选择业务实体' }],
        technicalExchangeType: [{ required: true, message: '请选择交流类型' }],
        technicalExchangeStartTime: [{ required: true, message: '请选择预计开始时间' }],
        technicalExchangeEndTime: [
            ...
            validator: (rule, value, callback) => {
              if (value && this.detailFormData.technicalExchangeStartTime) {
                ...
                if (startDate.getTime() >= endDate.getTime()) {
                  callback(new Error('预计结束时间需要大于预计开始时间'))
                }
              }
              callback()
            },
  ...
  methods: {
    ...
    async saveOrSubmit (type) {
      ...
      if (vendorsData.length === 0) {
        this.$message.warning('请至少新增一项邀请供应商！')
        return
      } else {
