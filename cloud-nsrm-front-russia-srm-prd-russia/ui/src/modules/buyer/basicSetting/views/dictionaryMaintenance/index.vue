      ...
      preArr: [
        ...
        },
        {
          ...
          options: [
            ...
            { label: this.$t('common.en'), value: 'en_US' }, // '英文'
            { label: this.$t('common.ja'), value: 'ja_JP' } // '日文'
          ]
        }
      ],
      preArr2: [
  ...
  methods: {
    ...
    addOneDict () {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          this.form.activeDate = this.form.activeDate
            ? parseTime(this.form.activeDate, '{y}-{m}-{d}')
            : ''
          this.form.inactiveDate = this.form.inactiveDate
            ? parseTime(this.form.inactiveDate, '{y}-{m}-{d}')
            : ''
          this.$http({
    ...
    addOneDictItem () {
      this.$refs.form2.validate(valid => {
        if (valid) {
          this.form2.activeDate = this.form2.activeDate
            ? parseTime(this.form2.activeDate, '{y}-{m}-{d}')
            : ''
          this.form2.inactiveDate = this.form2.inactiveDate
            ? parseTime(this.form2.inactiveDate, '{y}-{m}-{d}')
            : ''
          this.$http({
    ...
    editTab (type) {
      if (type == 'add') {
        ...
        this.form.activeDate = parseTime(new Date(), '{y}-{m}-{d}')
      } else {
        // 修改
