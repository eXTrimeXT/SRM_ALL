...
export default {
  ...
  data () {
    return {
      ...
      queryParam: {},
      integrationMode: [
        { value: 'Product', label: '产品工作流集成模式' },
        { value: 'IdeFlow', label: 'IDE工作流集成模式' },
        { value: 'Iframe', label: 'iframe嵌入页面模式' },
        { value: 'Self', label: '自带页面模式' },
        { value: 'Push', label: '无页面推送模式' },
        { value: 'IdeSdk', label: 'IdeSdk推送模式' },
        { value: 'None', label: '无工作流模式' }
      ]
    }
  },
  created () {
    ...
    editTab (type, row) {
      ...
      if (type === 'add') {
        ...
        tab = {
          ...
          params: { flag: 'add' },
          title: () => this.$t('dataConfMod.addProcessTemplate'), // "新增流程模板"
          name: 'workflowInfo'
        }
      } else {
        // 修改
        tab = {
          ...
          title: () => this.$t('dataConfMod.editProcessTemplate'), // "编辑流程模板"
          name: 'workflowInfo' + row.templateHeadId
        }
      }
