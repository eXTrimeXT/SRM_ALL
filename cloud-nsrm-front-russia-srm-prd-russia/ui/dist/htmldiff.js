...
(function() {
  ...
  DiffBuilder.prototype = {
    build: function() {
      ...
      // return (
      //   '<br/>[耗时：' + (new Date() - sd) + '毫秒]' + this.content.join('')
      // )
    ...
    insert: function(opt, tagCls) {
      this.insertTag(
        "ins",
        tagCls || "diffins",
        this.newWords.slice(opt.startInNew, opt.endInNew)
      );
    },
    ...
    del: function(opt, tagCls) {
      this.insertTag(
        "del",
        tagCls || "diffdel",
        this.oldWords.slice(opt.startInOld, opt.endInOld)
      );
    },
    ...
    extractConsecutiveWords: function(words, isTag) {
      ...
      return words.splice(
        0,
        idxOfFirstTag !== null ? idxOfFirstTag : words.length
      );
    },

    warpText: function(text, tagName, cssCls) {
      return (
        "<" + tagName + ' class="' + cssCls + '">' + text + "</" + tagName + ">"
      );
    },

    isOpeningTag: function(item) {
      return /^\s*<[^>]+>\s*$/gi.test(item);
    },
