  ...
  >
    ...
    <template v-if="visible">
      ...
      >
          ...
          <el-select
            ...
            :disabled="
              (editable && !!scope.row.sourceId) ||
                (isFrameworkAgreement === 'N' &&
                (scope.row.partnerType === '乙方' ||
        ...
        <!-- payRatio -->
        <!-- <template slot-scope="scope" slot="payRatio">
          <el-input
        ...
        >
          <el-input
            v-model="payPlans[scope.$index].taxNumber"
            :disabled="(editable && !!scope.row.sourceId) || illegal == 'view'"
            @change="changeHandle"
          />
        </template>
...
export default {
      ...
      columns: [
        ...
        },
        {
          attrs: {
            ...
            showOverflowTooltip: true,
            minWidth: '100',
            label: this.$t('dataConfMod.taxNumber'), // 税号
            prop: 'taxNumber'
          },
          slot: 'taxNumber'
        }
  ...
  methods: {
    ...
    buHandler (node, value, scope) {
      scope.row.partnerName = node ? node.ceeaCompanyName : null
