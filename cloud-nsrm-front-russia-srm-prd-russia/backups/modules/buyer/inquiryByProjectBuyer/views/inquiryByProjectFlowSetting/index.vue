      ...
      >
        ...
        >
          <el-table-column
            ...
            :formatter=" (row, column, value) => $getDictLabel('BARGAIN_SCOPE', value)"
          />
        </el-table-column>
        ...
        >
          <el-table-column
            ...
            width="85"
            show-overflow-tooltip
            :formatter="(row, column, cellValue) => $getDictLabel('BARGAIN_TYPE', cellValue)"
          />
        </el-table-column>
        ...
        >
          <el-table-column
            ...
            width="85"
            show-overflow-tooltip
            :formatter="(row, column, cellValue) => $getDictLabel('BRG_EVALUATE_METHOD', cellValue)"
          />
        </el-table-column>
...
<script>
import BargainFlowSettingDialog from './settingDialog'
...
export default {
  ...
  methods: {
    getQueryData (v) {
      ...
      this.$api.brg.inquiryByProject.processConfigList(filterParam).then(data => {
        if (data && data.data) {
    ...
    getBrgProcessNameList () {
      this.$api.brg.inquiryByProject.processConfigList({}).then(data => {
        if (data.data) {
          this.brgProcessNameList = []
          data.data.list.map(v => {
            this.brgProcessNameList.push({
    ...
    delOne (row) {
      this.$confirm(this.$t('common.delRow'), {
        ...
        this.$api.brg.inquiryByProject.processConfigDelete({ id: row.processConfigId }).then(() => {
          this.$message({
    ...
    validRow (row) {
      ...
      this.$api.brg.inquiryByProject.processConfigValidOrInvalid({}, row.processConfigId, validFlag).then(() => {
        this.$message({
