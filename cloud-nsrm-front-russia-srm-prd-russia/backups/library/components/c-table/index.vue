      ...
      <template v-slot="scope">
        <div :style="{ paddingLeft: `${scope.row._store.level * 16}px` }">
          ...
          >
            <i
              :class="
                scope.row._store.expanded ? 'el-icon-minus' : 'el-icon-plus'
              "
            />
          </span>
...
function updateVirtualContent (virtualStore, scrollTop, vnode) {
  ...
  if (
    startIndex === virtualStore.startIndex &&
    endIndex === virtualStore.endIndex
  ) {
    return
  }
    ...
    virtualBox: {
      inserted (el, binding, vnode) {
        ...
        vnode.context.$nextTick(() => {
          const elFixedBodyWrappers = el.querySelectorAll(
      ...
      componentUpdated (el, binding, vnode, oldVnode) {
        ...
        // 显示数量
        const showCount =
          Math.ceil(bodyHeight / unitHeight) + reserveTop + reserveBottom
        store.showCount = showCount
        store.maxDataLength = maxDataLength
        store.maxScrollTop =
          virtualWrapperHeight -
          bodyHeight -
          (reserveBottom + reserveTop) * unitHeight
        ...
        if (Array.isArray(data)) {
          data.forEach((dataItem, index) => {
            ...
            if (parentItem) {
              ...
              _store.parentList = [_store.parentId].concat(
                parentItem._store.parentList || []
              )
