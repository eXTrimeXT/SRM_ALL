        ...
        {...cancelButton, text: $t('bidMod.backTo')}
            ...
            // updateButtonConfig($form)
                      ...
                      field.visible = !$form.readPretty
                            ...
                            $values.auditStatus === 'APPROVED' &&
                            $table.getRowByIndex($self.index).applyStatus !== 'RETURNING'
                          }}`},"x-render-table-column":{width:150,customRender:!0,title:i18nExpression("purchaseDemand.requirementDate"),format:"yyyy-MM-dd","value-format":"yyyy-MM-dd","picker-options":{disabledDate(time){const today=new Date;return today.setHours(0),today.setMinutes(0),today.setSeconds(0),today.setMilliseconds(0),time.getTime()<today.getTime()}}},...feedbackLayoutIsPopover,"x-validator":{required:!0,message:i18nExpression("purchaseDemand.selectRequireDate")}},receiveAddress:{type:"string","x-component":"DictSelect","x-component-props":{code:"{{String($values.organizationId)}}",disabled:`{{
                            isReadOnly ||
                            $values.auditStatus === 'APPROVED' ||
                            $table.getRowByIndex($self.index).applyStatus === 'RETURNING'
