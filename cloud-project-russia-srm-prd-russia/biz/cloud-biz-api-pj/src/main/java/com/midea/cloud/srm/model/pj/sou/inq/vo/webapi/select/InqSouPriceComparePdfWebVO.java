...
import com.midea.cloud.srm.model.pj.sou.inq.vo.webapi.order.InqSouOrderItemWebVO;
 ...
 */
@Data
public class InqSouPriceComparePdfWebVO {
     ...
     * 转换方法
     */
    public static List<InqSouPriceComparePdfWebVO> convert(List<ApiInqSouOrderItemVO> orderItemList, List<SouVendor> vendorList,
        ...
        List<InqSouPriceComparePdfWebVO> voList = new ArrayList<>(orderItemList.size());
        InqSouPriceComparePdfWebVO vo;
        for (ApiInqSouOrderItemVO orderItem : orderItemList) {
            vo = new InqSouPriceComparePdfWebVO();
            ...
            if (StringUtils.isNotBlank(orderItem.getFormulaAttrValues())) {
                Map<Long/* factorId */, String/* price */> map = InqSouOrderItemWebVO.deserializationUserPrices(orderItem.getFormulaAttrValues());
     ...
     * 导出Excel
     */
    public static void exportPdf(HttpServletResponse response, String fileName, List<InqSouPriceComparePdfWebVO> voList) {
        try {
    ...
    public static void buildPdf(Document document, List<InqSouPriceComparePdfWebVO> voList) throws DocumentException, IOException {
        ...
        /* 增加表信息 */
        Map<String/* itemCode或者itemName(无编码物料) */, List<InqSouPriceComparePdfWebVO>> voMap = new HashMap<>(16);
        voList.forEach(vo -> {
        ...
        for (Map.Entry<String, List<InqSouPriceComparePdfWebVO>> entry : voMap.entrySet()) {
    ...
    private static void buildTable(Document document, Font font, int totalColumns, String itemCode,
                                   List<InqSouPriceComparePdfWebVO> voList) throws DocumentException {
        /* 1. 构建表格对象 */
        ...
        /* 3. 添加实际数据 */
        int index = -1;
        for (InqSouPriceComparePdfWebVO vo : voList) {
            index++;
            /* 列：物料描述(合并行) */
            if (index == 0) {
