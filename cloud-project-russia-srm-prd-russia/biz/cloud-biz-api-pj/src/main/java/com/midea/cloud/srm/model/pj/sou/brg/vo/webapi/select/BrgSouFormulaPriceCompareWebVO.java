 ...
 */
@Data
public class BrgSouFormulaPriceCompareWebVO {
     ...
     */
    public static BrgSouFormulaPriceCompareWebVO convert(Integer searchRound,
        ...
        BrgSouFormulaPriceCompareWebVO vo = new BrgSouFormulaPriceCompareWebVO();
        // 轮次
        ...
        for (SouItem souItem : itemList) {
            ...
            for (SouVendor vendor : vendorList) {
                ...
                if (orderItem != null) {
                    BrgSouOrderItem brgOrderItem = brgOrderItemMap.get(orderItem.getOrderItemId());
        ...
        extracted(itemList, formulaLineMap, vo);

        return vo;
    }

    private static void extracted(List<ApiBrgSouItemVO> itemList, Map<Long, List<PricingFormulaLine>> formulaLineMap, BrgSouFormulaPriceCompareWebVO vo) {
        // 物料价格公式明细
        vo.setOrderItemFormulaMap(new HashMap<>(itemList.size() << 3));
        for (ApiBrgSouItemVO item : itemList) {
            Set<Long> factorIds = new HashSet<>(32);
            List<PricingFormulaLine> formulaLineList = formulaLineMap.get(item.getFormulaId());
            ...
            for (PricingFormulaLine formulaLine : formulaLineList) {
                ...
                // 公式值
                formulaInfo.formulaValue = item.getFormulaValue();
            }
        }
    }
