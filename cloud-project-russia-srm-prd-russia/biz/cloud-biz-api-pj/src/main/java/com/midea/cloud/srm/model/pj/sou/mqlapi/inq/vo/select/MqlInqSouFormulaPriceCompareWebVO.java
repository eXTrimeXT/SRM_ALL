 ...
 */
@Data
public class MqlInqSouFormulaPriceCompareWebVO {

    @ApiModelProperty("轮次")
    private Integer round;
    @SuppressWarnings("AlibabaPojoNoDefaultValue")
    @ApiModelProperty("物料需求信息")
    private List<MqlInqSouItemVO> itemList = new ArrayList<>();
...
/**    souItem */
     ...
     */
    public static MqlInqSouFormulaPriceCompareWebVO convert(
            Integer searchRound,
        ...
        Set<Long> quotePriceFactorIds = new HashSet<>(formulaFactorMap.size() << 3);
        {
            for (List<EssentialFactor> factors : formulaFactorMap.values()) {
                for (EssentialFactor factor : factors) {
                    String supplierQuotedPriceText = "SUPPLIER_QUOTED_PRICE";
                    if (factor.getEssentialFactorFrom().equals(supplierQuotedPriceText)) {
        ...
        MqlInqSouFormulaPriceCompareWebVO vo = new MqlInqSouFormulaPriceCompareWebVO();
        // 轮次
        vo.round = searchRound;

        // 物料需求信息
        List<MqlInqSouItemVO> inqSouItemVOList = new ArrayList<>();
        if (CollectionUtils.isNotEmpty(souItemList)) {
            for (SouItem souItem : souItemList) {
                MqlInqSouItemVO inqSouItemVO = new MqlInqSouItemVO();
                BeanUtils.copyProperties(souItem, inqSouItemVO);
                inqSouItemVO.getInqSouItem().setNotaxTargetPrice(inqSouItemMap.get(inqSouItemVO.getSouItemId()).getNotaxTargetPrice());
        ...
        for (SouItem item : souItemList) {
            ...
            for (SouVendor vendor : vendorList) {
                ...
                orders.add(order);
                // 物料需求行ID
                order.orderItemId = item.getSouItemId();
                // 供应商ID
