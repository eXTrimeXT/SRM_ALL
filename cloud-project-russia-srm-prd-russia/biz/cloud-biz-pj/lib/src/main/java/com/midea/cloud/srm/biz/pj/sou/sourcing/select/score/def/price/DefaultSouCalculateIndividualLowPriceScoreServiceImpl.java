...
/**
 * 寻源核心 - 价格维度算分[单项 + 合理低价法]
 *
 * @author zhangwk12@midea.com
 * @since 2022/09/12
 */
@Service
public class DefaultSouCalculateIndividualLowPriceScoreServiceImpl implements SouCalculateScoreDimensionService, ISouCalculatePriceScoreService {
    ...
    public void calculateAndSet(long projectId, List<? extends SouScoreDimensionContextData> params, BigDecimal dimensionWeight) {
        /* 计算每个物料的最低供应商报价 */
        Map<Long/* souItemId */, BigDecimal/* 最低报价 */> minPriceMap = getIndividualMinPriceMap(params);

        params.forEach(data -> {
            BigDecimal priceScore = calculateLowPriceScore(data.getPrice(), minPriceMap.get(data.getSouItemId()));
            /* 按维度权重进行换算 */
...
//        合理低价
                && type.getScoreRuleType().equals(SouScoreRuleTypeEnum.MIN_PRICE);
    }
