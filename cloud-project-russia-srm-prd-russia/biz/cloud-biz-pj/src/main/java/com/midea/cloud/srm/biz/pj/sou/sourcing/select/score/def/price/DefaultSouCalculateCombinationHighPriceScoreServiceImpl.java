...
/**
 * 寻源核心 - 价格维度算分[组合 + 合理高价法]
 *
 * @author zhangwk12@midea.com
 * @since 2022/09/12
 */
@Service
public class DefaultSouCalculateCombinationHighPriceScoreServiceImpl implements SouCalculateScoreDimensionService, ISouCalculatePriceScoreService {
    ...
    public void calculateAndSet(long projectId, List<? extends SouScoreDimensionContextData> params, BigDecimal dimensionWeight) {
        ...
        /* 2. 收集每个组合的最高报价 */
        Map<String/* itemGroup */, BigDecimal> groupMaxPriceMap = getCombinationMaxPriceMap(params);
        /* 3. 计算价格得分 */
        params.forEach(data -> {
            /* 当前供应商对组合的总报价 */
            BigDecimal p = vendorGroupPriceMap.get(data.getVendorTab() + "_" + data.getItemGroup());
            /* 同组合不同供应商中的最高报价 */
            BigDecimal pMax = groupMaxPriceMap.get(data.getItemGroup());
            /* 计算得到价格得分 */
            BigDecimal priceScore = calculateHighPriceScore(p, pMax);
            /* 按维度权重进行换算 */
