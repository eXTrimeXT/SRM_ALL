...
values
    ((select max(ii.DICT_ITEM_ID) + 1 from scc_base_dict_item ii),
     'EXT_PR_REQ_FIX_PRICE_STATUS', '长城需求池定价状态', '长城需求池定价状态', 'zh_CN', '中文',
...
values
    ((select max(ii.DICT_ITEM_ID) + 1 from scc_base_dict_item ii),
     (select DICT_ID from scc_base_dict where DICT_CODE = 'EXT_PR_REQ_FIX_PRICE_STATUS' and LANGUAGE = 'zh_CN'),
    'DRAFT',      '未定价',          'zh_CN',       '中文',
