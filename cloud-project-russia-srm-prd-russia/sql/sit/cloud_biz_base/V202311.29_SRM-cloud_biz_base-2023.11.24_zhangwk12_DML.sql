...
values
    ((select max(ii.DICT_ITEM_ID) + 1 from scc_base_dict_item ii),
     'EXT_FIX_PRICE_STATUS', '长城定价单据状态', '长城定价单据状态', 'zh_CN', '中文',
...
values
    ((select max(ii.DICT_ITEM_ID) + 1 from scc_base_dict_item ii),
     (select DICT_ID from scc_base_dict where DICT_CODE = 'EXT_FIX_PRICE_STATUS' and LANGUAGE = 'zh_CN'),
    'DRAFT',      '拟定',          'zh_CN',       '中文',
