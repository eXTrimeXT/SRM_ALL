delete from scc_base_dict_item where DICT_ID in (select DICT_ID from scc_base_dict where DICT_CODE = 'PR_SOU_REQUIREMENT_FROM') and DICT_ITEM_CODE = 'YEAR';
...
values
    ((select max(ii.DICT_ITEM_ID) + 1 from scc_base_dict_item ii),
     'PR_SOU_REQUIREMENT_NO_PUBLIC', '招标计划不公示原因选择', '招标计划不公示原因选择', 'zh_CN', '中文',
...
values
    ((select max(ii.DICT_ITEM_ID) + 1 from scc_base_dict_item ii),
     (select DICT_ID from scc_base_dict where DICT_CODE = 'PR_SOU_REQUIREMENT_NO_PUBLIC' and LANGUAGE = 'zh_CN'),
    'GROUP_STRATEGY',      '项目集团战略级别保密',          'zh_CN',       '中文',
            current_date, '1',       'admin',    now(),         '127.0.0.1',
    '1',             'admin',         '127.0.0.1');
-- 3 ：限定供应商范围
insert into scc_base_dict_item
