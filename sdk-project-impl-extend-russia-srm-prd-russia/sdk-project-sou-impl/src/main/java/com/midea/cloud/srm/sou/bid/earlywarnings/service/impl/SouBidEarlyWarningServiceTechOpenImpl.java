...
@Slf4j
@Service
public class SouBidEarlyWarningServiceTechOpenImpl extends SouBidEarlyWarningServiceCommonImpl implements SouBidEarlyWarningService {
    @Autowired
    ...
    public String doWarning() {
        log.info("SouBidEarlyWarningService 预警技术标收标开始");
        Map<String, Object> params = super.getQueryParams();
        String techOpenTime = super.threeDayBefore();
        params.put("techOpenTime", techOpenTime);
        ...
        if(CollectionUtils.isNotEmpty(dataList)) {
            ...
            dataList.stream().forEach(data -> {
                ...
                Long diffPlan = diffDays(data.getTechOpenTime(), data.getPublishTime());
                ...
                var.put("${processName}", PROCESS_DATA_TECH_OPEN);
        ...
        log.info("SouBidEarlyWarningService 预警技术标收标结束");
