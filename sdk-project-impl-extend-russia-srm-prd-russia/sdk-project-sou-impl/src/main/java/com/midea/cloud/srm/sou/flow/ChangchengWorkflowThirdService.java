package com.midea.cloud.srm.sou.flow;
...
public class ChangchengWorkflowThirdService implements WorkflowThirdService {
    ...
    public FlowResponseDTO startThird(FlowCallbackDTO flowCallbackDTO) throws Exception {
        ...
        // 查询流程配置是否启用，没启用的直接返回一个InstanceId
         ...
         BpmResultDTO<BpmCreateResult> resultDTO = null;
         FlowResponseDTO flowResponseDTO = new FlowResponseDTO();
          if(isOpen){
              DictItem dictItem = baseExtClient.getDictItem("BPM2_ZBUSINESS_TYPE",businessType);
              BpmNewFlag bpmNewFlag = pjSouClient.getBpmNewFlag(new BpmNewFlag().setBusinessId(flowCallbackDTO.getBusinessId()).setBussinessType(businessType));
              if(dictItem!=null&&StringUtils.equals(dictItem.getItemDescription(), YesOrNo.YES.getValue())
                  ...
                  resultDTO = pjSouClient.startProcessByCategoty(flowCallbackDTO.getBusinessId(),flowCallbackDTO.getBusinessType(),bpmParam);
              }else{
                  resultDTO = pjSouClient.createProcessByCategory(JSONObject.parseObject(flowCallbackDTO.getFlowParam())
