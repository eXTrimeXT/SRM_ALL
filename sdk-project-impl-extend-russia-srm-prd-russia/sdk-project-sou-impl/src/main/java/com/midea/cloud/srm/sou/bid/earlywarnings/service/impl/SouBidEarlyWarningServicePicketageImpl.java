...
@Slf4j
@Service
public class SouBidEarlyWarningServicePicketageImpl extends SouBidEarlyWarningServiceCommonImpl implements SouBidEarlyWarningService {
    ...
    public String doWarning() {
        log.info("SouBidEarlyWarningService 预警定标开始");
        Map<String, Object> params = super.getQueryParams();
        String picketageTime = super.threeDayBefore();
        params.put("picketageTime", picketageTime);
        ...
        if(CollectionUtils.isNotEmpty(dataList)) {
            ...
            dataList.stream().forEach(data -> {
                ...
                Long diffActual = diffDays(new Date(), actual.getSumReportTime());
                Long diffPlan = diffDays(data.getPicketageTime(), data.getSumReportTime());
                ...
                var.put("${processName}", PROCESS_DATA_PICKETAGE);
        ...
        log.info("SouBidEarlyWarningService 预警定标结束");
