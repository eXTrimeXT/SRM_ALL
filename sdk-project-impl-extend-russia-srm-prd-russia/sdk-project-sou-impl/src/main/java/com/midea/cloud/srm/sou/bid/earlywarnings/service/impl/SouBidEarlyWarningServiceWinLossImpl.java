...
@Slf4j
@Service
public class SouBidEarlyWarningServiceWinLossImpl extends SouBidEarlyWarningServiceCommonImpl implements SouBidEarlyWarningService {
    @Autowired
    ...
    public String doWarning() {
        log.info("SouBidEarlyWarningService 预警中落标通知开始");
        Map<String, Object> params = super.getQueryParams();
        String winLossTime = super.threeDayBefore();
        params.put("winLossTime", winLossTime);
        ...
        if(CollectionUtils.isNotEmpty(dataList)) {
            ...
            dataList.stream().forEach(data -> {
                ...
                Long diffActual = diffDays(new Date(), actual.getPicketageTime());
                Long diffPlan = diffDays(data.getPublishWinLossTime(), data.getPicketageTime());
                ...
                var.put("${processName}", PROCESS_DATA_WIN_LOSS);
        ...
        log.info("SouBidEarlyWarningService 预警中落标通知结束");
