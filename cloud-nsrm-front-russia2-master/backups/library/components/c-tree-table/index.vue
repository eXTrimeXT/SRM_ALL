<template>
  ...
  >
    ...
    <el-table-column
      ...
      :label="showOrderNumber ? $t('common.sort') : ''"
    >
      <template slot-scope="scope">
        <div :style="{ paddingLeft: `${scope.row._store._level * indent}px` }">
          ...
          >
            <i
              :class="
                scope.row._store._expanded ? 'el-icon-minus' : 'el-icon-plus'
              "
            />
          </span>
          <span
...
export default {
  ...
  methods: {
    ...
    dataTransform (data, parentId) {
      ...
      if (Array.isArray(data)) {
        data.forEach((dataItem, index) => {
          ...
          treeDataItem._store._level =
            parent && parent._store ? parent._store._level + 1 : 0
          treeDataItem._store._hasChild = false
          treeDataItem._store._expanded = this.map[uid]
            ? this.map[uid]._store._expanded
            : this.expandAll
          ...
          if (
            treeDataItem[this.defaultProp.children] &&
            treeDataItem[this.defaultProp.children].length > 0
          ) {
            treeDataItem._store._hasChild = true
            const children = this.dataTransform(
              treeDataItem[this.defaultProp.children],
              uid
            )
    ...
    handleRowClassName ({ row, rowIndex }) {
      ...
      const show = _parent
        ? _parent._store._expanded && _parent._store._show
        : true
    ...
    getChildOrderNumber (parent, index) {
      return `${parent._store._orderNumber}${this.orderNumberSplit}${index +
        1}`
    },
    // 插入子节点
    ...
    insertBefore (insertData, row) {
      if (row) {
        ...
        if (_parent) {
          ...
          _parent._store._source[this.defaultProp.children].splice(
            _index,
            0,
            insertData
          )
    ...
    delete (row) {
      if (row) {
        ...
        if (_parent) {
          deleteData =
            _parent._store._source[this.defaultProp.children][_index]
