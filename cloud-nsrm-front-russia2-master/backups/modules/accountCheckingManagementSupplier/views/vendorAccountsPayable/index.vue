<template>
  <el-container
    class="flex-container-notab the_vendorAccountsPayable_wrapper"
    direction="vertical"
  >
    <el-main>
      <form-wrapper
...
export default {
  ...
  mounted () {
    ...
    // 字典信息查询
    const dictionaryCodes = [{ dictCode: 'ACCOUNT_PAYABLE_BILL_TYPE' }]
    getDictItemList(dictionaryCodes).then(res => {
      const [ACCOUNT_PAYABLE_BILL_TYPE] = res.data
      this.accountPayableBillType = adaptDictData(
        ACCOUNT_PAYABLE_BILL_TYPE.ACCOUNT_PAYABLE_BILL_TYPE
      )
    })
  },
  methods: {
...
.the_vendorAccountsPayable_wrapper {
  padding: 0 10px;
  .order-uploader {
