    ...
    <template v-for="(col, key) in innerHeader">
      ...
      >
        <template slot="header">
          <i class="toRequired" v-if="col.vendorNotEmptyFlag ==='Y'">*</i>{{ col.fieldName }}
        </template>
        <template slot-scope="scope">
          ...
          <template v-else-if="col.fieldCode === 'fromCounty'">
            <el-select
              ...
              @focus="(val) => { countyFocusHandle(val,scope.row) }"
            >
              <el-option
          ...
          <template v-else-if="col.fieldCode === 'toCounty'">
            <el-select
              ...
              @focus="(val) => { toCountyFocusHandle(val,scope.row) }"
            >
              <el-option
    ...
    methods: {
      ...
      changeLeg (val, row) {
        this.$http({
          ...
          this.requirementLineList.splice(this.requirementLineList.length - 1, 1)
        })
      },
      ...
      countyFocusHandle (val, row) {
        ...
        let parame = { regionLevelCode: '4.0', parentRegionCode: row.fromCityCode }
