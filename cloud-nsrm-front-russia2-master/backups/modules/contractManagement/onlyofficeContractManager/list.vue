...
export default {
  ...
  data () {
    return {
      ...
      preArr: [
        { prop: 'contractCode', label: this.$t('contractMod.contractNo') },
...
type: 'dict',
  code: 'ELEM_CONTRACT_TYPE' },
        { prop: 'contractNo', label: this.$t('contractMod.contractNo_1') },
        ...
        {
          prop: 'ceeaIfVirtual',
          label: this.$t('contractMod.ifVirtual'),
          type: 'select',
          options: [
      ...
      handler () {
        if (
          ...
          this.preFormObj = Object.assign(
            {},
            { contractStatus: this.$route.params.contractStatus }
          )
        } else if (
...
code: 'ELEM_CONTRACT_TYPE'
      ...
      {
        prop: 'mainContractNo',
        label: this.$t('contractMod.oldContractNo'),
        width: 150
      },
      {
        prop: 'modelName',
...
width: 100
      ...
      },
      {
        ...
        buttons: [
          ...
          //   show: row =>
          //     this.curRole === "BUYER" && row.contractStatus === "ARCHIVED",
          //   formattor: () => "发起付款申请"
          // },
          {
            callback: function (row) {
            ...
            show: function (row) {
              if (
                ...
                (_this.curRole === 'VENDOR' &&
                  row.contractStatus === 'SUPPLIER_CONFIRMING')
              ) {
                return true
              } else {
                return false
          ...
          },
          {
            ...
            formattor (val) {
              return _this.$t('common.close')
            },
            show: function (row) {
              if (
                _this.curRole === 'BUYER' &&
                ['ARCHIVED', 'WITHDRAW'].includes(row.contractStatus)
              ) {
                // 未发布 | 供应商确认
                return true
              } else {
          ...
          },
          {
            ...
            formattor (val) {
              return _this.$t('bidMod.approvalPass')
            },
            code: 'cm:contractManager:approvalOne',
            show: function (row) {
              if (
                _this.curRole === 'BUYER' &&
                row.contractStatus === 'UNDER_REVIEW'
              ) {
                return true
              } else {
                return false
          ...
          },
          {
            ...
            show: function (row) {
              if (
                _this.curRole === 'BUYER' &&
                row.contractStatus === 'SUPPLIER_CONFIRMED' &&
                row.createdBy === _this.globalNickname
              ) {
                return true
              } else {
  ...
  methods: {
    ...
    close (row) {
      ...
      // this.$confirm("当前操将关闭这个合同，确认关闭吗？", {
      //   confirmButtonText: this.$t("common.confirm"),
      //   cancelButtonText: this.$t("common.cancel"),
      //   type: "warning"
      // })
      //   .then(() => {
      //     this.$api.cm.buyer.main.contract
      //       .close(row.contractHeadId)
      //       .then(data => {
      ...
      if (this.contractCloseForm.fileuploadId) {
        ...
        this.$api.cm.buyer.main.contract
          ...
          .then(data => {
            ...
            // 更新状态
            // this.$api.cm.buyer.main.contract
            //   .close(this.contractCloseForm.contractHeadId)
            //   .then(data => {
            //     this.$message.success("关闭成功");
            //     this.getQuerydata();
            //     this.closeDialogVisible = false;
    ...
    exportOne () {
      ...
      // console.log("contractHeadIdArray",contractHeadIdArray)
    },
    addOne (contractType) {
      let rowId = null
      ...
      if (contractType !== 'MIAN_CONTRACT_ADD') {
        ...
        if (!this.currentRows.length) {
          this.$message.warning(
            this.$t('contractMod.msgContractManage[0]') + `${name}`
          ) // 请选择要以哪个合同为基础创建
          return
        }
        ...
        if (this.currentRows[0].contractStatus !== 'ARCHIVED') {
          this.$message.warning(
            this.$t('contractMod.msgContractManage[3]') + `${name}`
          ) // 只能已归档的合同才能创建
          return
        }
        ...
        console.log(rowId, contractOldCode)
      }
      this.$emit('tab-add', {
    ...
    launchPaymentRequest (row) {
      this.$api.cm.buyer.main.contract.getInfoById(row.contractHeadId).then(res => {
