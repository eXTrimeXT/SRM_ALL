    ...
    getExpiredDocuments () {
      this.$http({
        ...
        if (res.data && res.data?.list?.length) {
          ...
          let sign = filterList[0].authNum.split('_')
          let dictCode = `CERTIFICATE_TYPE_${sign[0]}`
          getDictItem(dictCode).then(res => {
            const labelList = filterList.map(item => {
              let obj = res.data.find(i => i.dictItemCode === item.authNum)
