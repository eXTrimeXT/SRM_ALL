            ...
            <template slot-scope="scope">
              <template v-if="scope.row['___editting'] && (col.editType === 'update' || (col.editType === 'add' && scope.row.___add))">
                ...
                <el-date-picker
                  ...
                  placeholder=""
                  :value-format="col.valueFormat"
                  :format="col.format || $formatDatePicker"
                  @change="scope.row['___editted'] = true;callback(col, scope.row)"
                />
                <el-date-picker
                  ...
                  placeholder=""
                  :value-format="col.valueFormat"
                  :format="col.format || $formatDatePickerTime"
                  @change="scope.row['___editted'] = true;callback(col, scope.row)"
                />
                <el-switch
...
<script>
  ...
  export default {
    name: 'TableEdit',
    components: { FilterColumn, PagerBar },
    props: {
      pageStep: { // 页数递增步数
        type: Array,
        default: function () {
    ...
    methods: {
      ...
      pageQuery (opr, size) {
        let queryParam = {}
        queryParam.__page = this.viewIndex
        queryParam.__pagesize = this.viewSize
        if (opr === '') {
          this.viewSize = size
        ...
        let _this = this
        _this.loading = true
        let paramsExt = Object.assign({ 'pageNum': queryParam.__page, 'pageSize': queryParam.__pagesize }, this.preQueryData,)
        this.$http({
          url: this.url,
          method: 'post',
          data: paramsExt
      ...
      tbDoLayout () {
        this.$nextTick(() => {
          this.$refs.tableEditGrid.doLayout()
        })
      },
      callback (col, row) {
        if (col.callback) {
      ...
      currentChange (row, old) {
        if (row == null) {
          this.$refs.tableEditGrid.setCurrentRow(old)
      ...
      add (obj) {
        ...
        this.$refs.tableEditGrid.setCurrentRow(this.tableData[0])
      },
      makeNewRow () {
        let tmp = {}
      ...
      checkChange (selection) {
        ...
        this.$emit('check-change', selection)
      },
      setData (data) {
        if (data != null) {
          let index = 0
