        ...
        <div class="searchBar">
          <div class="searchInner">
            <el-input
              ...
              :placeholder="$t('chatMod.placeholderTip')"
              ...
              >
                <div>
                  ...
                  <div class="imUuser">
                    ...
                    <span class="lastTime">{{ $parseTime(chatgroup.lastChattime) }}</span>
                  </div>
                  <div class="imCompany">
        ...
        <ul id="imUserUl">
          ...
          >
            <div>
              ...
              <div class="imUuser">
                <span class="userNameText">{{ $t('chatMod.xiaoM2') }}</span>
                <span class="lineState online">{{ $t('chatMod.online') }}</span>
                <span class="lastTime">20:01</span>
              </div>
              <div class="imCompany">
                {{ $t('chatMod.imCompany')}}<span class="unRead">5</span>
              </div>
            </div>
          </li>
        </ul>
      ...
      <div id="chartDailog">
              ...
              >
                ...
                >
                  <div v-if="chatdata.createdId === userIdCurrent">
                    <span>{{ $t('chatMod.youWithdraw') }}</span>
                  </div>
                  <div v-else>
                    {{ chatdata.createdUserBy + $t('chatMod.withdraw') }}
                  </div>
                </div>
                <div
                  v-else
        ...
        >
          ...
          >
            ...
            <span class="nameSpan">{{ $t('chatMod.addMembers') }}</span>
          </div>
          <!-- 采购商可以看见所有供应商 -->
          <div v-if="userType==='BUYER'">
            <div class="groupChatUserTit">
              {{ $t('quota.vendor') }}({{ chatgroupMembersCountVendor }})
              <span
            ...
            <ul>
              ...
              >
                ...
                >{{ $t('dataConfMod.remove') }}</span>
              </li>
            </ul>
          </div>
          <div>
            <div class="groupChatUserTit">
              {{ $t('common.buyer') }}({{ chatgroupMembersCountBuyer }})
              <span
  ...
  data () {
    return {
      ...
      hisMsgArr: [
        {
          userName: 'dsdsd',
          msgContent: this.$t('chatMod.receive'),
          type: 'ChatDirectionTypeReceive'
        },
        {
          userName: 'ttggg',
          msgContent: this.$t('mailManagement.send'),
  ...
  methods: {
    ...
    sendMsgHandle () {
      ...
      if (this.messageCurrent == null || this.messageCurrent == '') {
        // 提示，请输入发送内容
        this.$message.error(this.$t('chatMod.enterSendContent'))
        return
      }
      ...
      if (this.megType === 'reply') {
        ...
        if (chatdata.chatdataStates.length == 0) {
          ...
          this.$message.error(this.$t('chatMod.selectMember'))
          return
        }
    ...
    toGroupChatHandel (type) {
      ...
      if (type === 'addMembers') {
        ...
        if (this.chatgroupCurrent == null || this.chatgroupCurrent.chatgroupId == null) {
          this.$message.error(this.$t('chatMod.selectChatGroup'))
          return
        }
   ...
   // 搜索用户，只用于搜索聊天标题
    ...
    removeUserFromGroup (userInfo, index, type) {
      ...
      if (userInfo.userId == this.userIdCurrent) {
        this.$message.error(this.$t('chatMod.removeTip'))
      } else {
        this.$confirm(this.$t('chatMod.confirmRemove'), {
    ...
    withdrowHandel (data) {
      let _this = this
      // 确定是否撤回？
      this.$confirm(this.$t('chatMod.confirmWithdraw'), {
          confirmButtonText: this.$t('common.affirm'),
          cancelButtonText: this.$t('common.cancel'),
          type: 'warning'
        }).then(() => {
