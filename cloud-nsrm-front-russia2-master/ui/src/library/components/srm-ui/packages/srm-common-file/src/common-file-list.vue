...
export default {
  ...
  methods: {
    ...
    async showFilePreview (file) {
      ...
      const { data: { isTrue } } = await getFileSize(Secret.getValue(file.fileId))
      if (isTrue === 'Y') {
        this.$message.warning(this.$t('cusEntry.tipMessage.bigFileNoSupportPreview'))
        return false
      }
    ...
    async downloadFile (file) {
      ...
      if (file.fileName && file.fileName === 'Тендерное письмо-Tender Lette.docx') {
        downloadFileLink(
          `/api-sou/sourcing/init/downloadSouProjectInfo?projectId=${file.projectId}`
        ).catch(() => {
          this.$message.error(this.$t('components.eio.downloadFail')) // 下载失败
        })
      } else {
        const { data: { isTrue, sign, fileFullname, fileUrl } } = await getFileSize(Secret.getValue(file.fileId))
        // if (isTrue === 'Y') {
        //   // SRM系统文件流方式下载
        //   // const loadingIns = this.createLoading('文件下载中，请耐心等待')
        //   // try {
        //   //   await ossDownload(fileUrl, file.fileName)
        //   // } catch (err) {
        //   //   this.$message.warning(this.$t('cusEntry.tipMessage.downloadFail'))
        //   // } finally {
        //   //   loadingIns.close()
        //   // }
        //   // 打开新窗口浏览器下载
