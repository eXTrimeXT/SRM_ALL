...
const $handleFormulaQuote = (index: any, row: any, $form: any, $message: any) => {
  ...
  if (!row.orderCurrency) {
    // 请先选择币种！
    $message.warning(i18nExpression('bidMod.inpCurrency'))
    return
  }
  if (!row.taxKey) {
    // 请先选择税率！
    $message.warning(i18nExpression('bidMod.inpTaxKey'))
    return
  }
  data.priceEditRow = {
    ...row,
    ...
    orderItemList: {
      ...
      properties: generateXindexInOrder({
        ...
        noCodeItem: {
          type: 'string',
          'x-render-table-column': {
            title: i18nExpression('bidMod.biddingManagementBuyer.isNoCodeItem'),
            minWidth: 120
          },
        ...
        priceStartTime: {
          ...
          'x-component-props': {
            ...
            formatter: expression(`({ cellValue, row, column }) => {
              parseTime(row.priceStartTime, '{y}-{m}-{d}')
            }`)
          },
          'x-render-table-column': {
        ...
        priceEndTime: {
          ...
          'x-component-props': {
            ...yearMonthDaySelectorSegment['x-component-props'],
            formatter: expression(`({ cellValue, row, column }) => {
              parseTime(row.priceEndTime, '{y}-{m}-{d}')
            }`)
          },
          'x-render-table-column': {
        ...
        operation: {
          ...
          properties: {
            ...
            formula: {
              type: 'void',
              title: generateCharExpressionByFunction(({ $t, $readonly }) => $readonly ? $t('bidMod.formulaQuoteDetail') : $t('bidMod.formulaQuote')),
            ...
            ladder: {
              type: 'void',
              title: generateCharExpressionByFunction(({ $t, $readonly }) => $readonly ? $t('bidMod.ladderQuoteDetail') : $t('bidMod.ladderPrice')),
