...
const handleSuccess = (fileList: any, $form: any) => {
  $form.query('CatalogOnShelves').get('data').catalogOnShelvesAttaches = fileList
}
      ...
      properties: {
              ...
              properties: {
                ...
                priceLibraryNo: {
                  ...
                  'x-reactions': expression(`() => {
                    ...
                    setTimeout(() => {
                      $self.setComponentProps({
                        disabled: $form.query("CatalogOnShelves").get("data").isReadOnly || $form.values?.status === pageParams.statusJudge
                      })
                    })
                  }`),
                  readOnly: true,
        ...
        materialItem: {
          ...
          'x-reactions': expression(`() => {
            ...
            $self.readPretty = $form.query("CatalogOnShelves").get("data")?.isReadOnly
          }`),
          properties: {
            materialParams: {
              type: 'void',
              ...
              properties: {
                ...
                orderQuantityMinimum: {
                  ...
                  'x-reactions': expression(`() => {
                    ...
                    const sign = $form.query("CatalogOnShelves").get("data")?.isReadOnly
                    if (sign) return 
                    setTimeout(() => {
                      $self.setComponentProps({
        ...
        pictureItem: {
          ...
          properties: {
            catalogOnShelvesAttaches: {
              ...
              'x-component-props': {
                fileList: '{{$form.query("CatalogOnShelves").get("data").catalogOnShelvesAttaches}}',
                isReadOnly: '{{$form.query("CatalogOnShelves").get("data").isReadOnly}}',
                '@handleSuccess': '{{(res, file, fileList) => handleSuccess(fileList, $form)}}',
                '@setDefaultPic': '{{(file) => setDefaultPic(file, $form.query("CatalogOnShelves").get("data").catalogOnShelvesAttaches)}}',
                '@handleRemove': expression(`(newList, file) => {
                  if ($values.status) {
                    $form.query("CatalogOnShelves").get("data").attachesDeleteList.push(file)
                  }
                  $form.query("CatalogOnShelves").get("data").catalogOnShelvesAttaches = newList
                }`)
              }
            }
          }
