  ...
  methods: {
    updateInfo () {
      if (!this.apiConfig.updateApi) {
        this.$message.error(i18n.t('common.noConfigUpdateMethod'))
        return
      }
      if (!this.configInfo.listName) {
        this.$message.error(i18n.t('reportSetting.unSettingTable'))
        return
      }
    ...
    addInfo () {
      if (!this.apiConfig.updateApi) {
        this.$message.error(i18n.t('reportSetting.unSettingFun'))
        return
      }
      if (!this.configInfo.listName) {
        this.$message.error(i18n.t('reportSetting.unSettingTable'))
