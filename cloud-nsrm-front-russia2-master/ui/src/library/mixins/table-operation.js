  ...
  methods: {
    ...
    deleteHandle (id) {
      if (!this.operationFunction.deleteInfo) {
        this.$message.error(this.$t('common.noConfigDel'))
        return
      }
      if (!id) {
        this.$message.error(this.$t('common.noSelectDelete'))
        return
      }
    ...
    addHandle () {
      if (!this.componentConfig.add) {
        this.$message.error(this.$t('common.noConfigAdd'))
        return
      }
      this.mode = 'add'
      const tab = {
    ...
    editHandle (row, id) {
      if (!this.componentConfig.edit) {
        this.$message.error(this.$t('common.noConfigUpdate'))
        return
      }
      this.mode = 'edit'
      const tab = {
    ...
    viewHandle (row, id) {
      if (!this.componentConfig.view) {
        this.$message.error(this.$t('common.noConfigView'))
        return
      }
      this.mode = 'view'
      const tab = {
    ...
    extendHandle (row, id, mode) {
      ...
      if (!mode) {
        this.$message.error(this.$t('common.noConfig'))
        return
      }
      this.mode = mode
      const tab = {
