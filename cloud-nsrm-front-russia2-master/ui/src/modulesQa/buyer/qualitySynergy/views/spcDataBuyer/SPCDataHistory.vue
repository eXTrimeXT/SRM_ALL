        ...
        <el-collapse v-model="activeDims" class="tab-form-style">
          <el-collapse-item :title="$t('quality.spc.selectCondition')" name="1">
            <srm-row :gutter="50">
              <srm-col :initCol="4">
                ...
                >
                  <el-select
                    ...
                    :placeholder="$t('quality.spc.workGroup')"
                  >
                    <el-option
              ...
              <srm-col :initCol="4">
                <el-form-item :label="$t('quality.spc.erpCodeArray')" prop="erpCodeArray">
                  <el-select
                    v-model="form.erpCodeArray"
                    clearable
                    multiple
                    :placeholder="$t('quality.spc.erpCodeArray')"
                  >
                    <el-option
              ...
              <srm-col :initCol="4">
                <el-form-item :label="$t('quality.spc.workBatch')" prop="workBatch">
                  <el-select
                    ...
                    allow-create
                    default-first-option
                    :placeholder="$t('quality.spc.workBatch')"
                  >
                    <el-option
              ...
              <srm-col :initCol="4">
                <el-form-item :label="this.$t('quality.onLineEquipment')">
              ...
              <srm-col :initCol="4">
                <el-form-item :label="$t('quality.spc.productBill')" prop="productBill">
                  <el-select
                    ...
                    allow-create
                    default-first-option
                    :placeholder="$t('quality.spc.productBill')"
                  >
                    <el-option
              ...
              <srm-col :initCol="4/3">
                <el-form-item class="butCtrl">
                  <el-button size="mini" type="primary" @click="getQuerydata">
                    {{ $t('common.export') }}
                  </el-button>
                  <el-button size="mini" @click="onQuery">
                    {{ $t('common.analysis') }}
                  </el-button>
                  <el-button size="mini" @click="resetForm('applyForm')">
                    {{ $t('common.clear') }}
                  </el-button>
                </el-form-item>
              </srm-col>
...
import { spcData, qualityProject } from '@/modulesQa/buyer/qualitySynergy/api'
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        lineCode: [{ required: true, message: this.$t('quality.spc.seletLineCode') }],
        erpCodeArray: [{ required: true, message: this.$t('quality.spc.selectErpCodeArray') }]
      },
  ...
  methods: {
    getRules (params) {
      ...
      getMonitorSpcParam(transformParams).then(response => {
        if (response.data.records && response.data.records.length) {
          let ruleData = response.data.records[0] || {}
          this.form.sampleRule = this.sampleRule = ruleData.sampleRule // 控制图
          let rule = JSON.parse(ruleData?.rule || {})
          this.subGroupCount = rule.sub_group_size // 样本数量
        }
      })
    },
    getErpCodeList () {
      ...
      let transformParams = transformMQL.save('SpcRuleStandardBuyer', payload, 'query')
      this.$http({
    ...
    // 历史分析界面查询参数获取
    getCondition (params) {
      let transformParams = transformMQL.save('spcDataInputBuyer', [params], 'historyPageCondition')
      ...
      this.$refs.applyForm.validate((valid) => {
        if (valid) {
          ...
          if (this.selectId.length > 0) { // 已选点
            let transformParams = transformMQL.save('spcDataInputBuyer', this.selectId, 'historyAnalysisDatasFromIds')
            ...
            let transformParams = transformMQL.save('spcDataInputBuyer', [rest], 'historyAnalysisDatas')
    ...
    onQuery () {
      ...
      this.$refs.applyForm.validate((valid) => {
        if (valid) {
          ...
          this.form.erpCode = this.form.erpCodeArray.join(',')
          const { erpCodeArray, ...rest } = this.form
          this.spcPageDataShow = false
          let transformParams = transformMQL.save('spcDataInputBuyer', [rest], 'historyGraphDatas')
