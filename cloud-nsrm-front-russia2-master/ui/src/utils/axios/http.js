...
http.interceptors.request.use(
  config => {
    ...
    if (localStorage.getItem('globalization') == 'true') {
      config.headers['Cloud-Time-Zone'] = localStorage.getItem('cloudTimeZoneFormat')
    }
...
http.interceptors.response.use(
  response => {
    ...
    if (response.config.returnDirectly) {
      ...
      if (response.data && response.data.code === 'SRM_COMMON_00002') {
        ...
        MessageBox.confirm(i18n.t('common.reLogin'), i18n.t('components.approvalHead.tips.tip'), {
          confirmButtonText: i18n.t('common.confirm'),
          cancelButtonText: i18n.t('components.common.cancel'),
          type: 'warning',
          showClose: false,
    ...
    if (traceInfo) {
      // 这里不支持多语言，暂时用中文
      traceInfo = ' [error code:' + traceInfo + ']'
    } else {
      traceInfo = ''
    }
    ...
    if (res.code !== '0') {
      ...
      if (res.code === 'SRM_COMMON_00060') {
        ...
        return Promise.reject(i18n.t('common.appExpiration'))
      }
  ...
  error => {
    ...
    let message = error.message == 'canceled' ? i18n.t('common.canceledQuery') : error.message
    ...
    if (error.message.indexOf('timeout') !== -1 && !originalRequest._retry) {
      errorMessage = i18n.t('common.queryTimeOut')
      Message({
        ...messageConfig,
    ...
    if (!isCanceled) {
      ...
      if (error.code == 'ERR_BAD_RESPONSE') {
        Message({
          ...messageConfig,
          message: i18n.t('common.netWorkErr'),
          type: 'error',
          duration: 5 * 1000
        })
      } else {
