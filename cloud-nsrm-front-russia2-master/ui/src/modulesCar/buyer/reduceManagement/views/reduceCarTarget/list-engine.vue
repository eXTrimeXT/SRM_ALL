...
const $submitData = ($form:any, $queryEngine:any, $table:any, $index:any, row:any) => {
  $form.validate().then(() => {
    let regex = /^-?\d+(\.\d{1,4})?$/
    if (row.carTargetUnitPrice < 0 || !regex.test(row.carTargetUnitPrice.toString())) return app.$message.warning('年度降本目标(单价)为正数，最大支持4位小数')
...
const $approve = ($form:any, $queryEngine: any) => {
  ...
  if (selects.some(item => !item.annualPredictQuantity)) return app.$message.warning('年度预测台数为空不能提交审批')
  ...
  app.$confirm($t('是否确认提交审批'), '提示', {
    ...
    let payload = selects.map((row: any) => {
      return row.carTargetId
    })

    app.$http({
      url: '/api-cost/reduce/car/target/submitApproval',
      method: 'POST',
      data: { carTargetIds: payload },
      loading: true
    }).then((res: any) => {
      ...
      toolbar: {
        ...
        properties: {
          approve: {
            ...
            'x-component-props': {
              type: 'primary',
              ...buttonListItemVisibleByPermission('reduce:reduceCarTarget:approve'),
              '@click': expression(`() => {
      ...
      table: {
        ...
        properties: generateXindexInOrder({
          ...
          carTargetUnitPrice: {
            ...
            'x-render-table-column': {
              title: "{{$t('reduce.carTargetUnitPrice')}}",
              minWidth: 120
            },
          ...
          carTargetTotalAmount: {
            type: 'string',
            ...editTableFormItemValid,
            'x-render-table-column': {
              title: "{{$t('reduce.carTargetTotalAmount')}}",
              minWidth: 120,
              skipEditable: true
            },
            'x-reactions': expression(`(field) => {
                let row = $table.getRowByIndex($self.index)
              if(row && row.carTargetUnitPrice != undefined && row.annualPredictQuantity != undefined ){
                let value = row.carTargetUnitPrice * row.annualPredictQuantity
                setTimeout(()=>{
