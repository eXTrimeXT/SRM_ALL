<template>
  ...
  >
      ...
      <SrmRow>
        ...
        <SrmCol :init-col="3">
          <el-form-item :label="$t('common.creationTime')">
            <el-date-picker
              v-model="questionFormData.creationDate"
              type="date"
    ...
    <!-- 质疑回复 -->
    <h3 v-if="!userTypeFlag.isVendor || extReplayContent">{{ $t('cusEntry.supplement20250121.qaRespond') }}</h3>
    <Tinymce
    ...
    <!-- 质疑回复附件 -->
    <h3 v-if="!userTypeFlag.isVendor || replayFiles?.length > 0">{{ $t('cusEntry.supplement20250121.qaRespondFile') }}</h3>
    <FileDynamic
      ...
      :business-id="editRow.questionId || ''"
      :editable="!readonly && userTypeFlag.isBuyer && (techFlag || bidFlag)"
      :need-init="false"
    />
    ...
    <div slot="footer">
      ...
      <template v-if="userTypeFlag.isBuyer && !readonly">
        <!--暂存-->
        <!-- 技术专家、招标专家可编辑质疑回复内容 -->
        <el-button v-if="techFlag || bidFlag" type="primary" @click="tempSaveOrSubmit2('save', true)">
          {{ $t("common.staging") }}
        </el-button>

        <!-- 招标专家 提交 -->
        <el-button v-if="bidFlag" type="primary" @click="tempSaveOrSubmit2('submit', true)">
          {{ $t("common.submit") }}
        </el-button>
      </template>
...
export default {
  ...
  computed: {
    ...
    // 是否为招标专家
    bidFlag () {
      return this.userId === this.editRow.extBidUserId
    },

    // 是否为技术专家
    techFlag () {
      return this.userId === this.editRow.extTechUserId
    },
  ...
  methods: {
    ...
    async tempSaveOrSubmit2 (type, bol) {
      if (type === 'submit' && !this.extReplayContent) {
        // 请填写质疑回复
        this.$message.warning(this.$t('cusEntry.supplement20250218.msgInputQuestionAnswer'))
        return
      }
      const allparam = {
        ...this.questionFormData,
        ...
        sceneFiles: this.sceneFiles,
        replayFiles: this.replayFiles,
        questionStatus: type === 'save' ? 'CLARIFY_ING' : this.questionFormData.questionStatus
      }
      if (type == 'save') {
    ...
    ready2 (editorInstance) {
      if (this.readonly || !this.userTypeFlag.isBuyer || !(this.techFlag || this.bidFlag)) {
        editorInstance.setMode('readonly')
      }
    }
  }
