<template>
  <el-container class="flex-container-notab the_announcements_wrapper" direction="vertical">
    <el-main class="main-the-body">
      ...
      <MainHeader :l-span="22" :r-span="2">
        <template slot="left">
          <!-- 发布新公告 -->
          <AuthorityButton
            code="understandBiddingMag:publishNewNotice"
            type="primary"
      ...
      <TableView
        ...
        url="/api-pj/pjnotice/notice/listPage"
      />
    </el-main>
    <srm-dialog
      v-if="dialogVisible"
    ...
    >
      ...
      >
        ...
        <el-row type="flex">
          ...
          <el-form-item :label="$t('dataConfMod.noticeDetail')" prop="detail">
            <Tinymce id="announcementsTinymce" v-model="form.detail" :height="460" @setup="ready" />
          </el-form-item>
        </el-row>
        <el-row type="flex">
          <!-- 附件 -->
      ...
      <div v-if="flag === 'edit'" slot="footer">
        ...
        buttons: [
          {
            ...
            show: row => row.noticeStatus == 'UNPUBLISHED',
            code: 'understandBiddingMag:edit',
            formattor (val) {
          ...
          {
            callback: row => this.eidtItem(row),
            code: 'understandBiddingMag:view',
            formattor (val) {
          ...
          },
          {
            ...
            code: 'understandBiddingMag:delete',
            formattor (val) {
  ...
  methods: {
    ...
    publish () {
      ...
      this.$refs.form.validate((status, message) => {
        ...
        // }
        this.$http({
          url: '/api-pj/pjnotice/notice/publish',
          method: 'POST',
          data: this.form
        }).then(res => {
          this.$message({ type: 'success', message: res.message })
      ...
      this.$refs.form.validate((status, message) => {
        ...
        if (this.isModify) {
          ...
          const deleteNoticeVendorIds = metaDataId
            ...
            .filter(i => !!i)
          this.$http({
            url: '/api-pj/pjnotice/notice/modify',
            method: 'POST',
            data: this.form
          }).then(res => {
          ...
          this.$http({
            url: '/api-pj/pjnotice/notice/add',
            method: 'POST',
            data: this.form
          }).then(res => {
            this.$message({ type: 'success', message: res.message })
