      ...
      >
        <el-collapse v-model="activeDims" class="tab-form-style">
          ...
          <el-collapse-item :title="$t('orderMod.buyerOrderSynergy.orderDetailsForm')" name="1">
            ...
            >
              <template #ceeaPurchaseOrderDate>
                <el-date-picker
                  v-model="form.ceeaPurchaseOrderDate"
                  type="date"
          ...
          <!-- 预付信息 -->
          <el-collapse-item :title="$t('cusEntry.orderMod.prepaymentInfo')" name="2">
            <BaseForm
              ref="prepaymentInfo"
          ...
          <el-collapse-item :title="$t('orderMod.buyerOrderSynergy.orderDetailsList')" name="3">
            ...
            >
              ...
              <vxe-column
                field="materialNameShow"
                :title="$t('purchaseDemand.itemName')"
                width="150"
              />
              <vxe-column
                field="specificationShow"
              ...
              <vxe-column
                field="deliveryDate"
                :title="$t('purchaseDemand.promiseReceiveDate')"
                width="180"
                :edit-render="{}"
              >
                <template #edit="scope">
              ...
              <vxe-column
                field="ceeaPromiseReceiveDate"
                :title="$t('cusEntry.supplement20250121.actualDeliveryDate')"
                width="150"
              >
                <template #default="{ row }">
                  {{ $parseTime(row.ceeaPromiseReceiveDate) }}
                </template>
              </vxe-column>
              <!-- <vxe-column
                field="categoryName"
              ...
              <vxe-column
                field="currencyCode"
                :title="$t('orderMod.buyerOrderSynergy.currency')"
                width="120"
                :edit-render="{}"
              >
                <template #edit="scope">
                  <DictSelect
                    ...
                    @change-value="(value, dictItem) => {
                      scope.row.currencyId = dictItem ? dictItem.id : null
                      scope.row.currencyCode = dictItem ? dictItem.value : ''
                      scope.row.currencyName = dictItem ? dictItem.label : ''
                    }"
                  />
                  <el-input
                    v-else
              ...
              <vxe-column
                field="extExchangeRate"
                :title="$t('orderMod.exchangeRate')"
                width="120"
              />
              <vxe-column
                field="extStandardCurrencyCode"
                :title="$t('bid_mod.standardCurrency')"
                width="120"
              >
                <template #default="{ row }">
                  {{ getDictLabel('currency', row.extStandardCurrencyCode) }}
                </template>
              </vxe-column>
              <vxe-column
                field="ceeaTaxKey"
              ...
              <vxe-column
                field="extUserName"
                :title="$t('cusEntry.orderMod.extUserName')"
                width="120"
                :edit-render="{}"
              >
                <template #edit="scope">
              ...
              <vxe-column
                field="extUseDepartmentCode"
                :title="$t('cusEntry.orderMod.extUseDepartmentName')"
                width="120"
                :edit-render="{}"
              >
                <template #edit="scope">
              ...
              <vxe-column
                field="extWarrantyPeriod"
                :title="$t('cusEntry.orderMod.extWarrantyPeriod')"
                width="150"
                :edit-render="{}"
              >
                <template #edit="scope">
              ...
              <vxe-column
                field="extAttachId"
                :title="$t('cusEntry.supplement20250121.orderLineAttachment')"
                width="150"
                :edit-render="{}"
              >
                <template #edit="{ row }">
...
import { formItems, formItems1, detailColumn } from './data/detail'
...
export default {
  ...
  methods: {
    getDictLabel (code, val) {
      return val ? this.$getDictLabel (code, val) : ''
    },
    // 打印
    printBill () {
      let ureportTem = `database:${this.$store.getters.language == 'ru_RU' ? '采购订单打印模板俄语' : '采购订单打印模板'}.ureport.xml`
      const xml = encodeURIComponent(ureportTem)
    ...
    initDetail () {
      ...
      if (flag === 'add') {
        ...
        this.form.ceeaPurchaseOrderDate = parseTime(new Date(), '{y}-{m}-{d}', true)
      } else {
    ...
    async queryDetails (orderId) {
      ...
      this.$http({
        ...
        if (res && res.data) {
          ...
          this.form.extErpPaymentStatus = !this.form.extErpPaymentStatus ? 'UN_PAY' : this.form.extErpPaymentStatus
          // 查询多语言物料信息
          const materialIds = res.data[0].detailList?.map(item => item.materialId) || []
