<template>
  ...
  > -->
  <SrmDialog
    :title="$t('cusEntry.supplement20250121.createOrderFromSourcingResult')"
    size="xLarge"
    :visible.sync="dialogVisible"
    :close-on-click-modal="false"
    append-to-body
  ...
  >
    ...
    <TableView
      ...
      row-key="orderNumber"
      tableHeight="300px"
      url="/api-sup-ce/purchaseRequirement/selectWithFixPrice"
  ...
  data () {
    return {
      ...
      tableHeader: [
        ...
        {
          // label: '税率',
          label: () => this.$t('components.ocr.commodityTaxRate'),
          prop: 'taxRate',
          width: 120
        },
        {
  ...
  methods: {
    ...
    async afterQueryPriceData (tableData = []) {
      ...
      this.tableData = tableData.map(item => {
        ...
        // item.priceSource = '定价单'
        item.priceSource = this.$t("cusEntry.route.priceOrders")
    ...
    creatOrder () {
      ...
      const saveData = transformMQL.save('Order', this.selection, 'extCreateByFixPrice')
      this.$http({
        url: '/api-sup-ce/api-ql/Order/extCreateByFixPrice',
        method: 'POST',
        data: saveData,
        loading: true
      }).then(res => {
