    ...
    <el-main>
      <div v-if="showTitle">
        <p style="font-weight:bold;font-size: 14px;">
          <!-- 供应商报价对比详情 -->
          {{ $t("cusEntry.supplement20250205.supplierQuotationComparisonDetails") }}
        </p>
        <p style="color:red;">
          <!-- 注：统一折算成卢布进行价格对比 -->
          {{ $t("cusEntry.supplement20250205.convertToRublesForPriceComparison") }}
        </p>
      </div>
      <el-table
        border
...
export default {
  ...
  methods: {
      ...
      if (this.mergeFlag) {
        ...
        arr.map((item, index) => {
          if (item.extPackageName == arr[index + 1]?.extPackageName) {
            ...
            finalArr.push({
              // extPackageName: '小计',
              extPackageName: this.$t('cusEntry.supplement20250205.subtotal'),
        ...
        return [
          ...
          // },
          {
            ...
            extPackageName: this.$t('cusEntry.supplement20250205.totalExcludingTax'),
            priceList: firstItem?.priceList.map(item => ({
        ...
        return [
          ...
          {
            // itemDesc: '合计（未税）',
            itemDesc: this.$t('cusEntry.supplement20250205.totalExcludingTax'),
            priceList: firstItem?.priceList.map(item => ({
              extProvPriceSumNoTax: item.extTotalProvPriceSumNoTax
            }))
          }
    ...
    getDetail (projectId) {
      bidBuyerHttp.control.priceComparisonCa(projectId).then(res => {
        if (res && res.data) {
