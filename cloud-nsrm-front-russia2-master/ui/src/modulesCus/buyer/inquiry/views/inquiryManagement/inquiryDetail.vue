  ...
  methods: {
    ...
    async saveTempInviteVendorTab (type) {
      if (type === 'next') {
        ...
        const result = await this.$refs.inquiryDetailRequireInfoTab.validateForm(false)
        if (!result) {
          this.$message.error('请检查全部必填字段是否填写正确')
          return
        }
      }
      ...
      if (result) {
        ...
        // 保存，进行下一步
        const responseData = await this.saveOrSubmitInq(params, type)
        if (responseData) {
          if (type === 'next') {
    ...
    /* 保存 / 提交 询价信息 */
    saveOrSubmitInq (params, type = 'save') {
      let payload = this.getFormatAllParams(params)

      return new Promise(async resolve => {
        let response = null
        if (type === 'next') {
          response = await inqBuyerHttp.init.submitInq(payload)
        } else {
          response = await inqBuyerHttp.init.editInq(payload)
        }
        if (response) {
