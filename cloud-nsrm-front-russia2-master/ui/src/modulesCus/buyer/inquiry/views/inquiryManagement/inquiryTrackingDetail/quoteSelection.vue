  ...
  <div class="quote-selection">
    ...
    >
      ...
      <el-table-column
        align="center"
        prop="winVendorCode"
        :label="$t('cusEntry.bidMod.winVendorCode')"
        min-width="120"
        show-overflow-tooltip
      />
      <!--t 中标供应商 -->
      ...
      <el-table-column
        ...
        show-overflow-tooltip
        :formatter="row => {
          return row.winCurrency ? $getDictLabel('currency', row.winCurrency) : ''
        }"
      />
      <!-- 原币种 -->
      <el-table-column
        align="center"
        prop="vendorWinCurrency"
        :label="$t('cusEntry.supplement20250218.originalCurrency')"
        min-width="100"
        show-overflow-tooltip
        :formatter="row => {
          return row.vendorWinCurrency ? $getDictLabel('currency', row.vendorWinCurrency) : ''
        }"
      />
      <!--t 税率-->
      <el-table-column
      ...
      <el-table-column
        align="center"
        prop="olriginalWinStandardNotaxPrice"
      ...
      <el-table-column
        ...
        show-overflow-tooltip
        :formatter="row => {
          return row.winIsPrepaid ? $getDictLabel('YES_OR_NO', row.winIsPrepaid) : ''
        }"
      />
      <!-- 预付比例% -->
      <el-table-column
        align="center"
        prop="winPrepaidRatio"
      ...
      >
        ...
        >
          <template slot-scope="scope">
            {{ showValueByProps(scope.row.orderItemList, 'orderCurrency', item.vendorId) ? $getDictLabel('currency', showValueByProps(scope.row.orderItemList, 'orderCurrency', item.vendorId)) : '' }}
          </template>
        </el-table-column>
        <!-- 原币种 -->
        <el-table-column
          align="center"
          prop="vendorOrderCurrency"
          :label="$t('cusEntry.supplement20250218.originalCurrency')"
          width="100"
          show-overflow-tooltip
        >
          <template slot-scope="scope">
            {{ showValueByProps(scope.row.orderItemList, 'vendorOrderCurrency', item.vendorId) ? $getDictLabel('currency', showValueByProps(scope.row.orderItemList, 'vendorOrderCurrency', item.vendorId)) : '' }}
          </template>
        </el-table-column>
        <el-table-column
          align="center"
        ...
        >
          <template slot-scope="scope">
            {{ showValueByProps(scope.row.orderItemList, 'extExchangeRate', item.vendorId) }}
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('bidMod.taxRate2')"
        ...
        >
          <template slot-scope="scope">
            {{ showValueByProps(scope.row.orderItemList, 'orderNotaxPrice', item.vendorId) }}
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('cusEntry.bidMod.orderNotaxAmount')"
        ...
        >
          <template slot-scope="scope">
            {{ showValueByProps(scope.row.orderItemList, 'extIsPrepaid', item.vendorId) ? $getDictLabel('YES_OR_NO', showValueByProps(scope.row.orderItemList, 'extIsPrepaid', item.vendorId)) : '' }}
          </template>
        </el-table-column>
        <!-- 预付比例% -->
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            {{ showValueByProps(scope.row.orderItemList, 'extPrepaidRatio', item.vendorId) }}
          </template>
        </el-table-column>
        <!-- 物流费 -->
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            {{ showValueByProps(scope.row.orderItemList, 'extLogisticsCost', item.vendorId) }}
          </template>
        </el-table-column>
        <el-table-column
          align="center"
...
export default {
  ...
  methods: {
    ...
    cancelRow () {
      ...
      //   type: 'warning'
      // })
      this.$confirm(this.$t('cusEntry.supplement20250121.confirmCancelDataReturn'), this.$t('common.tips'), {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      }).then(() => {
    ...
    getSelectList (winRow) {
      inqBuyerHttp.select.getSelectListByProjectId({ projectId: this.header.projectId }).then(async res => {
        if (res.data) {
          const {
            itemList,
            vendorList
          ...
          this.quoteSelectionTable = itemList.map(item => {
            const data = resData.find(it => it.materialId === item.itemId)
            return {
              ...item,
              materialNameShow: data?.materialName,
              extMaterialModelShow: data?.extMaterialModel,
              winCurrency: 'RUB',
              orderItemList: item.orderItemList.map(it => ({...it, orderCurrency: 'RUB'}))
            }
          })
    ...
    async getQuoteSelectionDetailData () {
      ...
      if (response && response.data) {
        const materialIds = response.data.list.map(item => item.itemId)
        const resData = await this.getMaterialLang(materialIds)

        this.quoteSelectionTable = response.data.list.map(item => {
          const data = resData.find(it => it.materialId === item.itemId)
            return {
              ...item,
              materialNameShow: data?.materialName,
      ...
      if (toWin) {
        ...
        for (let item of this.multipleSelection) {
          if (!item.winAmount && item.winAmount !== 0) {
            // 请先填写中标数量
            this.$message.warning(this.$t('cusEntry.supplement20250121.submissionBank') + this.$t('vendorMod.msgSelBidder'))
            return
          }
        }
      }
