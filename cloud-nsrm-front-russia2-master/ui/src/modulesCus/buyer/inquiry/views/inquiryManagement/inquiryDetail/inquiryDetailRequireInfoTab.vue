<template>
  <div class="inquiry-detail-require-info-tab">
    ...
    <el-button
      v-if="header.createApprovalStatus !== 'APPROVED'"
      type="primary"
      @click="cancelDetail"
    >
      {{ $t('common.cancel') }}
    ...
    items: {
      handler (val) {
        ...
        if (list.length === 0) {
          ...
          const materialIds = list.map(item => item.itemId)
          this.$http({
            ...
            this.itemsTableData = list.map(item => {
              const data = res.data.find(it => it.materialId === item.itemId)
  ...
  methods: {
    ...
    exportDetail () {
      downloadFileLinkByPost(
        `/api-sou/npm/buyer/inq/init/exportJcHtHeadLineData?projectId=${this.formProjectId}`,
        this.$t('cusEntry.inq.exprotMaterialDetail')
      ).catch(() => {
    ...
    cancelDetail () {
      ...
      if (selectRecords.length === 0) {
        this.$message.warning(this.$t('cusEntry.tipMessage.selectCancelLine'))
        return false
      }
      inqBuyerHttp.init.cancelRows(selectRecords.map(item => item.souItemId)).then(res => {
        this.$message.success(this.$t('cusEntry.tipMessage.cancelSuccess'))
    ...
    /* 校验 */
    validateForm (type = true) {
      return new Promise(async resolve => {
        const { fullData } = this.$refs.xTable.getTableData()

        if (fullData.length === 0) {
          type && this.$message.warning(this.$t('bidMod.inpItemInfo'))
          resolve(false)
          return
        }

        const warningMessage = (index, message) => {
          type && this.$message.warning(this.$t('bidMod.warningMessage', { index: index + 1, message }))
          resolve(false)
        }
