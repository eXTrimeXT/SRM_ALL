      ...
      <MainHeader :l-span="22" :r-span="2">
        <template slot="left">
          ...
          <!-- <AuthorityButton code="expertApply:add" type="primary" @click="editTab('add',{})">
            {{ $t("cusEntry.supplement20250205.greenChannel") }}
          </AuthorityButton> -->
        </template>
      </MainHeader>
    ...
    return {
      ...
      searchFormConfig: [
        ...
        },
        {
          prop: 'creationDate',
          // label: '申请时间',
          label: () => this.$t('components.stratProcess.headers.docCreateTime'),
          type: 'daterange'
        }
      ],
      queryParam: {},
  ...
  mounted () {
    this.tableHeader = [
      ...
      },
      {
        ...
        buttons: [
          {
            show: row => ['DRAFT', 'WITHDRAW', 'REJECTED'].includes(row.applyStatus) && this.curUserId == row.createdId,
          ...
          {
            show: row => ['DRAFT'].includes(row.applyStatus) && this.curUserId == row.createdId,
          ...
          // 审批
          {
            ...
            show: row => ['APPROVING'].includes(row.applyStatus) && (this.curUserId == row.createdId || row.isApprover == 'Y'),
      ...
      const map = new Map([
        ...
        [
          'add',
          {
            ...
            // title: '专家申请绿色通道',
            title: this.$t('cusEntry.supplement20250205.expertApplyGreenChannel'),
            name: 'expertApply'
          }
        ],
        // 编辑
        [
          'edit',
          {
            ...
            title: this.$t('cusEntry.supplement20250205.expertApplication') + (row.expertApplyNo || ''),
    ...
    afterQuery () {
      this.$refs['list'].setTableData(async tableData => {
        ...
        tableData.forEach(tableItem => {
          let obj = queryTodoList.find(todoItem => tableItem.expertApplyId + '' === todoItem.businessId + '')
          if (obj) {
