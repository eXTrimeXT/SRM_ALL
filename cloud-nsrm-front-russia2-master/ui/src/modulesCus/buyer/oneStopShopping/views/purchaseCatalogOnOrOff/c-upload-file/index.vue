...
export default {
  ...
  methods: {
    ...
    onExceed (files, fileList) {
      // this.$message.warning(`本次选择了 ${files.length} 个文件，请勿选择超过${this.limit}个文件`);
      this.$message.warning(`${this.$t('cusEntry.supplement20250205.thisTimeChosen')} ${files.length} ${this.$t('cusEntry.supplement20250205.fileCountLimit')}${this.limit}${this.$t('cusEntry.supplement20250205.fileName')}`);
    },
    // 点击文件列表
    onPreview (file) {
      const { id, url } = file
