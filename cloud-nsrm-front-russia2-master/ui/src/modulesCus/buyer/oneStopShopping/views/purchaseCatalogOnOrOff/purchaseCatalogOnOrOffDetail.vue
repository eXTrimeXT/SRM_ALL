          ...
          >
            <template #priceLibraryNo>
              <el-input
                ...
                :disabled="isReadOnly || getFormObj.status === 'TO_BE_ON_SHELVES'"
                clearable
                @clear="clearFormData"
              >
                <em
                  ...
                  :class="[(isReadOnly || getFormObj.status === 'TO_BE_ON_SHELVES') ? 'el-input__icon' : 'off-cursor']"
            ...
            <template #extEffectiveDate>
              <el-date-picker
                v-model="materialForm.extEffectiveDate"
                type="date"
                :format="$formatDatePicker"
                disabled
              />
            </template>
            <template #extExpirationDate>
              <el-date-picker
                v-model="materialForm.extExpirationDate"
                type="date"
                :format="$formatDatePicker"
                disabled
              />
...
export default {
  ...
  data () {
    return {
      ...
      materialForm: {
        ...
        extAreaId: null,
        extAreaCode: null,
        extEffectiveDate: null,
        extExpirationDate: null,
        extGoodsCode: null,
  ...
  created () {
    ...
    this.materialForm.creationDate = parseTime(new Date(), '{y}-{m}-{d}', true)
    this.initDetail()
  },
  methods: {
    initDetail () {
    ...
    queryDetails (catalogOnShelvesId) {
      ...
      this.$http({
        ...
        const response = await this.$http({
          ...
          method: 'POST',
          data: { materialIds: [res.data[0].materialId], language: this.$i18n.locale },
          loading: true
        })
        const formObj = {
          ...rest,
          materialNameShow: response.data[0]?.materialName,
          specificationShow: response.data[0]?.extMaterialModel
        }
        // 渲染表单
        this.matchFileObj(formObj)
        // 附件渲染
    ...
    // 打开价格编号弹窗
    openQueryDialog () {
      if (this.isReadOnly || this.getFormObj.status === 'TO_BE_ON_SHELVES') return
    ...
    async timeOnBill () {
      // '是否确认定时上架？'
      const confirmResult = await this.$confirm(this.$t('cusEntry.supplement20250205.confirmScheduledListing'), {
    ...
    async timeOffBill () {
      // '是否确认取消定时上架？'
      const confirmResult = await this.$confirm(this.$t('cusEntry.supplement20250205.isConfirmCancelScheduledUpload'), {
        confirmButtonText: this.$t('common.affirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      }).catch(() => {
      ...
      this.$http({
        ...
        // '取消定时上架成功'
        this.$message.success(this.$t('cusEntry.supplement20250205.cancelScheduledListingSuccess'))
        this.cancelBill()
      })
    },
    // 上架
