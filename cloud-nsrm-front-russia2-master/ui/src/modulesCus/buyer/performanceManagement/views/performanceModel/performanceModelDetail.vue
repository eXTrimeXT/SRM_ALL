          ...
          >
            ...
            >
              <srm-row>
                ...
                <srm-col>
                  <!-- 模型类型 -->
                  <el-form-item :label="$t('cusEntry.supplement20250205.modelType')">
                    <DictSelect v-model="modeForm.attribute1" code="PERF_MODEL_TYPE" />
                  </el-form-item>
                </srm-col>
                <srm-col v-if="modeForm.attribute1 == 'ORDER'">
                  <el-form-item :label="$t('components.organization.COMPANY')" prop="organizationName">
              ...
              <div v-for="(dimItem, index) in perfTemplateDimWeightList" :key="index" class="dimCont">
                ...
                >
                  <srm-row>
                    ...
                    <!--权重-->
                    <srm-col v-if="modeForm.isFormula !== 'Y' && modeForm.attribute1 == 'PROJECT'">
                      <el-form-item
                ...
                >
                  ...
                  <el-table-column align="center" :label="$t('perfMod.indicatorLogic')" min-width="200">
                    <template slot-scope="scope">
                      <el-input v-model="scope.row.perfTemplateLine.indicatorLogic" :disabled="curOpt === 'view'" />
                    </template>
                  </el-table-column>
                  <el-table-column
                    align="center"
                  ...
                  <el-table-column
                    v-if="modeForm.isFormula !== 'Y' && modeForm.attribute1 == 'PROJECT'"
                    align="center"
  ...
  methods: {
    ...
    fatchDictData () {
      ...
      performanceManagement.getPefTempDimAndOrgCat().then(res => {
        let datas = res.data
        datas.forEach(e => {
          if (e.indicatorDimension == 'OHTER') {
            e.indicatorDimensionName = this.$t('dataConfMod.settingGuide.step6.0') // 其他
          }
          if (e.indicatorDimension == 'TEAM') {
            e.indicatorDimensionName =this.$t('cusEntry.supplement20250205.team') // 团队
          }
        })
        this.dimensionList = datas
      })
    },
    // 查询单据详情
    getFormDetail () {
        ...
        if (valid) {
          if (this.perfTemplateDimWeightList.length > 0) {
            ...
            this.perfTemplateDimWeightList.forEach(item => {
              if (this.modeForm.isFormula !== 'Y' && this.modeForm.attribute1 == 'PROJECT') {
                console.log(item.perfTemplateDimWeight.indicatorDimensionWeight, 'indicatorDimensionWeight')
            ...
            if (this.modeForm.isFormula !== 'Y') {
              if (this.modeForm.attribute1 == 'PROJECT') {
                ...
                if (bolDimensionWeight == 1) {
                  ...
                  return this.__jump_error__(
                    'modindicatorInfo',
                    null,
                    this.$t('cusEntry.supplement20250205.dimensionWeightSum') // '维度下面所有指标权重之和为100!'
                  )
                }
                ...
                if (indicatorDimensionWeight != 100) {
                  ...
                  return this.__jump_error__(
                    'modindicatorInfo',
                    null,
                    this.$t('cusEntry.supplement20250205.totalDimensionWeightSum') // '所有维度权重之和为100!'
                  )
                }
              }
            }
