    ...
    <el-main>
      ...
      >
        <template slot="left">
          ...
          <ExportExcel
            page-url="/api-pef/pj/perf/score-man-scoring-v1/listScoreManScoringPage"
        ...
        <el-main style="flex-grow: 1; display: flex; flex-direction: column; position: relative">
          ...
          >
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  {{ $getDictLabel('ORDER_SCORE_MAN_SCORE_STATUS', scope.row.scoringStatus) }}
                </template>
              </el-table-column>
              <el-table-column
                v-if="userType !== 'VENDOR'"
...
export default {
  ...
  data () {
    return {
      dictCodes: {
        scoringStatus: 'ORDER_SCORE_MAN_SCORE_STATUS',
        indicatorDimensionType: 'INDICATORS_DIM',
        evaluation: 'SCORE_IS'
      },
      tableHeader: [
        {
        ...
        {
          prop: 'indicatorDimensionType',
          label: this.$t('perfMod.indicatorDimensionType'),
          minWidth: 120
        },
        {
          prop: 'indicatorName',
        ...
        {
          prop: 'categoryName',
          label: this.$t('perfMod.modelCategory'),
          minWidth: 120
        },
        // 评分状态
        {
          prop: 'scoringStatus',
          label: this.$t('cusEntry.supplement20250205.scoreStatus'),
          minWidth: 120
        },
        {
          prop: 'indicatorLogic',
          label: this.$t('perfMod.indicatorLogic_1'),
          minWidth: 120
        },
        {
          prop: 'evaluation',
          label: this.$t('vendorMod.scoringMethod'),
          minWidth: 120
        },
        {
          prop: 'score',
          label: this.$t('bidMod.performScore'),
          minWidth: 120
        },
        {
          prop: 'comments',
          label: this.$t('perfMod.scoreShows'),
          minWidth: 120
        },
        {
          prop: 'perStartMonth',
        ...
        {
          prop: 'scoreNickName',
          label: this.$t('vendorMod.reviewPerson'),
          minWidth: 120
        },
        // 评分人工号
        {
          prop: 'scoreUserName',
          label: this.$t('cusEntry.supplement20250205.scoreUserName'),
          minWidth: 120
        },
        {
          prop: 'scoreDate',
        ...
        {
          prop: 'rejectRemark',
          label: this.$t('vendorMod.refuseMemo'),
          minWidth: 120
        },
        // 唯一标识
        {
          prop: 'scoreManScoringId',
          label: this.$t('cusEntry.supplement20250205.uniqueIdentifier'),
          minWidth: 120
        },
        {
          prop: 'errorMsg',
          label: this.$t('quickSearch.errorMsg'),
          minWidth: 120
        }
      ],
      userType: '',
      ...
      iModal1: {
        ...
        upLoadUrl: '/api-pef/pj/perf/score-man-scoring-v1/importScoreManScoringV1Excel'
      },
      extraData: {
        fileModular: 'pef',
  ...
  created () {
    ...
    this.queryForm = [
      ...
      // 评分状态
      {
        prop: 'scoringStatusList',
        label: this.$t('cusEntry.supplement20250205.scoreStatus'),
        type: 'dict', // 字典类型
        multiple: true,
        code: 'ORDER_SCORE_MAN_SCORE_STATUS' // 字典code
      },
      {
    ...
    this.preFormObj = {
        ifEndScored: null,
        ifScored: null,
        scoringStatusList: ["DRAFT", "REJECT"],
        ifQueryByScoreItemsCreatedBy: null
      }
    this.$nextTick(() => {this.getQuerydata(this.preFormObj)})
  },
  mounted () {},
  methods: {
    formatValue(value) {
    ...
    getQuerydata (v) {
      if (v.scoreDate) {
        const [scoreStartDate, scoreEndDate] = v.scoreDate
        v.scoreStartDate = scoreStartDate
        v.scoreEndDate = scoreEndDate
        Reflect.deleteProperty(v, 'scoreDate')
      }
        ...
        if (rowData) {
          ...
          if (row.indicatorsLines.length > 0) {
            if (['GREATER', 'GREATER_EQUAL'].includes(row.indicatorsLines[0].startSymbol)){
              ...
              this.$message.error(this.$t('perfMod.notWithinRange'))
            }
          } else {
            this.$message({
    ...
    saveDataHandle (isQuery) {
      ...
      this.$refs['graderTableForm'].validate((valid, graderForm) => {
        if (valid) {
          ...
          this.selections?.forEach(e => { // 不填写得分的时候得分为0
            if (!e.score || e.score == ''){
              e.score = 0
            }
          })
    ...
    confirmCalculateVisible () {
      let url = '/api-pef/pj/perf/score-man-scoring-v1/saveScoreManScoring'
      this.$http({
        url: url,
        method: 'POST',
        data: this.selections
