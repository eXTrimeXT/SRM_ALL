  ...
  created () {
    ...
    let queryFormCom = [
      ...
      { prop: 'contractName', label: this.$t('vendorMod.contractName') },  // '合同名称'
      {
        ...
        label: this.$t('cusEntry.reportManagement.performanceStatusType'),  //  '履约类型'
        type: 'dict', // 字典类型
      ...
      { prop: 'bidManagerFullPath', label:this.$t('cusEntry.supplement20250205.bidExpertDept') },  //  '招标专家部门'
      { prop: 'contractManagerFullPath', label: this.$t('cusEntry.supplement20250205.contractResponsibleDepartment') },  // '合同负责人部门'
      {
        prop: 'organizationId',
      ...
      { prop: 'scoreStart', label: this.$t('cusEntry.supplement20250205.scoreGreaterThan') },  // '评分大于'
      { prop: 'scoreEnd', label: this.$t('cusEntry.supplement20250205.scoreLessThan') },  // '评分小于'
      { prop: 'calcDate', label: this.$t('vendorMod.scoringTime'), type: 'daterange' }  // '评分时间'
    ]
    let queryFormBuyer = [
      {
        prop: 'companyName',
    ...
    this.tableHeader = [
      ...
      },
      {
        ...
        callback: function (row) {
          if (row.performanceType === 'PROJECT') {
            this.$message.info(this.$t('cusEntry.supplement20250205.fulfillmentTypeForFulfillmentProject'))  // '履约类型为履约项目'
            return false
          }
      ...
      {
        prop: 'projectStatus',
        label: this.$t('cusEntry.supplement20250121.overallRatingStatus'),  // '整体评分状态'
        width: '120',
        dataType: 'dict', // 数据类型为字典
      ...
      {
        prop: 'score',
        label: this.$t('vendorMod.vendorSelfScoring'),  // '得分'
        width: '120'
      },
      {
        prop: 'levelName',
        label: this.$t('cusEntry.reportManagement.level'),  // '等级'
        width: '120'
      },
      {
        prop: 'lastUpdateDate',
        label: this.$t('vendorMod.scoringTime'),  // '评分时间'
        width: '120',
        dataType: 'dateTime'
      },
      {
