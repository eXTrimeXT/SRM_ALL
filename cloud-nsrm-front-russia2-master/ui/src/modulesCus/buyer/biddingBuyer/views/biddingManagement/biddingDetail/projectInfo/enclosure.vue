  ...
  <div class="the_file_part">
    ...
    <div class="right_div">
      ...
      <!-- <FileDynamic
        ref="sceneAttachment"
        v-model="bidFileList"
        scene-module-code="SCENE_SOU_BIDFILE_ATTACHMENT"
        :business-id="biddingBase.projectId"
        :editable="!readOnly"
        :need-init="true"
      /> -->
      ...
      >
          ...
          <template v-slot="scope">
            <SrmCommonFile
              :default-file="{
                ...
                souRemark: scope.row.souRemark,
                projectId: scope.row.projectId
              }"
              :readonly="biddingBase.extBudget > 1200000 && [0, 1, 2, 3].includes(scope.$index)"
              :extra-data="fileInfo"
              @on-change="({file}) => outerFilesChange(file, scope.$index)"
            />
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template v-slot="{ $index }">
            <el-button
              v-if="biddingBase.extBudget <= 1200000 || (biddingBase.extBudget > 1200000 && $index > 3)"
              type="text"
              @click="outerFilesDeleteRow($index)"
            >
              {{ $t('common.delete') }}
            </el-button>
          </template>
