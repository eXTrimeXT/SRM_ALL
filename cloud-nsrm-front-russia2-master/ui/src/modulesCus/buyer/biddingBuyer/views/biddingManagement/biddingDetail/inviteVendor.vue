<template>
  ...
  <div style="padding-top: 10px">
    <el-form ref="form" :model="biddingBase">
      ...
      <el-form-item
        prop="extRecommendNo"
        :label="$t('cusEntry.bidMod.recommendOrderNum')"
      >
        <span
    ...
    >
      {{ $t('cusEntry.supplement20250121.getVendor') }}
    </el-button>
    ...
    >
      ...
      <!-- <el-table-column
        align="center"
        prop="extIsMainPoint"
        :label="$t('cusEntry.vendorMod.ifFocus')"
...
export default {
  ...
  methods: {
    ...
    extRecommendNoClick (item, index) {
      if (this.extRecommendNoList) {
        ...
        this.$emit('tab-add', {
          ...
          title: this.$t('perfMod.oracle') + row.extRecommendNo,
    ...
    getRecommVendorInfoByNo () {
      if (!this.biddingBase.extRecommendNo) {
        // 未绑定供应商推荐单号
        this.$message.error(this.$t('cusEntry.supplement20250121.noBind'))
        return
      }
      this.$http({
        ...
        if (res.data.souVendor.length > 0) {
          ...
          // 未查询到关联已审批的推荐供应商单据
          this.$message.error(this.$t('cusEntry.supplement20250121.noQuery'))
        }
      })
    },
    /* 查询邀请供应商数据 */
