...
export default {
  ...
  methods: {
    ...
    async tempSaveProjectInfo (type) {
      ...
      if (count > 1) {
        // 评标委员会主席只能设置一位
        this.$message.error(this.$t('cusEntry.supplement20250121.bidTips15'))
        return
      }
      ...
      // 校验附件至少存在一个
      if (this.biddingBaseInfo.extBudget > 1200000 && applyFileList.length == 0) {
        // 至少上传一份申请资料
        this.$message.error(this.$t('cusEntry.supplement20250121.bidTips16'))
        return
      }
      let applyFlag = applyFileList.some(item => !item.souDocId)
      if (this.biddingBaseInfo.extBudget > 1200000 && applyFlag) {
        // 请上传申请资料附件
        this.$message.error(this.$t('cusEntry.supplement20250121.bidTips17'))
        return
      }
      if (this.biddingBaseInfo.extBudget > 1200000 && bidFileList.length == 0) {
        // 至少上传一份招标文件
        this.$message.error(this.$t('cusEntry.supplement20250121.bidTips18'))
        return
      }
      let bidFlag = bidFileList.some(item => !item.souDocId)
      if (this.biddingBaseInfo.extBudget > 1200000 && bidFlag) {
        // 请上传招标文件附件
        this.$message.error(this.$t('cusEntry.supplement20250121.bidTips19'))
        return
      }
      ...
      if (!flag) {
        // 请填写招标计划时间
        this.$message.error(this.$t('cusEntry.supplement20250121.bidTips20'))
        return
      }
      const timeValid = this.$refs.biddingPlan.timeValidate()
      if (!timeValid) {
        // 请检查招标计划时间是否填写正确
        this.$message.error(this.$t('cusEntry.supplement20250121.bidTips21'))
        return
      }
      ...
      let submitData = {
        ...
        groupList,
        // 内部附件信息
        applyFileList: applyFileList.filter(item => !!item.souDocId),
        // 外部附件信息
        bidFileList: bidFileList.filter(item => !!item.souDocId),
        // 招标计划
