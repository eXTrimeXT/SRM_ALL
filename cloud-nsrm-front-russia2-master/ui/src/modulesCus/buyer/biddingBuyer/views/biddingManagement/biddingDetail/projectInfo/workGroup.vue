<template>
  <div class="work-group">
    ...
    >
      ...
      /> -->
      <!--工号-->
      <!-- 招标专家不可修改，区域负责人可修改, 非合并招标时其他人不允许修改，合并招标时可修改 -->
      <el-table-column
        align="center"
        prop="userName"
...
export default {
  ...
  watch: {
    detailData: {
      handler (val) {
        // 招标管理流程为询比价时 前端不显示技术评分行
        if (this.biddingBase.extSouProcess === 'INQUIRY') {
          this.groupList = val.filter(item => item.scoreAuth !== 'SOU_TECH') || []
        } else {
          this.groupList = val || []
        }
        this.$nextTick(() => {
