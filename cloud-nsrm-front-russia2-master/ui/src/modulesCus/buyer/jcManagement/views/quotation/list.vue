<template>
  ...
  >
    ...
    <!-- 集采需知 -->
    <SrmDialog
      :title="$t('cusEntry.supplement20250205.groupNotice')"
    ...
    >
      <div>
        ...
        >
          <!-- 附件 -->
          {{ $t("components.workedProcess.headers.attachment") }}
        </el-button>
  ...
  data () {
    return {
      ...
      tableHeader: [
        {
          prop: 'souNo',
          label: this.$t('bidMod.inquiryNo'), // '询价单号'
          width: 120,
          showType: 'button',
          btnStyle: 'text',
        ...
        {
          prop: 'projectStatus',
          label: this.$t('bidMod.inquiryStatus'),  // '询价状态'
          width: 120,
          dataType: 'dict',
        ...
        {
          prop: 'orderNo',
          label: this.$t('bidMod.quoteNo'),  // '报价单号'
          width: 150
        },
        {
          prop: 'orderStatus',
          label: this.$t('bid_mod.quoteStatus'),  // '报价状态'
          dataType: 'dict',
        ...
        {
          prop: 'orderEndTime',
          label: this.$t('bidMod.quotedeadline'),   // '报价截止时间'
          width: 150,
          formattor: val => (val ? parseTime(val, '{y}-{m}-{d} {h}:{i}:{s}') : '')
        },
        {
          prop: 'currentRound',
          label: this.$t('bidMod.bidingRound'),  // '轮次'
          width: 120
        },
        {
          prop: 'createdBy',
        ...
        {
          prop: 'publishTime',
          label: this.$t('components.notice.publishTime'),  // '发布时间'
          width: 120,
          formattor: val => (val ? parseTime(val, '{y}-{m}-{d} {h}:{i}:{s}') : '')
        },
        {
          ...
          buttons: [
            {
              ...
              formattor: () => this.$t('bidMod.doBiding1')  // '报价'
            },
            {
              // 已报价 && 接受报价中 && 允许撤回
  ...
  created () {
    this.preArr = [
      ...
      },
      {
        ...
        label: this.$t('cusEntry.supplement20250205.inquiryStage'), // 询价阶段
        type: 'dict',
        code: 'EXT_INQ_SOU_PROJECT_STATUS'
      },
      {
  ...
  methods: {
    downloadTemplate () {
      ...
      downloadFileLink(
        ...
        // '集采需知.docx'
        `${this.$t('cusEntry.supplement20250205.groupNotice')}.docx`
      ).catch(() => {
