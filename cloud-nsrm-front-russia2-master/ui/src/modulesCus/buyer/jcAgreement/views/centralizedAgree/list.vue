    ...
    <el-main>
      ...
      <MainHeader :l-span="22" :r-span="2">
        <template slot="left">
          ...
          <AuthorityButton
            type="primary"
            code="jc:jcAgreement:exprotDetail"
            @click="exprotDetail"
          >
          ...
          <MImport
            ...
            up-load-url="/api-sou/jcAgreement/importJcAgreementHead"
            type="default"
          ...
          <MImport
            ...
            up-load-url="/api-sou/jcAgreement/importJcAgreementLine"
            type="default"
...
import CentralizedAgreeDetail from './edit'
export default {
  name: 'CentralizedAgreeList',
  ...
  data () {
    return {
      ...
      searchFormConfig: [
        ...
        {
          prop: 'supplyArea',
          label: this.$t('cusEntry.reportManagement.supplyArea'), //'供应区域'
          type: 'dict',
          code: 'REGION',
          multiple: true
        }
  ...
  methods: {
    ...
    exprotHeader () {
      ...
      const queryParam = {
        agreementType: this.$t('cusEntry.inq.centralizedAgreeExportHeader'), // 集采协议
    ...
    exprotDetail () {
      downloadFileLinkByPost(
        '/api-sou/jcAgreement/exportJcHtHeadLineData',
        this.$t('cusEntry.inq.centralizedAgreeExport'),
        this.queryParam
      ).catch(() => {
        this.$message.error(this.$t('components.eio.downloadFail')) // 下载失败
      })
    ...
    getQueryData (params = {}) {
      let transformParams = {}
      const { creationDate, effectiveDate, supplyArea, ...rest } = params
      ...
      if (supplyArea && supplyArea.length) {
        transformParams.supplyArea = supplyArea.join()
      }
      this.queryParam = {
        ...rest,
        ...transformParams,
        agreementType: '集采协议' // 集采协议: 查询入参不能国际化
      }
    ...
    editTab (type, row) {
      const map = new Map([
        ...
        [
          'add',
          {
            component: CentralizedAgreeDetail,
            params: {
              flag: type,
              row,
              tabName: 'centralizedAgree'
            },
            title: this.$t('cusEntry.inq.centralizedAgreeExportHeader'),
        ...
        [
          'edit',
          {
            ...
            title: this.$t('cusEntry.inq.centralizedAgreeExportHeader') + (row.agreementCode || ''),
        ...
        [
          'change',
          {
            ...
            title: this.$t('cusEntry.supplement20250121.centralizedAgreeChange') + (row.agreementCode || ''), // '集采协议变更'
            name: row.agreementCode
          }
        ]
      ])
