<template>
  ...
  > -->
  <SrmDialog
    :title="readonly ? $t('cusEntry.supplement20250121.refundHistoryOfEarnestMoney') : $t('cusEntry.supplement20250121.detailsOfIntentionMoneyRefund')"
    size="large"
    :visible.sync="dialogVisible"
    :close-on-click-modal="false"
  >
    <div class="header">
      ...
      <div class="header-introduce">
        ...
        <p>{{ $t("cusEntry.bidMod.refundRemark") }}</p>
        <p>{{ $t("cusEntry.bidMod.projectCancellationText1") }}</p>
        <p>
          {{ $t("cusEntry.bidMod.projectCancellationText2") }}
        </p>
      </div>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column
        prop="refundAmount"
        :label="$t('cusEntry.supplement20250121.refundAmount_yuan')"
        minWidth="150"
        show-overflow-tooltip
        :render-header="_addStarToColumn"
      >
      ...
      >
        <template v-slot="scope">
          ...
          />
          <span v-else>{{ $parseTime(cope.row.expectRefundTime) }}</span>
        </template>
      </el-table-column>
      <!-- <el-table-column
        prop="refundReason"
      ...
      <el-table-column
        prop="refundReason"
        :label="$t('cusEntry.reportManagement.refundReason')"
        minWidth="150"
        :render-header="_addStarToColumn"
      >
        <template v-slot="scope">
    ...
    handleConfirm () {
      ...
      for (let item of this.tableData) {
        if (!item.refundReason) {
          ...
          this.$message.warning(this.$t('cusEntry.supplement20250121.pleaseSelectARefundReason'))
          return
        }
      }
