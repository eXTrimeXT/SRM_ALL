            ...
            <CountdownRound :deadline-time="orderEndTime" right @deadline="countdownRoundDeadline">
              <template #title>
                <p class="countdown-round-title">
                  <!-- 距<span class="red">第{{ baseInfo.currentRound }}轮</span>竞价结束仅剩： -->
...
export default {
  ...
  computed: {
    ...
    infoList () {
      ...
      return [
        {
          label: this.$t('bidMod.bidClosingTime'),  // '竞价截止时间'
          value: projectInfo.orderEndTime || '-',
          enable: true
        },
        {
          label: this.$t('bidMod.competitionLts.openRules'),  // '公开规则'
          value: this.$getDictLabel('SOU_AUCT_SCOPE_RULE', projectInfo.scopeRule) || '-',
          enable: true
        },
        {
          label: this.$t('bidMod.extendBidTrigger'),  // '延长竞价触发点'
          value: projectInfo.extendTrigger ? `${projectInfo.extendTrigger}${this.$t('bidMod.minute')}` : '-',
          enable: true
        },
        {
          label: this.$t('bidMod.competitionLts.competitionRules'),  // '竞价规则'
        ...
        {
          label: this.$t('competition.extendMaxOrderCount'),  // '每次延时最多可报次数'
          value: projectInfo.extendMaxOrderCount || '-',
          enable: true
        },
        {
          label: this.$t('bidMod.bidingCurrency2'),  // '报价币种'
          value: this.$getDictLabel('currency', projectInfo.standardCurrency) || '-',
          enable: true
        },
        {
          label: this.$t('bidMod.competitionLts.delayMinutes'),  // '延长竞价时长'
          value: projectInfo.extendMinute ? `${projectInfo.extendMinute}${this.$t('bidMod.minute')}` : '-',
          enable: true
        },
        {
          label: this.$t('bidMod.evaluateMethod'),  // '评分规则'
  ...
  methods: {
    ...
    async quoteOrderPrice (price) {
      ...
      if(!taxKey){
        this.$message.warning(this.$t('bidMod.selectTaxRate'))  // '请选择税率'
        return
      }
      if(!price){
        this.$message.warning(this.$t('competition.priceTip'))  // '请填写报价'
        return 
      }
      let params = {
      ...
      if (response?.data) {
        this.$message.success(this.$t('competition.orderQuotationSuccess'))  // '报价成功'
        this.getOrderDetails()
      }
    },
