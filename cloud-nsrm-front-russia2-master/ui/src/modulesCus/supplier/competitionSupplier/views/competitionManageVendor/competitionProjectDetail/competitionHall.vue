  ...
  <div class="competition-hall">
    <div class="hall-info">
      <SrmRow>
        ...
        <SrmCol>
          <p>
            <span>{{ $t('bidMod.biddingType') }}:</span>{{ $getDictLabel('SOU_AUCT_SCORE_RULE_TYPE', baseInfo.scoreRuleType) }}
          </p>
        </SrmCol>
        <!--报价截止时间-->
        <SrmCol>
          <p>
            <span>{{ $t('bidMod.quotedeadline') }}:</span>{{ $parseTime(baseInfo.orderEndTime) }}
          </p>
        </SrmCol>
    ...
    >
      ...
      <el-table-column
        align="center"
        prop="pricePercent"
        :label="(baseInfo.scoreRuleType === 'MIN_PRICE' ? $t('bidMod.amplitude') : $t('cusEntry.supplement20250205.growthRate')) + '(%)'"
        width="100"
        show-overflow-tooltip
      />
    </el-table>
