  ...
  <div class="technology-score-detail">
    ...
    <div class="display-form-content">
      <SrmRow>
        ...
        <!--发布时间-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.releaseDatetime') }}</span>{{ $parseTime(bargainData.publishTime) }}
        </SrmCol>
        <!--投标截止时间-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.enrollEndDatetime') }}</span>{{ $parseTime(bargainData.orderEndTime) }}
        </SrmCol>
        <!--编制人-->
        <SrmCol :init-col="3">
        ...
        <!--投标开始时间-->
        <SrmCol :init-col="3">
          <span>{{ $t('bidMod.bidingStartDatetime') }}</span>{{ $parseTime(bargainData.orderStartTime) }}
        </SrmCol>
        <!--预计投标地点-->
        <SrmCol :init-col="3">
