<template>
  <el-container class="flex-container-aside-bargain the-bargain-detail-wrap">
    ...
    <el-container class="flex-container flex-container-right" direction="vertical">
      <!-- 头部信息 -->
      <DetailHeader v-if="bargainBaseInfo.souNo" :bargain-base="bargainBaseInfo" />
      ...
      <el-main style="padding-right: 3px">
        <el-tabs v-model="activeTab" style="height: 100%">
          ...
          >
            <OrionWorkflowTab
              :scope-id="bargainBaseInfo.projectId"
          ...
          >
            <OrionBondManagement
              ...
              :base-info="{ id: bargainBaseInfo.projectId, idKey: 'projectId' }"
...
import { WORKFLOW_MODEL_ID } from 'lib@/composition/bargainLts/utils'
import SignUpManagement from './bargainDetail/signUpManagement.vue'
import BidingControl from './bargainDetail/bidingControl.vue'
import BusinessManagement from './bargainDetail/businessManagement'
import TechManagement from './bargainDetail/techManagement'
import Evaluation from './bargainDetail/evaluation'
import InviteVendor from './bargainDetail/inviteVendor'
import ScoreRule from './bargainDetail/scoreRule'
import ProjectInfo from './bargainDetail/projectInfo'
import RequireInfo from './bargainDetail/requireInfo'
import DetailHeader from './bargainDetail/detailHeader'
import BargainDetailMenu from './bargainDetail/bargainDetailMenu'
...
export default {
  ...
  computed: {
    ...
    pricingType () {
      return {
        // 普通报价
        isSimplePricing: this.bargainBaseInfo.orderType === SOU_ORDER_TYPE_ENUM.SIMPLE,
        // 公式报价
        isFormulaPricing: this.bargainBaseInfo.orderType === SOU_ORDER_TYPE_ENUM.FORMULA,
        // 模型报价
        isModelPricing: this.bargainBaseInfo.orderType === SOU_ORDER_TYPE_ENUM.MODEL
      }
    },
  ...
  async created () {
    // 查询招标流程配置
    await this.getBrgProcessConfigIdList()

    if (!this.pageFlag.isAdd) {
      // 编辑 查看 审批
      this.bargainBaseInfo.projectId = this.$attrs.params.row.projectId
  ...
  methods: {
    /* 查询流程配置 */
    async getBrgProcessConfigIdList (pageSize = 1000) {
      ...
      if (response && response.data) {
        ...
        if (total > pageSize) {
          ...
          await this.getBrgProcessConfigIdList(total)
          return
        }
    ...
    async getFormDetail (type) {
      const response = await brgBuyerHttp.init.getProjectInfo(this.bargainBaseInfo.projectId)
      ...
      // 基础信息
      this.bargainBaseInfo = JSON.parse(JSON.stringify(project))
      this.$nextTick(() => {
        // 清除表单校验提示 默认不需要
    ...
    setMenuNodeConfig (processId, type) {
      ...
      if (process) {
        this.$refs.bargainDetailMenu.setMenuNodeConfig(processId, type)
      }
    },
    ...
    async menuItemClick (data) {
      // 没有单据ID || 没有流程ID
      if (!this.bargainBaseInfo.projectId || !this.bargainBaseInfo.processConfigId) {
    ...
    async getProcessNode (processNodeList) {
      if (!this.bargainBaseInfo.projectId) {
        return
      }
      ...
      if (processNodeList && Array.isArray(processNodeList)) {
        ...
        response = await brgBuyerHttp.process.projectNodes(this.bargainBaseInfo.projectId)
      }

      if (response && response.data) {
        if (this.$refs.bargainDetailMenu) {
          this.$refs.bargainDetailMenu.updateTreeMenuData(response.data)
        }
      }
    },
