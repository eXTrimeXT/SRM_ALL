<template>
  ...
  <el-form :model="projectRequirementsForm">
    <SrmRow style="padding-top: 10px">
      ...
      <SrmCol :init-col="3">
        ...
        <el-form-item :label="$t('bidMod.pricingType')" label-width="80px">
          <DictSelect
            v-model="bargainBaseInfo.orderType"
            code="SOU_ORDER_TYPE"
            :disabled="readonly"
    ...
    >
      ...
      <!--组合-->
      <vxe-column
        v-if="bargainBase.orderWay === SOU_ORDER_WAY_ENUM.COMBINED"
        field="itemGroup"
...
<script>
import { brgBuyerHttp, bidBuyerHttp } from 'modb@/bargain/api'
import {
  SOU_ORDER_TYPE_ENUM,
...
import ModelQuoteDialog from 'lib@/composition/bargainLts/modelQuote/modelQuoteDialog'
...
export default {
  ...
  methods: {
    ...
    async getProjectRequirementsData () {
      const response = await brgBuyerHttp.init.getRequireInfo(this.bargainBaseInfo.projectId)
      ...
      if (response && response.data) {
        ...
        this.$nextTick(() => {
          this.orderTypeChange(this.bargainBaseInfo.orderType, false)
        })
      }
    },
    ...
    orderTypeTransformOptions (options) {
      // 过滤模板报价
      return options.filter(item => ![SOU_ORDER_TYPE_ENUM.TEMPLATE,SOU_ORDER_TYPE_ENUM.MATERIAL_COST_SEPARATION].includes(item.value))
    },
    ...
    addRow () {
      this.$refs.xTable.insertAt({
        ...
        remark: '',
        // 带入项目信息中的价格有效期
        priceStartTime: this.bargainBase.priceStartTime || '',
        priceEndTime: this.bargainBase.priceEndTime || '',
        requireDate: null,
        formulaValue: null
      }, -1)
    },
    ...
    validateTableData () {
      ...
      const warningMessage = (index, message) => {
        ...
        this.$message.warning(`${this.$t('components.ocr.diTip')}${index + 1}${this.$t('common.row')} ${message}`)
      }
      ...
      for (const [index, item] of fullData.entries()) {
        ...
        // }
        // 是否进价格库
        if (this.bargainBase.isSyncToPriceLibrary === 'Y' && (!item.priceStartTime || !item.priceEndTime)) {
          // 请录入定价开始时间和结束时间!
          warningMessage(index, this.$t('bidMod.bidMsgList[31]'))
          return { status: false }
        }
        // 组合决标
        if (this.bargainBase.orderWay === SOU_ORDER_WAY_ENUM.COMBINED) {
          if (!item.itemGroup) {
            // 组合决标的决标方式，请输入组合！
    ...
    async saveRequirement (type) {
      ...
      const response = await brgBuyerHttp.init.editRequireInfo({
        projectId: this.bargainBaseInfo.projectId,
        requireDesc: this.bargainBaseInfo.requireDesc,
        orderType: this.bargainBaseInfo.orderType,
