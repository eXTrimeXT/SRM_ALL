...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        motorcycleTypeCode: [{ required: true, message: this.$t('productionPrepare.enterVehicleTypeCode') }], // 车型编码
        motorcycleTypeName: [{ required: true, message: this.$t('productionPrepare.enterVehicleTypeName') }], // 车型名称
        itemCode: [{ required: true, message: this.$t('vendorMod.enterMaterialCode') }], // 物料编码
        itemName: [{ required: true, message: this.$t('dataConfMod.msgInputItemName') }], // 物料名称
        vendorCode: [{ required: true, message: this.$t('quality.selectSupplierCode') }], // 供应商编码
        vendorName: [{ required: true, message: this.$t('vendorMod.enterVendorName') }], // 供应商名称
        problemExchangeStage: [{ required: true, message: this.$t('productionPrepare.selectOccurrencePhase') }], // 发生阶段
        problemExchangeRiskLevel: [{ required: true, message: this.$t('productionPrepare.selectRiskLevel') }], // 风险等级
        problemExchangeInfo: [{ required: true, message: this.$t('vendorMod.enterDescriptionProblem') }], // 问题描述
        problemExchangeReason: [{ required: true, message: this.$t('cusEntry.supplement20250211.inputReasonDescriptionAndStrategy') }], // 原因描述及对策
        problemExchangeResult: [{ required: true, message: this.$t('purchaseDemand.processingResult') }] // 处理结果
      },
