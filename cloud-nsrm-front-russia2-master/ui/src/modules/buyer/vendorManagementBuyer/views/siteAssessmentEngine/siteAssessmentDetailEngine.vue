...
const $disabledAdd = ( $form ) => {
  ...
  return states.addressReadOnly || states.bolType == 4 || ['PUBLISH', 'SUBMITTED', 'APPROVED'].includes(states.orderStatus) || $form.readPretty
}
...
const initButtonConfig = ($form: any) => {
  setTimeout(() => {
    ...
    if (!reviewResult || reviewResult == '') {
      componentInstance.buttonConfigInfo.save.name = i18nExpression('common.staging')  // '暂存'
      componentInstance.buttonConfigInfo.submit.name = i18nExpression('common.submit')  // '提交'
    }
  }, 50)
}
...
const schema = defineSchemas({
  ...
  state: {
    ...
    'x-data': {
      ...
      titleDisabled:false // 头部三个头字段是否置灰
    }
  },
  SiteForm: {
    type: 'void',
...
const $submits = async (type, $form, $queryEngine, $message, $t, $bus) => {
  ...
  const tablefileList = $form.query('.fileList').get('value')
  ...
  tableorgCateJournals.forEach((datas) => {
    if (datas.quoted == 'Y') {
      bolOrgCate = 0
    }
  })
  if (bolOrgCate) {
    app.$message.error(app.$t('vendorMod.certificationAndCategory'))
    return false
  }
  console.log(values)
  ...
  const run = async () => {
    if (type == 'SAVE') { // 暂存的时候
      if ([null, undefined, 'DRAFT'].includes(approveStatus)) { // 新增或者编辑的时候
      ...
      if (!reviewResult || reviewResult == '') { // 没有最终结论的时候
        ...
        $queryEngine.request.save(values, { query: { '*':{},
          ...
          const componentInstance = $form.query('.SchemaWorkflow').take().componentProps.componentInstance
          componentInstance.setWorkflowBusinessId(res.data[0]?.siteFormId || null)
