  ...
  >
    <el-main>
      <CWorkflowMulti
        ...
        @confirm="(type, comment) => submittedHandle(type)"
        @close-tab="back"
      >
        <div class="stepDiv">
          <VendorAccessSteps
          ...
          >
            <el-form
              ...
              :disabled="
                curRole === 'VENDOR' ||
                  curOpt === 'view' ||
                  (orderStatus !== 'DRAFT' && orderStatus !== '' && orderStatus !== 'REJECTED')
              "
            >
              <srm-row>
                <srm-col>
        ...
        <template v-if="curOpt != 'view'" slot="buttonOne">
          ...
          <el-button
            v-if="
              curRole === 'BUYER' && ['DRAFT', '', 'REJECTED', 'WITHDRAW'].includes(orderStatus)
                // (orderStatus === 'DRAFT' || orderStatus === '' || orderStatus === 'REJECTED')
            "
            @click="stagingHandle"
          >
...
export default {
  ...
  methods: {
    ...
    getCompanyObj (val, data) {
      ...
      this.form.reviewFormId = ''
      this.orgCateJournals = []
      this.isTrial = false
    },
    // 查询是否物料试用
    ...
    getReviewFormObj (val, data) {
      ...
      if (val) {
        ...
        this.$set(this.form, 'reviewFormNumber', '')
        this.orgCateJournals = []
      }
    },
