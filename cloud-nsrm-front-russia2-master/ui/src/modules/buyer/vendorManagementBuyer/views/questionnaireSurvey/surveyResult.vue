              ...
              <div class="question-list">
                <el-row :gutter="32" class="question-listBOx">
                  <el-col :span="12" class="result-left">
                    <strong>
                      <!-- 供应商反馈数量(已反馈数量/总数量) -->
                      {{ $t("cusEntry.supplement20250211.supplierFeedbackRatio") }}{{ backCount }}/{{ totalCount }}</strong>
...
export default {
  ...
  data () {
    return {
      ...
      preArr: [
        ...
        {
          prop: 'resultFlag',
          label: this.$t('dashboard.loopMode'), // '反馈状态'
          type: 'dict',
          code: 'RESULT_FLAG'
        }
      ],
  ...
  mounted () {
    ...
    this.tableHeader = [
      {
        prop: 'surveyTitle',
        label: this.$t('dataConfMod.questionnaire')  // '问卷标题'
      },
      {
        prop: 'vendorName',
      ...
      {
        prop: 'feedbackTime',
        label: this.$t('perfMod.vFeedbackTime'),  // '反馈时间'
        formattor: (val, item) => {
          return item.resultFlag === 'N' ? '-' : this.$parseTime(val, '{y}-{m}-{d}')
        }
      },
      {
        prop: 'operation',
        ...
        buttons: [
          {
            ...
            // '问卷报告'
            formattor: () => this.$t('vendorMod.questionnaireReport')
          }
        ]
      }
    ]
    this.fileHeader = [
      {
        prop: 'fileName',
        label: this.$t('bidMod.designation'),  // '名称'
        showType: 'slot',
        slot: 'downloadFileName'
      },
      {
        prop: 'vendorCode',
        label: this.$t('vendorMod.uploadSupplierCode')  // '上传供方编码'
      },
      {
        prop: 'vendorName',
        label: this.$t('vendorMod.uploadSupplierName')  // '上传供方名称'
      },
      {
        prop: 'uploadDate',
        label: this.$t('components.fileupload.uploadDate'),  // '上传时间'
        dataType: 'dateTime'
      }
    ]
  ...
  methods: {
    ...
    batchDownload () {
      if (!this.multipleSelection || !this.multipleSelection.length) {
        // '请选择需要下载的附件'
        this.$message.error(this.$t('vendorMod.selectAttachment'))
        return
      }
      let promiseArr = []
      ...
      if (!promiseArr.length) {
        // '选择数据没有附件'
        this.$message.error(this.$t('vendorMod.noAttachment'))
        return
      }
      ...
      this.$http({
        ...
        .then(data => {
          if (data) {
            ...
            this.chartData = {
              // legend: ['未反馈', '已反馈'],
              legend: [this.$t('vendorMod.noFeedback'), this.$t('vendorMod.givenFeedback')],
              seriesData: [
                { name: this.$t('vendorMod.noFeedback'), value: params.totalCount - params.backCount },
                { name: this.$t('vendorMod.givenFeedback'), value: params.backCount }
              ],
              color: [
                '#5470C6',
                '#91CC75',
    ...
    editTab (type, row) {
      if (type === 'edit') {
        let tab = {
          ...
          title: () => this.$t('vendorMod.questionnaireReport1'),  // '问卷调查报告'
