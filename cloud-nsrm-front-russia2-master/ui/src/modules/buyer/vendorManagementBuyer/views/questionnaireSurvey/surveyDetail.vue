...
export default {
  ...
  data () {
    return {
      ...
      tableHeaderSelected: [
        {
          prop: 'vendorName',
          // '已选供应商'
          label: this.$t("vendorMod.selectedVendor")
        }
      ],
      ...
      surveySelectionList: [
        {
          ...
          selectionValue: null,
          // '选项1'
          selectionHolder: this.$t('cusEntry.supplement20250211.option1'),
          descriptionFlag: 'N',
          description: null
        },
        {
          ...
          selectionValue: null,
          // '选项2'
          selectionHolder: this.$t('cusEntry.supplement20250211.optionTwo'),
          descriptionFlag: 'N',
          description: null
        }
      ],
      fileList: [],
      rules: {
        buId: [{ required: true, message: this.$t('common.BusinessEntity') }],  // '请选择业务实体'
        endDate: [{ required: true, message: this.$t('vendorMod.feedbackDeadline') }],  // '请选择反馈截止时间'
        feedbackFlag: [{ required: true, message: this.$t('vendorMod.permissionToView') }],  // '请选择查看结果权限'
        surveyTitle: [{ required: true, message: this.$t('vendorMod.enterQuestionnaireTitle') }] //'请输入问卷标题'
      },
      preArr: [
        {
          prop: 'vendorName',
        ...
        {
          prop: 'categoryName',
          label: this.$t('vendorMod.category')  // '采购品类'
        },
        {
          prop: 'materialName',
  ...
  watch: {
    'surveyQuestion.selectionCount': {
      ...
      handler: function (n, o) {
        let arr = [{ value: 1, label: this.$t('dashboard.noControl') }]  // '不控制'
        ...
        for (let i = 0; i < count; i++) {
          arr1.push({
            ...
            selectionValue: this.surveySelectionList[i]
              ...
              : null,
              // '选项'
            selectionHolder: this.$t('vendorMod.option') + (i + 1),
          ...
          this.surveySelectionList = [
            {
              selectionCode: 'A',
              selectionValue: null,
              selectionHolder: this.$t('cusEntry.supplement20250211.option1')  // '选项1'
            }
          ]
        } else {
          let count = this.surveyQuestion.selectionCount
  ...
  methods: {
    ...
    vendorAdd (rows) {
      if (!this.surveyId) {
        // '请先暂存基本信息'
        this.$message.error(this.$t('vendorMod.saveBasicInfoFirst'))
        return
      }
      this.$http({
    ...
    handleVendorDelete () {
      if (!this.selectedRows.length) {
        // '请勾选需删除数据'
        this.$message.warning(this.$t('vendorMod.checkDeleteData'))
        return
      }
    ...
    vendorDownloadTemplate () {
      downloadFileLinkByPost(
        ...
        // '供应商导入模板下载.xlsx',
        this.$t('cusEntry.supplement20250211.supplierImportTemplateDownloadXlsx'),
      ).catch(err => {
    ...
    copyQuestion (item) {
      // '确定复制此题目？'
      this.$confirm(this.$t('vendorMod.sureCopy'), {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      })
    ...
    comfirmSave () {
      if (!this.surveyQuestion.emptyFlag) {
        // '请选择是否必填'
        this.$message.error(this.$t('vendorMod.ifRequire'))
        return
      }
      if (!this.surveyQuestion.questionName) {
        // '请输入题目'
        this.$message.error(this.$t('vendorMod.enterQuestion'))
        return
      }
      let index = this.surveySelectionList.findIndex(item => !item.selectionValue)
      if (index > -1 && this.surveyQuestion.questionType !== 'Q') {
        // '请填写选项'
        this.$message.error(this.$t('vendorMod.enterOption') + (index + 1))
        return
      }
      if (
        ...
        // '请选择员工调研范围'
        this.$message.error(this.$t('vendorMod.scopeOfEmployeeSurvey'))
        return
      }
      ...
      if (this.surveyQuestion.jumpEnd === 'Y' && this.surveySelectionList.length > 0) {
        ...
        if (surveyJumpEnd && surveyJumpEnd.length === 0) {
          // '请选择选项跳转到末尾'
          this.$message.error(this.$t('vendorMod.jumpToTheEnd'))
          return
        }
      }
      // 校验新增时是否已存在相同的题目
        ...
        if (questionNameList && questionNameList.length > 0) {
          questionNameList.map(v => {
            if (
              ...
              // '已有该题目'
              this.$message.warning(this.$t('vendorMod.topicExisted'))
    ...
    clearAll () {
      // '是否清除所有已选供应商'
      this.$confirm(this.$t('vendorMod.clearSelectedSuppliers'), {
        confirmButtonText: this.$t('common.confirm'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      })
    ...
    preview () {
      if (!this.allParams.surveyTitle) {
        // '标题不能为空'
        this.$message.error(this.$t('vendorMod.blankTitle'))
        return
      }
      if (!this.allParams.surveyQuestionDTOList.length) {
        // '题目列表不能为空'
        this.$message.error(this.$t('vendorMod.questionListEmpty'))
        return
      }
      let tab = {
        ...
        title: () => this.$t('vendorMod.surveyPreview'),  // '问卷调查预览'
    ...
    toResult () {
      let tab = {
        ...
        title: () => this.$t('dashboard.questionnaireSurvey'),  // '问卷调查反馈结果'
    ...
    saveInfo () {
      ...
      if (getCheckDuplicateTopic) {
        // '问卷详情含有重复的题目'
        this.$message.warning(this.$t('vendorMod.duplicateQuestion'))
        return false
      }
      this.$http({
      ...
      this.$refs.info.validate(valid => {
        if (valid) {
          if (!this.selectList.length) {
            // '已选供应商不能为空'
            this.$message.error(this.$t('vendorMod.supplierEmpty'))
            return
          }
