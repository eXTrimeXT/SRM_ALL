...
// @ts-ignore
import {dataTimeSelectorSegment, formGridSegment, RenderEngine} from 'lib@/components/render-engine'
import {
  connect,
  defineSchemas,
  expression, generateXindexInOrder, i18nExpression, useAutoMountInstanceToField
...
import { forms } from "modb@/vendorManagementBuyer/views/cooperationEndedEngine/form";
...
const viewUpdateButton = ($form: any) => {
  ...
  console.log(componentInstance.workflowParamsInfo.integrationMode, 'integrationMode')
...
const handleButtonConfig = ($form: any, componentInstance: any) => {
  ...
  updateWorkflowconfig(componentInstance, orderId, tabDisabled, {procTitle:$form.values.orgCatFormNumber})
}
                  ...
                  collapse: {
                    ...
                    properties: generateXindexInOrder({
                      ...
                      cooperationEndForm: {
                        ...
                        'x-component-props': {
                          title: i18nExpression('vendorMod.cooperationEndedForm')
                        },
                  ...
                  collapseBusiness: {
                    ...
                    properties: generateXindexInOrder({
                      ...
                      businessForm: {
                        ...
                        'x-component': 'CollapseItem',
                        'x-component-props': {
                          title: i18nExpression('vendorMod.businessMatter')
                        },
                        'x-query-engine-skip': true,
                        properties: {
                          businessList: {
...
const $submits = async (type, $form, $queryEngine, $message, $t, $bus) => {
  ...
  if (['CATEGORY_EXIT', 'CATEGORY_FORZEN', 'CATEGORY_THAW'].includes(submitData.supplierControlType)) { // 如果是品类
    let bol2 = 0
    let optSelectRange = rangeDatas.rangeList.length > 0 ? rangeDatas.rangeList : rangeDatas.getRangeList() // 控制范围
    ...
    if (bol2 == 0) {
      app.$message.warning(
        app.$t('dataConfMod.msgInputCate')
      )// 请输入品类
      return false
    }
    let bol1 = 0
    let selectedCategoryList = rangeDatas.categoryList.length > 0 ? rangeDatas.categoryList : rangeDatas.getCategoryList() // 控制明细
    ...
    if (bol1 == 0) {
      app.$message.warning(
        app.$t('dataConfMod.msgInputUnit2')
      )// 请输入组织
      return false
    }
  } else if (['ORGANIZATION_EXIT', 'ORGANIZATION_FORZEN', 'ORGANIZATION_THAW'].includes(submitData.supplierControlType)) { // 组织退出
    let bol1 = 0
    let optSelectRange = rangeDatas.rangeList.length > 0 ? rangeDatas.rangeList : rangeDatas.getRangeList() // 控制范围
  ...
  if ($form.query('.businessMatterType').take()?.value) {
    const businessMatterType = JSON.parse(JSON.stringify($form.query('.businessMatterType').take()?.value))
    submitData.businessMatterType = businessMatterType.join(',')
  }
  ...
  const run = async () => {
    ...
    if (type == 'SAVE') { // 暂存的时候
        $queryEngine.request.save(submitData, { query: {"*":{},OrgCatFormCategoryRange: {'*': {}},OrgCatFormCategoryDetail: {'*': {}}}, loading: true }).then(() => {
      ...
      $queryEngine.request.save(submitData, { query: {"*":{},OrgCatFormCategoryRange: {'*': {}},OrgCatFormCategoryDetail: {'*': {}}},tree: true, loading: true }).then((res) => {
        // $form.query('.businessMatterType').take().value = submitData.businessMatterType.split(',')
        submitData.orgCatFormId = res.data[0].orgCatFormId
        const componentInstance = $form.query('.SchemaWorkflow').take().componentProps.componentInstance
        componentInstance.setWorkflowBusinessId(res.data[0]?.orgCatFormId || null)
        componentInstance.setWorkflowTabDisabled(true)
        componentInstance.setWorkflowBusinessVariables({procTitle:$form.values.orgCatFormNumber})
