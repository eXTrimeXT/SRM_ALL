...
const customUpdateButton = computed(() => (buyer() && !$readOnlyFlag && ['CONFIRMING'].includes(workflowStatus.value)))

const viewUpdateButton = computed(() => (buyer() && !$readOnlyFlag && !['CONFIRMING'].includes(workflowStatus.value)))
...
const initButtonConfig = ($form: any) => {
  setTimeout(() => {
    ...
    componentInstance.buttonCustom.PASS.view = false
    componentInstance.buttonCustom.REJECT.view = false
...
const handlePass = ($queryEngine:any, form:any, $bus:any) => {
  $queryEngine.request.baseRequest(
    {
      type: 'PurCatalogChange',
      loading: true,
      query: {
        '*': {}
      },
    ...
    $bus.$emit('PurCatalogChange')
  })
}

const handleReject = ($queryEngine:any, form:any, $bus:any) => {
  $queryEngine.request.baseRequest(
    {
      type: 'PurCatalogChange',
      loading: true,
...
const $submit = async (type:string, $form:any, $queryEngine:any, $confirm:any, $message:any, $bus:any) => {
  ...
  if (type === 'PASS') return handlePass($queryEngine, form, $bus)
  if (type === 'REJECT') return handleReject($queryEngine, form, $bus)
  ...
  if (type === 'SAVE') {
    ...
    return $queryEngine.request.baseRequest(
      ...
      const changeId = res.originalData?.records[0] || ''
      const componentInstance = $form.query('.SchemaWorkflow').take().componentProps.componentInstance
      componentInstance.setWorkflowBusinessId(changeId)
