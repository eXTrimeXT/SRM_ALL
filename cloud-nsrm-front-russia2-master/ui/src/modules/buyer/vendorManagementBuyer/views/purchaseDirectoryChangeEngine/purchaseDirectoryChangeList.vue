...
import PurchaseDirectoryDetail from './purchaseDirectoryChangeDetail'
import PurchaseDirectoryChangeDetail from 'modb@/vendorManagementBuyer/views/purchaseDirectoryChange/purchaseDirectoryChangeDetail'
    ...
    properties: {
        ...
        properties: {
          exportExcel: {
            ...
            'x-component-props': {
              ...exportExcelSegment,
              type: 'primary',
              pageUrl: "/api-sup/api-ql/PurCatalogChange/query",
              meiqlKey: "PurCatalogChange", // meiQl 表格key
              filterParams: queryFieldValueExpression('query'),
              tableHeader: queryFieldStatePropertyExpression('PurCatalogChange.table', 'data.columns'),
              dictCodes: {
      ...
      table: {
        ...
        properties: generateXindexInOrder({
          ...
          changeNo: {
            type: 'string',
            title: i18nExpression('purchase.SourceChangeOrderNo'),  // '货源变更单号'
          ...
          operation: {
            ...
            properties: {
              ...
              edit: {
                ...
                'x-reactions': changeFieldVisibleByDeps(['.changeStatus'], `['DRAFT'].includes($deps[0]) ||
                  (['REJECTED', 'WITHDRAW'].includes($deps[0]) && !$vendor()) || (['FIRST_REJECT'].includes($deps[0]) && $vendor())
                `)
              },
              approve: {
                type: 'void',
                ...
                'x-reactions': changeFieldVisibleByDeps(['.changeStatus'], `['SUBMITTED'].includes($deps[0]) && !$vendor()`)
              },
              management: {
                type: 'void',
                ...
                'x-reactions': changeFieldVisibleByDeps(['.changeStatus'], `['CONFIRMING'].includes($deps[0]) && !$vendor()`)
              },
              submitApproval: {
                type: 'void',
                title: "{{$t('bidMod.submitApproval')}}",
                'x-component-props': {
                  '@click': expression(`({row}) => {
                    // todo -- 待确认
                    $editTab('submit',row)
                ...
                'x-reactions': changeFieldVisibleByDeps(['.changeStatus'], `['FIRST_APPROVED'].includes($deps[0])  && !$vendor()`)
              },
...
const $editTab = (type:string, row:Object) => {
  let name, title
  if (type === 'add') {
    name = i18nExpression('vendorMod.supplyList')  // '货源清单'
    title = i18nExpression('vendorMod.newSourcelList')  //'新增货源清单'
    ...
    emitTabAdd({
      ...
      params: {
        flag: 'add',
        row,
        tabName: i18nExpression('route.purchaseDirectoryChange')  // '货源变更'
      },
      title: i18nExpression('route.purchaseDirectoryChange'),  // '货源变更'
      name: i18nExpression('route.purchaseDirectoryChange')  // '货源变更'
    })
    return
  } else { // edit/view
    // '货源变更'
    name = i18nExpression('route.purchaseDirectoryChange') + row.changeNo
    // '货源变更'
    title = i18nExpression('route.purchaseDirectoryChange') + row.changeNo
  }
  let tab = {
