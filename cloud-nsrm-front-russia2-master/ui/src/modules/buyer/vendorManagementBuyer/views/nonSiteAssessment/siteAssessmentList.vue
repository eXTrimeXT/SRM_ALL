  ...
  >
    <el-main>
      ...
      <MainHeader>
        <template slot="left">
          <AuthorityButton
            code="sup:nonSiteAssessmentList:add"
            type="primary"
            @click="editTab('add')"
          >
            {{ $t('common.add') }}
      ...
      <TableView
        ...
        url="/api-sup/serviceSiteForm/listPageByParm"
      />
    </el-main>
  </el-container>
</template>
...
import quaOfReviewDetail from 'modb@/vendorManagementBuyer/views/nonQuaOfReview/quaOfReviewDetail'
import { getDictItem } from '@/api/common'
import { adaptDictData, parseTime, findMenuIdByPath } from '@/utils'
import { siteAssessmentApi } from 'modb@/vendorManagementBuyer/api/vendorManagement'
...
export default {
  ...
  data () {
    return {
      ...
      preArr: [
        {
          prop: 'siteFormNumber',
          label: () => this.$t('vendorMod.siteOrderInfo') // 供应商评审单号
        },
        {
        ...
        {
          prop: 'assessmentType',
          label: () => this.$t('vendorMod.siteType'), // 供应商评审类型
          type: 'dict', // 字典类型
    ...
    $route: {
      ...
      handler () {
        if (
          ...
          this.$route.params.funName === 'nonSiteAssessment'
        ) {
  ...
  created () {
    ...
    _this.tableHeader = [
      ...
      // {
      //   prop: 'workFlowInfo',
      //   label: () => _this.$t('vendorMod.approvalProcess'), // '审批流程'
      //   width: 70,
      //   align: 'center',
      //   showType: 'button',
      //   btnStyle: 'text',
      ...
      //   width: 120,
      //   formattor (val) {
      //     return '<span class="iconfont iconscc-process"></span>'
      //   },
      //   callback: function (row) {
      //     this.intoFlow('view', row) // 查看流程详情
      //   }.bind(this),
      //   show: function (row) {
      //     if (row.fdId) {
      //       return true
      ...
      },
      {
        ...
        buttons: [
          ...
          },
          {
            ...
            code: 'sup:nonSiteAssessmentList:edit',
            formattor (val) {
          ...
          },
          {
            ...
            code: 'sup:nonSiteAssessmentList:approval',
            show: function (row) {
              if (['PUBLISH', 'SUBMITTED'].includes(row.approveStatus) && row.reviewResult != '') {
                return true
              } else {
          ...
          },
          {
            ...
            code: 'sup:nonSiteAssessmentList:delete',
            formattor (val) {
          ...
          },
          {
            ...
            code: 'sup:nonSiteAssessmentList:abandon',
            formattor (val) {
  ...
  methods: {
    ...
    approvalOne (row) {
      return false
      // this.$http({
      //   url: '/api-sup/review/serviceSiteForm/pass',
      //   method: 'POST',
      //   data: { siteFormId: row.siteFormId },
      //   loading: true
      // })
      //   .then(data => {
      //     this.getQuerydata()
    ...
    intoFlow (type, row) {
      ...
      if (type === 'view') {
        ...
        tab = {
          ...
          params: {
            flag: 'view',
            tabName: 'workflowReport' + row.siteFormNumber,
            flowParame: {
              // 流程参数
              fdId: row.fdId
            }
          },
          title: () => row.siteFormNumber + this.$t('common.approvalDetail'), // '审批详情',
          name: 'workflowReport' + row.siteFormNumber
        }
      }
      this.$emit('tab-add', tab)
    }
