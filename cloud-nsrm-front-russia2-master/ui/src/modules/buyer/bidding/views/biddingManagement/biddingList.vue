...
import { judgeAbandonProject, judgeDeleteProject } from 'lib@/composition/biddingLts/utils'
...
export default {
  ...
  data () {
    return {
      tableViewUrl: bidBuyerHttp.init.listPageUrl,
      scoreRuleName: '',
      tableHeader: [
        // 项目编号
        {
  ...
  watch: {
    $route: {
      handler () {
        ...
        // 工作流 或者其他地方 跳转过来
        if (routeParams.from === 'fromFun' && routeParams.funName === 'biddingList') {
          let projectId = null
          let title = null
        ...
        // 需求池跳转
        if (routeParams.from === 'demandPoolManagement' && routeParams.funName === 'biddingManagementLTS') {
  ...
  methods: {
    ...
    async abandonRow (row) {
      ...
      const response = await bidBuyerHttp.init.cancel({
        ...
        [
          'edit',
          {
            ...
            params: {
              flag: type,
              row: row,
              tabName: `biddingDetail${row.souNo}`
            },
            title: row.souName || row.souNo,
            name: `biddingDetail${row.souNo}`
          }
        ],
        // 查看
        [
