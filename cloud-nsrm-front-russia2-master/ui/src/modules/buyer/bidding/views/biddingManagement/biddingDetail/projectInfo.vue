            ...
            validator: (_rule, value, callback) => {
              if (value) {
                ...
                if (startDate.getTime() > endDate.getTime()) {
                  callback(new Error(this.$t('bidMod.priceEndTimeLessStart')))  // '价格有效期至需要小于价格有效期自'
                }
              }
              callback()
            },
          ...
          { required: true, message: this.$t('bidMod.bidMsgList[36]') },
          {
            validator: (_rule, value, callback) => {
              if (value) {
                ...
                const endDate = new Date(value)
                if (startDate.getTime() > endDate.getTime()) {
                  callback(new Error(this.$t('bidMod.orderEndTimeLessStart')))  // '截止时间需要小于开始时间'
                }
              }
              callback()
            },
        ...
        bankAccountName: [
          {
            validator: (_rule, value, callback) => {
              if (this.bondConfigVisible && !value) {
                callback(new Error(this.$t('vendorMod.msgBankAccountName')))  // '请输入账户名称'
              }
              callback()
            },
            trigger: ['change', 'blur']
        ...
        bankBranchName: [
          {
            validator: (_rule, value, callback) => {
              if (this.bondConfigVisible && !value) {
                callback(new Error(this.$t('bidMod.bidMsgList.17')))  // '请输入开户支行'
              }
              callback()
            },
            trigger: ['change', 'blur']
