  ...
  <el-container class="barcoderuleheadEdit" direction="vertical">
    <el-main>
      <div class="form-container">
        <el-form ref="form" :model="form" :rules="rules">
          <srm-row :gutter="32">
            <srm-col :span="6">
              <!-- 规则名称 -->
              <el-form-item prop="ruleName" :label="this.$t('barcodeManageNew.ruleName')">
                <el-input v-model="form.ruleName" maxlength="50" show-word-limit />
              </el-form-item>
            </srm-col>
            <srm-col :span="6">
              <!-- 规则描述 -->
              <el-form-item prop="ruleDesc" :label="this.$t('barcodeManageNew.ruleDes')">
                <el-input v-model="form.ruleDesc" maxlength="100" show-word-limit />
              </el-form-item>
            </srm-col>
            <srm-col :span="6">
              <!-- 开始日期 -->
              <el-form-item prop="startDate" :label="this.$t('components.beginDate')">
                <el-date-picker
            ...
            <srm-col :span="6">
              <!-- 结束时间 -->
              <el-form-item prop="endDate" :label="this.$t('components.common.endTime')">
                <el-date-picker
            ...
            <srm-col :span="6">
              <!-- 所属箱型业务 -->
              <el-form-item prop="businessBoxType" :label="this.$t('barcodeManageNew.boxTypeBusiness')">
                <dict-select
            ...
            <srm-col :span="6">
              <!-- 品类信息 -->
              <el-form-item prop="categoryName" :label="this.$t('vendorMod.categoryInfo')">
                <QuickSearch
      ...
      <el-collapse v-model="activeLine">
        <!-- 明细 -->
        <el-collapse-item :title="this.$t('vendorMod.details')" name="1">
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ruleName: [{ required: true, message: this.$t('barcodeManageNew.fillRuleName') }],  // '请填写规则名称！'
        startDate: [{ required: true, message: this.$t('barcodeManageNew.selectStartDate') }],  // '请选择开始日期！'
        businessBoxType: [{ required: true, message: this.$t('barcodeManageNew.selectBoxTypeBusiness')}],  // '请选择所属箱型业务！' 
        categoryName: [{ required: true, message: this.$t('barcodeManageNew.selectCategoryInfo') }]  // '请选择品类信息！'
      },
      readOnly: false,
  ...
  methods: {
    ...
    async save () {
      if (this.realDataSource.length < 1) {
        return this.$message.warning(this.$t('barcodeManageNew.addOneDetail')) // '请至少添加一条明细数据！'
      }
