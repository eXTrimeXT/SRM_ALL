      ...
      columns: [
        {
          attrs: {
            prop: 'outerBoxId',
            label: this.$t('cusEntry.supplement20250211.outerBoxId')  // 外箱id
          },
          slot: 'outerBoxId'
        },
        {
          attrs: {
            prop: 'fieldType',
            label: this.$t('cusEntry.supplement20250211.expandFieldType')  // '扩展字段类型'
          },
          slot: 'fieldType'
        },
        {
          attrs: {
            prop: 'fieldCode',
            label: this.$t('cusEntry.supplement20250211.expandFieldCodeDtoField')  // '扩展字段编码（dto属性字段）'
          },
          slot: 'fieldCode'
        },
        {
          attrs: {
            prop: 'fieldDesc',
            label: this.$t('cusEntry.supplement20250211.expandedFieldDescription')  // '扩展字段描述'
          },
          slot: 'fieldDesc'
        },
        {
          attrs: {
            prop: 'fieldVal',
            label: this.$t('cusEntry.supplement20250211.extendedFieldValue')  // '扩展字段数值'
          },
          slot: 'fieldVal'
        },
        {
    ...
    */
        ...
        if (flag === 'add') {
          ...
          if (type == '1') {
            ...
            const loading = this.$loading({
              lock: true,
              text: this.$t('cusEntry.supplement20250211.autoGenerateAndBindBarcodes'),  // '自动生成内外箱条码，并且自动绑定，请稍等！！！'
      ...
      this.$http({
        ...
        .then((data) => {
          ...
          if (isNull(data.data.categoryCode)) {
            this.$message({
              type: 'warning',
              message: this.$t('hierarchical.categorynumber')  // '该物料没有绑定品类编号！'
            })
          } else {
    ...
    onInputBlur () {
      if (this.form.comments.length >= 60) {
        this.$message({
          type: 'warning',
          message: this.$t(this.$t('cusEntry.supplement20250211.currentCharLengthExceedsLimit'))  // '当前字符长度超过字数限制，最大值60个字符'
        })
      }
    }
  }
