      ...
      actions: {
        paginationQuery: {
          ...
          transformResponse: (res: string) => {
            ...
            if (data.data?.ref?.StorageReturn) {
              const keys = Object.keys(data.data.ref.StorageReturn ?? {})
              keys.forEach(key => {
                const item = data.data.ref.StorageReturn[key]
                item.handleDate = item.type === 'RECEIVE' ? item.receiveDate : item.returnToSupplierDate
              })
            }
            return data
          }
      ...
      toolbar: {
        ...
        properties: {
          ...
          exportExcel: {
            ...
            'x-component-props': {
              ...exportExcelSegment,
              pageUrl: '/api-sup-ce/api-ql/StorageReturn/query',
              tableHeader: queryFieldStatePropertyExpression('StorageReturn.table', 'data.columns'),
              dictCodes: {
      ...
      table: {
        ...
        properties: generateXindexInOrder({
          ...
          receiveOrderLineNo: {
            type: 'string',
            'x-render-table-column': {
              title: i18nExpression('orderMod.receiveOrderLineNo'), // 接收行号
              minWidth: 120
            }
          },
...
<template>
  <RenderEngine :schema="schema" :scope="scope" schemaKey="StorageReturn" />
</template>
