...
export default {
  ...
  'x-component-props': {
    title: i18nExpression('orderMod.orderChangeAttach') // 订单变更附件信息
  },
  properties: {
    toolbar: {
      type: 'void',
      ...
      properties: {
        add: {
          ...
          'x-component-props': {
            type: 'primary',
            '@click': expression(`() => {
              $self.query('.orderChangeAttaches').take().componentProps.componentInstance.addRow()
             }`)
          }
        }
      }
    ...
    orderChangeAttaches: {
      ...
      'x-query-engine-relation': 'orderChangeAttaches:*',
