...
export default {
  ...
  properties: {
    ...
    pagination: {
      ...
      'x-component-props': {
        pageNum: expression('$form.query(\'Order\').get(\'data\').orderchangeRecordsPageNum'),
        pageSize: expression('$form.query(\'Order\').get(\'data\').orderchangeRecordsPageSize'),
        total: expression('$form.query(\'Order\').get(\'data\').orderchangeRecordsTotal'),
        pageSizes: [5, 15, 30, 60, 120, 300, 600, 1000, 1500],
        '@current-change': expression(`(num) => {
                $form.query('Order').get('data').orderchangeRecordsPageNum = num
                $getOrderchangeRecordsData($form)
              }`),
        '@size-change': expression(`(size) => {
                $form.query('Order').get('data').orderchangeRecordsPageSize = size
                $getOrderchangeRecordsData($form)
              }`)
      }
    }
