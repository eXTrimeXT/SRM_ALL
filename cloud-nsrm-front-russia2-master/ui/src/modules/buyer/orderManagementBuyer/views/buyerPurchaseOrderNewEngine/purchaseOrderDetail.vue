...
const $saveBill = (type: string, $form: any, $queryEngine: any, $message: any, $bus: any) => {
  ...
  values.detailList.forEach((item: any) => {
    item.ceeaPlanReceiveDate = item.ceeaPlanReceiveDate ? parseTime(item.ceeaPlanReceiveDate, '{y}-{m}-{d} {h}:{i}:{s}', true) : ''
    item.ceeaPromiseReceiveDate = item.ceeaPromiseReceiveDate ? parseTime(item.ceeaPromiseReceiveDate, '{y}-{m}-{d} {h}:{i}:{s}', true) : ''
