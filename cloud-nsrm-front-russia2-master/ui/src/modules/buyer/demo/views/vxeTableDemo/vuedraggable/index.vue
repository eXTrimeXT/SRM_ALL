...
export default {
  ...
  data () {
    return {
      dictClass: createDictClass(
        {
          ...
          currentField: [
            {
              id: 1,
              value: this.$t('accountMod.unitPrice'),  // '单价'
              label: this.$t('accountMod.unitPrice')  //this.$t('accountMod.unitPrice')
            }
          ],
          selectField: [
            {
              id: 1,
              value: this.$t('bid_mod.quantity'),  // '数量'
      ...
      componentsTable: [],
      commandItems: [
        { key: 'item-1', component: 'select', type: 'bracket', label: this.$t('cusEntry.supplement20250211.bracketNotation') }, // '括号'
        { key: 'item-3', component: 'select', type: 'connector', label: this.$t('priceModel.costElement.connector') }, // '连接符'
        { key: 'item-4', component: 'select', type: 'conditionComparator', label: this.$t('cusEntry.supplement20250211.operatorNotation') },  // '操作符'
        { key: 'item-4', component: 'input', type: 'value', label: this.$t('materialMainData.attributeValue') },  // '属性值'
        { key: 'item-5', component: 'select', type: 'currentField', label: this.$t('cusEntry.supplement20250211.currentAttributeField') },  // '当前属性字段'
        { key: 'item-6', component: 'select', type: 'selectField', label: this.$t('cusEntry.supplement20250211.selectedAttributeField') }  // '所选属性字段'
      ],
      // 递增的key
      keyIndex: 1000
    }
  ...
  methods: {
    ...
    add (value) {
      ...
      this.componentsList.push({
        ...
        name: this.$t('cusEntry.supplement20250211.componentName') + this.keyIndex,  // '组件-'
