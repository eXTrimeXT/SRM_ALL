  ...
  const DynamicClass = {
    ...
    load: (attribute, customMethods) => {
      ...
      if (attribute.placeholder) {
        ...
        let languageCode = attribute.languageCode
        let label = languageCode ? i18n.t(languageCode) : propertyDescribe
        componentProperty['placeholder'] = i18n.t('common.pleaseInput') + label // attribute.placeholder
      }
      if (attribute.lSpan) {
      ...
      if (attribute.propList) {
        for (let i = 0; i < attribute.propList.length; i++) {
          ...
          if (attribute.componentType === 'EL_BUTTON' && propItem.propName === 'buttonNameText') {
            ...
            slots.text.default.push({ text: i18n.t('fileDynamic.' + propItem.propValue) }) // propItem.propValue
          }
