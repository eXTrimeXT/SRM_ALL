<template>
  <div class="approval-report">
    ...
    <PaymentTypeDialog
      ...
      :business-type="approvalHeader.sourceFromType"
      :edit-row="viewRow"
      readonly
    />
...
export default {
          ...
          if (this.pageType.isInquiry) {
            ...
            if (current.isLadder === 'Y') {
              ...
              last[`vendor_${current.vendorId}_isLadder`] = {
                ...
                ladderList: current.ladderPriceList
              }
            }
  ...
  created () {
    ...
    if (this.approvalHeader.sourceFromId && this.pageType) {
  ...
  methods: {
    ...
    getApprovalReportData () {
      ...
      if (this.pageType.isAuct) {
        // 新版竞价
        this.generatePriceFilterReportForAuct()
      }
    },
    /** 查询新版竞价 */
    generatePriceFilterReportForAuct () {
      let transformParams = transformMQL.save('AuctSouProjectForBuyer', [{ projectId: this.approvalHeader.sourceFromId }], 'generatePriceReport')
      console.log('transformParams', transformParams)
      this.$http({
        url: '/api-sou/api-ql/AuctSouProjectForBuyer/generatePriceReport',
        method: 'POST',
        ...
        if (records.length) {
          this.arrangeReportForBidAndBrg(records[0], 'AUCT')
        }
      })
    },
    /* 查询简易询价报表数据 */
    ...
    getGeneratePriceFilterReportForInquiry () {
      this.$http({
        url: `/api-sou/buyer/inq/select/generatePriceReport/${this.approvalHeader.sourceFromId}`,
        method: 'GET',
        loading: true
      }).then(data => {
    ...
    getGeneratePriceFilterReportForBiding () {
      this.$http({
        url: `/api-sou/buyer/bid/select/generatePriceReport/${this.approvalHeader.sourceFromId}`,
        method: 'GET',
        loading: true
      }).then(data => {
    ...
    /* 查询项目式询价报表数据 跟招标基本一致 */
    // /api-sou/buyer/brg/select/generatePriceReport/{sourceFromId}
    getGeneratePriceFilterReportForBargain () {
      this.$http({
        url: `/api-sou/buyer/brg/select/generatePriceReport/${this.approvalHeader.sourceFromId}`,
        method: 'GET',
        loading: true
      }).then(data => {
    ...
    generatePriceFilterReportForComp () {
      this.$http({
        url: `/api-sou/buyer/comp/select/generatePriceReport/${this.approvalHeader.sourceFromId}`,
        method: 'GET',
        loading: true
      }).then(data => {
