<template>
  ...
  >
    ...
    <div v-if="false" class="ladder-price-form-row">
      <div class="form-col">
        <!--阶梯价类型-->
    ...
    >
      ...
      >
        <template v-slot="scope">
          <el-input
            v-if="!isReadOnly"
            v-model.number="scope.row.orderNotaxPrice"
            type="number"
            :disabled="isReadOnly"
          />
  ...
  methods: {
    ...
    addLadderPrice () {
      for (const i of this.ladderPriceTable) {
        if (i.beginQuantity === 0 || i.endQuantity === 0) {
          // 数量不为0！
          this.$message.warning(this.$t('bidMod.common.ladderMsg3'))
          return
        }
    ...
    saveLadderItems () {
      ...
      for (let x = 0; x < this.ladderPriceTable.length; x++) {
        ...
        if (!this.ladderPriceTable[x].orderNotaxPrice) {
