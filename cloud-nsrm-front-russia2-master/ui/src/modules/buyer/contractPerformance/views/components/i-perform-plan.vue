    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, scope) => {
            if (this.readPretty == true) {
              return this.$parseTime(scope.row.planStartDate)
            } else {
              return (
                <el-date-picker
        ...
        },
        {
          ...
          render: (h, scope) => {
            if (this.readPretty == true) {
              return this.$parseTime(scope.row.planEndDate)
            } else {
              return (
                <el-date-picker
        ...
        {
          prop: 'remarks',
          label: this.$t('perfMod.specialRemarks'),  // 特殊备注
          minWidth: 150,
          render: (h, scope) => {
            if (this.readPretty == true) {
            ...
            if (complete) {
              return (
                <el-button type="text" onClick={() => this.handOver(scope.row, 'file')} >
                  { `${ thsi.$t("components.workedProcess.headers.attachment") }（${scope.row.fileNum || 0}）` }
                </el-button>
              )
            }
            return null
