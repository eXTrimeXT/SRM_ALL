<template>
  <el-container
    class="contractPerformanceBillEdit"
    direction="vertical"
  >
    <el-main>
      <el-tabs
        ...
        type="border-card"
      >
        <el-tab-pane :label="$t('supRisk.baseInfo')">
    ...
    >
      ...
      <template #footer>
        ...
        <template v-if="isHandoverApprove">
          <el-button
            type="primary"
            :disabled="readOnly"
            @click="handoverApprove"
          >
          <!-- 通过 -->
            {{ $t("components.approvalHead.headers.pass") }}
          </el-button>
...
export default {
  ...
  created () {
    ...
    if (row.perOrderId) {
      this.perOrderId = row.perOrderId
  ...
  methods: {
    ...
    async handover (row, disabled) {
      ...
      this.isEditable = ['REJECTED', 'UNFINISHED_WORK', 'SUPPLIER_SUBMITTED'].includes(
        row.planStatus
      )
      this.isHandoverApprove = ['PURCHASE_SUBMIT'].includes(row.planStatus)
    ...
    async saveOrderPlanPay () {
      let perDelivOrderDetaList = this.deliveryOrderData.perDelivOrderDetaList
      let bolDeliveryNum = 1
      perDelivOrderDetaList.forEach(e => {
        if (!e.deliveryNum) {
          bolDeliveryNum = 0
        }
      })
      if (bolDeliveryNum == 0) {
        this.$message.error(this.$t('contract_mod.enterDeliveryQuantity'))
        return false
      }
    ...
    async save () {
      ...
      let perOrderInPlanList = data.perOrderInPlanList
      if (perOrderInPlanList.length > 0) {
        let totalNum = 0
        let nameBol = 1
        let timeBol = 1
        perOrderInPlanList.forEach(e => {
          totalNum += Number(e.planInvoiceProportion)
          if (!e.nodePersonId) {
            nameBol = 0
          }
          if (!e.planInvoiceCompleteDate) {
            timeBol = 0
          }
        })
        if (nameBol == 0) {
          this.$message.error(this.$t('contract_mod.nodePersonRequired'))
          return false
        }
        if (totalNum != 100) {
          this.$message.error(this.$t('contract_mod.plannedInvoicingRatios'))
          return false
        }
        if (timeBol == 0) {
          this.$message.error(this.$t('contract_mod.planFieldMust'))
          return false
        }
      }
      const res = await this.$api.cmPerform.buyer.main.performOrder.saveOrUpdatePerOrder(data)
      this.perOrderId = res.data
      this.$message.success(res.message)
      this.refresh()
    },
    async submit () {
    ...
    billValidate () {
      return new Promise((resolve) => {
        let checkFields = [
          ...
          {
            key: 'planStartDate',
            message: this.$t('contract_mod.planStartDateRequired'),
            activeName: '1'
          },
          {
            key: 'planEndDate',
        ...
        checkFields = [
          {
            key: 'nodePersonName',
            message: this.$t('contract_mod.nodePlanRequired'),
            activeName: '2'
          },
          {
    ...
    refresh () {
      if (!this.perOrderId) return
      this.initFormData(this.perOrderId)
    },
    cancel () {
      const { tabName } = this.$attrs.params
      this.$emit('tab-remove', tabName)
      this.__setTabTodo('contractPerformanceBillList.getQuerydata')
    }
  }
}
</script>
<style scoped lang="scss">
.contractPerformanceBillEdit {
  .title {
    margin: 10px 0;
    font-size: 14px;
    font-weight: bolder;
