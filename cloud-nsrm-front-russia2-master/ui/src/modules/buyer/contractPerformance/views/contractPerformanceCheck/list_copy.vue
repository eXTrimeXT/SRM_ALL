...
export default {
  ...
  created () {
    ...
    this.tableHeader = [
      ...
      {
        prop: 'contractNo',
        label: this.$t('bidMod.compactIndex'),  // '合同序号'
        minWidth: 130
      },
      {
        prop: 'nodePersonName',
      ...
      },
      {
        ...
        buttons: [
          ...
          },
          {
            ...
            formattor (val) {
              //  '交付确认'
              return this.$t('supRisk.deliverConfirm')
            },
            show: row => {
    ...
    editTab (type, row = {}) {
      ...
      let tab = {
        ...
        params: {
          flag: type,
          row,
          tabName: this.$t('contractMod.contractAcceptance') + name  // '合同验收'
        },
        // title: name ? '合同验收' + name : '新增合同验收',
        title: name ? this.$t('contractMod.contractAcceptance') + name : this.$t('contractMod.newContractAcceptance'),
        name: this.$t('contractMod.contractAcceptance') + name  // '合同验收'
      }
