      ...
      <MainHeader>
        ...
        >
          ...
          <ExportExcel
            ...
            page-url="/api-pef/vendorAsses/listPage"
            type="default"
          />
        </template>
      </MainHeader>
...
export default {
  ...
  created () {
    ...
    this.tableHeader = [
      ...
      {
        prop: 'sourceType',
        label: this.$t('perfMod.sourceType'),  // '来源单据类型'
        width: '120',
      ...
      },
      {
        ...
        buttons: [
          {// 审批
            ...
            formattor (val) {
              return _this.$t('vendorMod.doApproval')
            },
            show: function (row) {
              // 拟定和撤回的单据可以编辑
              if (
                _this.curRole === 'BUYER' && row.status === 'SUBMITTED'
              ) {
                return true
              } else {
                return false
          ...
          },
          {
            ...
            formattor (val) {
              return _this.$t('perfMod.deal')
            },
            show: function (row) {
              if (
                _this.curRole === 'BUYER' &&
                row.status === 'VENDOR_FEEDBACK' &&
                row.mIsFeedback === 'N'
              ) {
                return true
              } else {
  ...
  methods: {
    ...
    notifyVendor () {
      if (this.currentRows && this.currentRows.length > 0) {
        ...
        performanceManagement.notifySupplier(rows).then(res => {
          this.$message({
            message: res.message,
            type: 'success'
          })
    ...
    delRowData (row) {
      ...
      this.$confirm(this.$t('perfMod.sureDeleteData'), {
        ...
        .then(() => {
          performanceManagement.vendorAssesDel({ vendorAssesId }).then(res => {
            this.$message({
              message: res.message,
              type: 'success'
            })
