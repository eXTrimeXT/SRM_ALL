    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
      <el-form ref="ratingForm" :model="submitModel.submitform" :rules="submitModel.rules">
        <srm-row>
          ...
          <srm-col :initCol="2">
            <el-form-item :label="$t('perfMod.orgName')" prop="organizationName">
              <!-- <el-input v-model="submitModel.submitform.organizationName" ></el-input> -->
              <OrganizationSelector
                ref="ouSelector"
                v-model="submitModel.submitform.organizationId"
                :parent-id="-1"
                node-type="OU"
          ...
          <srm-col :initCol="2">
            <el-form-item :label="$t('perfMod.scoreStart')" prop="scoreStart">
              <el-input v-model="submitModel.submitform.scoreStart" type="number" />
            </el-form-item>
          </srm-col>
          <srm-col :initCol="2">
            <el-form-item :label="$t('perfMod.scoreEnd')" prop="scoreEnd">
...
export default {
  ...
  created () {
    ...
    this.tableHeader = [
      ...
      {
        prop: 'scoreStart',
        label: () => _this.$t('perfMod.scoreStart') // '综合绩效得分>',
      },
      {
        prop: 'scoreEnd',
        label: () => _this.$t('perfMod.scoreEnd') // '综合绩效得分<=',
      },
      {
        prop: 'creationDate',
        dataType: 'dateTime',
  ...
  methods: {
    ...
    addNew () {
      ...
      this.submitModel.submitform.organizationCode = null
      this.submitModel.submitform.organizationId = null
      this.submitModel.submitform.organizationName = null
      ...
      this.$refs.ratingForm.validate(valid => {
        if (!valid) {
          ...
          if (scoreStart > scoreEnd) {
            this.$message.error('绩效评分区间配置有误请检查')
            return false
          }
