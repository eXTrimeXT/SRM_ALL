        ...
        targetPrice: {
          ...
          'x-content': generateCharExpressionByFunction(({ $values }) => {
            return $values.header?.currentRound === 1 && ['ACCEPT_ORDER', 'ORDER_END'].includes($values.header?.extProjectStatus)
              ? i18nExpression('bidMod.setTargetPrice')  // '设定目标价'
              : i18nExpression('bidMod.viewTargetPrice')  // '查看目标价'
          })
        },
        // 本轮需报价供应商数量
                ...
                '@click': generateCharFunctionExpression(async ({ $values, $queryEngine, $prompt, $t, $message }, { row }) => {
                  const promptResult = await $prompt(
                    ...
                    i18nExpression('bidMod.cancelBiding1'),
                    {
                      ...
                      inputErrorMessage: i18nExpression('bidMod.discardReasonMsg')  // '作废原因必填并且长度不能超过500字符！'
                    }
                  )
                  ...
                  if (!promptResult.value) {
                    // '请输入撤回原因！'
                    $message.warning(i18nExpression('bidMod.withdrawTips1'))
                    return
                  }
                  if (promptResult.value.length > 250) {
                    // '撤回原因长度最多250个字符！'
                    $message.warning(i18nExpression('bidMod.withdrawTips2'))
                    return
                  }
                  ...
                  if (response) {
                    // '作废成功！'
                    $message.success(i18nExpression('bidMod.discardSuccess'))
