...
const printInnerBoxByOuterId = async (id:string,$queryEngine: any)=>{
  $queryEngine.request.baseRequest({
    ...
    if(setTemplatePath.length == 1){
      ...
      // '内箱存在多个模板，无法打印'
      app.$message.warning(t('cusEntry.supplement20250211.innerBoxMultipleTemplatesPrintError'))
    }
  })
}
        ...
        properties: generateXindexInOrder({
          ...
          relationMaterialQuantity: {
            ...
            'x-render-table-column': {
              ...
              description: i18nExpression('cusEntry.supplement20250211.outerBoxCapacityAndLabel')  // '即1个外箱可装的物料数量，贴1个外箱标签'
            }
          },
          // 已绑定内箱数量
          boundInnerBoxQuantity: {
            type: 'string',
            'x-render-table-column': {
              title: i18nExpression('orderMod.boundInnerBoxQuantity2'), // '已绑定内箱数量'
              minWidth: 150,
              icon: 'el-icon-question',
              // 1个外箱已绑定的内箱数量
              description: i18nExpression('buyerDeliveryOrder.prompt17') // '1个外箱已绑定的内箱数量'
            }
          },
          // 待绑定内箱物料数量
          leftMaterialQuantity: {
            type: 'string',
            'x-render-table-column': {
              title: i18nExpression('orderMod.leftMaterialQuantity'),  // '待绑定内箱物料数量'
              minWidth: 150,
              icon: 'el-icon-question',
              // 关联物料数量-已绑定内箱数量*内箱关联物料数量
              description: i18nExpression('cusEntry.supplement20250211.associatedMaterialQuantity')  // '关联物料数量-已绑定内箱数量*内箱关联物料数量'
            }
          },
          // 绑定状态
          boundFlag: {
          ...
          operation: {
            ...
            properties: {
              ...
              printOutBar: {
                ...
                'x-content': i18nExpression('cusEntry.supplement20250211.printOuterBoxBarcode'),  // '打印外箱条码'
                'x-component-props': {
                  type: 'text',
                  '@click': expression(`() => {
                    console.log(i18nExpression('cusEntry.supplement20250211.printOuterBoxBarcode'))  // '打印外箱条码'
                    let row = $table.getRowByIndex($self.index)
              ...
              printInnerBar: {
                type: 'void',
                'x-component': 'AuthorityButton',
                'x-content': i18nExpression('cusEntry.supplement20250211.printInnerBoxBarcode'),
                'x-component-props': {
                  type: 'text',
                  '@click': expression(`() => {
                    console.log('打印内箱条码')
