    ...
    properties: {
      ...
      query: {
        ...
        properties: generateXindexInOrder({
          ...
          tagRuleName: {
            type: 'string',
            title: "{{ $t('cusEntry.supplement20250211.barcodeRuleName') }}",  // 条码规则名称
            'x-component': 'QuickSearchWrapper',
            'x-component-props': {
      ...
      table: {
        ...
        properties: generateXindexInOrder({
          ...
          tagRuleCode: {
            type: 'string',
            title: "{{ $t('cusEntry.supplement20250211.barcodeRuleCode') }}",  // 条码规则编码
            'x-render-table-column': {
              'min-width': 120
            }
          },
      ...
      dialog: {
        type: 'void',
        title: "{{ $t('cusEntry.supplement20250211.newBarcodeGenerationRule') }}",  // 新增条码生成规则
        'x-component': 'RDialog',
        'x-component-props': {
          ...
          form: {
            ...
            properties: generateXindexInOrder({
              tagGenerateRuleDimension: {
                type: 'string',
                title: "{{ $t('cusEntry.supplement20250211.barcodeGenerationRuleDimension') }}",  // 条码生成规则维度
                required: true,
...
const $handleFn = throttle(($self:any, values:any, $message:any, $confirm:any, $queryEngine:any, done:any) => {
  ...
  $queryEngine.request.baseRequest({
    ...
    if (result?.data.length) {
      // '该品类/物料已存在一条默认的规则，是否修改?'
      $confirm(t('cusEntry.supplement20250211.isModifyDefaultRuleForCategoryOrMaterial'), {
