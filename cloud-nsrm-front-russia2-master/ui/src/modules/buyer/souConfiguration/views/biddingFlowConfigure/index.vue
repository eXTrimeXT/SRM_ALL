    ...
    <el-main>
      ...
      >
        ...
        <el-table-column
          ...
          :label="$t('themeConfig.processName')"
...
export default {
  ...
  data () {
    return {
      ...
      formWrapperArray: [
        // 流程标题
        { prop: 'processConfigName', label: this.$t('themeConfig.processTitle') }
      ],
      pageInfo: {
        total: 0,
        pageNum: 1,
  ...
  methods: {
    ...
    async operationRow (type, row) {
      ...
      const response = await bidBuyerHttp.process[type](row.processConfigId)
      if (response) {
