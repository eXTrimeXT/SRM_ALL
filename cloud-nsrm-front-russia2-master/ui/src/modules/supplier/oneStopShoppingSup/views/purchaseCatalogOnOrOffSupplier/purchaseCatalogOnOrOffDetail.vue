<template>
  <el-container class="flex-container the-purInvoice-detail" direction="vertical">
          ...
          >
            <template #priceLibraryNo>
              <el-input
                ...
                :disabled="isReadOnly || getFormObj.status === 'REJECTED'"
                clearable
                @clear="clearFormData"
              >
                <em
    ...
    <!-- 价格库 -->
    <price-search
      url="/api-sup-ce/sup/catalogOnShelves/getPriceLibraryForCatalogOnShelves"
  ...
  methods: {
    ...
    async queryDetails (id) {
      const { data } = await this.$http({
        url: '/api-sup-ce/sup/catalogOnShelves/getDetailPage',
        method: 'GET',
        params: { id },
        loading: true
      })
    ...
    hasDefaultImg () {
      if (this.catalogOnShelvesAttaches.length < 1) {
        // 请至少上传一张物料图片！
        this.__jump_error__('imgRef', null, this.$t('cusEntry.supplement20250205.uploadAtLeastOneMaterialImage'))
        return false
      }
      ...
      if (!sign) {
        // 请设置一张默认主图！
        this.__jump_error__('imgRef', null, this.$t('cusEntry.supplement20250205.defaultMainImage'))
        return false
      }
      return true
    },
    ...
    // 提交审批
    async submitBill () {
      const confirmResult = await this.$confirm(this.$t('bidMod.submitApprovalSure'), {
      ...
      await this.$http({
        url: '/api-sup-ce/sup/catalogOnShelves/submit',
        method: 'POST',
        data: {
    ...
    async saveBill () {
      const { data } = await this.$http({
        url: '/api-sup-ce/sup/catalogOnShelves/addOrUpdateOnShelves',
        method: 'POST',
        data: {
          catalogOnshelves: this.getFormObj,
          catalogOnShelvesAttaches: this.fileList
