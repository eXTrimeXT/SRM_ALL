...
import orderForm from 'modb@/vendorManagementBuyer/views/purchaseDirectoryEngine/components/orderForm'
import attrForm from 'modb@/vendorManagementBuyer/views/purchaseDirectoryEngine/components/attrForm'
import file from 'modb@/vendorManagementBuyer/views/purchaseDirectoryEngine/components/file'
        ...
        back: {
          ...
          'x-component-props': {
            ...
            '@click': expression(`() => {
              ...
              app.$confirm($t('outsource.goBackConfirm'), $t('components.approvalHead.tips.tip'), {
...
const $submit = async (type:string, $form:any, $queryEngine:any, $confirm:any, $message:any, $bus:any) => {
  ...
  if (type === 'submit') {
    ...
    if (rest.startDate >= rest.endDate) {
      $message.warning(t('vendorMod.materialBaseInfo'))
      return
    }
  }
  return $queryEngine.request.baseRequest(
    {
      ...
      payload: [
        {
          ...form,
          catalogStatus: type === 'submit' ? 'VALID' : 'DRAFT'
        }
      ],
      action: 'save'
    }
    ...
    if (type === 'save') {
      ...
      $bus.$emit('PurchaseCatalogVendor')
    }
  })
}
...
<template>
  <RenderEngine schemaKey="vendorPurchaseDirectoryDetail" :pageAttrs="$attrs" :scope="scope" :schema="schema" />
</template>
