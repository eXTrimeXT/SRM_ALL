...
export default {
  ...
  created () {
    ...
    this.tableHeader = [
      {
        prop: 'changeNo',
        label: () => _this.$t('purchase.SourceChangeOrderNo'),
        width: 130
      },
      {
        prop: 'vendorCode',
      ...
      },
      {
        ...
        label: () => _this.$t('components.organization.ORG'), // 业务实体
        width: 130
      },
      {
        prop: 'invName',
        label: () => _this.$t('components.organization.INV'),
        width: 130
      },
      {
        prop: 'materialCode',
      ...
      {
        prop: 'startDate',
        label: () => _this.$t('common.effectTime'), // 生效时间
        width: 100,
        formattor (val) {
      ...
      {
        prop: 'endDate',
        label: () => _this.$t('vendorMod.expirationTime'), // 失效时间
        width: 100,
        formattor (val) {
          return val ? parseTime(val, '{y}-{m}-{d}') : ''
        }
      ...
      },
      {
        ...
        buttons: [
          ...
          },
          {
            ...
            formattor (val) {
              return this.$t('bidMod.submitApproval')
            },
            show: row => {
  ...
  methods: {
    inValid (row) {
      this.$confirm(this.$t('vendorMod.confirmInvalidate'), {
    ...
    downloadTemplate () {
      downloadFileLink('/api-sup/purchaseCataLog/importExcelTemplate', this.$t('logisticsMod.importTemplateXLSX')).catch(
        () => {
    ...
    editTab (type, row = {}) {
      ...
      let tab = {
        ...
        params: {
          flag: type,
          row,
          tabName: this.$t('route.purchaseDirectoryChange') + name
        },
        title: name ? this.$t('route.purchaseDirectoryChange') + name : this.$t('route.purchaseDirectoryChange'),
        name: this.$t('route.purchaseDirectoryChange') + name
      }
