      ...
      },
      {
        ...
        buttons: [
          ...
          // '审批'
          {
            ...
            show: row => ['SUBMITTED'].includes(row.changeStatus) &&
                          row.workflowAuditStatus === 'WAIT' &&
                          this.userType === 'BUYER' &&
                          this.flowWithTabMode.includes(row.integrationMode)
          },
  ...
  methods: {
    ...
    delRowData (row) {
      ...
      this.$confirm(this.$t('common.confirmDelete'), {
        ...
        .then(() => {
          vendorInfoChangeApi.changeInfoDel({ changeId }).then((res) => {
            this.$message({
              message: res.message,
              type: 'success'
            })
    ...
    async editTab (type, row) {
      ...
      if (type === 'add') {
        if (this.userType === 'VENDOR') {
          let res = await vendorInfoChangeApi.ifAddInfoChange(this.companyIdAdd)
    ...
    async rejectOrRecallRow (type, row) {
      ...
      const promptResult = await this.$prompt(
        '',
        this.$t('cusEntry.supplement20250211.titleCause', {title}), // `${title}原因
        {
          confirmButtonText: this.$t('common.confirm'),
          cancelButtonText: this.$t('components.common.cancel'),
          inputValidator: value => !(!value || value.length > 500),
          inputErrorMessage: this.$t('cusEntry.supplement20250211.maxLength500', {title}) // ${title}原因必填并且长度不能超过500字符！
        }
      )
      ...
      vendorInfoChangeApi.saveOrUpdatuOrderByStatus(url, datas).then(() => {
