...
import { vendorInfoChangeApi } from 'mods@/vendorManagementSupplier/api/vendorInfoChange'
...
export default {
  ...
  methods: {
    ...
    fatchOldData () {
      ...
      if (changeId) {
        vendorInfoChangeApi.getInfoByChangeId({ changeId }).then((res) => {
    ...
    submitHandleFuntion (types) {
      ...
      if (this.baseInfoModelChange.baseInfoForm.registCurrency == '' || this.baseInfoModelChange.baseInfoForm.registCurrency == null) {
        // 请输入币种
        this.$message.error(this.$t('cusEntry.supplement20250211.inputCurrency'))
        return false
      }
    ...
    // 数据处理
    dataHandle (type) {
      console.log(this.financeInfoDataChange)
      const submitData = {}
      ...
      const financeInfos = this.formatDimFields(this.financeDimFieldContexts, this.financeInfoDataChange) // 财务信息
      ...
      if (bolFileuploadId) {
        // 请输入相关认证信息必填数据
        this.$message.error(this.$t('cusEntry.supplement20250211.requiredAuthInfo'))
        return false
      }
      ...
      vendorInfoChangeApi.saveOrUpdateChannelInfo(url, submitData).then(async res => {
        if (res) {
          if (res.code == '0') {
            this.$message({
              message: res.message,
    ...
    formatDimFields (dimFieldArr, modelData) {
      ...
      if (dimFieldArr && dimFieldArr.length > 0) {
        ...
        modelData.forEach((item, index) => {
          if (item.opType === 'add' || item.opType === 'update' || item.opType === 'delete') {
            let dyObj = {}
    ...
    buyerReject () {
      this.$prompt('', this.$t('vendorMod.rejectReason'), {
        ...
        inputErrorMessage: this.$t('vendorMod.rejectLimit500')
      }).then(({ value }) => {
        this.$http({
...
::v-deep .formClassAll .el-select,.formClassAll .el-input-all,.formClassAll .el-date-editor,.formClassAll .el-input-group{
  width: 60%;
}
...
::v-deep .the-vendorInfoChangeDetail-detail {
  .sub_header {
    margin: 0 0 10px;
  }
...
::v-deep .companyInfoFillChange {
  padding-bottom: 40px;
}
::v-deep .download-link-wrap .close-icon {
  cursor: pointer;
  display: inline-block;
  vertical-align: middle;
}
::v-deep .the_info_message_wrapper .el-form-item__content {
  line-height: 14px;
}
::v-deep .vue-treeselect__multi-value-item.vue-treeselect__multi-value-item-new {
  background: #efd9df;
}
::v-deep .c-upload-file .el-upload--text{
  height: 30px !important;
}
::v-deep .c-download-link.ellipsis{
  height: 30px !important;
  line-height: 30px !important;
}
::v-deep .el-form .vue-treeselect__control{
  height: 30px !important;
}
