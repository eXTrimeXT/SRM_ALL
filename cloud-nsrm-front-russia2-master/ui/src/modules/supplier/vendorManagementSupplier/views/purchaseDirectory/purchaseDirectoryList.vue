...
export default {
  ...
  created () {
    ...
    this.tableHeader = [
      ...
      {
        prop: 'invName',
        label: () => _this.$t('components.organization.INV'), // 库存组织
        width: 130
      },
      {
        prop: 'materialCode',
      ...
      {
        prop: 'dataSource',
        label: () => _this.$t('vendorMod.dataSources'), // 数据来源
        width: 180,
        dataType: 'dict',
    ...
    editTab (type, row = {}) {
      ...
      let tab = {
        ...
        params: {
          flag: type,
          row,
          tabName: this.$t('vendorMod.supplyList') + name
        },
        title: name ? this.$t('vendorMod.supplyList') + name : this.$t('vendorMod.newSourcelList'),
        name: this.$t('vendorMod.supplyList') + name
      }
