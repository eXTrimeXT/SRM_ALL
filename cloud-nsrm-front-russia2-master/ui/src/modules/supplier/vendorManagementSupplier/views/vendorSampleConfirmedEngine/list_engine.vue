...
const { getCurrentUserInfo } = usePageHelper()

onActivated(() => {
  const params = app.$route.params
  if (params.from === "fromFun" && app.$route?.params.taskIndex === 2) {
    let row = {
      sampleId: app.$route?.params?.formId,
      sampleNumber: app.$route?.params?.formNo
    }
    $editTab('view', row)
  }
})
      ...
      actions: {
        paginationQuery: {
          transformRequest: expression(`(data,headers) => {
            ...
            //   data.payload.filter = {}
            // }
            // data.payload.filter.vendorName = vendorName
            return data
          }`)
        }
      }
          ...
          requireSendTime: {
            ...
            'x-component-props': {
              ...
              formatter: expression(`({ cellValue, row, column }) => {
                parseTime(row.requireSendTime, '{y}-{m}-{d}')
              }`)
            },
                  ...
                  fulfill: {
                    schema: {
                      'x-visible': expression(`
                        ['PUBLISHED'].includes($deps[0])
                      `)
                    }
                  }
                },
                'x-component-props': {
                  '@click': expression('({ row }) => $editTab("vendorEdit", row)')
                }
              },
              refused: {
                type: 'void',
                ...
                'x-component-props': {
                  '@click': expression('({ row }) => $editTab("vendorEditRefuse", row,$queryEngine)')
                }
              }
            }
          }
...
<template>
  <RenderEngine schemaKey="vendorSampleConfirmedVendorList" :pageAttrs="$attrs" :schema="schema" :scope="scope" />
</template>
