            ...
            >
              ...
              >
                <srm-row>
                  ...
                  <!-- 实际考核金额(不含税) -->
                  <srm-col >
                    <el-form-item :label="$t('perfMod.actualAssessmentAmountN')">
...
export default {
  ...
  methods: {
    ...
    getOrderFormDetail (vendorAssesId = this.curOrderId) {
      perVendorApi.vendorAssesQueryById({ vendorAssesId }).then(res => {
    ...
    indicatorDimensionChange (val) {
      ...
      if (val) {
        let params = {}
        params.indicatorType = 'PERFORMANCE'
    ...
    getPefTempIndicatorByDim (params) {
      perVendorApi.getPefTempLineByDim(params).then(res => {
    ...
    findPerfTemplateLineAndIndsLine (params) {
      perVendorApi.findPerfTemplateLineAndIndsLine(params).then(res => {
        this.indicatorLineList = res.data.perfTemplateIndsLineList
      })
    },
    evaluationResultsChange () {
      this.$http({
        url: '/api-pef/vendorAsses/getAssessmentPenalty',
        method: 'POST',
        data: {
          indicatorDimension: this.assessmentForm.indicatorDimension, // 指标维度
          ceeaIndicatorLineDes: this.assessmentForm.ceeaIndicatorLineDes, // 评选结果
          assessmentDate: this.assessmentForm.assessmentDate // 审核时间
        },
        loading: true
      }).then(res => {
        this.assessmentForm.assessmentPenalty = res.data
      })
    },
    // 报价输入金额改变
    ...
    approvalSB () {
      ...
      // 提交审批
      perVendorApi.vendorAssesModify(submitData).then(res => {
        if (res.code == '0') {
    ...
    notifySupplier () {
      ...
      perVendorApi.notifySupplier([submitData]).then(res => {
        this.$message({
          message: res.message,
          type: 'success'
        })
    ...
    vendorFeedback () {
      ...
      perVendorApi.vendorFeedback(submitData).then(res => {
        this.$message({
          message: res.message,
          type: 'success'
        })
    ...
    submitProcessRes () {
      ...
      perVendorApi.buyersProcess(submitData).then(res => {
        this.$message({
          message: res.message,
          type: 'success'
        })
    ...
    dataHandle () {
      ...
      if (this.curOpt === 'add' || this.curOpt === 'adds') {
        // 新增
        perVendorApi.vendorAssesAdd(submitData).then(res => {
          this.$message({
            message: res.message,
            type: 'success'
          })
