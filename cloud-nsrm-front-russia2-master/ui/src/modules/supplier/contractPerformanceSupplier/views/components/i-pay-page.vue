  ...
  data () {
    return {
      ...
      columns: [
        ...
        {
          prop: 'fileId',
          label: this.$t('components.workedProcess.headers.attachment'),
          render: (h, scope) => {
            return (
              <SrmCommonFile
                extra-data="fileInfo"
        ...
        { prop: 'lastUpdatedFullName', label: this.$t('components.fileupload.uploadUserName') },
        { prop: 'lastUpdateDate', label: this.$t('components.fileupload.uploadDate'), dataType: 'dateTime' },
        {
          prop: 'operation',
            ...
            if (this.mode === 'handOver') {
              return (
                <el-popover placement="top" width="160" v-model={this.visible}>
                  <p>{ this.$t('cusEntry.supplement20250205.confirmDelete') }</p>
