...
const schema = defineSchemas({
  ...
  filePreview: {
    type: 'void',
    title: expression(`$form.query('PerInvoice').get('data').fileSourceName + '-' + $t('common.preview')`),
    'x-component': 'RDialog',
    'x-component-props': {
      size: 'xLarge',
      footer: true,
  ...
  uploadInvoice: {
    ...
    'x-component-props': {
      visible: false,
      headerText: {
        title: $t('cusEntry.supplement20250211.editInvoice'), // 编辑发票
...
const $invoicingVerification = ($form) => { // 校验是否一个开票单只能有一种里程碑
  ...
  if (perInvoiceDetailList?.length > 0) {
    ...
    if (!bol2) {
      app.$message.warning($t('cusEntry.supplement20250211.inputMilestoneNode'))
      return false
    }
    if (!bol) {
      app.$message.warning($t('cusEntry.supplement20250211.openInvoiceMilestoneChoice'))
      return false
    }
    return bol
  } else {
