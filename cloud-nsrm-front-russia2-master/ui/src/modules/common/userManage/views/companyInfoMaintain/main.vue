                      ...
                      <srm-col :initCol="3">
                        ...
                        <el-form-item>
                          ...
                          <el-radio-group
                            ...
                            @change="managementChange($event, $t('vendorMod.msgIfPass1'))"
                          >
                      ...
                      <srm-col :initCol="3">
                        ...
                        <el-form-item>
                          ...
                          <el-radio-group
                            ...
                            @change="managementChange($event, $t('vendorMod.msgIfPass2'))"
                          >
                            <el-radio :label="$t('common.Y')">
                              {{
                                $t("common.yes")
                      ...
                      <srm-col :initCol="3">
                        ...
                        <el-form-item>
                          ...
                          <el-radio-group
                            ...
                            @change="managementChange($event, $t('vendorMod.msgIfPass3'))"
                          >
                            <el-radio :label="$t('common.Y')">
                              {{
                                $t("common.yes")
...
export default {
    ...
    return {
      ...
      relModel: {
        ...
        rules: {
          ...
          // orgId: [{ required: true, message: this.$t('common.BusinessEntity')') }]
        }
      },
      outerField: [
        'overseasRelation',
  ...
  methods: {
    ...
    phoneChange (data) {
      ...
      if (!reg.test(data)) {
        this.$message.error(this.$t('dataConfMod.enterCorrectMobile'))
      }
    },
    emailChange (data) {
      var reg = /\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/
      if (!reg.test(data)) {
        this.$message.error(this.$t('dataConfMod.enterCorrectEmail'))
      }
    },
    // 维度间的级联
    selectChanges (val) {
      ...
      _this.$refs.baseInfoForm.validate(valid => {
        if (valid) {
          ...
          const contactInfosRequiredKeys = [
            { key: 'contactName', message: this.$t('cusEntry.supplement20250211.contactNameTip') }, // 缺少姓名
            { key: 'ceeaContactMethod', message: this.$t('cusEntry.supplement20250211.ceeaContactMethodTip') }, // 缺少联系方式
            { key: 'email', message: this.$t('cusEntry.supplement20250211.emailTip') } // 缺少邮箱
          ]
          for (const [index, item] of new Map(this.allParam.contactInfos.map((item, index) => [index, item]))) {
            ...
            if (errorItem) {
              // 替换提示行字符
              const tip = this.$t('vendorMod.contactInfo') + this.$t('bidMod.warningMessage', { index: index + 1 , message: errorItem.message })
          ...
          if (this.allParam.bankInfos.length === 0) {
            this.$message.warning(this.$t('dataConfMod.addBankInfo'))
            return
          }
          // 需要校验数据
          const bankInfosRequiredKeys = [
            { key: 'bankCode', message: this.$t('cusEntry.supplement20250211.bankCodeTip') }, // 缺少银行代码
            { key: 'bankAccountName', message: this.$t('cusEntry.supplement20250211.bankAccountNameTip') }, // 缺少账号名称
            { key: 'bankAccount', message: this.$t('cusEntry.supplement20250211.bankAccountTip') }, // 缺少银行账号
            { key: 'currencyCode', message: this.$t('cusEntry.supplement20250211.currencyCodeTip') } // 缺少币种
          ]
          for (const [index, item] of new Map(this.allParam.bankInfos.map((item, index) => [index, item]))) {
            ...
            if (errorItem) {
              // 替换提示行字符
              const tip = this.$t('vendorMod.bankInfo') + this.$t('bidMod.warningMessage', { index: index + 1 , message: errorItem.message })
              this.$message.warning(tip)
              return
            }
          }
          ...
          if (bolManage) {
            this.$message.error(this.$t('common.pleaseUpload') + bolManage + this.$t('components.workedProcess.headers.attachment'))
            return false
          }
