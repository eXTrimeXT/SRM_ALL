...
export default {
  ...
  created () {
    this.queryForm = [
      {
        prop: 'systemName',
        label: this.$t('systemconfig.label1') // '系统名称'
      },
      {
        prop: 'type',
        label: this.$t('interfacelog.label2'), // 传输类型
        type: 'dict',
        code: 'INTERFACE_LOG_TYPE'
      },
      {
        prop: 'protocol',
        label: this.$t('interfacelog.label1'), // '协议类型'
        type: 'dict',
        code: 'INTERFACE_LOG_SERVICE_TYPE'
      }
    ]
    ...
    handleTestClick (index, row) {
      ...
      if (systemId) {
        systemConfigApi.interfaceTest({ systemId }).then(res => {
          if (res) {
              this.$message({
                message: this.$t('interfacelog.callSuccessful'),
                type: 'success'
              })
              this.getQuerydata()
            }
